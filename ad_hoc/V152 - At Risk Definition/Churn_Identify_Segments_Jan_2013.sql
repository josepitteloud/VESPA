...
--select count(1) from bcg_base_20130111
--grant all on bcg_base_20130111 to public;
--create base at 18-Jan-2013 at account level
  select distinct account_number
    into glasera.bcg_base_20130111
    from sk_prod.cust_subs_hist
   where subscription_sub_type = 'DTV Primary Viewing'
     and status_code = 'AC'
     and effective_from_dt <= '2013-01-11'
     and effective_to_dt > '2013-01-11'
     and effective_from_dt < effective_to_dt;
--9991215 Row(s) affected

create unique hg index ind_acc on bcg_base_20130111(account_number);

grant select on bcg_base_20130111 to public;


--append target date
alter table bcg_base_20130111
        add target_dt date default null;

update bcg_base_20130111
   set target_dt = '2013-01-11';
...
;
-------------------------9991212 Row(s) affected
commit;



create hg index indx_fam_key on bcg_base_20130111 (cb_key_family);

create hg index indx_ind_key on bcg_base_20130111(cb_key_individual);

create hg index indx_hh_key on bcg_base_20130111(cb_key_household);
...
--9991166 Row(s) affected

--check
  select num_mix
        ,count(*)
    from bcg_base_20130111
group by num_mix;
...
--------------692983 Row(s) affected




delete from bcg_base_20130111 where roi_flag = 1;
----692983 Row(s) affected
...
DELETE
  FROM bcg_base_20130111   --The target table containing Staff accounts
...
-------------------------------8691027 Row(s) affected

/*select count(*), lifestage from bcg_base_20130111 group by lifestage;
*/
...
create hg index ind_sii on bcg_base_20130111(service_instance_id);
...
update bcg_base_20130111
  ...
  from bcg_base_20130111 as base
       inner join sk_prod.cust_single_account_view as sav on base.account_number = sav.account_number;
--9252751 Row(s) affected


create hg index ind_pc on bcg_base_20130111(cb_address_postcode);
...
select count(*), box_subscription from bcg_base_20130111 group by box_subscription
...
select count(*),tenure_latest from bcg_base_20130111 group by tenure_latest
...
COMMIT;-----------------110747150 Row(s) affected
...
select product_holding, count(*), sum(churn_cuscan) from bcg_base_20130111 group by product_holding

select top 10 * from bcg_base_20121130
select tv_package, count(*), sum(churn_cuscan) from bcg_base_20130111 group by tv_package order by tv_package
select tv_package, count(*), sum(churn_cuscan) from bcg_base_20130111 group by tv_package order by mix_pack
...
select cmsh.account_number
      ...
      inner join bcg_base_20130111 base
...
----------------------------------New Line--------------------
 ...
 select  bbs.account_number
    ...
    FROM sk_prod.CUST_NON_SUBSCRIPTIONS as cns
         inner join bcg_base_20130111 as bbs on cns.account_number = bbs.account_number
...
select count(*) from bcg_base_20130111 where value_seg_updated = 'UNSTABLE' and tenure_group_latestactive in ('01 1-3 months','02 4-6 months','03 6-12 months')



select count(*),tenure_group_latestactive from bcg_base_20130111 group by tenure_group_latestactive

select count(*), tenure_group_latestactive from bcg_base_20130111 group by tenure_group_latestactive
select count(*), Box_Subscription1 from bcg_base_20130111 group by Box_Subscription1*/
...
select count(*),longtenure_cuscan_rule1 from bcg_base_20130111 group by longtenure_cuscan_rule1;

select count(*),EOC_cuscan_rule1 from bcg_base_20130111 group by EOC_cuscan_rule1;


select count(*), affluence from bcg_base_20130111 group by affluence;
select count(*),unstable_cuscan_rule1 from bcg_base_20130111 group by unstable_cuscan_rule1;
...
--sybase alter bug
-- select cast(null as date) as churn_date,cast(null as varchar) as churn_type,cast(0 as bit) as churn_flag,* into bcg_base_temp from bcg_base_20130111;
-- drop table bcg_base_20130111;
-- select * into bcg_base_20130111 from bcg_base_temp;
--
alter table bcg_base_20130111
add (churn_date                 date            default null
    ,churn_type                 varchar(20)     default null
...
select top 10 value_seg_updated,tenure,* from bcg_base_20130111
select count(1),count(distinct account_number) from bcg_base_20130111
...
------9252754 Row(s) affected
  ...
  select count(distinct account_number)
    ...
    from atrisk_results_20130111 as bas
         inner join SK_PROD.CUST_SINGLE_ACCOUNT_VIEW as sav on bas.account_number = sav.account_number


select top 20 * from atrisk_results_20130111

grant all on atrisk_results_20130111 to public;
