...
select count(1) from bcg_base_20130411
grant all on bcg_base_20130411 to public;
--create base at 18-Apr-2013 at account level
  select distinct account_number
    into greenj.bcg_base_20130411
    from sk_prod.cust_subs_hist
   where subscription_sub_type = 'DTV Primary Viewing'
     and status_code = 'AC'
     and effective_from_dt <= '2013-04-11'
     and effective_to_dt > '2013-04-11'
     and effective_from_dt < effective_to_dt;
--9953433      Row(s) affected

create unique hg index ind_acc on bcg_base_20130411(account_number);

grant select on bcg_base_20130411 to public;


--append target date
alter table bcg_base_20130411
        add target_dt date default null;

update bcg_base_20130411
   set target_dt = '2013-04-11';
...
update bcg_base_20130411
  ...
  FROM bcg_base_20130411 as base
...
create hg index indx_fam_key on bcg_base_20130411 (cb_key_family);

create hg index indx_ind_key on bcg_base_20130411(cb_key_individual);

create hg index indx_hh_key on bcg_base_20130411(cb_key_household);
...
delete from bcg_base_20130411 where roi_flag = 1;
...
DELETE
  FROM bcg_base_20130411   --The target table containing Staff accounts
...
/*select count(*), lifestage from bcg_base_20130411 group by lifestage;
*/
...
create hg index ind_sii on bcg_base_20130411(service_instance_id);
...
create hg index ind_pc on bcg_base_20130411(cb_address_postcode);
...
select count(*), box_subscription from bcg_base_20130411 group by box_subscription
...
select count(*),tenure_latest from bcg_base_20130411 group by tenure_latest
...
select product_holding, count(*), sum(churn_cuscan) from bcg_base_20130411 group by product_holding

select top 10 * from bcg_base_20121130
select tv_package, count(*), sum(churn_cuscan) from bcg_base_20130411 group by tv_package order by tv_package
select tv_package, count(*), sum(churn_cuscan) from bcg_base_20130411 group by tv_package order by mix_pack
...
select cmsh.account_number
      ...
      inner join bcg_base_20130411 base
...
----------------------------------New Line--------------------
 ...
 select  bbs.account_number
    ...
    FROM sk_prod.CUST_NON_SUBSCRIPTIONS as cns
         inner join bcg_base_20130411 as bbs on cns.account_number = bbs.account_number
...
select count(*) from bcg_base_20130411 where value_seg_updated = 'UNSTABLE' and tenure_group_latestactive in ('01 1-3 months','02 4-6 months','03 6-12 months')



select count(*),tenure_group_latestactive from bcg_base_20130411 group by tenure_group_latestactive

select count(*), tenure_group_latestactive from bcg_base_20130411 group by tenure_group_latestactive
select count(*), Box_Subscription1 from bcg_base_20130411 group by Box_Subscription1*/
...
select count(*),longtenure_cuscan_rule1 from bcg_base_20130411 group by longtenure_cuscan_rule1

select count(*),EOC_cuscan_rule1 from bcg_base_20130411 group by EOC_cuscan_rule1


select count(*), affluence from bcg_base_20130411 group by affluence
select count(*),unstable_cuscan_rule1 from bcg_base_20130411 group by unstable_cuscan_rule1
...
--sybase alter bug
-- select cast(null as date) as churn_date,cast(null as varchar) as churn_type,cast(0 as bit) as churn_flag,* into bcg_base_temp from bcg_base_20130411;
-- drop table bcg_base_20130411;
-- select * into bcg_base_20130411 from bcg_base_temp;
--
alter table bcg_base_20130411
add (churn_date                 date            default null
    ,churn_type                 varchar(20)     default null
...
select top 10 value_seg_updated,tenure,* from bcg_base_20130411
select count(1),count(distinct account_number) from bcg_base_20130411
...
--2. Current Vespa customers
  select case when colour <> 'Red' then 'X' else coalesce(cuscan_type,'X') end as cuscan_type
    ...
    from ( select distinct account_number 
           ...
           where status_vespa = 'Enabled' ) as sbv
...
-- 4 Candidates for Vespa Panels likely to rteturn data reliably
  select case when colour <> 'Red' then 'X' else coalesce(cuscan_type,'X') end as cuscan_type
        ,sum(case when value_seg = 'Bedding In'                    then 1 else 0 end) as bedding_in
        ,sum(case when value_seg = 'Unstable'                      then 1 else 0 end) as unstable
        ,sum(case when value_seg not in ('Bedding In', 'Unstable') then 1 else 0 end) as other
...
select   wat.account_number
        ,min(cbk_day) as cbk_day
    into #account_numbers_cbk_day_April_2013
    from vespa_analysts.waterfall_base                as wat
         left join sk_prod.VALUE_SEGMENTS_DATA as cvs on wat.account_number = cvs.account_number
         left join atrisk_results                     as bas on bas.account_number = wat.account_number
         left join vespa_analysts.accounts_to_exclude as exc on bas.account_number = exc.account_number
...
--delete from #temp_account_cbk_day where account_number in (select account_number from kinnairt.DRX_595_BOXES_LIST);

select * from #account_numbers_cbk_day_April_2013;
