...
-- execute V079_result_calculation '2012-10-03', 'Test_Run';



if object_id('V079_result_calculation') is not null then drop procedure V079_result_calculation endif;


create procedure V079_result_calculation
    @scanning_day        date = NULL,     -- Date of daily table caps to cache
    @parRunIdentifier    varchar(10)
as
begin
    ...
    set @varSql = '
            ...
            select
              ...
              from V079_Daily_Events_Capped_##^!!2!!^##__' || @parRunIdentifier || ' as da,
    ...
    set @varSql = 'if object_id(''V079_Res_' || @parRunIdentifier || '__Aggr_Results'') is not null drop table V079_Res_' || @parRunIdentifier || '__Aggr_Results'
    execute(@varSql)

    set @varSql = 'if object_id(''V079_Res_' || @parRunIdentifier || '__Raw_Data'') is not null drop table V079_Res_' || @parRunIdentifier || '__Raw_Data'
    execute(@varSql)



    set @varSql = '
                           alter table V079_aggregate_collection rename V079_Res_' || @parRunIdentifier || '__Aggr_Results
                          '
    execute(@varSql)

    set @varSql = '
                           alter table V079_pull_cache rename V079_Res_' || @parRunIdentifier || '__Raw_Data
                          '
    execute(@varSql)
