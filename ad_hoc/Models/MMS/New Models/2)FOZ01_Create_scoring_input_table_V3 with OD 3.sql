proc sql;
connect to sybaseiq
     ( DATABASE=olive_prod  PORT=8000  HOST="upiqm070.bskyb.com" SERVER=olive_iqm_prod07 authdomain="Sybase PRD") ;
execute( SETUSER SAS_MM  ) by sybaseiq;
execute(

DROP VARIABLE IF EXISTS @start_date;
DROP VARIABLE IF EXISTS @end_date;
DROP VARIABLE IF EXISTS @end_date_lag;

CREATE  VARIABLE @start_date DATE;
CREATE  VARIABLE @end_date DATE;
CREATE  VARIABLE @end_date_lag DATE;

SET @start_date   = '2000-01-01'; /* Set to be before the scoring_week */
SET @end_date     = '2999-01-01'; /* Set to be after the scoring_week */
SET @end_date_lag = '2999-01-01'; /* Set to be after the scoring_week */


/* --------------This part is added by FOZ01 ----------------*/

drop variable if exists @sky_month;
drop variable if exists @obs_dt ;

create variable @sky_month varchar(254);
create variable @obs_dt date;

set @sky_month = (select x_subs_month_and_year from sky_calendar where calendar_date=today());
set @obs_dt = dateadd(day,-1,(select min (calendar_date) from sky_calendar where x_subs_month_and_year = @sky_month)); /* last Thursday of the month*/
...
DROP    VARIABLE IF EXISTS @scoring_week;
CREATE  VARIABLE @scoring_week CHAR(6);

SET  @scoring_week  = (select subs_week_and_year from sky_calendar where calendar_date =@obs_dt);
...
/*  5.2 OD fields */
    ...
    SELECT       a.account_number
                ...
                ,CASE   WHEN DATEDIFF(MONTH, date_last_od , base_dt ) > 15 THEN 16 ELSE  DATEDIFF(MONTH, date_last_od , base_dt )  END as OD_Months_since_Last
    ...
    /*  5.5.2 Missing values */
        ...
        SET          ADSL_Enabled             = CASE WHEN ADSL_Enabled             IS NULL                         THEN 'Unknown' ELSE ADSL_Enabled             END
                  ...
                  /* --,Simple_Segment           = CASE WHEN Simple_Segment           IS NULL OR  Simple_Segment = '' THEN 'Unknown' ELSE Simple_Segment           END */
                    ...
                    ,DTV_PCs_ever = CASE WHEN DTV_PCs_ever IS NULL                         THEN 0         ELSE DTV_PCs_ever END
...
/*  6.1 Create binning table */
    ...
    DROP TABLE IF EXISTS New_Uplift_Models_Input_Table;
  ...
  /*--              ,BB_RTM */
             ...
             /*  -- ,Simple_Segment */
                ...
                ,CASE WHEN DTV_PCs_ever IS NULL  THEN  0 ELSE DTV_PCs_ever   END AS DTV_Pending_cancels_ever
    ...
    INTO        New_Uplift_Models_Input_Table
    FROM        CS_raw_test
    WHERE       country = 'UK'
    AND         base_dt BETWEEN @start_date AND @end_date_lag; /* Because we don't know upsell for last month */

    CREATE HG INDEX id1 ON New_Uplift_Models_Input_Table (account_number);
    CREATE DATE INDEX iddt ON New_Uplift_Models_Input_Table (base_dt);

/*  6.2 Permissions to public */

 GRANT SELECT ON New_Uplift_Models_Input_Table TO PUBLIC;
