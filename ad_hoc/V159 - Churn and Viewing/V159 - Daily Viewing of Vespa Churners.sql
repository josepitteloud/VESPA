...
---Generate a list of all those who cuscan between 26/12/12 and 18/4/13---
select
                account_number
...
WHERE           csh.status_start_dt between '2012-12-26' and '2013-04-18'
...
,po_dt
,pre_po_dt
into #v159_all_vespa_churners
from account_status_at_period_start as a 
left outer join #cuscan_in_period as b
on a.account_number=b.account_number
where b.account_number is not null and status_at_2012_12_26 = 'AC' and vespa_weight_2012_12_26 >0
;
commit;

select * into v159_all_vespa_churners from #v159_all_vespa_churners; commit;
commit;

create hg index idx1 on v159_all_vespa_churners(account_number);
commit;

--select count(*) from v159_all_vespa_churners;
-----------------------------------------------------------------------------------------------------------------------------------------------------
...
IF object_ID ('V159_Daily_viewing_summary_churners_all') IS NOT NULL THEN
            DROP TABLE  V159_Daily_viewing_summary_churners_all
END IF;

CREATE TABLE  V159_Daily_viewing_summary_churners_all
...
IF object_ID ('V159_Daily_viewing_summary_churners') IS NOT NULL THEN
            DROP TABLE  V159_Daily_viewing_summary_churners
END IF;
--select * from V159_Daily_viewing_summary_churners;
CREATE TABLE  V159_Daily_viewing_summary_churners
    ( 
            Account_Number                             varchar(20)  not null
            ,viewing_day                      date
...
;
--delete from V159_Daily_viewing_summary_churners where viewing_day<'2012-08-01'; commit;

-- Date range of programmes to capture
SET @snapshot_start_dt  = '2012-05-29';  --Had to restart loop half way through
--SET @snapshot_start_dt  = '2012-01-01';  --Original
SET @snapshot_end_dt    = '2012-07-31';
delete from V159_Daily_viewing_summary_churners_all where account_number is null or account_number is not null;

commit;
create hg index idx1 on V159_Daily_viewing_summary_churners_all(Account_Number);
create hg index idx2 on V159_Daily_viewing_summary_churners_all(programme_trans_sk);
commit;
...
SET @viewing_var_sql = '
        insert into V159_Daily_viewing_summary_churners_all(
                cb_row_ID
                ,Account_Number
                ,Subscriber_Id
                ,programme_trans_sk
...
insert into V159_Daily_viewing_summary_churners
select a.account_number
...
--select viewing_day , count(*) from V159_Daily_viewing_summary_churners group by viewing_day order by viewing_day ;
--select top 5000 * into V159_Daily_viewing_summary_churners_test from V159_Daily_viewing_summary_churners;commit;
commit;
--select dateformat(viewing_starts,'HH') as hourss , count(*) from vespa_analysts.VESPA_DAILY_AUGS_20130126 group by hourss
--select top 5000 * from V159_Daily_viewing_summary_churners
--select viewing_day,count(*) from V159_Daily_viewing_summary_churners group by viewing_day order by viewing_day
--viewing_day,count()
--'2012-05-28',1778

--select * from V159_Daily_viewing_summary_churners;format
commit;

--Create Single Account Summaries--

commit;
create  hg index idx1 on V159_Daily_viewing_summary_churners (account_number);
commit;
...
--'2013-02-04'
SET @snapshot_start_dt  = '2013-02-04'; --Rerrun from here (phase II starts 14th Aug 2012--
--SET @snapshot_start_dt  = '2012-08-14';  --Original
SET @snapshot_end_dt    = '2013-04-17';
delete from V159_Daily_viewing_summary_churners_all where account_number is null or account_number is not null
--select top 100 * from vespa_analysts.VESPA_DAILY_AUGS_20130401
--select top 100 * from V159_Tenure_10_16mth_Viewing
-- Build string with placeholder for changing daily table reference
SET @viewing_var_sql = '
        insert into V159_Daily_viewing_summary_churners_all(
...
--delete from V159_Daily_viewing_summary_churners where viewing_day>='2013-02-04'; commit;  select * into V159_Daily_viewing_summary_churners_COPY from V159_Daily_viewing_summary_churners; commit;
--drop table #totals;
...
into #totals
from V159_Daily_viewing_summary_churners as a
left outer join v159_all_vespa_churners as b
on a.account_number = b.account_number
...
/*
select * from v159_all_vespa_churners;

,sum(case when viewing_day  between dateadd(month,-1,po_dt) and po_dt-1 then total_duration_terrestrial+total_duration_free_non_terrestrial else 0 end) as month_pre_churn_pay
,sum(case when viewing_day  between dateadd(month,-1,po_dt) and po_dt-1 then total_duration-(total_duration_terrestrial+total_duration_free_non_terrestrial) else 0 end) as month_pre_churn_free
...
select * from  V159_Daily_viewing_summary_churners
select * from V159_epg_data_since_oct_2011;


select count(*) from vespa_analysts.VESPA_DAILY_AUGS_20130306


select count(*) from vespa_analysts.ph1_VESPA_DAILY_AUGS_20120119
commit;
...
cb_row_ID
                ...
                ,capped_flag
               ,capped_event_end_time
from vespa_analysts.ph1_VESPA_DAILY_AUGS_20120101


select top 100 * from  vespa_analysts.ph1_VESPA_DAILY_AUGS_20120401
select top 100 * from  sk_prod.Vespa_programme_schedule where channel_name = 'BBC 1 S East' and broadcast_start_date_time_utc='2012-04-01 18:00:00'
select top 100 * from sk_prod.vespa_epg_dim where programme_trans_sk = 201204020000015017
select top 100 * from sk_prod.Vespa_programme_schedule where pk_programme_instance_dim = 201204020000007634

SELECT       as programme_trans_sk
            ,service_key
            ,Channel_Name
            ,programme_instance_name as epg_title
            ,programme_instance_duration as duration
            ...
            ,broadcast_daypart as x_broadcast_Time_Of_Day
            ,pay_free_indicator
INTO  V159_epg_data_since_oct_2011
FROM sk_prod.Vespa_programme_schedule
...
from vespa_analysts.VESPA_DAILY_AUGS_20130205 as a
left outer join V159_epg_data_since_oct_2011 as b 
on a.programme_trans_sk=b.programme_trans_sk
group by channel_name
order by total_duration desc
;

--select top 100 * from  vespa_analysts.VESPA_DAILY_AUGS_20130205 where programme_;
select  pk_programme_instance_dim as programme_trans_sk
            ,service_key
            ,Channel_Name
            ,programme_instance_name as epg_title
            ...
            ,pay_free_indicator
FROM 
WHERE cast (tx_date_utc as date) between '2013-02-05'  and  '2013-02-05' and channel_name = 'Sky1'
order by tx_date_utc


select broadcast_start_date_time_utc from sk_prod.Vespa_programme_schedule  where pk_programme_instance_dim=628993474
select * from V159_epg_data_since_oct_2011  where programme_trans_sk=628993474
select cast (tx_date_utc  as date) as ev_date , count(*) as records from V159_epg_data_since_oct_2011 group by ev_date order by ev_date
-- where programme_trans_sk=628993474
select count(*) from vespa_analysts.ph1_VESPA_DAILY_AUGS_20120813
select count(*) from vespa_analysts.VESPA_DAILY_AUGS_20120814
