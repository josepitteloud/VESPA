...
--select count(*) from  vespa_analysts.VESPA_DAILY_AUGS_20130401;
--select top 5000 * into V159_Daily_viewing_summary_churners_test from V159_Daily_viewing_summary_churners;commit;
commit;
...
select po_dt ,count(*) ,sum(case when po_dt-pre_po_dt=32 then 1 else 0 end) as days_to from v159_all_vespa_churners as b group by po_dt order by po_dt

select po_dt-pre_po_dt as days_to ,count(*)  from v159_all_vespa_churners as b group by days_to order by days_to

select pre_po_dt as days_to ,pre_po_dt+32 as extra ,count(*)  from v159_all_vespa_churners as b where po_dt = '2013-04-02' group by days_to,extra order by days_to
...
select po_dt , count(*)  from v159_all_vespa_churners as b
