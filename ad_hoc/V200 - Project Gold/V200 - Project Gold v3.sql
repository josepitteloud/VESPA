...
,min(h_residence_type_v2) as household_property_type
...
,a.household_ownership_type
,a.household_property_type
,a.affluence_septile
...
select household_ownership_type
,count(*) as UK_Household 
, sum(currently_active_hh) as active_uk_hh
,sum(case when currently_active_hh=1 and  PROD_LATEST_ENTITLEMENT_PREM_SPORTS>0 then 1 else 0 end ) as active_uk_hh_on_sports

, sum(case when currently_active_hh=1 and analogue_customer=1 then 1 else 0 end ) as active_uk_hh_analogue_customer
, sum(case when currently_active_hh=1 and analogue_customer=1 and  PROD_LATEST_ENTITLEMENT_PREM_SPORTS>0 then 1 else 0 end ) as active_uk_hh_analogue_customer_on_sports

,sum(case when currently_active_hh=0 and (total_syscan_accounts_at_hh+total_cuscan_accounts_at_hh)>0  then 1 else 0 end ) as inactive_uk_hh_previously_active
,sum(case when currently_active_hh=0 and (total_syscan_accounts_at_hh+total_cuscan_accounts_at_hh)>0 and  PROD_LATEST_ENTITLEMENT_PREM_SPORTS>0 then 1 else 0 end ) as inactive_uk_hh_previously_active_had_sports
...
from #uk_view
group by household_ownership_type
order by household_ownership_type;
commit;

---Lapsed Household Analysis--

select EPG.Genre_Description
       , EPG.Sub_Genre_Description
,EPG.Channel_Name
,count(*) as records
from 
sk_prod.Vespa_programme_schedule EPG
WHERE  cast(broadcast_start_date_time_utc as date) = '2013-08-31'
GROUP  BY EPG.Genre_Description
       , EPG.Sub_Genre_Description
,EPG.Channel_Name
;
...
sk_prod.Vespa_programme_schedule EPG
where
cast(broadcast_start_date_time_utc as date) = '2013-08-31' 
and channel_name = 'Sky Sports 1' 
