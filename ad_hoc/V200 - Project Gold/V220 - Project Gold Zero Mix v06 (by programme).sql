...
,sum(case when ent_cat_prod_changed='Y' and new_contribution>old_contribution then 1 else 0 end) as all_upgrades
,sum(case when ent_cat_prod_changed='Y' and new_contribution<old_contribution then 1 else 0 end) as all_downgrades
,max(case when ent_cat_prod_changed='Y' and new_contribution>old_contribution then effective_from_dt else null end) as date_of_last_upgrade
,max(case when ent_cat_prod_changed='Y' and new_contribution<old_contribution then effective_from_dt else null end) as date_of_last_downgrade
...
,box_type_group=case          when box_type in ('A) HD Combi','C) HDx Combi','E) Sky+ Combi','G) Multiroom') then 'a) Multiple Boxes'  
                        when box_type in ('B) HD','D) HDx','E) Sky+ Combi','F) Sky+') then 'b) Single Non-FDB Box' 
                        when box_type in ('H) FDB') then 'c) FDB Box' else 'b) Single Non-FDB Box'  end ---Set non-Matches to Most popular group
...
commit;
---Produce Pivot of Activity for Output----
...
select non_premium_pay_duration
,count(*) as accounts
from v200_zero_mix_full_account_list
group by non_premium_pay_duration
order by non_premium_pay_duration
