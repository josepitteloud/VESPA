...
,max(case when watched_15min_plus=1 then 1 else 0 end) as did_watch
,min(case when arsenal_fixture=1 then '01: Arsenal' when chelsea_fixture=1 then '02: Chelsea' 
when man_city_fixture=1 then '03: Man City' when manchester_united_fixture=1 then '04: Man Utd' else '06: Other' end) as fixture_type
into #summary_by_fixture
from v223_all_accounts_possible_and_actual_CL_days
group by account_number
,channel_name_inc_hd_staggercast_channel_families
,broadcast_start_date_time_local
...
/*
select * from #summary_by_fixture;
...
from dbarnett.v223_sports_epg_lookup_aug_12_jul_13 
where left (programme_instance_name,3) = 'UCL'

--programme_sub_genre_type='Champions League'
group by programme_instance_name
,channel_name_inc_hd_staggercast_channel_families
,broadcast_start_date_time_local
,programme_sub_genre_type
...
,broadcast_start_date_time_local
from dbarnett.v223_sports_epg_lookup_aug_12_jul_13 
where channel_name_inc_hd_staggercast_channel_families='ITV1'
...
,programme_instance_name
,min(cast(broadcast_start_date_time_local as date)) as bcast_date
from dbarnett.v223_sports_epg_lookup_aug_12_jul_13 
where (programme_instance_name like '%Live%' or  programme_instance_name like 'UCL%') and broadcast_start_date_time_local<'2013-06-01'
group by  programme_sub_genre_type
,programme_instance_name
order by  programme_sub_genre_type
,bcast_date
,programme_instance_name
...
,broadcast_start_date_time_local
,channel_name_inc_hd_staggercast_channel_families
,sum(could_have_watched) as could_watch_fixture
,sum(did_watch) as did_watch_fixture
from #summary_by_rights_broadcast_time
