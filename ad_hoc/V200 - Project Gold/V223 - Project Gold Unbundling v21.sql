...
commit;
--exec gen_create_table 'dbarnett.v223_Unbundling_pivot_activity_data_cut_down_for_powerpivot'
--select top 10 * from dbarnett.v223_Unbundling_pivot_activity_data_cut_down_for_powerpivot 

/*
select decile_prog_engaged_niche_sports_sky_sports 
,decile_prog_engaged_niche_sports_non_sky_sports 
,count(*)
from dbarnett.v223_Unbundling_pivot_activity_data
group by decile_prog_engaged_niche_sports_sky_sports 
,decile_prog_engaged_niche_sports_non_sky_sports 
order by decile_prog_engaged_niche_sports_sky_sports 
,decile_prog_engaged_niche_sports_non_sky_sports 
*/
...
select minutes_wwe_sky_sports
,case when (minutes_wwe_sky_sports *account_weight)<0 then 0 else (minutes_wwe_sky_sports *account_weight) end  as minutes_wwe_sky_sports_total
,case when (minutes_sport_sky_sports *account_weight)<0 then 0 else (minutes_sport_sky_sports *account_weight) end  as minutes_sport_sky_sports_total

 from v223_unbundling_viewing_summary_by_account where minutes_wwe_sky_sports_total>=minutes_sport_sky_sports_total

select account_number	,total_viewing_duration_sports	,viewing_duration_Sky_Sports_total	
,viewing_duration_WWE	,viewing_duration_Sky_Sports_WWE	,viewing_duration_SBO_WWE	,viewing_duration_Sky_1_or_2_WWE	,account_weight, days_with_viewing
 from v223_unbundling_viewing_summary_by_account
where percentile_minutes_wwe_sky_sports	=1 and percentile_minutes_sport_sky_sports=40

commit;

---Total Amount of programming of Sport by genre/sub genre--
--drop table #programmes_deduped;
select programme_sub_genre_type
,channel_name_inc_hd_staggercast_channel_families
,broadcast_start_date_time_utc
,genre_description
,sub_genre_description
,programme_instance_duration
into #programmes_deduped
from dbarnett.v223_sports_epg_lookup_aug_12_jul_13 
where channel_name_inc_hd_staggercast_channel_families not in ('Anytime','SBO')
group by programme_sub_genre_type
,channel_name_inc_hd_staggercast_channel_families
,broadcast_start_date_time_utc
,genre_description
,sub_genre_description
...
,genre_description
,sub_genre_description
,sum(programme_instance_duration) as total_programme_duration
,count(*) as programmes
from #programmes_deduped
group by programme_sub_genre_type
--,channel_name_inc_hd_staggercast_channel_families
,genre_description
,sub_genre_description
order by total_programme_duration desc
;

select * from #programmes_deduped where programme_sub_genre_type='WWE'
...
select sum(account_weight) from dbarnett.v223_Unbundling_pivot_activity_data
