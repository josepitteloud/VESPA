select rank() over ( partition by subscriber_id
       ...
       ,case
        ...
        ,case
         ...
         ,genre_description
                 ,service_key
into VEA_21_27_Jan_Viewing_Events_Subscriber_Null
...
and Duration > 6
--32,997 Row(s) affected

-- Remove events that have second and later programmes
delete from VEA_21_27_Jan_Viewing_Events_Subscriber_Null where Program_Order > 1
--18,769 Row(s) affected
...
select rank() over ( partition by subscriber_id
       ...
       ,case
        ...
        ,case
         ...
         ,genre_description
         ,service_key
into VEA_21_27_Jan_Viewing_Events_account_number_null
from sk_prod.vespa_events_all
where panel_id = 12
and EVENT_START_DATE_TIME_UTC   >= '2013-01-21 00:00:00'
and EVENT_START_DATE_TIME_UTC   <= '2013-01-27 23:59:59'
...
and Duration > 6
--7,605,628 Row(s) affected

-- Remove events that have second and later programmes
delete from VEA_21_27_Jan_Viewing_Events_account_number_null where Program_Order > 1
--2,661,977 Row(s) affected
...
select rank() over ( partition by subscriber_id
       ...
       ,case
        ...
        ,case
         ...
         ,genre_description
         ,service_key
into VEA_21_27_Jan_Viewing_Events_Both_null
from sk_prod.vespa_events_all
where panel_id = 12
and EVENT_START_DATE_TIME_UTC   >= '2013-01-21 00:00:00'
and EVENT_START_DATE_TIME_UTC   <= '2013-01-27 23:59:59'
...
and account_number is null
and Duration > 6
--4,041 Row(s) affected

-- Remove events that have second and later programmes
delete from VEA_21_27_Jan_Viewing_Events_Both_null where Program_Order > 1
--2,156 Row(s) affected
