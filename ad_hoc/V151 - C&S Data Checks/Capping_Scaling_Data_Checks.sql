/*
 ...
 Capping & Scaling data checks between Vespa and Production environments

*/

------------------------------------------------------------------------------------------------------------

select min(event_start_date_time_utc),max(event_start_date_time_utc) from sk_prod.VESPA_DP_PROG_VIEWED_CURRENT

select date(event_start_date_time_utc),count(*)
from sk_prod.VESPA_DP_PROG_VIEWED_CURRENT
where event_start_date_time_utc >= '2012-02-25 00:00:00'
and event_start_date_time_utc <= '2012-03-03 23:59:59'
...
group by    Viewing_Type_Detailed
           ...
           ,Event_Start_hour
--672 Row(s) affected
select * from Duration_After_Capping_Old

--Total Viewing Time After Capping for the old structure
select sum(datediff(second,EVENT_START_DATE_TIME_UTC,coalesce(Capped_Event_End_Time,EVENT_END_DATE_TIME_UTC)))*1.0/3600 as Sum_Duration_hour_Old
from VEA_21_27_Jan_Viewing_Events_Final
...
select sum(datediff(second,EVENT_START_DATE_TIME_UTC,coalesce(Capping_end_date_time_utc,EVENT_END_DATE_TIME_UTC)))*1.0/3600 as Sum_Duration_hour_New
from VEA_21_27_Jan_Viewing_Events_Final
