...
are adsmartable (adsmart_account_numbers) and also to find those boxes that are adsmart_capabale (adsmart_capable_account_numbers).
The basic difference between the two tables is that for the latter is that the constraint relating to viewing consent
cust_viewing_data_capture_allowed is removed (it has been set to 'Y' for the former).
