...
-- ####################################################################################################
-- ####################################################################################################
--            This version uses a CUSTOM INPUT table - "Scaling3_00_Input"
-- ####################################################################################################
-- ####################################################################################################
-- ####################################################################################################
...
--Part 1 - First fill the table:
  create table Scaling3_00_Input(account_number varchar(30)
...
;
  ...
  select account_number
   ...
   where movement in ('Account to add to panels 6/7, eventually for panel 12','Account to add to panel 12 from panel 6 or 7')
;
...
;
  ...
  select min(coalesce(cbk_day,1)) as dy
        ,cur.account_number
        ,movement
    into #temp
    from #current as cur
         left join panbal_amends as ame on cur.account_number = ame.account_number
         left join vespa_analysts.waterfall_scms_callback_data as cal on ame.account_number = cal.account_number
   where (movement is null or movement in ('Account to remove from panel 12','Leave on panel 12'))
     and rq <= 0.5
group by cur.account_number
        ,movement
;
...
--598173
  ...
  insert into Scaling3_00_Input(account_number
                               ,subscriber_id)
  select acc.account_number
        ,card_subscriber_id
    from #accounts as acc
         inner join #sky_box_count as sbc on acc.account_number = sbc.account_number
;

  update Scaling3_00_Input as bas
...
--(run "SC3 - 0 - table creation.sql" (in iSQL)

--Part 3 run the code to create the prcoedures (from Part L down)

--Part 4 run this code to run the procedures
begin

    declare @build_ID int
    declare @varStartDate date
    set @build_ID='999'
    set @varStartDate = '2013-10-17'

    commit

    execute SC3_v1_1__do_weekly_segmentation_CUSTOM @varStartDate, @build_ID, now()
    commit

    execute SC3_v1_1__scale_Vespa_panel_CUSTOM      @varStartDate, now(), @build_ID
    commit

end;

--Part 5 calculate effective sample size for the current panel
  select account_number
    into #sbv
...
;
  ...
  select sum(calculated_scaling_weight * calculated_scaling_weight) as large
   ...
   where adjusted_event_start_date_vespa = '2013-10-03'
;

select (small * small)/large from #ess1; --180365.131605266

--Part 6 calculate effective sample size for the proposed panel
  select sum(weighting * weighting) as large
        ,sum(weighting) as small
    into #ess2
    from SC3_Todays_panel_members  as tpm
         inner join SC3_Weightings as wei on tpm.scaling_segment_id = wei.scaling_segment_id
   where scaling_day='2013-10-03'
;

select (small * small)/large from #ess2; --521789.061492216


select count(1) from SC3_Todays_panel_members;
...
IF object_id('SC3_v1_1__do_weekly_segmentation_CUSTOM') IS NOT NULL THEN DROP PROCEDURE SC3_v1_1__do_weekly_segmentation_CUSTOM END IF;

create procedure SC3_v1_1__do_weekly_segmentation_CUSTOM
...
begin
     ...
     execute citeam.logger_add_event @Scale_refresh_logging_ID, 3, 'SC3: Profiling Sky UK base as of ' || dateformat(@profiling_thursday,'yyyy-mm-dd') || '.'
     commit
     ...
     begin
         ...
         if @tablespacename = 'vespa_analysts'
             EXECUTE citeam.logger_create_run 'ScalingSegmentation'           , 'SC3: Segmentation build for ' || dateformat(@profiling_thursday, 'yyyy-mm-dd') || '.', @Scale_refresh_logging_ID output
         else
         begin
             set @tablespacename = coalesce(@tablespacename, user)
             EXECUTE citeam.logger_create_run 'SC3 Dev ' || @tablespacename || 'SC3: Segmentation build for  ' || dateformat(@profiling_thursday, 'yyyy-mm-dd') || '.', @Scale_refresh_logging_ID output
         end
     end
     commit
...
--              ,h_household_composition -- may as well pull out the item we need given we're ranking and deleting
     ...
     INSERT INTO SC3_scaling_weekly_sample (
         ...
         ,vespa_universe  -- Need to include default
         ,isba_tv_region
         ,hhcomposition
         ,tenure
         ,num_mix
     ...
     select  account_number
            ,case
                ...
                when flag = 1 and cust_viewing_data_capture_allowed = 'N' then 'Adsmartable but no consent'
     ...
     commit
     execute citeam.logger_add_event @Scale_refresh_logging_ID, 3, 'L02: Midway 2a/8 (Accounts with no universe)', coalesce(@QA_catcher, -1)
     commit
     ...
     */
     ...
     commit
     execute citeam.logger_add_event @Scale_refresh_logging_ID, 3, 'L02: Complete! (Variables)', coalesce(@QA_catcher, -1)
     commit
     ...
     commit
     execute citeam.logger_add_event @Scale_refresh_logging_ID, 3, 'L03a: Midway (Population Segment lookup)', coalesce(@QA_catcher, -1)
     commit
     ...
     commit
     execute citeam.logger_add_event @Scale_refresh_logging_ID, 3, 'L03b: Midway (Weighting Segment lookup)', coalesce(@QA_catcher, -1)
     commit
     ...
     execute citeam.logger_add_event @Scale_refresh_logging_ID, 3, 'L03: Complete! (Segment ID assignment)', coalesce(@QA_catcher, -1)
     commit
     ...
     commit
     execute citeam.logger_add_event @Scale_refresh_logging_ID, 3, 'L04: Complete! (Segments published)', coalesce(@QA_catcher, -1)
     commit
     ...
     execute citeam.logger_add_event @Scale_refresh_logging_ID, 3, 'SC3: base segmentation complete!'
     commit

end; -- of procedure "SC3_v1_1__do_weekly_segmentation_CUSTOM"
commit;
...
IF object_id('SC3_v1_1__prepare_panel_members_CUSTOM') IS NOT NULL THEN DROP PROCEDURE SC3_v1_1__prepare_panel_members_CUSTOM END IF;

create procedure SC3_v1_1__prepare_panel_members_CUSTOM
...
begin
     ...
     commit
     execute citeam.logger_add_event @Scale_refresh_logging_ID, 3, 'SC3: Deciding panel members for ' || dateformat(@scaling_day,'yyyy-mm-dd') || ' using profiling of ' || dateformat(@profiling_date,'yyyy-mm-dd') || '.'
     commit
...
select top 10 * from Scaling3_00_Input where subscriber_id is null
     insert into #raw_logs_dump
     select distinct account_number, service_instance_id
     from Scaling3_00_Input
...
--601935
     ...
     from #raw_logs_dump
     commit
     execute citeam.logger_add_event @Scale_refresh_logging_ID, 3, 'A01: Midway 1/2 (Log extracts)', coalesce(@QA_catcher, -1)
     commit
...
--522703
     ...
     commit
     execute citeam.logger_add_event @Scale_refresh_logging_ID, 3, 'A01: Complete! (Panel members)', coalesce(@QA_catcher, -1)
     commit

     execute citeam.logger_add_event @Scale_refresh_logging_ID, 3, 'SC3: panel members prepared!'
     commit

end; -- of procedure "SC3_v1_1__prepare_panel_members_CUSTOM"
commit;



IF object_id('SC3_v1_1__make_weights_CUSTOM') IS NOT NULL THEN DROP PROCEDURE SC3_v1_1__make_weights_CUSTOM END IF;

create procedure SC3_v1_1__make_weights_CUSTOM
...
begin
     ...
     execute citeam.logger_add_event @Scale_refresh_logging_ID, 3, 'SC3: Making weights for ' || dateformat(@scaling_day,'yyyy-mm-dd') || ' using profiling of ' || dateformat(@profiling_date,'yyyy-mm-dd') || '.'
     commit
     ...
     inner join vespa_analysts.SC3_Segments_lookup_v2_1 as sl
     ...
     commit
     execute citeam.logger_add_event @Scale_refresh_logging_ID, 3, 'B01: Complete! (Segmentation totals)', coalesce(@QA_catcher, -1)
     commit
     ...
     SET @iteration      = 0
     SET @cntr_var       = 1
     SET @scaling_var    = (SELECT scaling_variable FROM vespa_analysts.SC3_Variables_lookup_v2_1 WHERE id = @cntr)
     ...
     BEGIN
             ...
             WHILE @cntr_var < 6
             BEGIN
                         SELECT @scaling_var = scaling_variable FROM vespa_analysts.SC3_Variables_lookup_v2_1 WHERE id = @cntr_var

                         EXECUTE('
                         INSERT INTO SC3_category_working_table (universe,profile,value,sky_base_accounts,vespa_panel,sum_of_weights)
                             SELECT  srs.universe
                                    ,@scaling_var
                                    ,ssl.'||@scaling_var||'
                             ...
                             FROM SC3_weighting_working_table AS srs
                                     inner join vespa_analysts.SC3_Segments_lookup_v2_1 AS ssl ON srs.scaling_segment_id = ssl.scaling_segment_id
             ...
             SELECT @scaling_var = scaling_variable FROM vespa_analysts.SC3_Variables_lookup_v2_1 WHERE id = @cntr
             ...
             FROM SC3_weighting_working_table
                     inner join vespa_analysts.SC3_Segments_lookup_v2_1 AS ssl ON SC3_weighting_working_table.scaling_segment_id = ssl.scaling_segment_id
     ...
     commit
     execute citeam.logger_add_event @Scale_refresh_logging_ID, 3, 'B02: Midway (Iterations)', coalesce(@QA_catcher, -1)
     commit

     -- Now push convergence details out to the tracking tables: the first one provides a convergence summary at a category level

     INSERT INTO SC3_category_subtotals (scaling_date,universe,profile,value,sky_base_accounts,vespa_panel,category_weight
     ...
     commit
     execute citeam.logger_add_event @Scale_refresh_logging_ID, 3, 'B02: Complete (Calculate weights)', coalesce(@QA_catcher, -1)
     commit
     ...
     commit
     execute citeam.logger_add_event @Scale_refresh_logging_ID, 3, 'B03: Midway 1/4 (Midway weights)', coalesce(@QA_catcher, -1)
     commit
     ...
     commit
     execute citeam.logger_add_event @Scale_refresh_logging_ID, 3, 'B03: Midway 2/4 (Midway intervals)', coalesce(@QA_catcher, -1)
     commit
     ...
     commit
     execute citeam.logger_add_event @Scale_refresh_logging_ID, 3, 'B03: Midway 3/4 (VIQ interface)', coalesce(@QA_catcher, -1)
     commit

     execute citeam.logger_add_event @Scale_refresh_logging_ID, 3, 'B03: Complete! (Publish weights)'
     commit
     execute citeam.logger_add_event @Scale_refresh_logging_ID, 3, 'SC3: Weights made for ' || dateformat(@scaling_day, 'yyyy-mm-dd')
     commit

end; -- of procedure "SC3_v1_1__make_weights_CUSTOM"
commit;
...
-- table.
IF object_id('SC3_v1_1__scale_Vespa_panel_CUSTOM') IS NOT NULL THEN DROP PROCEDURE SC3_v1_1__scale_Vespa_panel_CUSTOM END IF;

create procedure SC3_v1_1__scale_Vespa_panel_CUSTOM
...
begin
    ...
    begin
        ...
        EXECUTE citeam.logger_create_run @log_title, 'SC3: Scaling initiated ('|| dateformat(@scaling_day, 'yyyy-mm-dd') ||')', @Scale_refresh_logging_ID output
    ...
    -- Do all the work!
    EXECUTE SC3_v1_1__prepare_panel_members_CUSTOM   @scaling_day, @batch_date, @Scale_refresh_logging_ID
    ...
    commit
    EXECUTE SC3_v1_1__make_weights_CUSTOM            @scaling_day, @batch_date, @Scale_refresh_logging_ID
    commit
...
--    EXECUTE SC3_v1_1__daily_QA_units          @Scale_refresh_logging_ID
    commit
    execute citeam.logger_add_event @Scale_refresh_logging_ID, 3, 'SC3: Scaling completed ('|| dateformat(@scaling_day, 'yyyy-mm-dd') ||')'
    commit

end; -- of procedure "SC3_v1_1__scale_Vespa_panel_CUSTOM"
...
--grant execute on SC3_v1_1__do_weekly_segmentation_CUSTOM     to CITeam, vespa_analysts, sk_prod, sk_prodreg;
--grant execute on SC3_v1_1__prepare_panel_members_CUSTOM      to CITeam, vespa_analysts, sk_prod, sk_prodreg;
--grant execute on SC3_v1_1__make_weights_CUSTOM               to CITeam, vespa_analysts, sk_prod, sk_prodreg;
--grant execute on SC3_v1_1__daily_QA_units             to CITeam, vespa_analysts, sk_prod, sk_prodreg;
--grant execute on SC3_v1_1__scale_Vespa_panel_CUSTOM          to CITeam, vespa_analysts, sk_prod, sk_prodreg;
