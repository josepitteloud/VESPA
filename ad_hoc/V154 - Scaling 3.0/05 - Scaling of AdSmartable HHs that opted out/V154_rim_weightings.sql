...
be found in V154_segments_aggregated, with the SQL for this table coming from
vespa_sample
...
if object_id('V154_weighting_working_table') is not null drop table V154_weighting_working_table
CREATE TABLE V154_weighting_working_table (
...
CREATE HG INDEX indx_un on V154_weighting_working_table(sky_base_universe)

COMMIT

--------------------------------------------------------------- E02 - SC2_category_subtotals
-- This table contains historic information and should not be deleted
if object_id('V154_category_subtotals') is not null drop table V154_category_subtotals
CREATE TABLE V154_category_subtotals (
     scaling_date           date
    ,sky_base_universe      VARCHAR(50)
    ,profile                VARCHAR(50)
    ,value                  VARCHAR(70)
...
create index indx_date on V154_category_subtotals(scaling_date)
create hg index indx_universe on V154_category_subtotals(sky_base_universe)
create hg index indx_profile on V154_category_subtotals(profile)
...
if object_id('V154_category_working_table') is not null drop table V154_category_working_table
CREATE TABLE V154_category_working_table (
     sky_base_universe      VARCHAR(50)
    ,profile                VARCHAR(50)
    ,value                  VARCHAR(70)
    ,sky_base_accounts      DOUBLE
...
create hg index indx_universe on V154_category_working_table(sky_base_universe)
create hg index indx_profile on V154_category_working_table(profile)
create hg index indx_value on V154_category_working_table(value)

COMMIT
--go

truncate table V154_weighting_working_table

INSERT INTO V154_weighting_working_table (scaling_segment_id, sky_base_universe, sky_base_accounts, vespa_panel)
select updated_scaling_segment_id as scaling_segment_id, sky_base_universe, sky_base_accounts, vespa_accounts
     from V154_segments_aggregated
    where sky_base_accounts > 0
commit
...
-- individually until the category sum of weights converge to the population category subtotals
     ...
     SET @iteration      = 0
     SET @cntr_var       = 1
     SET @scaling_var    = (SELECT scaling_variable FROM glasera.V154_Variables_lookup_v1_1 WHERE id = @cntr)
     set @scaling_count  = (select count(*) from glasera.V154_Variables_lookup_v1_1)
--      SET @scaling_var    = (SELECT scaling_variable FROM vespa_analysts.SC2_Variables_lookup_v2_1 WHERE id = @cntr)

-- The V154_weighting_working_table table contains subtotals and sum_of_weights for all segments represented by
-- the sky base.
...
update V154_weighting_working_table
...
commit
     ...
     -- In this scenario, the person running the code should send the results of the V154_metrics for that
     -- week to analytics team for review. ## What exactly are we checking? can we automate any of it?

     WHILE @cntr <= @scaling_count
     BEGIN
             DELETE FROM V154_category_working_table

             SET @cntr_var = 1
             WHILE @cntr_var <= @scaling_count
             BEGIN
                         SELECT @scaling_var = scaling_variable FROM glasera.V154_Variables_lookup_v1_1 WHERE id = @cntr_var
--                          SELECT @scaling_var = scaling_variable FROM vespa_analysts.SC2_Variables_lookup_v2_1 WHERE id = @cntr_var

                         EXECUTE('
                         INSERT INTO V154_category_working_table (sky_base_universe,profile,value,sky_base_accounts,vespa_panel,sum_of_weights)
                             SELECT  srs.sky_base_universe
                              ...
                              FROM V154_weighting_working_table AS srs
                                     inner join V154_segments_aggregated AS ssl ON srs.scaling_segment_id = ssl.updated_scaling_segment_id
             ...
             UPDATE V154_category_working_table
             ...
             SELECT @convergence = SUM(convergence_flag) FROM V154_category_working_table
             SET @iteration = @iteration + 1
             SELECT @scaling_var = scaling_variable FROM glasera.V154_Variables_lookup_v1_1 WHERE id = @cntr
--              SELECT @scaling_var = scaling_variable FROM vespa_analysts.SC2_Variables_lookup_v2_1 WHERE id = @cntr

             EXECUTE('
             UPDATE V154_weighting_working_table
             SET  V154_weighting_working_table.category_weight = sc.category_weight
                 ,V154_weighting_working_table.sum_of_weights  = V154_weighting_working_table.sum_of_weights * sc.category_weight
             FROM V154_weighting_working_table
                     inner join V154_segments_aggregated AS ssl ON V154_weighting_working_table.scaling_segment_id = ssl.updated_scaling_segment_id
                     inner join V154_category_working_table AS sc ON sc.value = ssl.'||@scaling_var||'
     ...
     SELECT @sky_base = SUM(sky_base_accounts) FROM V154_weighting_working_table
     SELECT @vespa_panel = SUM(vespa_panel) FROM V154_weighting_working_table
     SELECT @sum_of_weights = SUM(sum_of_weights) FROM V154_weighting_working_table
     ...
     INSERT INTO V154_category_subtotals (scaling_date,sky_base_universe,profile,value,sky_base_accounts,vespa_panel,category_weight
     ...
     FROM V154_category_working_table

     -- The V154_metrics table contains metrics for a particular scaling date. It shows whether the
    ...
    IF object_id('V154_full_accounts_aggregated') IS NOT NULL DROP TABLE V154_full_accounts_aggregated
    select wei.scaling_segment_id, agg.*, wei.segment_weight
            into V154_full_accounts_aggregated
            from V154_segments_aggregated agg
      inner join V154_weighting_working_table wei
              on agg.updated_scaling_segment_id = wei.scaling_segment_id
    commit
