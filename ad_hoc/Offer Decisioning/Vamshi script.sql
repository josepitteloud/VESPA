...
WHERE effective_to_dt >=  @start_dt 
group by
        a.account_number,
        a.subscription_id,
        a.currency_code,
...
from (
    ...
    from (
            ...
            from    cust_bills cb
                ...
                where           cb.payment_due_dt >= @start_dt 
...
select a.account_number,
   ...
   --    into MCKINSEY_FULL_CALLS_HIST_NEW_1
       ...
       where cast(created_date as date) >= @start_dt
...
select a.account_number,
     ...
     --  into MCKINSEY_FULL_CONTACT_HIST
       from cust_contact a
       where cast(created_date as date) BETWEEN  @start_dt AND @end_dt
       and contact_channel = 'I PHONE COMMUNICATION'
       and contact_grouping_identity is not null
       group by a.account_number, event_date
...
select a.account_number,
       ...
       where typeofevent = 'TA'
       and event_dt BETWEEN  @start_dt AND @end_dt
       group by a.account_number, event_date
...
********************************************************************* */
select DISTINCT a.* into NORYD.MCKINSEY_CONTRACT_HIST
...
from CITEAM.offer_usage_all;----106164661 Row(s) affected
 WHERE  created_dt BETWEEN  @start_dt AND @end_dt
...
select account_number,
  ...
  --     into MCKINSEY_FULL_BOX_HIST
	   ...
	   WHERE box_replaced_dt >= @start_dt
...
--      into MCKINSEY_FULL_SIMPLE_SEGMENT_HIST
	   ...
	   WHERE observation_date >=  DATEADD(MONTH,-3, @start_dt)
       group by account_number,
       segment,
       segment_lev2,
...
WHERE created_date  BETWEEN  @start_dt AND @end_dt
...
AS (
	SELECT acq.account_number
		...
		, RTM_LEVEL_3
		, row_number() OVER (
			PARTITION BY acq.account_number
			, acq.event_dt ORDER BY ord.order_dt DESC
				, order_number DESC
			) Order_Rnk
