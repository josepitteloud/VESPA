...
create OR REPLACE procedure SC3_v1_1__prepare_panel_members
...
--     execute logger_add_event @Scale_refresh_logging_ID, 3, 'A01: Midway 1/2 (Log extracts)', coalesce(@QA_catcher, -1)
     ...
     -- First moving the unique account numbers in...
        ...
        SELECT
                        o.account_number
                ,       scaling_segment_id
        FROM #panel_options o, SC_MANUAL_PANEL_ACCOUNT_LIST m
...
--     execute citeam.logger_add_event @Scale_refresh_logging_ID, 3, 'SC3: Making weights for ' || dateformat(@scaling_day,'yyyy-mm-dd') || ' using profiling of ' || dateformat(@profiling_date,'yyyy-mm-dd') || '.'
     ...
     commit
        ...
        update SC3_weighting_working_table    as      w
...
--      SET @scaling_var    = (SELECT scaling_variable FROM vespa_analysts.SC2_Variables_lookup_v2_1 WHERE id = @cntr)
     ...
     -- week to analytics team for review. ## What exactly are we checking? can we automate any of it?
        ...
        BEGIN
                ...
                TRUNCATE TABLE SC3_category_working_table
                COMMIT
                ...
                WHILE @cntr_var <= @scaling_count
                        BEGIN
                                ...
                                EXECUTE('
                                        INSERT INTO SC3_category_working_table(
                                                        sky_base_universe
                                                ,       profile
                                                ,       value
                                                ,       sky_base_accounts
                                        ...
                                        FROM
                                                                        SC3_weighting_working_table AS srs
                ...
                EXECUTE('
                        UPDATE  SC3_weighting_working_table
                        SET
                                        SC3_weighting_working_table.category_weight    =       sc.category_weight
                        ...
                        FROM
                                ...
                                inner join      vespa_analysts.SC3_Segments_lookup_v1_1        AS      ssl             ON      SC3_weighting_working_table.scaling_segment_id =       ssl.scaling_segment_id
