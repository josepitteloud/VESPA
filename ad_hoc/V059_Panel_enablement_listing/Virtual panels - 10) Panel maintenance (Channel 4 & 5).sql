...
commit;
  ...
  -- ##### Load additional accounts manually (top up - probably one-off procedure) #####
-- insert into VirtPan_Channel45_01_Universe_Selection
--             (Account_Number, Subscriber_id, Source, Random_Num)
--   select
--         a.Account_Number,
--         cast(b.card_subscriber_id as int),
--         'Manual top up - 01/03/2013',
--         rand(number(*) * @var_multiplier)
--     from bednaszs.VirtPan_13_Selection_20130301_Final a,
--          sk_prod.cust_card_subscriber_link b left join VirtPan_Channel45_01_Universe_Selection c
--                                                     on cast(b.card_subscriber_id as int) = c.Subscriber_id
--                                                    and c.Run_Date = today()
--    where b.current_flag = 'Y'
--      and a.Account_Number = b.Account_Number
--      and c.Subscriber_Id is null;                                                     -- Only add new Suscriber Ids
-- commit;
--
...
select count(distinct Account_Number) as Cnt_Accounts, count(*) as Cnt_Boxes
  from VirtPan_Channel45_01_Universe_Selection
 where Channel4_Panel_Flag = 1;

select count(distinct Account_Number) as Cnt_Accounts, count(*) as Cnt_Boxes
  from VirtPan_Channel45_01_Universe_Selection
 where Channel5_Panel_Flag = 1;
...
select
vp1, vp2 ,Vespa_Panel, count(*) as cnt
from vespa_analysts.vespa_broadcast_reporting_vp_map base
group by vp1, vp2 ,Vespa_Panel
