...
if object_id('CP2_box_lookup') is not null then drop table CP2_box_lookup end if;
...
if object_id('CP2_relevant_boxes') is not null then drop table CP2_relevant_boxes end if;
...
if object_id('CP2_calculated_viewing_caps') is not null then drop table CP2_calculated_viewing_caps end if;
...
create unique index forcing_uniqueness on CP2_calculated_viewing_caps
    (event_start_hour, event_start_day, initial_genre, box_subscription, pack_grp, live_timeshifted_events);
...
-- a good thing.
if object_id('CP2_capping_buckets') is not null then drop table CP2_capping_buckets end if;
...
create unique index for_uniqueness on CP2_capping_buckets
    (event_start_hour, event_start_day, initial_genre, live_timeshifted_events);
...
if object_id('CP2_capped_data_holding_pen') is not null then drop table CP2_capped_data_holding_pen end if;
create table CP2_capped_data_holding_pen (
    ...
    ,timeshifting               varchar(20)                         -- 'LIVE' or 'VOSDAL' (same day as live) or 'PLAYBACK7' (playback within 7 days) or 'PLAYBACK28' (otherwise)
...
-- each capping strand
IF object_id('CP2_QA_viewing_control_totals') IS NOT NULL then DROP TABLE CP2_QA_viewing_control_totals end if;
create table CP2_QA_viewing_control_totals (
    ...
    ,primary key (build_date, data_state, program_air_date, live_timeshifted_events, genre)
);
...
IF object_id('CP2_QA_viewing_control_distribs') IS NOT NULL then DROP TABLE CP2_QA_viewing_control_distribs end if;
...
IF object_id('CP2_QA_event_control_distribs') IS NOT NULL then DROP TABLE CP2_QA_event_control_distribs end if;
...
IF object_id('CP2_QA_daily_average_viewing') IS NOT NULL then DROP TABLE CP2_QA_daily_average_viewing end if;
...
IF object_id('CP2_QA_viewing_control_cap_distrib') IS NOT NULL then  DROP TABLE CP2_QA_viewing_control_cap_distrib end if;
