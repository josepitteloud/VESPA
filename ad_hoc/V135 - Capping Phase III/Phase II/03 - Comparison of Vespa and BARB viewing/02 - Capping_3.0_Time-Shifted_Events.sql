...
-- select top 10* from Capping2_01_Viewing_Records
    --------------------------------------------------------------------------------
    -- B) - Get The viewing Data
    --------------------------------------------------------------------------------
    ...
    where live_recorded in ('LIVE','RECORDED')
      ...
      and right(account_number, 1) = '6'       -- Randomly chosen number, selected so that we have a 10% sample. Comment out for all accounts.
--       and right(account_number, 2) = '64'       -- Randomly chosen number, selected so that we have a 1% sample. Comment out for all accounts.
--       and right(account_number, 2) between '83' and '87'       -- Randomly chosen set of numbers, selected so that we have a 5% sample. Comment out for all accounts.
    commit
    ...
    */
    ...
    -------------------------------------------------------------------------------------------------
    --Bringing in the ntile correction factor in other to make teh comparison with Barb more dynamic

    if object_id('ntile_investigation') is not null drop table ntile_investigation
    create table ntile_investigation ( live_timeshifted_events integer
                             ,time_period varchar (7)
                             ,ntile_correction integer
                             )
--      Code which will later be used when we dynamically update the table
        insert into ntile_investigation
                select *
                  from capping_threshold_corrections
--         select distinct(ntile_correction) as current_value_of_ntile from ntile_investigation

--      Current optimal ntile values
--     insert into ntile_investigation values (0, '23-3', 0)
--     insert into ntile_investigation values (0, '4-14', 2)
--     insert into ntile_investigation values (0, '15-19',2)
--     insert into ntile_investigation values (0, '20-22',0)
--     insert into ntile_investigation values (1, 'null', 5)
--     insert into ntile_investigation values (2, 'null', 1)
--     insert into ntile_investigation values (3, 'null', 1)
--     insert into ntile_investigation values (4, 'null', 4)

--     insert into ntile_investigation values (0, '23-3', -5)
--     insert into ntile_investigation values (0, '4-14', -5)
--     insert into ntile_investigation values (0, '15-19',-5)
--     insert into ntile_investigation values (0, '20-22',-5)
--     insert into ntile_investigation values (1, 'null', -5)
--     insert into ntile_investigation values (2, 'null', -5)
--     insert into ntile_investigation values (3, 'null', -5)
--     insert into ntile_investigation values (4, 'null', -5)

    declare @ntile_correction integer

    select @ntile_correction = ntile_correction
      from ntile_investigation
...
--     ,sec_ntile as cap_ntile
    --Added case statement to ensure that cap_ntile is between 0 and 200 (17/12/2013)
    ,case       when pri_ntile is null and sec_ntile+@ntile_correction > 20 then sec_ntile+@ntile_correction-20
                when pri_ntile is null and sec_ntile+@ntile_correction <= 20 then 0
                when pri_ntile+@ntile_correction > sec_ntile then sec_ntile
                when pri_ntile+@ntile_correction > 200 then 200
                when pri_ntile+@ntile_correction <   0 then   0
                else pri_ntile+@ntile_correction
                end as cap_ntile
...
-------------------------------------------------------------------------------------------------------
    ...
    commit
-- select @ntile_correction as current_value_of_ntile_correction
...
--     ,sec_ntile as cap_ntile
    --Added case statement to ensure that cap_ntile is between 0 and 200 (17/12/2013)
    ,case when pri_ntile is null and sec_ntile+@ntile_correction > 25 then sec_ntile+@ntile_correction-25
          when pri_ntile is null and sec_ntile+@ntile_correction <= 25 then 0
          when event_start_hour in (4,5,10,11,12,13,14) and pri_ntile+@ntile_correction > sec_ntile then sec_ntile
          when event_start_hour in (4,5,10,11,12,13,14) and pri_ntile+@ntile_correction > 200 then 200
          when event_start_hour in (4,5,10,11,12,13,14) and pri_ntile+@ntile_correction <   0 then   0
          when event_start_hour in (4,5,10,11,12,13,14) then pri_ntile+@ntile_correction
          when event_start_hour in (6,7,8,9) and datepart(weekday,event_date) in (1,7) and pri_ntile+@ntile_correction > sec_ntile then sec_ntile
          when event_start_hour in (6,7,8,9) and datepart(weekday,event_date) in (1,7) and pri_ntile+@ntile_correction > 200 then 200
          when event_start_hour in (6,7,8,9) and datepart(weekday,event_date) in (1,7) and pri_ntile+@ntile_correction <   0 then   0
          when event_start_hour in (6,7,8,9) and datepart(weekday,event_date) in (1,7) then pri_ntile+@ntile_correction
          when event_start_hour in (6,7,8,9) and datepart(weekday,event_date) in (2,3,4,5,6) and pri_ntile+@ntile_correction > sec_ntile then sec_ntile
          when event_start_hour in (6,7,8,9) and datepart(weekday,event_date) in (2,3,4,5,6) and pri_ntile+@ntile_correction > 200 then 200
          when event_start_hour in (6,7,8,9) and datepart(weekday,event_date) in (2,3,4,5,6) and pri_ntile+@ntile_correction <   0 then   0
          when event_start_hour in (6,7,8,9) and datepart(weekday,event_date) in (2,3,4,5,6) then pri_ntile+@ntile_correction
     end as cap_ntile
    ,cast(null as integer) as min_dur_mins
    into CP2_h4_14
    from CP2_nt_4_19
...
--     ,ntile as cap_ntile
    --Added case statement to ensure that cap_ntile is between 0 and 200 (17/12/2013)
    ,case
        when ntile+@ntile_correction > ntile then ntile
        when ntile+@ntile_correction <   0   then 0
        else ntile+@ntile_correction
     end as cap_ntile
    ,cast(null as integer) as min_dur_mins
    into CP2_h15_19
    from CP2_nt_4_19
...
------------------------------------------------------------------------------------------------------
    ...
    ,max(ntile_2) as sec_ntile
--     ,sec_ntile as cap_ntile
    --Added case statement to ensure that cap_ntile is between 0 and 200
    ,case       when pri_ntile is null and sec_ntile+@ntile_correction > 20 then sec_ntile+@ntile_correction-20
                when pri_ntile is null and sec_ntile+@ntile_correction <= 20 then 0
                when pri_ntile+@ntile_correction > sec_ntile then sec_ntile
                when pri_ntile+@ntile_correction > 200 then 200
...
-- **********************************************************************************************
-- The following was added on 20/12/2012 as we are only using a 1% sample, we may have an issue when we
-- do not get viewing data that matches a certain criteria then we will get nulls in this model
    update CP2_calculated_viewing_caps
    set max_dur_mins = @min_cap_bound_minutes
    where (
        max_dur_mins < @min_cap_bound_minutes
