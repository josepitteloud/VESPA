...
create procedure CP2_build_days_caps
    ...
    ,@sample            bit = NULL      -- Do we take a sample of the data, use only when we are running during capping calibration.
as
begin
...
-- THe following is the original code used in teh capping script, however since we have to put everything into a
-- cahracter variable and execute that we can no longer have comments in our code.
-- We keep this here for legacy issues and to show the comments so that a future user has a better idea of what is going on
-- ============================================================================================================================
...
--           and subscriber_id is not null                                    -- There shouldn't be any nulls, but there are
-- ============================================================================================================================
    ...
    select @var_sql = '
        ...
        select
           ...
           ,case
            ...
            ,null                                         as followed_by_standbyin
              ,datepart(day,
                        case
                            when (EVENT_START_DATE_TIME_UTC <  ''2012-03-25 01:00:00'') then EVENT_START_DATE_TIME_UTC
                            when (EVENT_START_DATE_TIME_UTC <  ''2012-10-28 02:00:00'') then dateadd(hour, 1, EVENT_START_DATE_TIME_UTC)
                            when (EVENT_START_DATE_TIME_UTC <  ''2013-03-31 01:00:00'') then EVENT_START_DATE_TIME_UTC
                            when (EVENT_START_DATE_TIME_UTC <  ''2013-10-27 02:00:00'') then dateadd(hour, 1, EVENT_START_DATE_TIME_UTC)
                            when (EVENT_START_DATE_TIME_UTC <  ''2014-03-30 01:00:00'') then EVENT_START_DATE_TIME_UTC
                              else NULL
                          end)                            as event_start_day
              ,datepart(hour,
                        case
        ...
        where live_recorded in (''LIVE'',''RECORDED'')
          ...
          and DK_EVENT_START_DATEHOUR_DIM >= ' || @varEventStartHour || '
          and DK_EVENT_START_DATEHOUR_DIM <= ' || @varEventEndHour || '
          and subscriber_id is not null'

        -- Add to code when doing capping calibration so that we are only running on a 10% sample.
        -- May look to change the 6 to a random number in future, possibly as one of the variables that get passed into the script as you want to ensure always having the
        -- same sample during calibration.
        if @sample = 1 select @var_sql = @var_sql || ' and right(account_number, 1) = ''6'''       -- Randomly chosen number, selected so that we have a 10% sample. Comment out for all accounts.
...
--  Updating the Capping2_01_Viewing_Records table with followed_by_standbyin field
    ...
    */
    ...
    -------------------------------------------------------------------------------------------------
    --Bringing in the ntile correction factor in order to make the comparison with Barb more dynamic
    --The table capping_threshold_corrections is updated in the capping calibration part of the script
...
--
    ...
    select @ntile_correction = ntile_correction
     ...
     where live_timeshifted_events = 0
       ...
       and weekday_indicator = (case when day(@capping_date) between 2 and 6 then 1 else 0 end)
    ...
    --case statement to ensure that cap_ntile is between 0 and max_ntile
    ,case
        when max_ntile-10+@ntile_correction > max_ntile then max_ntile
        when max_ntile-10+@ntile_correction <   0   then 0
        else max_ntile-10+@ntile_correction
     end as cap_ntile
...
-------------------------------------------------------------------------------------------------------
    ...
    --case statement to ensure that cap_ntile is between 0 and max_ntile
    ,case when event_start_hour in (4,5,10,11,12,13,14) and max_ntile-10+@ntile_correction > max_ntile then max_ntile
          when event_start_hour in (4,5,10,11,12,13,14) and max_ntile-10+@ntile_correction <   0 then   0
          when event_start_hour in (4,5,10,11,12,13,14) then max_ntile-10+@ntile_correction
          when event_start_hour in (6,7,8,9) and datepart(weekday,event_date) in (1,7) and max_ntile-10+@ntile_correction > max_ntile then max_ntile
          when event_start_hour in (6,7,8,9) and datepart(weekday,event_date) in (1,7) and max_ntile-10+@ntile_correction <   0 then   0
          when event_start_hour in (6,7,8,9) and datepart(weekday,event_date) in (1,7) then max_ntile-10+@ntile_correction
          when event_start_hour in (6,7,8,9) and datepart(weekday,event_date) in (2,3,4,5,6) and max_ntile-10+@ntile_correction > max_ntile then max_ntile
          when event_start_hour in (6,7,8,9) and datepart(weekday,event_date) in (2,3,4,5,6) and max_ntile-10+@ntile_correction <   0 then   0
...
--------------------------------------------------------------------------------------------------------
    ...
    ,initial_genre
    ,max(ntile_1) as max_ntile
    ,case when max_ntile-10+@ntile_correction > max_ntile then max_ntile
          when max_ntile-10+@ntile_correction <   0 then   0
          else max_ntile-10+@ntile_correction
     end as cap_ntile
    ,cast(null as integer) as min_dur_mins
...
-------------------------------------------------------
    ...
    ,event_start_day
    ,max(ntile_lp) as max_ntile
    ,case when max_ntile-6+@ntile_correction > max_ntile then max_ntile
          when max_ntile-6+@ntile_correction <   0 then   0
          else max_ntile-6+@ntile_correction
     end as cap_ntile
    ,cast(null as integer) as min_dur_mins
    into CP2_2p
    from CP2_nt_lp
...
-----------------------------------------------------------------------
    ...
    ,event_start_day
    ,max(ntile_lp) as max_ntile
    ,case when max_ntile-2+@ntile_correction > max_ntile then max_ntile
          when max_ntile-2+@ntile_correction <   0 then   0
          else max_ntile-2+@ntile_correction
     end as cap_ntile
    ,cast(null as integer) as min_dur_mins
    into CP2_3p
    from CP2_nt_lp
...
--------------------------------------------------------------------------
    ...
    ,event_start_day
    ,max(ntile_lp) as max_ntile
    ,case when max_ntile-1+@ntile_correction > max_ntile then max_ntile
          when max_ntile-1+@ntile_correction <   0 then   0
          else max_ntile-1+@ntile_correction
     end as cap_ntile
    ,cast(null as integer) as min_dur_mins
    into CP2_4p
    from CP2_nt_lp
