...
begin
    ...
    -------------------------------------------------------------------------------------------------
    if object_id('view_dupe_culling_1') is not null drop table view_dupe_culling_1
    if object_id('view_dupe_culling_2') is not null drop table view_dupe_culling_2

    -- First off: Kick out the duplicates out that come in from the weird day wrapping stuff
    select subscriber_id, adjusted_event_start_time, x_viewing_start_time, max(Tx_Start_Datetime_UTC) as latest_start
    into view_dupe_culling_1
    from Capping2_01_Viewing_Records
    group by subscriber_id, adjusted_event_start_time, x_viewing_start_time
    having count(1) > 1

    commit
    create unique index fake_pk on view_dupe_culling_1 (subscriber_id, adjusted_event_start_time, x_viewing_start_time)
    commit

    -- OK, so find keys for all the things that need to be deduplicated out:
    select vr.cb_row_id, 1 as Dummy
    into view_dupe_culling_2
    from view_dupe_culling_1 as vdc1
    inner join Capping2_01_Viewing_Records as vr
    on  vdc1.subscriber_id              = vr.subscriber_id
    and vdc1.adjusted_event_start_time  = vr.adjusted_event_start_time
    and vdc1.x_viewing_start_time       = vr.x_viewing_start_time
    and vdc1.latest_start              <> vr.Tx_Start_Datetime_UTC           -- Want to keep the one that started the latest...

    commit
    create unique index fake_pk on view_dupe_culling_2 (cb_row_id)
    commit

    -- Now to do the deduplication:
    delete from Capping2_01_Viewing_Records where cb_row_id in (select cb_row_id from view_dupe_culling_2)
    commit
    ...
    into cumulative_playback_corrections
    from Capping2_01_Viewing_Records
    where live = 0
    ...
    commit
    create unique index sequencing_key on cumulative_playback_corrections (subscriber_id, adjusted_event_start_time, sequencer)
    commit
    ...
    into cumulative_playback_corrections_2
    from cumulative_playback_corrections

    commit

    create unique index fake_pk on cumulative_playback_corrections_2 (cb_row_ID)
    ...
    from Capping2_01_Viewing_Records
    inner join cumulative_playback_corrections_2 as cpc2
    on Capping2_01_Viewing_Records.cb_row_id = cpc2.cb_row_id

    set @QA_catcher = -1

    select @QA_catcher = count(1)
    from cumulative_playback_corrections_2
    ...
    if object_id('uncapped_viewing_totals') is not null drop table uncapped_viewing_totals

    select subscriber_id,
        round(sum(datediff(ss, X_Viewing_Start_Time, X_Viewing_End_Time)) / 60.0, 0) as total_box_viewing
    into uncapped_viewing_totals
    from Capping2_01_Viewing_Records
    where daily_table_date = @target_date
    group by subscriber_id
    ...
    delete from CP2_event_listing

    commit

--    select case when subscriber_id is null then 0 else 1 end as sub,date(EVENT_START_DATE_TIME_UTC) as d,count(1) as cow
--    from sk_prod.vespa_events_all group by sub, d
    ...
    */
    ...
    set initial_genre          = fpie.genre
       ...
       ,initial_channel_name   = upper(trim(fpie.channel_name)) -- Wait, didn't we uppercase and trim that when we imported it? Can't we do straight joins now?
    from CP2_event_listing
    ...
    select @QA_catcher = count(1)
    from CP2_event_listing
    where initial_channel_name is not null
    ...
    set pack=cl.pack
    from CP2_event_listing
    inner join vespa_analysts.CP2_channel_lookup as cl      -- explicit schema because there's that central lookup we want to refer to
    on CP2_event_listing.initial_channel_name = cl.epg_channel
    ...
    select   Live
            ...
            ,initial_genre
            ,event_dur_mins
            ,ntile(200) over (partition by Live,event_start_day, event_start_hour order by x_event_duration) as ntile_lp
    ...
    into CP2_nt_lp
    FROM CP2_ntiles_week
    where Live=0 and Viewing_Event_Type = 'Playback'
    group by Live,event_start_day, event_start_hour,DOW,ntile_lp

    commit
 --create capping limits for VOSDAL	
  if object_id('CP2_nt_lp') is not null drop table CP2_nt_lp
    ...
    into CP2_nt_lp
    FROM CP2_ntiles_week
    where Live=0 and Viewing_Event_Type = 'VOSDAL'
    group by Live,event_start_day, event_start_hour,DOW,ntile_lp

    commit
	-- Case when the Broadcast start date = event start date ie VOSDAL----
...
order by air_view_delta
    ...
    ,case when pri_ntile is null then sec_ntile-25
          when event_start_hour in (4,5,10,11,12,13,14) then pri_ntile-25
          when event_start_hour in (6,7,8,9) and datepart(weekday,event_date) in (1,7) then pri_ntile-20
          when event_start_hour in (6,7,8,9) and datepart(weekday,event_date) in (2,3,4,5,6) then pri_ntile-25
     end as cap_ntile
    ,cast(null as integer) as min_dur_mins
    into CP2_h4_14
    from CP2_nt_4_19
    ...
    ,max(ntile_2) as sec_ntile
    ,case when pri_ntile is null then sec_ntile-20 else pri_ntile-25 end as cap_ntile
    ,cast(null as integer) as min_dur_mins
    into CP2_h20_22
    from CP2_nt_20_3
    ...
    FROM CP2_nt_lp
    where Live=0 and Viewing_Event_Type = 'Playback'
    group by Live,event_start_date, Broadcast_start_date,event_start_hour
	
	--identify ntile threshold for VOSDAL events
	SELECT Live
    ,event_start_date
	,Broadcast_start_date
	,event_start_hour
    ...
    */
    ...
    update Capping2_01_Viewing_Records
    set Capping2_01_Viewing_Records.pack = t2.pack
    from Capping2_01_Viewing_Records
    inner join vespa_analysts.CP2_channel_lookup as t2
    on Capping2_01_Viewing_Records.initial_channel_name = t2.epg_channel
    commit
...
/*
THIS IS CURRENTLY PROBABLY REDUNDANT

create procedure CP2_refresh_caps
as
begin

-------------------------------------------------------------------------------------------------
-- L01) THE OUTER LOOP: LOGGER & SETUP
-------------------------------------------------------------------------------------------------

declare @scanning_day           date    -- Date of immediate consideration

declare @active_profiling_day   date    -- Date for which the metadata profiling is applied
-- This guy needs to be tracked separately because, with the prioritisation stuff,
-- we don't know that any two subsequent days we process are actually going to be
-- chronologically adjacent, or even in the same week, so we need to check which
-- week we're in and which week the profiling is good for, and queue metadata
-- rebuilds when necessary. We'll also need another date to store some of the
-- temporary date checking values:
declare @test_day               date
declare @loop_counter           integer
declare @loop_max               integer
declare @still_time             bit
...
-- For control totals into the logging:
declare @QA_catcher             integer

-- First, find out whether it's a live or a dev build:
...
if lower(@tablespacename) = 'vespa_analysts'
    set @run_Identifier = 'VespaCapping2Cache'
else
begin
    ...
    set @run_Identifier = 'CP2 test ' || upper(right(@tablespacename,1)) || upper(left(@tablespacename,2))
end

execute logger_create_run @run_Identifier, @Refresh_identifier, @CP2_build_ID output

-------------------------------------------------------------------------------------------------
-- L02) ESTABLISH TIME BOUNDS
-------------------------------------------------------------------------------------------------
    if object_id('completed_cap_days') is not null drop table completed_cap_days

-- So this guy is kind of complicated as we're trying to manage prioritised historical builds,
-- full builds back to the beginning of Vespa, and regular operational updates all through the
-- same scheduler mechanism.

-- The period bounds for what we're building come from the centralised rebuild prioritisation
-- periods view. Now we just have to worry about what we've already processed, as indicated
-- from the control total capping tables:
select distinct build_date
into completed_cap_days
from CP2_QA_viewing_control_totals
where data_state like '4%'      -- Okay, so if there are partial builds in there we'll have a lot of trouble, but yeah.

commit
create unique index fake_pk on completed_cap_days (build_date)
commit

    if object_id('sequenced_build_days') is not null drop table sequenced_build_days
select
    calendar_date
    ,rank() over (order by build_priority desc, calendar_date asc) as sequencer
into sequenced_build_days
from CP2_cache_prioritisation_intervals as vcp
left join completed_cap_days ccd
on vcp.calendar_date = ccd.build_date
where ccd.build_date is null            -- consider only those that aren't already built

-- Now we have a table with all the dates that still need capping treatment, and a flag indicating
-- the order in which they get treated. Awesome.
commit
    if object_id('completed_cap_days') is not null drop table completed_cap_days
commit
...
select @QA_catcher = count(1)
from sequenced_build_days

execute logger_add_event @CP2_build_ID, 3, 'L02: Complete! (Prioritise build days)', coalesce(@QA_catcher, -1)
commit

-- Now we've got to loop through all the days in this table and build caps for each:
set @loop_counter = 1
select @loop_max = max(sequencer) from sequenced_build_days
set @active_profiling_day = null
commit

while @loop_counter <= @loop_max
begin

    -- Okay, so all prioritisation and existing updates aside, what's the next day to cap?
    select @scanning_day = calendar_date
    from sequenced_build_days
    where sequencer = @loop_counter

-------------------------------------------------------------------------------------------------
-- L03) PREPARE WEEKLY PROFILING STUFF
-------------------------------------------------------------------------------------------------

    -- So, does the date we're capping have the same profiling Thursday as the metadata build we
    -- already have? If not we'll have to bulid it again...

    -- So what day should we be taking for the metadata build on this day?
    -- Grab the last Thursday before the date we're building.
    select @test_day = max(calendar_date)
    from sk_prod.sky_calendar
    where calendar_date <= @scanning_day
    and subs_last_day_of_week = 'Y'

    if @test_day <> @active_profiling_day or @active_profiling_day is null
    begin
        execute logger_add_event @CP2_build_ID, 3, 'L03: New week detected, kicking of metadata build.'
        commit

        execute CP2_Profile_Boxes @test_day
        -- Remember that we profiled it to here
        set @active_profiling_day = @test_day
    end
    -- If they already matched, then we've already got metadata on the profiling day we want

    -- ## QA: check that the box lookup table isn't empty

-------------------------------------------------------------------------------------------------
-- L04) BUILD & CACHE CAPS FOR EACH DAY
-------------------------------------------------------------------------------------------------

    -- Check if the daily table exists:
--    if object_id('sk_prod.VESPA_STB_PROG_EVENTS_' || dateformat(@scanning_day,'yyyymmdd')) is null
--begin
--        -- no daily table, put warnings into the logger.
--        execute logger_add_event @CP2_build_ID, 2, 'L04: No Daily Table! (' || dateformat(@scanning_day,'yyyymmdd') || ')'
--    end
--    else
--    begin

        -- Build the day's caps:
        execute CP2_build_days_caps @scanning_day, @CP2_build_ID

        commit

-------------------------------------------------------------------------------------------------
-- L05) QA ON RECENT CONSTRUCTIONS
-------------------------------------------------------------------------------------------------

        -- This also has to be done a day at a time, since (because of the funky historical prioritisation)
        -- we don't know exactly which day will have been capped this cycle once the proc has finished.

        execute CP2_QA_on_cap_build @scanning_day, @CP2_build_ID

        -- heh, this proc doesn't exist yet though, but when it does it will QA that one day of caps.

        commit
--    end

-------------------------------------------------------------------------------------------------
-- L06) THE OUTER LOOP: EXIT CONTROLS
-------------------------------------------------------------------------------------------------

    -- If we didn't process to the end of where we wanted to be, then we'll bump the capping cache
    -- proc onto the next day's queue by returning and exit code of 11 which indicates a queue bounce
    -- (refer to scheduler table source code for details).

    -- Is there time to continue to build more cap cache stuff?
    execute CITeam.VES024_Is_There_Still_Time @still_time

    commit

    -- Loop exit controlled with a break rather than loop termination.
    if @still_time = 0
    begin
        -- We're not going to kill the various intermediate tables at this point, we might need them
        -- for manual QA a bit later, outside the loop.
        execute logger_add_event @CP2_build_ID, 3, 'Capping 2: Terminates! Aproached business hours, postponing further cap builds.'

        set @loop_counter = @loop_max + 1 -- I should trust Sybase more, but I don't
        commit

        return 11 -- 11 is the code for the scheduler to bounce the task into the next day's queue
    end
    else
        set @loop_counter = @loop_counter + 1
    end

    -- If we get to here, the build all finished, so we'll report the caps are all up to date.
    execute logger_add_event @CP2_build_ID, 3, 'Capping 2: Complete! All cap builds up to date.'

    commit

end; -- procedure CP2_refresh_caps

-- CITeam owns the scheudler:
grant execute on CP2_refresh_caps to CITeam;
