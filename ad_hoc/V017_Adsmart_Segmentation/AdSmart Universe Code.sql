...
--if object_id('AdSmart') is not null then drop table AdSmart end if;

drop table AdSmart;
commit;


CREATE TABLE AdSmart(                                                   -- Create table with a list of variables
        ...
        ,cb_address_postcode       VARCHAR(10)     -- postcode of account
        ,Cable_area                VARCHAR(3)      -- Cable Area Flag
        ,currency_code             VARCHAR(5)      -- Distiguish ROI accounts
        ,TA_attempts               INTEGER         -- TA attempts Last 12 months
        ,TA_Saves                  INTEGER         -- TA saves Last 12 months
        ,Pending_cancel            INTEGER         -- Pending Cancelation flag
        ,Pend_Can_date             DATE            -- Cancelation Date
        ,Premiums_downgrades       INTEGER         -- Premium downgrades L12
        ,Movies_downgrades         INTEGER         -- Movies downgrades L12
        ,Sports_downgrades         INTEGER         -- Sports downgrades L12
        ,current_offer             INTEGER         -- account currently in offer
        ,Offer_count               INTEGER         -- Number of offers
        ,offer_expires30           INTEGER         -- account offer expires next 30 days
        ,value_segment             VARCHAR(50)     -- Current Value Segment
        ,sky_id                    bigint          -- Sky id created
        ,Sky_Go_Reg                INTEGER         -- Sky Go number of downloads 12 months
        ,HDTV                      INTEGER         -- Current HD TV subscription
        ,Ent_Extra                 INTEGER         -- Current Entertainment Extra
        ,sky_sports_1              tinyint         -- Current Sky Sports 1 Subs
        ,sky_sports_2              tinyint         -- Current Sky Sports 2 Subs
        ,movies_1                  tinyint         -- Current Sky Movies 1 Subs
        ,movies_2                  tinyint         -- Current Sky Movies 1 Subs
        ,Disney                    integer         -- Current Disney Subs
        ,ESPN_Subscribers          INTEGER         -- Current ESPN subscription
        ,Anytime_plus              INTEGER         -- Anytime+ activated
        ,multiroom                 INTEGER         -- Current Multiroom Subscription
        ,skyplus                   INTEGER         -- customer has skyplus
        ,Total_MR                  INTEGER         -- Total number of MR boxes
        ,T_AdSm_box                INTEGER         -- Total Adsmartable boxes
        ,talk_product              VARCHAR(50)     -- Current Sky Talk product
        ,BB_type                   VARCHAR(50)     -- Current BB product
        ,box_type                  VARCHAR(50)     -- Primary Box Type
        ,vespa                     INTEGER         -- Box returning data
        ,panel_id_vespa            INTEGER         -- panel_id
        ,Sky_cust_life             VARCHAR(20)     -- based on Sky Tenure
        ,Total_miss_pmt            INTEGER         -- Number of unbilled payments Last 12 months
        ,h_lifestage               VARCHAR(50)     -- household lifestage
        ,HomeOwner                 VARCHAR(50)     -- data defined from Experians ConsumerView
        ,Financial_outlook         VARCHAR(50)     -- data defined from Experians ConsumerView
        ,Demographic               VARCHAR(50)     -- data defined from Experians ConsumerView
        ,Lifestage                 VARCHAR(2)      -- data defined from Experians ConsumerView
        ,Family_Lifestage          VARCHAR(2)      -- data defined from Experians ConsumerView
        ,tenure                    VARCHAR(2)      -- data defined from Experians ConsumerView
        ,h_fss_v3_group            VARCHAR(2)      -- data defined from Experians ConsumerView --change to Z because there is a M category
        ,h_mosaic_uk_2009_group    VARCHAR(2)      -- data defined from Experians ConsumerView
        ,h_mosaic_uk_2009_type     VARCHAR(2)      -- data defined from Experians ConsumerView
        ,PAF                       INTEGER         -- data defined from Experians ConsumerView
        ,H_AFFLUENCE               VARCHAR(2)      -- data defined from Experians ConsumerView
        ,Sky_Reward_L12            INTEGER         -- Sky Rewards Last 12 months
        ,Sky_Events_L12            INTEGER         -- Sky Events Last 12 months
        ,Sky_T_Rewards             INTEGER         -- total Sky Reward (rewards + events)
        ,Kids_Aged_LE4             varchar(1)      -- data defined from AXCIOM
        ,Kids_Aged_5to11           varchar(1)      -- data defined from AXCIOM
        ,Kids_Aged_12to17          varchar(1)      -- data defined from AXCIOM
        ...
        ,Mirror_has_children       VARCHAR(50)     -- data defined from Experians ConsumerView
        ,Mirror_ABC1               VARCHAR(1)      -- data defined from Experians ConsumerView
...
--      create index on AdSmart
CREATE   HG INDEX idx01 ON AdSmart(account_number);
commit;
...
--      insert into AdSmart file
INSERT INTO AdSmart (account_number, currency_code,cb_key_household)
...
update ADsmart
set Sky_Events_L12 = case when Sky_Events_L12 is not null then Sky_Events_L12 else 0 end
,Sky_Reward_L12    = case when Sky_Reward_L12 is not null then Sky_Reward_L12 else 0 end
;
...
--select top 100 * from Sky_events
...
---------------------------------------------------------- Anytime + Downloaded L6
SELECT distinct account_number
               ,1 AS Anytime_Active_L6M
INTO APLUS_DOWN
FROM   sk_prod.CUST_ANYTIME_PLUS_DOWNLOADS
WHERE  last_modified_dt BETWEEN today() - 180 AND today()
...
--      create index on #Anytime_plus file
CREATE   HG INDEX idx06 ON APLUS_DOWN(account_number);
commit;

-- alter table to add field
ALTER TABLE AdSmart ADD Anytime_plus_usage tinyint default 0;
commit;

--      update file
UPDATE AdSmart
SET base.Anytime_plus_usage = Aplus.Anytime_Active_L6M
FROM AdSmart AS Base
INNER JOIN APLUS_DOWN AS Aplus
ON base.account_number = APlus.account_number;
commit;
...
SELECT  base.account_number
         ,Max(CASE WHEN offer_end_dt          > @today THEN '1' ELSE '0'        END) AS Current_offer
         ,Max(CASE WHEN offer_end_dt          < DATEADD(day,+30,@today) THEN '1' ELSE '0'        END) AS offer_expires30
         ,count(*) as InOffer
INTO     offers
...
--select Current_offer, count(*) from offers group by Current_offer
--select sum(convert(integer,Current_offer)), sum(convert(integer,offer_expires30)), sum(convert(integer,InOffer)) from offers
...
-- delete temp file
drop table boxtype_ac; commit;
drop table scaling_box_level_viewing; commit;
...
--select panel_id_vespa,count(*) from AdSmart group by panel_id_vespa;
...
--       ,(RTRIM(Lifestage)||RTRIM(Family_Lifestage)||RTRIM(tenure)||RTRIM(h_mosaic_uk_2009_group)||RTRIM(h_mosaic_uk_2009_type)||RTRIM(h_fss_v3_group)) AS exp
        ,CASE WHEN (cb_address_status = '1' and cb_address_dps IS NOT NULL and cb_address_organisation IS NULL) THEN 1
              ...
              END as PAF
INTO nodupes
FROM sk_prod.EXPERIAN_CONSUMERVIEW AS CV, ADSMART AS base
...
ALTER table     NoDupes ADD   h_lifestage_desc             varchar(50); commit;
ALTER table     NoDupes ADD   Mirror_has_children          varchar(50); commit;
ALTER table     NoDupes ADD   HomeOwner                    varchar(50); commit;
ALTER table     NoDupes ADD   Financial_outlook            varchar(50); commit;
ALTER table     NoDupes ADD   Demographic                  varchar(50); commit;
ALTER table     NoDupes ADD   Mirror_ABC1                  varchar(1) ; commit;
ALTER table     NoDupes ADD   Kids_Aged_LE4                varchar(1) ; commit;
ALTER table     NoDupes ADD   Kids_Aged_5to11              varchar(1) ; commit;
ALTER table     NoDupes ADD   Kids_Aged_12to17             varchar(1) ; commit;
ALTER table     NoDupes ADD   H_AFFLUENCE                  varchar(2) ; commit;
...
--select top 100 * from AdSmart;
--select count(*) from AdSmart;
--select h_fss_v3_group, count(*) from AdSmart group by h_fss_v3_group;



-----------------------------------------------------------------------------------------
--MIRROR MEN AND MIRROR WOMEN
-----------------------------------------------------------------------------------------
...
--------------------------------------------------------------------------------
--Mirror Men & Mirror Women Derivation
--------------------------------------------------------------------------------

create variable @cb_change_date date
...
--select top 100 * from mirror_men_and_women_seg


        --      update AdSmart file
UPDATE AdSmart
SET      MIRROR_MEN_MIN       = EXP.MIRROR_MEN
        ,MIRROR_WOMEN_MIN     = EXP.MIRROR_WOMEN
FROM AdSmart  AS Base
INNER JOIN mirror_men_and_women_seg AS EXP
ON base.cb_key_household = EXP.cb_key_household;
commit;

--replace missing
Update AdSmart
set MIRROR_MEN_MIN      = case when MIRROR_MEN_MIN is not null then MIRROR_MEN_MIN      else 'MVII' end
    ,MIRROR_WOMEN_MIN   = case when MIRROR_WOMEN_MIN is not null then MIRROR_WOMEN_MIN  else 'WVII' end
;

drop table ageband; commit;
drop table mirror_men_and_women; commit;
drop table mirror_men_and_women2; commit;
drop table mirror_men_and_women_seg; commit;

-- select top 100 * from sk_prod.EXPERIAN_CONSUMERVIEW;
-- select top 100 * from AdSmart;
-- select count(*) from AdSmart;

--select MIRROR_MEN_MIN, count(*) from AdSmart group by MIRROR_MEN_MIN;
...
--select barb_desc_itv, count(*) from AdSmart group by barb_desc_itv;
...
into Acct_cust
from  AdSmart  AS Base inner join sk_prod.CUST_SINGLE_ACCOUNT_VIEW as exp
        ON base.account_number = EXP.account_number;
commit;

Update ADsmart
SET  base.ACCT_CUST_ACCOUNT_ID = sm.ACCT_CUST_ACCOUNT_ID
FROM ADsmart as base INNER JOIN Acct_cust AS sm
ON base.account_number = sm.account_number;
commit;
...
/*
ALTER TABLE AdSmart     ADD (cb_key_db_person       varchar(50));
ALTER TABLE AdSmart     ADD (cb_key_individual      bigint);
ALTER TABLE AdSmart     ADD (government_region      varchar(50));
ALTER TABLE AdSmart     ADD (bt_fibre_area          varchar(10));
ALTER TABLE AdSmart     ADD (exchange_id            varchar(10));
ALTER TABLE AdSmart     ADD (exchange_status        varchar(10));
ALTER TABLE AdSmart     ADD (exchange_unbundled     varchar(10));
ALTER TABLE AdSmart     ADD (isba_tv_region         varchar(50));
*/

Alter table AdSmart Add Country varchar(50);
commit;
...
FROM ADsmart as base INNER JOIN temp_Dinesh AS sm
ON base.account_number = sm.account_number;
commit;
...
ALTER TABLE AdSmart ADD (model_score  integer);
commit;

Update ADsmart
SET  base.model_score = sm.model_score
FROM ADsmart as base INNER JOIN modelscore AS sm
...
ALTER TABLE AdSmart     ADD (IUL_Affluence_group  varchar(25) default 'X) Missing');
...
ALTER TABLE AdSmart     ADD (Affluence_group  varchar(25) default 'X) Missing');
commit;
...
--select top 100 * from AdSmart;
--select Affluence_group, count(*) from AdSmart group by Affluence_group;
...
Alter table AdSmart add CURRENT_PACKAGE varchar(50);
commit;
...
Alter table AdSmart Add  HOUSEHOLD_COMPOSITION varchar(35)  default 'UNCLASSIFIED'; commit;
Alter table AdSmart Add  cb_address_town       varchar(50)  default 'Missing'; commit;
...
-- select HOUSEHOLD_COMPOSITION, count(*) from AdSmart group by HOUSEHOLD_COMPOSITION;
-- select cb_address_town, count(*) from AdSmart group by cb_address_town;
...
Alter table AdSmart Add metropolitan_area_and_itv_region varchar(70); commit;
Alter table AdSmart Add pc_area                          varchar(5); commit;
Alter table AdSmart Add postcode_town                    varchar(70); commit;
...
--SELECT top 100 * from AdSmart;
--select lukcat_fr_de_nrs, count(*) from sk_prod.CACI_SOCIAL_CLASS group by lukcat_fr_de_nrs
...
Alter table AdSmart Add  social_grade varchar(15)  default 'UNCLASSIFIED';
commit;

Update AdSmart
set base.social_grade = sc.social_grade
from AdSmart as base inner join caci_sc as sc
...
select account_number
        ...
        ,(case when datediff(day,acct_first_account_activation_dt,today())     <= 365      then '0-1 Year'
               when datediff(day,acct_first_account_activation_dt,today())     > 365
                   and datediff(day,acct_first_account_activation_dt,today())  <= 730      then '1-2 Years'
               when datediff(day,acct_first_account_activation_dt,today())     > 730
                   and datediff(day,acct_first_account_activation_dt,today())  <= 3650     then '2-10 Years'
               when datediff(day,acct_first_account_activation_dt,today())     > 3650      then '10+ Years'
               else null
               end) as cust_tenure
into custenure
...
Alter table AdSmart Add cust_tenure varchar(20);
commit;

Update AdSmart
set base.cust_tenure = ct.cust_tenure
from AdSmart as base inner join custenure as ct
...
--QA
--select top 100 * from custenure where cust_tenure = '0-1 Year' and acct_first_account_activation_dt <= '20111107'
...
Alter table AdSmart Add head_of_HH_gender          integer;     commit;
Alter table AdSmart Add head_of_HH_actual_age      integer;     commit;
Alter table AdSmart Add head_of_HH_age_band        varchar(20); commit;

update AdSmart
set head_of_HH_gender            = temp.p_gender
    ,head_of_HH_actual_age       = temp.p_actual_age
from AdSmart as base inner join lifestage_age as temp
...
--select top 1000 * from AdSmart

-------------------------------------------------------------------------------------------------------------

---- FILE NAMES NEED TO BE CHANGED TO REFLECT CHANGE IN DATA DATE

-- grant access to most recent AdSmart table
GRANT SELECT ON Adsmart TO PUBLIC;
commit;

-- create date stamped version of ADSmart table
drop table AdSmart_20121129;                                    -- date of run
commit;

select *
into AdSmart_20121129
from adsmart;
commit;

GRANT SELECT ON AdSmart_20121129 TO PUBLIC;
commit;
...
--select mirror_abc1, count(*) from adsmart group by mirror_abc1
--select social_grade, count(*) from adsmart group by social_grade
--select Kids_Aged_LE4, count(*) from adsmart group by Kids_Aged_LE4
--select count(*) from AdSmart_20121129
