...
from    (
			...
			*/
			select  *
					,case	when software_version like '%.65.00%' 	then 'R11.1' 
							...
							when software_version like 'R12%' 		then 'R12' 
							else 'Other' 
					end 	as A_B
			from	table_date_range(skyplus.skyplus_sessions_,timestamp('2016-10-13'),timestamp('2016-10-24'))
			where	(
						software_version like 'R11%' or
						software_version like 'R12%'
					)
		...
		INNER JOIN	R12_Gate.ThePanel	as PM
		...
		inner join	(
						...
						from	(
									select	thedate
											,viewing_card
											,software_version
									from    table_date_range(skyplus.skyplus_sessions_,timestamp('2016-10-13'),timestamp('2016-10-24')) as base
									group	by	thedate
												,viewing_card
												,software_version
								)	as x
								INNER JOIN	R12_Gate.ThePanel	as y
