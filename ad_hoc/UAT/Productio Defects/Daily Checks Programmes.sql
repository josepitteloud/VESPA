...
**Project Name: 					DAILY CHECKS (PROGRAMME)
...
------------------------------------
/* A: GENERATING PROGRAMME CHECKS */
------------------------------------

select	fact.DT																								as TheDate
		...
		,sum(case when fact.dk_programme_dim > 0 then 1 else 0 end) 										as records_with_prog_dim
		,count(distinct fact.dk_programme_dim) 																as total_programme_dim
		,sum(case when fact.dk_programme_dim in (null,-1) then 1 else 0 end) 								as records_without_programme_dim
...
from	(
			...
			select	dk_event_start_datehour_dim/100																	as DT
					,PK_VIEWING_PROGRAMME_INSTANCE_FACT																as key_
					,duration
					,to_timestamp(
									substring	(	
