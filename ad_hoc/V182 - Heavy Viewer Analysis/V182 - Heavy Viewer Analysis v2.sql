--drop table v182_duration_by_channel_and_account; drop table v182_duration_by_channel_day; drop table v182_account_viewing_days; commit;
...
--set @channel_num_latest=2; set @channel_num_max=10;

set @channel_name = (select channel_name_inc_hd_staggercast from v182_distinct_channel_list where rank_id = @channel_num_latest);
--select @channel_name;
--drop table v182_heavy_channel_users; drop table v182_total_duartion_from_heavy_viewers; drop table v182_latest_channel_name ;
create table v182_heavy_channel_users (
account_number varchar(50)
,channel_name_inc_hd_staggercast varchar(200)
)
;
create table v182_total_duartion_from_heavy_viewers
(channel_name_inc_hd_staggercast varchar(200)
,top_viewers_channel varchar(200)
,weighted_duration float
,weighted_duration_rank integer
)
;

create table v182_latest_channel_name 
(channel_name_inc_hd_staggercast varchar(200)
);
...
insert into v182_latest_channel_name 
(select channel_name_inc_hd_staggercast from v182_distinct_channel_list where rank_id = @channel_num_latest
)

insert into v182_heavy_channel_users (
 select  a.account_number
,b.channel_name_inc_hd_staggercast
from v182_duration_by_channel_and_account as a
left outer join v182_latest_channel_name  as b
on a.channel_name_inc_hd_staggercast=b.channel_name_inc_hd_staggercast
where b.channel_name_inc_hd_staggercast is not null and  a.channel_view_rank<=10
group by a.account_number
,b.channel_name_inc_hd_staggercast
 )

--Get all Viewing of these accounts---
insert into v182_total_duartion_from_heavy_viewers
(
select a.channel_name_inc_hd_staggercast
,c.channel_name_inc_hd_staggercast as top_viewers_channel
,sum(total_duration*account_weight) as weighted_duration
, rank() over (order by weighted_duration desc) as rank_id 
from v182_duration_by_channel_and_account as a
left outer join v182_account_list as b
on a.account_number = b.account_number
left outer join v182_heavy_channel_users as c
on a.account_number = c.account_number
where b.days_viewing_captured>=20 and c.account_number is not null and b.account_weight>0 and a.channel_name_inc_hd_staggercast<>c.channel_name_inc_hd_staggercast
group by a.channel_name_inc_hd_staggercast
,top_viewers_channel
)

delete from v182_heavy_channel_users where account_number is null or account_number is not null
delete from v182_latest_channel_name  where channel_name_inc_hd_staggercast is null or channel_name_inc_hd_staggercast is not null

'

;
--select top 100 * from v182_duration_by_channel_and_account
...
select top_viewers_channel
,max(case when weighted_duration_rank=1 then channel_name_inc_hd_staggercast else null end) as channel_rank_01
,max(case when weighted_duration_rank=2 then channel_name_inc_hd_staggercast else null end) as channel_rank_02
,max(case when weighted_duration_rank=3 then channel_name_inc_hd_staggercast else null end) as channel_rank_03
,max(case when weighted_duration_rank=4 then channel_name_inc_hd_staggercast else null end) as channel_rank_04
,max(case when weighted_duration_rank=5 then channel_name_inc_hd_staggercast else null end) as channel_rank_05
,max(case when weighted_duration_rank=6 then channel_name_inc_hd_staggercast else null end) as channel_rank_06
,max(case when weighted_duration_rank=7 then channel_name_inc_hd_staggercast else null end) as channel_rank_07
,max(case when weighted_duration_rank=8 then channel_name_inc_hd_staggercast else null end) as channel_rank_08
,max(case when weighted_duration_rank=9 then channel_name_inc_hd_staggercast else null end) as channel_rank_09
,max(case when weighted_duration_rank=10 then channel_name_inc_hd_staggercast else null end) as channel_rank_10
,sum(weighted_duration) as total_weighted_duration
from v182_total_duartion_from_heavy_viewers
group by top_viewers_channel
order by total_weighted_duration desc
--
;
...
select top 500 * from v182_total_duartion_from_heavy_viewers
select count(*) from v182_total_duartion_from_heavy_viewers

select top 5000 * from v182_total_duartion_from_heavy_viewers where top_viewers_channel = 'Sky Sports 1' order by weighted_duration_rank
select top 5000 * from v182_total_duartion_from_heavy_viewers where top_viewers_channel = 'Sky Arts 1' order by weighted_duration_rank
