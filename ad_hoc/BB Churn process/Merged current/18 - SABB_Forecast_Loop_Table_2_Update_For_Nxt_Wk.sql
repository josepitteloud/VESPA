...
BEGIN
	...
	SET BB_offer_rem_and_end_raw = CASE WHEN BB_Offer_Applied = 1 THEN DATEDIFF(day, end_date, Curr_Offer_end_Date_Intended_BB) ELSE BB_offer_rem_and_end_raw - 7 END
		...
		--- Updating my_sky_login_3m_raw 
			...
			DECLARE @dt DATE 
			SET @dt = COALESCE ((SELECT MIN (end_date) FROM FORECAST_Looped_Sim_Output_Platform) , (SELECT end_date FROM Forecast_Loop_Table_2) )
	...
	INNER JOIN citeam.CUST_Fcast_Weekly_Base AS a ON a.account_number = b.account_number
	...
	LEFT JOIN SABB_BB_Calls_prob_TABLE AS c ON b.Call_group = c.Prob_Group AND b.segment_sa = c.segment_sa AND Rand_call BETWEEN Lower_limit AND UPPER_LIMIT 
	...
	UPDATE Forecast_Loop_Table_2 AS a
	SET SABB_forecast_segment = convert(VARCHAR(4), CASE WHEN node IN (22, 46, 49, 70, 75, 71) THEN 1
