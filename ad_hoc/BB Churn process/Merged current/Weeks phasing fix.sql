...
SELECT subs_week_and_year 
    , end_date 
    , CASE WHEN DAY(end_date) BETWEEN 24 AND 31  THEN 1 ELSE 0 END eom 
    , SUM(eom) OVER (PARTITION  BY churn_type  ORDER BY subs_week_and_year DESC) batch
    , churn_type
    , COUNT(*) hits 
 INTO weeks_fix  
FROM BB_Churn_cancellations_6
GROUP BY subs_week_and_year , churn_type , eom, end_date 
...
SELECT subs_week_and_year
    ...
    , 'HM' AS churn_type 
    , CASE WHEN DAY(end_date) BETWEEN 24 AND 31  THEN 1 ELSE 0 END eom 
    , SUM(eom) OVER (ORDER BY subs_week_and_year DESC) batch
...
SELECT subs_week_and_year
    ...
    , CASE WHEN DAY(end_date) BETWEEN 1 AND 7  THEN 1 ELSE 0 END eom 
    , SUM(eom) OVER (ORDER BY subs_week_and_year DESC) batch
FROM FORECAST_Looped_Sim_Output_Platform
GROUP BY subs_week_and_year
    , end_date
...
UPDATE weeks_fix_draft
SET new_week = b.subs_week_and_year 
FROM weeks_fix_draft AS a 
