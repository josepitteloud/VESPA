...
BEGIN
	...
	DECLARE @mx_dt DATE
	SET @mx_dt = (SELECT max(end_date) FROM citeam.CUST_Fcast_Weekly_Base)
	...
	LEFT JOIN citeam.nowtv_accounts_ents 	AS c ON a.account_number = c.account_number AND  @mx_dt BETWEEN period_start_date AND period_end_date 
	WHERE a.bb_active = 1 AND a.dtv_active = 0 
