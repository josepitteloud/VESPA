--SELECT * INTO FORECAST_Looped_Sim_Output_Platform_prefix from FORECAST_Looped_Sim_Output_Platform
--GO 
/*----------------------------------------------------------------------------------------------------------*/
...
-- Trend coefficients
	SELECT * INTO FCAST_Regr_Coeffs 		FROM SABB_Regression_Coefficient(ForeCAST_Start_Wk, 2);
	MESSAGE CAST(now() as timestamp)||' | Calculate historic rates and trends for foreCAST DONE' TO CLIENT;
--Intraweek PL probablilities
	SELECT * INTO IntraWk_AB_Pct 			FROM Intraweek_ABs_Dist(ForeCAST_Start_Wk);
	SELECT * INTO IntraWk_PC_Pct 			FROM Intraweek_PCs_Dist(ForeCAST_Start_Wk);
	SELECT * INTO IntraWk_BCRQ_Pct 			FROM Intraweek_BCRQ_Dist(ForeCAST_Start_Wk);
-- Intended PC Duration Distribution
	SELECT * INTO DTV_PC_Duration_Dist 		FROM PC_Duration_Dist(ForeCAST_Start_Wk);
-- PC Reactivation, PC ABs 
	SELECT * INTO PC_PL_Status_Change_Dist 	FROM PC_Status_Movement_Probabilities(ForeCAST_Start_Wk);
-- AB Reactivation
	SELECT * INTO AB_PL_Status_Change_Dist 	FROM AB_Status_Movement_Probabilities(ForeCAST_Start_Wk);
-- Offer Duration Distribution
	SELECT * INTO Offer_Applied_Dur_Dist 	FROM SABB_Offer_Applied_Duration_Dist(ForeCAST_Start_Wk, 6);
-- Last 6 Wks activations to sample in simulation
	CALL SABB_Build_ForeCAST_New_Cust_Sample(ForeCAST_Start_Wk);
-- Weekly Activation Volumes
	CALL SABB_ForeCAST_Create_Opening_Base(ForeCAST_Start_Wk,sample_pct);
*/
	SELECT * INTO Activation_Vols 			FROM SABB_ForeCAST_Activation_Vols(Y2W01,Y3W52);
	MESSAGE CAST(now() as timestamp)||' | Calculate historic rates and trends for foreCAST DONE' TO CLIENT;
...
/*
	CALL SABB_my_sky_login_prob ;
	CALL SABB_BB_Calls_prob ;	
*/
----------------------------------------------------------------------------------------------------------------
...
----------------------------------------------------------------------------------------------------------------
	MESSAGE CAST(now() as timestamp)||' | Run simulation loop' TO CLIENT;
		...
		SET counter = 1;
		SET True_Sample_Rate = (SELECT CAST(sum(CASE WHEN sample = 'A' THEN 1 ELSE 0 END) AS FLOAT) / count(*) FROM FORECAST_Base_Sample );
...
GRANT SELECT ON SABB_predicted_values       TO spencerc2, jcartwright, vespa_group_low_security;
GRANT SELECT ON Fcast_Regr_Coeffs           TO spencerc2, jcartwright, vespa_group_low_security;		
GRANT SELECT ON IntraWk_PC_Pct          	TO spencerc2, jcartwright, vespa_group_low_security;		
GRANT SELECT ON IntraWk_AB_Pct          	TO spencerc2, jcartwright, vespa_group_low_security;		
GRANT SELECT ON IntraWk_BCRQ_Pct        	TO spencerc2, jcartwright, vespa_group_low_security;		
GRANT SELECT ON DTV_PC_Duration_Dist        TO spencerc2, jcartwright, vespa_group_low_security;		
GRANT SELECT ON PC_PL_Status_Change_Dist    TO spencerc2, jcartwright, vespa_group_low_security;		
GRANT SELECT ON AB_PL_Status_Change_Dist    TO spencerc2, jcartwright, vespa_group_low_security;		
GRANT SELECT ON Offer_Applied_Dur_Dist      TO spencerc2, jcartwright, vespa_group_low_security;		
GRANT SELECT ON Activation_Vols          	TO spencerc2, jcartwright, vespa_group_low_security;		
GRANT SELECT ON ForeCAST_Loop_Table         TO spencerc2, jcartwright, vespa_group_low_security;
GO		
