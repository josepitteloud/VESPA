...
BEGIN
	...
	SELECT mor.account_number
		, mor.status_code
		, mor.event_dt
		, cast(DATEADD(day, 65, mor.event_dt) as date) as AB_Future_Sub_Effective_Dt
	...
	SET Next_status_code = next_cancel_status
		, ab_effective_to_dt=next_cancel_dt
	...
	SET Next_status_code = 'CN'
		, AB_Future_Sub_Effective_Dt_End_Dt=cast(AB_Future_Sub_Effective_Dt - datepart(weekday, AB_Future_Sub_Effective_Dt + 2) + 7 AS DATE)
	WHERE Next_status_code = 'BCRQ';

	UPDATE #AB_Intended_Churn
	SET  AB_Future_Sub_Effective_Dt_End_Dt=cast(AB_Future_Sub_Effective_Dt - datepart(weekday, AB_Future_Sub_Effective_Dt + 2) + 7 AS DATE);
	...
	SELECT AB_s.*
		...
		, CASE 	WHEN sc.calendar_date + 7 BETWEEN event_dt AND AB_Effective_To_Dt THEN 'AB' 
				WHEN sc.calendar_date + 7 BETWEEN AB_Effective_To_Dt AND AB_Future_Sub_Effective_Dt_End_Dt THEN 
--						else 
						Next_Status_Code END AS Status_Code_EoW							 
	...
	delete from #AB_PL_status where Wks_To_Intended_Churn is null or Status_Code_EoW is null;
