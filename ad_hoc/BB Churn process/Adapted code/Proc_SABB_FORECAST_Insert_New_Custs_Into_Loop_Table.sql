	...
	BEGIN
		...
		SELECT *
			...
			, rand(number(*)  * @multiplier + 1) e1
			, rand(number(*)  * @multiplier + 2) e2
 			, rand(number(*)  * @multiplier + 3) e3
			, rand(number(*)  * @multiplier + 4) e4
			, rand(number(*)  * @multiplier + 5) e5
			, rand(number(*)  * @multiplier + 6) e6
			, rand(number(*)  * @multiplier + 7) e7
		...
		SELECT *
			, row_number() OVER ( ORDER BY rand_sample2 ) Rand_Rnk
		...
		SELECT 
			...
			, e6 
			, e7 
			, COUNT(*) OVER (PARTITION BY node_SA) 						y1					
			, CAST(row_number() OVER (PARTITION BY node_SA ORDER BY e1) AS FLOAT)  y2
			, y2 / y1
			, CAST(0 AS TINYINT) 
	...
	GRANT EXECUTE
		ON SABB_Forecast_Insert_New_Custs_Into_Loop_Table_2 TO CITeam, spencerc2;
