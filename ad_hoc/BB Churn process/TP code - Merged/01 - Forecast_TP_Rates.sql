...
BEGIN
	...
	IF EXISTS #TP_weekly_agg;
		SELECT subs_year
			, subs_week_of_year AS subs_week
			, cast(subs_week_and_year AS INT) AS subs_week_and_year
			, node_TP AS TP_forecast_segment
			, Count() AS n
			, cast(sum(BB_Offer_Applied) AS REAL) AS BB_Offer_Applied				-- TODO: Affected by the bb_offerapplied
		...
		INTO #TP_weekly_agg
		FROM jcartwright.DTV_Fcast_Weekly_Base_2 AS agg
	...
	DROP TABLE IF EXISTS #TP_forecast_summary_2;
