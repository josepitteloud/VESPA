CREATE OR REPLACE PROCEDURE Forecast_TP_Loop_Table_2_Actions (IN TP_Counter INT, IN Rate_Multiplier REAL) sql security invoker
...
BEGIN
	...
	------ SysCan trend ------
	UPDATE TP_ForeCAST_Loop_Table_2 AS a
	SET pred_bb_enter_SysCan_YoY_Trend = Coalesce(d.Grad_Coeff * 4 * (cast(TP_Counter - 1 AS REAL) / 53), 0)
	FROM TP_ForeCAST_Loop_Table_2 AS a
	LEFT JOIN TP_FCAST_Regr_Coeffs AS d ON a.TP_forecast_segment = d.fcast_segment
	...
	------ CusCan trend ------
	UPDATE TP_ForeCAST_Loop_Table_2 AS a
	SET pred_bb_enter_CusCan_YoY_Trend = Coalesce(d.Grad_Coeff * 4 * (cast(TP_Counter - 1 AS REAL) / 53), 0)
	FROM TP_ForeCAST_Loop_Table_2 AS a
	LEFT JOIN TP_FCAST_Regr_Coeffs AS d ON a.TP_forecast_segment = d.fcast_segment
		--         and d.LV = Forecast_Start_Wk
	...
	------ HM trend ------
	UPDATE TP_ForeCAST_Loop_Table_2 AS a
	SET pred_bb_enter_HM_YoY_Trend = Coalesce(d.Grad_Coeff * 4 * (cast(TP_Counter - 1 AS REAL) / 53), 0)
	FROM TP_ForeCAST_Loop_Table_2 AS a
	LEFT JOIN TP_FCAST_Regr_Coeffs AS d ON a.TP_forecast_segment = d.fcast_segment
		--         and d.LV = Forecast_Start_Wk
	...
	------ 3rd party trend ------
	UPDATE TP_ForeCAST_Loop_Table_2 AS a
	SET pred_bb_enter_3rd_party_YoY_Trend = Coalesce(d.Grad_Coeff * 4 * (cast(TP_Counter - 1 AS REAL) / 53), 0)
	FROM TP_ForeCAST_Loop_Table_2 AS a
	LEFT JOIN TP_FCAST_Regr_Coeffs AS d ON a.TP_forecast_segment = d.fcast_segment
		--         and d.LV = Forecast_Start_Wk
	...
	UPDATE TP_ForeCAST_Loop_Table_2 AS a
	SET pred_BB_Offer_Applied_YoY_Trend = Coalesce(d.Grad_Coeff * 4 * (cast(TP_Counter - 1 AS REAL) / 53), 0)
	FROM TP_ForeCAST_Loop_Table_2 AS a
	LEFT JOIN TP_FCAST_Regr_Coeffs AS d ON a.TP_forecast_segment = d.fcast_segment
		--         and d.LV = Forecast_Start_Wk
	...
	INTO #im
	FROM (SELECT *
		FROM TP_IntraWk_AB_Pct
		WHERE next_status_code NOT IN ('AP')
				UNION
		SELECT *
		FROM TP_Intrawk_PC_Pct
		WHERE next_status_code NOT IN ('AP')
				UNION
		SELECT *
		FROM TP_Intrawk_BCRQ_Pct
		WHERE next_status_code NOT IN ('AP')
		) AS x
	GROUP BY churn_type
		, pseudo_status
	...
	INTO #wm
	FROM (
		SELECT * FROM TP_PC_PL_Status_Change_Dist
		UNION
		SELECT * FROM TP_AB_PL_Status_Change_Dist
		) AS x
