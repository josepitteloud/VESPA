...
--================================================================================================================================================	   
--================================================================================================================================================
pitteloudj.SABB_BB_Calls_prob as
begin
  ...
  if exists(select tname from syscatalog where creator = USER_NAME() and UPPER(tname) = UPPER('SABB_BB_Calls_prob_TABLE') and tabletype = 'TABLE')
    drop table pitteloudj.SABB_BB_Calls_prob_TABLE
  select account_number,
    'segment_sa'=convert(varchar(20),null),
    'curr_count'=MAX(case when end_date = @mx_dt then BB_all_calls_1m_raw else 0 end), -- BB_all_calls_1m_raw on the current week
    'count_1w'=MAX(case when end_date = DATEADD(day,-7,@mx_dt) then BB_all_calls_1m_raw else 0 end), -- -- BB_all_calls_1m_raw on the Previous week
    'Calls_LW'=case when curr_count < count_1w then 0 else curr_count-count_1w end, -- Logins made last week
    'L_12'=MAX(case when end_date between DATEADD(month,-12,@mx_dt) and DATEADD(week,-1,@mx_dt) then BB_all_calls_1m_raw else 0 end), -- Max Login in the past 12 month
    'L_9'=MAX(case when end_date between DATEADD(month,-9,@mx_dt) and DATEADD(week,-1,@mx_dt) then BB_all_calls_1m_raw else 0 end), -- Max Login in the past 9 month
    'L_6'=MAX(case when end_date between DATEADD(month,-6,@mx_dt) and DATEADD(week,-1,@mx_dt) then BB_all_calls_1m_raw else 0 end), -- Max Login in the past 6 month
    'L_3'=MAX(case when end_date between DATEADD(month,-3,@mx_dt) and DATEADD(week,-1,@mx_dt) then BB_all_calls_1m_raw else 0 end), -- Max Login in the past 3 month
    'Prob_Group'=case when L_12 = 0 then 1
    when L_9 = 0 then 2
    when L_6 = 0 then 3
    when L_3 = 0 then 4
    else 5 -- Picking the longest Group
    end into #BB_CALLS_prob_1
  ...
  select a.Prob_Group,
    ...
    from #BB_CALLS_prob_3 as a
      left outer join #BB_CALLS_prob_3 as b on a.segment_sa = b.segment_sa
  ...
  commit work
  create lf index id1 on pitteloudj.SABB_BB_Calls_prob_TABLE(Prob_Group)
  create lf index id2 on pitteloudj.SABB_BB_Calls_prob_TABLE(Calls_LW)
  create hg index id3 on pitteloudj.SABB_BB_Calls_prob_TABLE(Lower_limit)
  create hg index id4 on pitteloudj.SABB_BB_Calls_prob_TABLE(UPPER_LIMIT)
  grant select on pitteloudj.SABB_BB_Calls_prob_TABLE to citeam,vespa_group_low_security
  commit work
