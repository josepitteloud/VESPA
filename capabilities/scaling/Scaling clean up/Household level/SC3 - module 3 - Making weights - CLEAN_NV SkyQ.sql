CREATE OR REPLACE PROCEDURE SC3_v1_1__make_weights_clean_NV_skyQ
...
BEGIN
	...
	DECLARE @NV_target BIGINT
	DECLARE @NV_factor FLOAT
	DECLARE @V_factor FLOAT
		DECLARE @dev FLOAT
	COMMIT
	MESSAGE cast(now() as timestamp)||' | M1.3 Start 'TO CLIENT

	
			if object_id('VESPA_HOUSEHOLD_WEIGHTING_NV') is not null drop table VESPA_HOUSEHOLD_WEIGHTING_NV 
			create table VESPA_HOUSEHOLD_WEIGHTING_NV (
				...
				, nv_flag					BIT 			DEFAULT 0
				,primary key (account_number, scaling_date)
			)

			create dttm index for_loading on VESPA_HOUSEHOLD_WEIGHTING_NV (build_date)
			create hg index idx1 on VESPA_HOUSEHOLD_WEIGHTING_NV (account_number)
			create hg index idx2 on VESPA_HOUSEHOLD_WEIGHTING_NV (cb_key_household)
			create date index idx3 on VESPA_HOUSEHOLD_WEIGHTING_NV (scaling_date)

			grant select on VESPA_HOUSEHOLD_WEIGHTING_NV to vespa_group_low_security, sk_prodreg
	...
	COMMIT
	MESSAGE cast(now() as timestamp)||' | M1.3 SC3_weighting_working_table insert DONE: '||@@rowcount TO CLIENT
	...
	JOIN SC3_Segments_lookup_SKYQ  AS sl ON SC3_weighting_working_table.scaling_segment_id = sl.scaling_segment_id
	...
	FROM SC3_Todays_panel_members_nv
	WHERE scaling_segment_id IS NOT NULL
	GROUP BY scaling_segment_id

	MESSAGE cast(now() as timestamp)||' | M1.3 #segment_distribs created: '||@@rowcount TO CLIENT
	...
	COMMIT
	MESSAGE cast(now() as timestamp)||' | M1.3 Iteration starts ' TO CLIENT
	...
	SET @scaling_var = ( SELECT scaling_variable FROM SC3_Variables_lookup_v1_2 WHERE id = @cntr )
	SET @scaling_count = ( SELECT COUNT(scaling_variable) FROM SC3_Variables_lookup_v1_2 )
	...
	BEGIN
		...
		BEGIN
			...
			FROM SC3_Variables_lookup_v1_2
			WHERE id = @cntr_var
			...
			EXECUTE ('INSERT INTO SC3_category_working_table (sky_base_universe,profile,value,sky_base_accounts,vespa_panel,sum_of_weights) '
                            ...
                            ||' JOIN SC3_Segments_lookup_SKYQ  AS ssl ON srs.scaling_segment_id = ssl.scaling_segment_id'
		...
		SELECT @dev = SUM(abs(sky_base_accounts - sum_of_weights)) FROM SC3_category_working_table
		...
		EXECUTE ('UPDATE SC3_weighting_working_table '
				...
				||' FROM SC3_weighting_working_table '
                ||' JOIN SC3_Segments_lookup_SKYQ  AS ssl ON SC3_weighting_working_table.scaling_segment_id = ssl.scaling_segment_id '
		...
		COMMIT
		MESSAGE cast(now() as timestamp)||' | Scaling variable: '||@scaling_var TO CLIENT
		MESSAGE cast(now() as timestamp)||' | Convergence: '||@convergence TO CLIENT
		MESSAGE cast(now() as timestamp)||' | Deviation: '||@dev TO CLIENT
	...
	END
	MESSAGE cast(now() as timestamp)||' | M1.3 Iteration ends ' TO CLIENT
	COMMIT
	...
	FROM SC3_Intervals
	JOIN SC3_Todays_panel_members_nv AS tpm ON SC3_Intervals.account_number = tpm.account_number
	...
	FROM SC3_Todays_panel_members_nv AS tpm
	...
	IF (SELECT count(1) FROM VESPA_HOUSEHOLD_WEIGHTING_NV WHERE scaling_date = @scaling_day ) > 0
	BEGIN
		DELETE FROM VESPA_HOUSEHOLD_WEIGHTING_NV
		WHERE scaling_date = @scaling_day
	END

	COMMIT

	INSERT INTO VESPA_HOUSEHOLD_WEIGHTING_NV
	...
	JOIN SC3_Todays_panel_members_nv AS tpm 		ON ws.account_number = tpm.account_number -- Filter for today's panel only
		AND ws.profiling_date = @profiling_date

	COMMIT

	MESSAGE cast(now() as timestamp)||' | M1.3 NV re-weighting 'TO CLIENT
		
	SELECT 
		@NV_target = @sky_base * (nv / 100)
	FROM Empty_logs_totals_2
	WHERE dt = @scaling_day
	
	
	MESSAGE cast(now() as timestamp)||' | M1.3 @sky_base '|| @sky_base TO CLIENT
	MESSAGE cast(now() as timestamp)||' | M1.3 @NV_target '|| @NV_target TO CLIENT
	
	SELECT @NV_factor = @NV_target / SUM(scaling_weighting)  
	FROM VESPA_HOUSEHOLD_WEIGHTING_NV
	WHERE nv_flag = 1 
		AND scaling_date = @scaling_day
	
	MESSAGE cast(now() as timestamp)||' | M1.3 @NV_factor: '|| @NV_factor TO CLIENT
	
	UPDATE VESPA_HOUSEHOLD_WEIGHTING_NV
	SET scaling_weighting = scaling_weighting * @NV_factor
	WHERE nv_flag = 1 AND scaling_date = @scaling_day
		
	SELECT sky_base_universe, SUM(sum_of_weights) w
	INTO #ow
	FROM SC3_weighting_working_table
	GROUP BY sky_base_universe
	COMMIT 	
	SELECT  c.sky_base_universe
			, CAST(max(d.w) AS FLOAT) t1
			, CAST(SUM(CASE WHEN nv_flag = 1 THEN scaling_weighting ELSE 0 END) AS FLOAT) t2 
			, CAST(SUM(CASE WHEN nv_flag = 0 THEN scaling_weighting ELSE 0 END) AS FLOAT) t3 
			, v_factor = (t1- t2)/ t3
	INTO #v_factor
	FROM VESPA_HOUSEHOLD_WEIGHTING_NV 		AS a 
	JOIN SC3_Sky_base_segment_snapshots 	AS b ON a.account_number= b.account_number AND profiling_date =  @profiling_date
	JOIN SC3_weighting_working_table		AS c ON b.population_scaling_segment_id = c.scaling_segment_id
	JOIN #ow 								AS d ON d.sky_base_universe = c.sky_base_universe 
	WHERE a.scaling_date = @scaling_day
	GROUP BY c.sky_base_universe
	COMMIT 
	
	/*
	SELECT @V_factor = (@sky_base - @NV_target) / SUM(scaling_weighting)
	FROM VESPA_HOUSEHOLD_WEIGHTING_NV
	WHERE nv_flag = 0 AND scaling_date = @scaling_day
	*/
	--MESSAGE cast(now() as timestamp)||' | M1.3 @V_factor: '|| @V_factor TO CLIENT
	
	UPDATE VESPA_HOUSEHOLD_WEIGHTING_NV
	SET scaling_weighting = scaling_weighting * V_factor
	FROM VESPA_HOUSEHOLD_WEIGHTING_NV 	AS a 
	JOIN SC3_scaling_weekly_sample		AS b ON a.account_number = b.account_number 
	JOIN #v_factor						AS c ON c.sky_base_universe = b.sky_base_universe
	WHERE nv_flag = 0 AND scaling_date = @scaling_day
