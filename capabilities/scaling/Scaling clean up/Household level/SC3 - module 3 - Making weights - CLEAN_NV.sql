CREATE OR REPLACE PROCEDURE SC3_v1_1__make_weights_clean_NV
...
BEGIN
	...
	SET @scaling_count = ( SELECT COUNT(scaling_variable) FROM vespa_analysts.SC3_Variables_lookup_v1_1 )
	...
	BEGIN
		...
		BEGIN
			...
			EXECUTE ('INSERT INTO SC3_category_working_table (sky_base_universe,profile,value,sky_base_accounts,vespa_panel,sum_of_weights) '
                            ...
                            ||' JOIN vespa_analysts.SC3_Segments_lookup_v1_1 AS ssl ON srs.scaling_segment_id = ssl.scaling_segment_id'
		...
		EXECUTE ('UPDATE SC3_weighting_working_table '
				...
				||' FROM SC3_weighting_working_table '
                ||' JOIN vespa_analysts.SC3_Segments_lookup_v1_1 AS ssl ON SC3_weighting_working_table.scaling_segment_id = ssl.scaling_segment_id '
