...
CREATE OR REPLACE PROCEDURE V289_M11_02_SC3_v1_1__prepare_panel_members_clean_nv
...
BEGIN
	...
	if object_id('SC3_Todays_panel_members_nv') is not null 
	drop table SC3_Todays_panel_members_nv 
	create table SC3_Todays_panel_members_nv (
	...
	TRUNCATE TABLE SC3_todays_panel_members_nv
	COMMIT -- (^_^)
	...
	INSERT INTO SC3_todays_panel_members_nv (
		account_number
		,scaling_segment_id
		)
	SELECT account_number
	...
	MESSAGE cast(now() as timestamp)||' | M11.1 #SC3_todays_panel_members_nv insert. Rows:'||@@rowcount TO CLIENT
	...
	SELECT DISTINCT 
	  a.account_number
	, sbss.vespa_scaling_segment_id
	, 1 
	FROM EMPTY_logs_Netezza_consolidated as a 
	INNER JOIN SC3_Sky_base_segment_snapshots AS sbss ON a.account_number = sbss.account_number
	WHERE a.v_date = @scaling_day
		AND sbss.profiling_date = @profiling_date
		AND a.account_number NOT IN (SELECT account_number FROM SC3_todays_panel_members_nv)
