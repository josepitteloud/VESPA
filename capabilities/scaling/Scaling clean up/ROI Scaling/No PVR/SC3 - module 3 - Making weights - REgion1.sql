...
BEGIN
	...
	JOIN SC3_ROI_Segments_lookup_no_pvr_r2 	AS sl ON a.scaling_segment_id = sl.scaling_segment_id
	...
	COMMIT
	CREATE UNIQUE INDEX fake_pk ON #segment_distribs_roi (scaling_segment_id)
	COMMIT
	...
	FROM SC3_ROI_weighting_working_table 	AS a 
	INNER JOIN #segment_distribs_roi 		AS sd ON SC3_ROI_weighting_working_table.scaling_segment_id = sd.scaling_segment_id AND a.bb_flag = sd.bb_flag
	...
	-- In this scenario, the person running the code should send the results of the SC3_ROI_metrics for that
	-- week to analytics team for review. ## What exactly are we checking? can we automate any of it?
	MESSAGE cast(now() as timestamp)||' | SC3.3 Iteration Start' TO CLIENT
	CREATE TABLE #convergence_table (Iteration int, convergence int) 
	...
	BEGIN
		...
		BEGIN
			...
			EXECUTE ('INSERT INTO SC3_ROI_category_working_table (sky_base_universe,profile,value,sky_base_accounts,vespa_panel,sum_of_weights) '
                            ...
                            ||' JOIN SC3_ROI_Segments_lookup_no_pvr_r2 AS ssl ON srs.scaling_segment_id = ssl.scaling_segment_id'
		...
		INSERT INTO #convergence_table_roi
		SELECT  @iteration,  @convergence 
		...
		EXECUTE ('UPDATE SC3_ROI_weighting_working_table '
                ...
                ||' JOIN SC3_ROI_Segments_lookup_no_pvr_r2 AS ssl ON SC3_ROI_weighting_working_table.scaling_segment_id = ssl.scaling_segment_id '
	...
	END
	MESSAGE cast(now() as timestamp)||' | SC3.3 Iteration END' TO CLIENT
	COMMIT
	...
	JOIN SC3_ROI_Sky_base_segment_snapshots AS ws 	ON wwt.scaling_segment_id = ws.population_scaling_segment_id
	...
	SELECT * from #convergence_table_roi
	DROP TABLE #convergence_table_roi
