...
BEGIN
	...
	CREATE TABLE #raw_logs_dump_temp_roi (
					account_number VARCHAR(20) NOT NULL
					,service_instance_id VARCHAR(30) NOT NULL
							)

	COMMIT -- (^_^)
	SET @query = 'INSERT INTO #raw_logs_dump_temp_roi ' 
		|| 'SELECT DISTINCT' 
		|| 'account_number ' 
		|| ',service_instance_id ' 
		|| 'FROM ' || @dp_tname 
	...
	MESSAGE cast(now() as timestamp)||' | M11.1 #raw_logs_dump_temp_roi creation. Rows:'||@@rowcount TO CLIENT
	
	COMMIT -- (^_^)
	CREATE hg INDEX idx1 ON #raw_logs_dump_temp_roi (account_number)
	CREATE hg INDEX idx2 ON #raw_logs_dump_temp_roi (service_instance_id)
	COMMIT -- (^_^)

	CREATE TABLE #raw_logs_dump_roi (
		account_number VARCHAR(20) NOT NULL
		,service_instance_id VARCHAR(30) NOT NULL
		)
	...
	INSERT INTO #raw_logs_dump_roi
	SELECT DISTINCT account_number
		,service_instance_id
	FROM #raw_logs_dump_temp_roi

	MESSAGE cast(now() as timestamp)||' | M11.1 #raw_logs_dump_roi creation. Rows:'||@@rowcount TO CLIENT
	
	COMMIT -- (^_^)
	CREATE INDEX some_key ON #raw_logs_dump_roi (account_number)
	COMMIT -- (^_^)
	...
	INTO #panel_options_roi
	FROM #raw_logs_dump_roi
	GROUP BY account_number

	MESSAGE cast(now() as timestamp)||' | M11.1 #panel_options_roi creation. Rows:'||@@rowcount TO CLIENT
	
	COMMIT -- (^_^)
	CREATE UNIQUE INDEX fake_pk ON #panel_options_roi (account_number)
	COMMIT -- (^_^)

	DROP TABLE #raw_logs_dump_temp_roi
	DROP TABLE #raw_logs_dump_roi
	COMMIT -- (^_^)

	-- Getting this list of accounts isn't enough, we also want to know if all the boxes
	-- of the household have returned data.
	UPDATE #panel_options_roi
	SET expected_boxes = sbss.expected_boxes
		,scaling_segment_id = sbss.vespa_scaling_segment_id
	FROM #panel_options_roi
	INNER JOIN SC3_ROI_Sky_base_segment_snapshots AS sbss ON #panel_options_roi.account_number = sbss.account_number
