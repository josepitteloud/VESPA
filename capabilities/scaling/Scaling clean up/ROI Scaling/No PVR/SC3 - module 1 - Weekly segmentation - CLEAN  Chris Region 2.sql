CREATE OR REPLACE PROCEDURE SC3_ROI_do_weekly_segmentation_no_pvr_RC_2_tenure
...
BEGIN
	...
	SET a.ROI_account_flag = CASE  WHEN b.fin_currency_code = 'EUR' AND cust_active_dtv = 1 THEN 1 
		...
		,a.ROI_Region = CASE 	WHEN UPPER (a.ROI_County) IN ('DUBLIN') THEN 'Dublin'
								...
								WHEN UPPER (a.ROI_County) IN ('CLARE','KERRY','LIMERICK','TIPPERARY','WATERFORD','CORK') THEN 'Munster'
								ELSE 'Not Defined'
								END
		,a.cb_key_individual = b.cb_key_individual
		,a.cust_viewing_data_capture_allowed = b.cust_viewing_data_capture_allowed
		,a.tenure = CASE   WHEN datediff(day,acct_first_account_activation_dt,@profiling_thursday) <=  730 THEN 'A) 0-2 Years'
    ...
    FROM SC3_ROI_scaling_weekly_sample AS a
	JOIN /*vespa_analysts.*/SC3_ROI_Segments_lookup_no_pvr_rC2 AS ssl ON trim(lower(a.sky_base_universe)) = trim(lower(ssl.sky_base_universe))
														 AND left(a.ROI_Region, 20) = left(ssl.ROI_Region, 20)
														 AND a.Package        = ssl.Package
														 AND left(a.tenure, 2)         = left(ssl.tenure, 2)
														 AND a.	     = ssl.no_of_stbs
