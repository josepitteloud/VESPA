...
BEGIN
	...
	JOIN SC3_ROI_Segments_lookup_no_pvr AS sl ON SC3_ROI_weighting_working_table.scaling_segment_id = sl.scaling_segment_id
	...
	MESSAGE cast(now() as timestamp)||' | SC3.3 #segment_distribs creation. Rows:'||@@rowcount TO CLIENT
	COMMIT
	...
	FROM SC3_ROI_weighting_working_table
	INNER JOIN #segment_distribs AS sd ON SC3_ROI_weighting_working_table.scaling_segment_id = sd.scaling_segment_id
	...
	BEGIN
		...
		BEGIN
			...
			EXECUTE ('INSERT INTO SC3_ROI_category_working_table (sky_base_universe,profile,value,sky_base_accounts,vespa_panel,sum_of_weights) '
                            ...
                            ||' JOIN SC3_ROI_Segments_lookup_no_pvr AS ssl ON srs.scaling_segment_id = ssl.scaling_segment_id'
		...
		EXECUTE ('UPDATE SC3_ROI_weighting_working_table '
				...
				||' FROM SC3_ROI_weighting_working_table '
                ||' JOIN SC3_ROI_Segments_lookup_no_pvr AS ssl ON SC3_ROI_weighting_working_table.scaling_segment_id = ssl.scaling_segment_id '
	...
	SELECT * from #convergence_table
	DROP TABLE #convergence_table
