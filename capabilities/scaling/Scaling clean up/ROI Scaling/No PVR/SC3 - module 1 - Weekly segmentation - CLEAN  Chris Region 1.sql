CREATE OR REPLACE PROCEDURE SC3_ROI_do_weekly_segmentation_no_pvr_RC_1
...
BEGIN
	...
	SET a.ROI_account_flag = CASE  WHEN b.fin_currency_code = 'EUR' AND cust_active_dtv = 1 THEN 1 
		...
		,a.ROI_Region = CASE 	WHEN UPPER (a.ROI_County) IN ('DUBLIN') THEN 'Dublin'
								WHEN UPPER (a.ROI_County) IN ('KILDARE','LAOIS','LONGFORD','LOUTH','MEATH','OFFALY','WESTMEATH','WICKLOW','CARLOW','KILKENNY','WEXFORD') THEN 'Leinster (Exc. Dublin)'
								WHEN UPPER (a.ROI_County) IN ('CAVAN','DONEGAL','GALWAY','LEITRIM','MAYO','MONAGHAN','ROSCOMMON','SLIGO') THEN 'Ulster & Connacht'
								WHEN UPPER (a.ROI_County) IN ('CLARE','KERRY','LIMERICK','TIPPERARY','WATERFORD') THEN 'Munster (Exc. Cork)'
								WHEN UPPER (a.ROI_County) IN ('CORK') THEN 'Cork'
								ELSE 'Not Defined'
								END
	...
	UPDATE #weekly_sample_roi 
	SET ROI_Region = CASE 	WHEN UPPER (ROI_County) IN ('DUBLIN') THEN 'Dublin'
							...
							WHEN UPPER (ROI_County) IN ('CLARE','KERRY','LIMERICK','TIPPERARY','WATERFORD') THEN 'Munster (Exc. Cork)'
							WHEN UPPER (ROI_County) IN ('CORK') THEN 'Cork'
							ELSE 'Not Defined'
							END
    ...
    FROM SC3_ROI_scaling_weekly_sample AS a
	JOIN /*vespa_analysts.*/SC3_ROI_Segments_lookup_no_pvr_rC1 AS ssl ON trim(lower(a.sky_base_universe)) = trim(lower(ssl.sky_base_universe))
