...
greenj.SC2_Recreate_Env; --to clear data
...
--(run "SC2 - 0 - table creation.sql" (in iSQL)
...
begin
    ...
    set @build_ID='999'
    set @varStartDate = '2013-11-14' --isn't actually used

    commit

    execute SC2_v2_1__do_weekly_segmentation_CUSTOM @varStartDate, @build_ID, now()
    commit

    execute SC2_v2_1__scale_Vespa_panel_CUSTOM      @varStartDate, now(), @build_ID
    commit
...
--Part 6 calculate effective sample size for the proposed panel
  select sum(weighting * weighting) as large
    ...
    from SC2_Todays_panel_members  as tpm
         inner join SC2_Weightings as wei on tpm.scaling_segment_id = wei.scaling_segment_id
   where scaling_day = @max_date
;
select * from #ess2
drop table #ess2
...
select min(weighting)
    from SC2_Todays_panel_members  as tpm
         inner join SC2_Weightings as wei on tpm.scaling_segment_id = wei.scaling_segment_id
   where scaling_day = @max_date

select @max_date = max(scaling_day) from SC2_Weightings
...
--522703
     ...
     update #panel_options
       ...
       from #panel_options
            inner join SC2_Sky_base_segment_snapshots as sbss on #panel_options.account_number = sbss.account_number
...
select max(adjusted_event_start_date_vespa) from sk_prod.viq_viewing_data_scaling


select top 10 * from sk_prod.viq_viewing_data_scaling

select sum(calculated_scaling_weight),count(1) from sk_prod.viq_viewing_data_scaling
where adjusted_event_start_date_vespa = @max_date
