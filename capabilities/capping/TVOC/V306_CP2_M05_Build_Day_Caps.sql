...
/* The pre-standby flags have already been set as part of the Netezza extract. Also this update, updates all events so that they are a pre-standby event, which is not right
...
as begin
        ...
        */
        ...
        -- JT for lag events there will be a number of capping segments that are not common between the 2 periods
        -- So the above update query will leave a large number of events with null max_dur_mins. These will are get capped later
        -- So need to update all nulls to @max_cap_bound_minutes
...
update CP2_event_matching
                set     capped_event_end_time =
                                case
                                        when matched_start_time is null and matched_end_time <= X_Adjusted_Event_End_Time then matched_end_time
                                        when datediff (ss,adjusted_event_start_time, matched_start_time) > 120
                                                        and dateadd(ss,- datediff (ss,adjusted_event_start_time, matched_start_time), matched_end_time) <= X_Adjusted_Event_End_Time
                                                        then dateadd(ss,- datediff (ss,adjusted_event_start_time, matched_start_time), matched_end_time)
                                        when matched_end_time <= X_Adjusted_Event_End_Time then matched_end_time
                                        else X_Adjusted_Event_End_Time
                                end,
