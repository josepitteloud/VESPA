...
-- if object_id('CP2_build_days_caps') is not null drop procedure CP2_build_days_caps;
-- commit;

-- go

-- create or replace procedure CP2_build_days_caps
...
-- begin
     ...
     create variable    @target_date       date = NULL     ;--(^_^) -- Date of daily table caps to cache
     create variable    @CP2_build_ID      bigint = NULL   ;--(^_^) -- Logger ID (so all builds end up in same queue)

     set @target_date = '2014-11-10';
     set @CP2_build_ID = 1234;
...
--     declare @playback_days          tinyint         -- How many days back worth of playback viewing data to consider
    create variable @playback_days          tinyint;--(^_^)         -- How many days back worth of playback viewing data to consider
    set     @playback_days = 28;--(^_^)                     -- Want to be able to treat timeshifting of up to 28 days (consistency with BARB)
...
--     declare @BARB_day_cutoff_time   time            -- because we don't know if the day starts at 2AM or 4 or 6 or 9 and this means it's easily changed
    create variable @BARB_day_cutoff_time   time ;--(^_^)           -- because we don't know if the day starts at 2AM or 4 or 6 or 9 and this means it's easily changed
    set     @BARB_day_cutoff_time = '02:00:00';--(^_^)      -- Treating the day as 2AM to 2AM. Currently not really using this though?

    create variable @max_cap_bound_minutes  integer ;--(^_^)        -- The largest a capping bound can possibly be. This is important as we have to get viewing records out of the previous day's table to cap ea
    set     @max_cap_bound_minutes = 120;--(^_^)

    create variable @min_cap_bound_minutes  integer ;--(^_^)        -- The smallest a capping bound can possibly be.
    set     @min_cap_bound_minutes = 20;--(^_^)

    create variable @min_view_duration_sec  tinyint ;--(^_^)        -- The bound below which views are ignored, in seconds ("surfing")
    set     @min_view_duration_sec  = 6;--(^_^)

    create variable @uncapped_sample_pop_max integer ;--(^_^)       -- The maximum number of uncapped events to consider per bucket for selecting new end times for each capped event
    set     @uncapped_sample_pop_max = 10000;--(^_^)        -- About 2.5 hours to process the matching loop for one day; good thing we're scheduling these overnight
                                                    -- Update: Now pulled back to 10k given we're also batching by initial channel now too

    create variable @var_sql                varchar(15000) ;--(^_^)  -- For dynamic SQL over daily tables, though we're only capping one day so there's no looping
    create variable @QA_catcher             integer ;--(^_^)

    create variable @gmt_start              date ;--(^_^)             -- To capture when the clocks go back in Autumn
    create variable @bst_start              date ;--(^_^)            -- To capture when the clocks go forward in Spring

    set @bst_start = dateadd(dy, -(datepart(dw, datepart(year, today()) || '-03-31') -1),datepart(year, today()) || '-03-31') ;--(^_^) -- to get last Sunday in March
    set @gmt_start = dateadd(dy, -(datepart(dw, datepart(year, today()) || '-10-31') -1),datepart(year, today()) || '-10-31') ;--(^_^) -- to get last Sunday in October
...
--     commit
    ...
    --------------------------------------------------------------------------------
    if object_id('CP2_Viewing_Records')                 is not null drop table CP2_Viewing_Records;--(^_^)
    if object_id('CP2_01_Viewing_Records')              is not null drop table CP2_01_Viewing_Records;--(^_^)
    if object_id('Uncleansed_Viewing_Totals')           is not null drop table Uncleansed_Viewing_Totals;--(^_^)
    if object_id('CP2_4BARB_internal_viewing')          is not null drop table CP2_4BARB_internal_viewing;--(^_^)
    if object_id('CP2_4BARB_view_endpoints')            is not null drop table CP2_4BARB_view_endpoints;--(^_^)
    if object_id('cumulative_playback_corrections')     is not null drop table cumulative_playback_corrections;--(^_^)
    if object_id('cumulative_playback_corrections_2')   is not null drop table cumulative_playback_corrections_2;--(^_^)
    if object_id('CP2_viewing_control_cap_distrib')     is not null drop table CP2_viewing_control_cap_distrib;--(^_^)
    if object_id('CP2_viewing_control_distribs')        is not null drop table CP2_viewing_control_distribs;--(^_^)
    if object_id('CP2_viewing_control_totals')          is not null drop table CP2_viewing_control_totals;--(^_^)
    ...
    create hg   index idx2 on Capping2_01_Viewing_Records (Subscriber_Id);--(^_^)
    create dttm index idx3 on Capping2_01_Viewing_Records (Adjusted_Event_Start_Time);--(^_^)
    create dttm index idx4 on Capping2_01_Viewing_Records (X_Viewing_Start_Time);--(^_^)
    create hg   index idx5 on Capping2_01_Viewing_Records (bucket_id);--(^_^)
    create lf   index idx6 on Capping2_01_Viewing_Records (event_start_hour);--(^_^)
    create lf   index idx7 on Capping2_01_Viewing_Records (event_start_day);--(^_^)



    create variable @varBroadcastMinDate  int;--(^_^)
    create variable @varEventStartHour    int;--(^_^)
    create variable @varEventEndHour      int;--(^_^)

    set @varBroadcastMinDate  = (dateformat(@target_date - @playback_days, 'yyyymmdd00'));--(^_^)
    set @varEventStartHour    = (dateformat(@target_date - 1, 'yyyymmdd23'));--(^_^)      -- Event to start no earlier than at 23:00 on the previous day
    set @varEventEndHour      = (dateformat(@target_date, 'yyyymmdd23'));--(^_^)          -- Event to start no later than at 23:59 on the next day
    ...
    insert into Capping2_01_Viewing_Records
              (cb_row_ID,Account_Number,Subscriber_Id,X_Type_Of_Viewing_Event,Adjusted_Event_Start_Time,X_Adjusted_Event_End_Time,
               ...
               Service_Key,daily_table_date,live,genre,sub_genre,epg_channel,channel_name,program_air_date,program_air_datetime,
               event_start_day,event_start_hour)
    select
          ...
          ,case when live_recorded = 'LIVE' then 1 else 0 end as live
    ...
    where live_recorded in ('LIVE','RECORDED')
      ...
      and Panel_id in (11,12) -- double check with EDM implementation
      and type_of_viewing_event <> 'Non viewing event'  -- may be defunct
    ...
    commit
    ;--(^_^)
    -- select count() from Capping2_01_Viewing_Records; -- 29,136,304

    -- Start off the control totals:
    if object_id('Capping2_tmp_Uncleansed_Viewing_Totals') is not null drop table Capping2_tmp_Uncleansed_Viewing_Totals;--(^_^)
    ...
    commit
    ;--(^_^) -- select top 20 * from Capping2_tmp_Uncleansed_Viewing_Totals;
    -- select avg(total_box_viewing) from Capping2_tmp_Uncleansed_Viewing_Totals where subscriber_id <> -1;

    delete from CP2_QA_daily_average_viewing
     where build_date = @target_date
    commit
    ;--(^_^) -- select top 20 * from CP2_QA_daily_average_viewing;
...
--     commit
    ...
    -------------------------------------------------------------------------------------------------
    if object_id('Capping2_tmp_View_dupe_Culling_1') is not null drop table Capping2_tmp_View_dupe_Culling_1 ;--(^_^)

    -- only interested in first instance within a viewing event -> genre
...
--     commit
    ...
    -- fields, so we still need this funny in-between table... -- is this stil true???
    commit
...
--     from Capping2_tmp_Cumulative_Playback_Corrections_2
    ...
    IF object_id('Capping2_tmp_Cumulative_Playback_Corrections') is not null drop table Capping2_tmp_Cumulative_Playback_Corrections    ;--(^_^)
    IF object_id('Capping2_tmp_Cumulative_Playback_Corrections_2') is not null drop table Capping2_tmp_Cumulative_Playback_Corrections_2    ;--(^_^)
...
--     commit
    ...
    if object_id('Capping2_tmp_Uncapped_Viewing_Totals') is not null drop table Capping2_tmp_Uncapped_Viewing_Totals    ;--(^_^)
...
--     commit
    ...
    create index    for_joins           on CP2_event_listing (account_number)   ;--(^_^)
    create index    start_time_index    on CP2_event_listing (Adjusted_Event_Start_Time)    ;--(^_^)
    create index    init_channel_index  on CP2_event_listing (initial_channel_name) ;--(^_^)
    create index for_the_joining_group  on CP2_event_listing (event_start_hour, event_start_day, initial_genre, box_subscription, pack_grp, Live)   ;--(^_^)
    create index by_bucket_index        on CP2_event_listing (bucket_id, pack_grp, box_subscription)    ;--(^_^)
    ...
    commit
    ;--(^_^)
    -- select top 20 * from CP2_event_listing
    -- select count() from CP2_event_listing; -- 19,865,006
    -- select count() from CP2_event_listing where subscriber_id = -1; -- 1,270
...
--     commit
    ...
    commit
    ;--(^_^) --9271298 Row(s) affected
    -- select count() from CP2_First_Programmes_In_Event; --19,865,006
...
-- 
    ...
    -- Will have this information from Netezza extract

    --add pack & network [Update: Network no longer in play]
    update CP2_event_listing base
       set base.pack = trim(cl.channel_pack)
      from Vespa_Analysts.Channel_Map_Dev_Service_Key_Attributes as cl -- may need to be updated (prod version)
     where base.Initial_Service_Key = cl.Service_Key

    commit
    ;--(^_^) 19346401 Row(s) affected
...
--     commit
    ...
    from CP2_box_lookup as bl   -- created in another procedure, but won't be needed since we have this from the Netezza extract
    ...
    commit
    ;--(^_^) 19864768 Row(s) affected
    -- select count() from CP2_event_listing; -- 19865006
...
--     commit
    ...
    ;--(^_^)
    -- 400 Row(s) affected
    -- select count() from CP2_capping_buckets; --0
    ...
    commit
    ;--(^_^)
    -- select count() from CP2_capping_buckets; -- 400
...
--     commit
    ...
    -------------------------------------------------------------------------------------------------
    -- C04) BUILDING N-TILES FOR CAPPING -- already given in Netezza extract
    -------------------------------------------------------------------------------------------------
    ...
    select   Live
            ...
            ,ntile(200) over (partition by Live,event_start_day,event_start_hour,box_subscription,pack_grp,initial_genre order by x_event_duration) as ntile_1
            ,ntile(200) over (partition by Live,event_start_day,event_start_hour,pack_grp,initial_genre order by x_event_duration) as ntile_2
            ,x_event_duration
            ,viewed_duration
            ,num_views
    ...
    commit
    ;--(^_^) 19854360 Row(s) affected
    -- select count() from CP2_ntiles_week; -- 19854360
...
--     commit
    ...
    --create capping limits for start hours 4-19 -> linked to P/S-box-dependent segment definitions
    ...
    and Live=1
    group by Live,event_date,event_start_day,event_start_hour,box_subscription,pack_grp,initial_genre,ntile_1

    commit
    ;--(^_^) 146082 Row(s) affected
...
--     commit
    ...
    and Live=1
    group by Live,event_start_day,event_start_hour,box_subscription,pack_grp,initial_genre,ntile_2

    commit
    ;--(^_^) 71895 Row(s) affected
...
--     commit
    ...
    ,case when pri_ntile is null then sec_ntile-20 else pri_ntile-0 end as cap_ntile
...
--     commit
    ...
    ,case when pri_ntile is null then sec_ntile-25
          when event_start_hour in (4,5,10,11,12,13,14) then pri_ntile-2
          when event_start_hour in (6,7,8,9) and datepart(weekday,event_date) in (1,7) then pri_ntile-2
          when event_start_hour in (6,7,8,9) and datepart(weekday,event_date) in (2,3,4,5,6) then pri_ntile-2
     end as cap_ntile
    ,cast(null as integer) as min_dur_mins
    into CP2_h4_14
    from CP2_nt_4_19
...
--     commit
    ...
    commit
    ;--(^_^)    2082  Row(s) affected
    -- select top 20 * from CP2_calculated_viewing_caps;
    -- select count() from CP2_calculated_viewing_caps; --3857
...
--     commit
    ...
    commit
    ;--(^_^)    17 Row(s) affected
    -- select top 20 * from CP2_QA_viewing_control_cap_distrib;
    -- select count() from CP2_QA_viewing_control_cap_distrib; -- 11
...
--     commit
    ...
    */
    ...
    commit
    ;--(^_^)    29135024 Row(s) affected
    -- select count() from Capping2_01_Viewing_Records; -- 29136304
    ...
    */
    ...
    commit
    ;--(^_^)    29136304 Row(s) affected
    -- select count() from CP2_QA_viewing_control_totals; --2071
    ...
    commit
    ;--(^_^)    565 Row(s) affected
    -- select count() from CP2_QA_viewing_control_totals; --2636
...
--     commit
    ...
    create variable @number_of_capped_events float
    ;--(^_^)
...
--     commit
    ...
    */
    ...
    commit
    ;--(^_^)
    -- select top 20 * from CP2_capping_buckets;
...
--         end
    ...
    -- than a showstopper.
    -- select top 20 * from CP2_capped_events_with_endpoints;
    -- select count() from CP2_capped_events_with_endpoints; --736569
    -- select @the_bucket, @max_buckets;
...
--     commit
    ...
    commit
    ;--(^_^)
    --  select count() from CP2_capped_events_with_endpoints; -- 19865006
...
--     commit
    ...
    commit
    ;--(^_^)    0 Row(s) affected
    -- select top 20 * from CP2_capped_data_holding_pen;
    -- select count() from CP2_capped_data_holding_pen where capped_flag = 0; 0
    ...
    commit
    ;--(^_^)    29136304 Row(s) affected
    -- select distinct capped_flag from CP2_capped_data_holding_pen; 11
...
-- 
    ...
    group by program_air_date, live, genre
    ;--(^_^)    565 Row(s) affected
    -- select count() from CP2_QA_viewing_control_totals;   -- 3201
...
-- if object_id('CP2_Profile_Boxes') is not null drop procedure CP2_Profile_Boxes;
-- commit;

-- go

create or replace procedure CP2_Profile_Boxes
...
if object_id('CP2_refresh_caps') is not null drop procedure CP2_refresh_caps;
commit;
-- go
...
-- if object_id('CP2_clear_transient_tables') is not null drop procedure CP2_clear_transient_tables;
-- commit;

-- go

create or replace procedure CP2_clear_transient_tables
as
begin
...
CP2_build_days_caps '2014-11-10', 1234;
CP2_Profile_Boxes '2014-11-06', 1234;

CP2_clear_transient_tables;
