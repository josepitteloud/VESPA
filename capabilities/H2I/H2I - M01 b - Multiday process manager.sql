...
AS BEGIN
	...
	BEGIN 	
		...
		BEGIN
			...
			SET @sql_= 
				'CALL dba.sp_create_table (''vespa_shared'',' ||''''||@tablename||''''||','||''''||
				'scaling_day AS date,' || 
		...
		BEGIN
			...
			SET @sql_= 'CALL dba.sp_create_table (''vespa_shared'',' ||''''||@tablename||''''||','||''''||'source AS VARCHAR (5),thedate AS datetime,hhsize AS int,viewers_size AS int,session_size AS int, hits AS int'||''''||')'
			...
			SET @sql_= 'CALL dba.sp_create_table (''vespa_shared'',' ||''''||@tablename||''''||','||''''||'source AS VARCHAR (5),ageband AS VARCHAR (10),genre AS VARCHAR (9),hits AS int,sow AS int'||''''||')'
			...
			SET @sql_= 'CALL dba.sp_create_table (''vespa_shared'',' ||''''||@tablename||''''||','||''''||'source AS VARCHAR (5),hhsize AS int,hits AS int,ukbase AS float'||''''||')'
		...
		BEGIN
			...
			BEGIN 
				...
				EXECUTE (@sql_2)
			END
			MESSAGE cast(now() as timestamp)||' | @ M01B.5: V289_M13_individual_viewing_live_vosdal table DONE' TO CLIENT
			------------------------------------------------------	
			-- M11 	V289_M11_04_VESPA_INDIVIDUAL_WEIGHTING_
			------------------------------------------------------
			MESSAGE cast(now() as timestamp)||' | @ M01B: V289_M11_04_VESPA_INDIVIDUAL_WEIGHTING_'	
			
			SET @tablename 	= 'V289_M11_04_VESPA_INDIVIDUAL_WEIGHTING_'||DATEFORMAT(@current_day, 'YYYYMMDD')||'_'||RIGHT(CAST('00'||@run_id AS VARCHAR),3)
			SET @sql_		= 'CALL dba.sp_create_table (''vespa_shared'',' ||''''||@tablename||''''||','||''''||' ACCOUNT_NUMBER AS varchar(20), HH_person_number AS integer, scaling_date AS DATE, scaling_weighting AS double, build_date AS DATETIME '||''''||')'
			...
			BEGIN 
				SET @sql_2 = 'INSERT INTO vespa_shared.'||@tablename
				SET @sql_2 = @sql_2 ||' SELECT * FROM V289_M11_04_VESPA_INDIVIDUAL_WEIGHTING_'
				SET @sql_2 = @sql_2 ||' COMMIT '
				SET @sql_2 = @sql_2 ||' create hg index hg1 on vespa_shared.'||@tablename||' (account_number)'
				SET @sql_2 = @sql_2 ||' create lf index hg3 on vespa_shared.'||@tablename||' (HH_person_number)'
				SET @sql_2 = @sql_2 ||' commit'
			...
			BEGIN 
				MESSAGE 'Table creation in vespa shared  failed, results stored in local schema at: '||@tablename TO CLIENT 
				SET @sql_2 = 'SELECT * INTO '||@tablename
				SET @sql_2 = @sql_2 ||' FROM V289_M11_04_VESPA_INDIVIDUAL_WEIGHTING_'
				SET @sql_2 = @sql_2 ||' create hg index hg1 on vespa_shared.'||@tablename||' (account_number)'
				SET @sql_2 = @sql_2 ||' create lf index hg3 on vespa_shared.'||@tablename||' (HH_person_number)'
				SET @sql_2 = @sql_2 ||' commit'
				...
				EXECUTE (@sql_2)
			END
				MESSAGE cast(now() as timestamp)||' | @ M01B.5: V289_M11_04_VESPA_INDIVIDUAL_WEIGHTING_ table DONE' TO CLIENT
			...
			SET @sql_		= 'CALL dba.sp_create_table (''vespa_shared'',' ||''''||@tablename||''''||','||''''||'account_number AS varchar(20), person_number AS integer, ind_scaling_weight AS double, gender AS integer, age_band AS integer, head_of_hhd AS integer, hhsize AS integer'||''''||')'
