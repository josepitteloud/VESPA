        ...
        */
        ...
        begin
                ...
                SELECT DISTINCT 
                        ...
                        , '  ' /*sav.*/cb_address_line_1 			------------ Not available for now
            ...
            /* ****************************** Temporary skipped due to    cb_address_line_1      is not available
 ...
 */
                ...
                SELECT vh.account_number
                        , vh.cb_key_household
                        --, vh.cb_address_line_1					-- Not in use for now 
                        --, ex.cb_address_line_1 AS linex			-- Not in use for now 
                        , COUNT(DISTINCT ex.cb_key_db_person) + MAX(CAST(h_number_of_children_in_household_2011 as INT))  AS  HH_composition
                        , Children_count        = MAX(CAST(h_number_of_children_in_household_2011 as INT))
                ...
                --CREATE HG INDEX idal ON #t3(cb_address_line_1)          commit -- NOT in use for now
                COMMIT
                ---------------------   Table Update
                UPDATE V289_M08_SKY_HH_view
                SET     a.Children_count        = b.Children_count
                        ,   a.HH_composition        = b.HH_composition
               --         ,   a.cb_address_line_1     = b.linex 					-- Not in use for now 
                        ,   a.non_matching_flag     = 1
                        ,   a.edited_add_flag       = 1
                FROM V289_M08_SKY_HH_view as a
                JOIN #t3 as b ON a.account_number = b.account_number  AND a.cb_key_household = b.cb_key_household and rank_1 = 1 --AND a.cb_address_line_1 = b.cb_address_line_1 -- Not in use for now
				...
				DELETE FROM V289_M08_SKY_HH_composition WHERE HH_person_number > 15 -- **********Capping statement of hh>15 persons. Delete when is fixed 
                
                MESSAGE cast(now() as timestamp)||' | @ M08.5 Individual table populated. Rows:'||@@rowcount TO CLIENT
