...
AS BEGIN
    ...
    /*    if (datediff(day,@last_update_dt,today()) > 6 or @last_update_dt is null)
      ...
      */                  
                ...
                JOIN (  SELECT DISTINCT account_number FROM VIQ_VIEWING_DATA_SCALING
                        WHERE adjusted_event_start_date_vespa = @event_date )   as viq ON sav.account_number    = viq.account_number
...
----------------------------
        ...
        INTO #perb
        FROM    (SELECT house_id, count(*) ind FROM skybarb GROUP BY house_id) as f
        JOIN    (SELECT DISTINCT household_number FROM skybarb_fullview ) as b
                        on b.household_number = f.house_id
        GROUP bY ind
