...
begin
    ...
    -- declare  @Scale_refresh_logging_ID  bigint = 3874
        ...
        from
                ...
                inner join      thompsonja.SC3i_Segments_lookup_v1_1        as      l_sc3i  -- on l_sc3.sky_base_universe = l_sc3i.sky_base_universe -- NOT USED IN THIS VERSION
        ...
        into #t1
        from SC3I_Sky_base_segment_snapshots b inner join thompsonja.SC3i_Segments_lookup_v1_1 l on b.population_scaling_segment_id = l.scaling_segment_id
        ...
        */
        ...
        from
                ...
                inner join      thompsonja.SC3i_Segments_lookup_v1_1        as      l_sc3i  on      l_sc3.isba_tv_region                                            =       l_sc3i.isba_tv_region           -- finally, match the attributes to give us the new SC3I segment IDs
                                                                                                                                                        and l_sc3.package                                                               =       l_sc3i.package
        ...
        from
                ...
                inner join  thompsonja.SC3I_Segments_lookup_v1_1 l on p.population_scaling_segment_id = l.scaling_segment_id
        ...
        select distinct account_number
        into #t2
        from SC3I_Todays_panel_members p inner join thompsonja.SC3i_Segments_lookup_v1_1 l on p.scaling_segment_id = l.scaling_segment_id
        where (gender = 'U' and age_band <> '0-19') -- exclude U gender except for 0-19 (almost all 0-19 are U)
        --        or hhcomposition = 'D) Unclassified HHComp' -- high numbers of zero vespa segemnts driving lower effective sample size
        --        or l.sky_base_universe = 'Adsmartable but no consent' -- Oct test data has very few Adsmart No Consent on the panel so exclude
        --        or (age_band = '20-24' and hhcomposition = 'B) Singles') -- Small segments
...
-- -- Keep the totals for age/gender groups the same but apply Barb %
        ...
        -- from
             ...
             -- inner join
                -- thompsonja.SC3i_Segments_lookup_v1_1 l
...
-- --      inner join vespa_analysts.SC2_Segments_lookup_v1_1 as sl
     -- inner join thompsonja.SC3i_Segments_lookup_v1_1 as sl
...
-- --      SET @scaling_var    = (SELECT scaling_variable FROM vespa_analysts.SC2_Variables_lookup_v2_1 WHERE id = @cntr)
     ...
     -- BEGIN
             ...
             -- BEGIN
                         ...
                         -- INSERT INTO SC3I_category_working_table (sky_base_universe,profile,value,sky_base_accounts,vespa_panel,sum_of_weights)
                             ...
                             -- FROM SC3I_weighting_working_table AS srs
                                     -- inner join thompsonja.SC3i_Segments_lookup_v1_1 AS ssl ON srs.scaling_segment_id = ssl.scaling_segment_id
             ...
             -- FROM SC3I_weighting_working_table
                     -- inner join thompsonja.SC3i_Segments_lookup_v1_1 AS ssl ON SC3I_weighting_working_table.scaling_segment_id = ssl.scaling_segment_id
...
*/
     -- Log the profiling date being used for the build
      ...
      COMMIT -- (^_^)
        ...
        */
        ...
        from
                                        (
                                                ...
                                                from    thompsonja.SC3i_Segments_lookup_v1_1
                                        )       as      a
                cross join      (
        ...
        */
        ...
        from            thompsonja.SC3i_Segments_lookup_v1_1        as      l
     ...
     from
                 ...
                 -- inner join vespa_analysts.SC2_Segments_lookup_v1_1 as sl
                 inner join thompsonja.SC3i_Segments_lookup_v1_1    as      sl              on      SC3I_weighting_working_table.scaling_segment_id =       sl.scaling_segment_id
     COMMIT -- (^_^)
