...
CREATE OR REPLACE PROCEDURE v289_m11b_Scaling_Wrapper
...
AS BEGIN
		...
		DECLARE @logbatch_id 	VARCHAR(100)
		DECLARE @logrefres_id   VARCHAR (40)
		   
		----------------------------------
		-- M11b.1 - Initialising the logger
		----------------------------------  
		
/*		MESSAGE cast(now() as timestamp)||' | Begining  M11b.1 - Initialising the logger' TO CLIENT
		...
		MESSAGE cast(now() as timestamp)||' | @ M11b.1: Initialising the logger DONE' TO CLIENT
*/
		-------------------------------------
