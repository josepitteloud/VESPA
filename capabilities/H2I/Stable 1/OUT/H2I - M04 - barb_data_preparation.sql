		...
		from    (
					...
					from    (
								...
								select  a.household_number
										,dense_rank() over	(
																partition by    cast(start_time_of_session as date)
                                                                                ,a.household_number
																order by        a.set_number
										...
										,case   when cast(start_time_of_session as time) between '00:00:00.000' and '05:59:59.000' then 'night'
												when cast(start_time_of_session as time) between '06:00:00.000' and '08:59:59.000' then 'breakfast'
												when cast(start_time_of_session as time) between '09:00:00.000' and '11:59:59.000' then 'morning'
												when cast(start_time_of_session as time) between '12:00:00.000' and '14:59:59.000' then 'lunch'
												when cast(start_time_of_session as time) between '15:00:00.000' and '17:59:59.000' then 'early prime'
												when cast(start_time_of_session as time) between '18:00:00.000' and '20:59:59.000' then 'prime'
												when cast(start_time_of_session as time) between '21:00:00.000' and '23:59:59.000' then 'late night'
										end     as session_daypart
										,b.person_number
										,b.processing_weight	as processing_weight
										,case when a.person_1_viewing   = 1 and person_number = 1   then processing_weight*a.duration_of_session else 0 end as person_1
										,case when a.person_2_viewing   = 1 and person_number = 2   then processing_weight*a.duration_of_session else 0 end as person_2
										,case when a.person_3_viewing   = 1 and person_number = 3   then processing_weight*a.duration_of_session else 0 end as person_3
										,case when a.person_4_viewing   = 1 and person_number = 4   then processing_weight*a.duration_of_session else 0 end as person_4
										,case when a.person_5_viewing   = 1 and person_number = 5   then processing_weight*a.duration_of_session else 0 end as person_5
										,case when a.person_6_viewing   = 1 and person_number = 6   then processing_weight*a.duration_of_session else 0 end as person_6
										,case when a.person_7_viewing   = 1 and person_number = 7   then processing_weight*a.duration_of_session else 0 end as person_7
										,case when a.person_8_viewing   = 1 and person_number = 8   then processing_weight*a.duration_of_session else 0 end as person_8
										,case when a.person_9_viewing   = 1 and person_number = 9   then processing_weight*a.duration_of_session else 0 end as person_9
										,case when a.person_10_viewing  = 1 and person_number = 10  then processing_weight*a.duration_of_session else 0 end as person_10
										,case when a.person_11_viewing  = 1 and person_number = 11  then processing_weight*a.duration_of_session else 0 end as person_11
										,case when a.person_12_viewing  = 1 and person_number = 12  then processing_weight*a.duration_of_session else 0 end as person_12
										,case when a.person_13_viewing  = 1 and person_number = 13  then processing_weight*a.duration_of_session else 0 end as person_13
										,case when a.person_14_viewing  = 1 and person_number = 14  then processing_weight*a.duration_of_session else 0 end as person_14
										,case when a.person_15_viewing  = 1 and person_number = 15  then processing_weight*a.duration_of_session else 0 end as person_15
										,case when a.person_16_viewing  = 1 and person_number = 16  then processing_weight*a.duration_of_session else 0 end as person_16
