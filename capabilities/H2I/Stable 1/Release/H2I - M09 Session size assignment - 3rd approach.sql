...
AS BEGIN
    ...
    create lf index tmp1_idx_2 on #tmp1(overlap_batch)
        ...
        IF EXISTS(SELECT tname FROM syscatalog WHERE creator = ${SQLFILE_ARG001}  and tabletype = 'TABLE' and upper(tname) = 'TEMP_EVENT')   drop table temp_event
        SELECT
                        event_ID
                        , dt.account_number
                        , dt.subscriber_id
        ...
        IF EXISTS(SELECT tname FROM syscatalog WHERE creator = ${SQLFILE_ARG001}  and tabletype = 'TABLE' and upper(tname) = 'EVENTS_1_BOX')   drop table events_1_box
    ...
    WHERE adj_hh < 1
		...
		------------------------------------------------------------------------------------------------------------------------
		------------------------------------------------------------------------------------------------------------------------
        IF EXISTS(SELECT tname FROM syscatalog WHERE creator = ${SQLFILE_ARG001}  and tabletype = 'TABLE' and upper(tname) = 'OVERLAPPING_EV')   drop table overlapping_ev
        ...
        commit
        ------------------------------------------------------------------------------------------------------------------------
       IF EXISTS(SELECT tname FROM syscatalog WHERE creator = ${SQLFILE_ARG001}  and tabletype = 'TABLE' and upper(tname) = 'NEW_MATRIX')   drop table  new_matrix
        SELECT
              mx.thedate
            , mx.segment_id
            , mx.viewing_size
