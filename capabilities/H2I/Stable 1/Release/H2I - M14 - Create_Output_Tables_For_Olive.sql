...
create or replace procedure ${SQLFILE_ARG001} .V289_M14_Create_Final_Olive_Output_Tables
AS BEGIN
...
--*/
	...
	FROM
		(	SELECT tname 
			...
			FROM syscatalog
			WHERE creator = ${SQLFILE_ARG001} 
					AND UPPER(tname) = UPPER ('V289_M14_event_individuals_%')
					AND UPPER(tname) <> UPPER ('V289_M14_event_individuals_CURRENT')
					AND (tabletype = 'VIEW' OR tabletype = 'TABLE')
		)   AS  A
	WHERE   RNK < 3
	COMMIT -- (^_^)
	...
	FROM
		(	SELECT tname 
			...
			FROM syscatalog
			WHERE creator = ${SQLFILE_ARG001} 
					AND UPPER(tname) = UPPER ('V289_M14_individuals_details_%')
					AND UPPER(tname) <> UPPER ('V289_M14_individuals_details_CURRENT')
					AND (tabletype = 'VIEW' OR tabletype = 'TABLE')
		)   AS  A
	WHERE   RNK < 3
	COMMIT -- (^_^)
