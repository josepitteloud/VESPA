...
--                                              ,datepart(year,today())-datepart(year,demo.date_of_birth)       as age
                                                ...
                                                --,case when s1+s2+s3+s4+s5+s6 = 10 then 1 else 0 end   as digital_hh
                                                ,s4     as digital_hh
                                into    skybarb
...
--------------------------------------
				...
				*/
                ...
                from    (
                                                        ...
                                                        from    (
                                                                                ...
                                                                                select  viewing.household_number
                                                                                                ,viewing.pvf_pv2
                                                                                                ,dense_rank() over      (partition by    cast(viewing.local_start_time_of_session as date) ,viewing.household_number
                                                                                                                            order by        viewing.set_number ,viewing.local_start_time_of_session
                                                                                                                                        )   as session_id
                                                                                                ,dense_rank() over      (partition by    viewing.household_number order by        viewing.local_tv_event_start_date_time||'-'||viewing.set_number
                                                                                                                                                )   as event_id
                                                                                                ,set_number
                                                                                ...
                                                                                --from    ripolile.barb_daily_ind_prog_viewed_output  as viewing  -- [UNCOMENT FOR FINAL VERSION]
                                                ...
                                                inner join  skybarb as z on  mega.household_number   = z.house_id and mega.person_number      = z.person

                commit
				
				MESSAGE cast(now() as timestamp)||' | @ M04.1: skybarb_fullview CREATED:'||@@rowcount TO CLIENT
                ...
                MESSAGE cast(now() as timestamp)||' | @ M04.1: Final BARB Data Preparation DONE: ' ||@@rowcount TO CLIENT
