...
----------------------------
                ...
                select   event_id, overlap_batch into #longest_batch
                from
                (select          event_id, overlap_batch, dense_rank() over  (
                                                                                partition by    event_id
                                                                                order by        chunk_duration_seg desc
                                                                        )   as dur_rank
               ...
               commit
                ...
                create hg index i2 on #longest_batch(overlap_batch)
                commit
                ...
                select  dpdata.account_number
                                ...
                                ,case when overlap.event_id is null then        case when barb_min_start_date_time_utc <> '2999-12-31 00:00:00' then barb_min_start_date_time_utc else null end -- Not overlap, so just need to deal with events that have not been MA
                                      when overlap.event_id is not null then -- Overlaps
                                                case when l.overlap_batch is null
                ...
                from    v289_m07_dp_data_tempshelf          as dpdata
                                ...
                                left join #longest_batch l on overlap.event_id = l.event_id and overlap.overlap_batch = l.overlap_batch
