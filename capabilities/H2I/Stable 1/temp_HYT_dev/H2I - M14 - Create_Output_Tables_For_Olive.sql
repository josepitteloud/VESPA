...
AS BEGIN
	...
	-- M14.1 Create monthly output tables for Olive
	-----------------------------------------------
	MESSAGE CAST(NOW() AS TIMESTAMP)||' | Begining M14.1 - Determining required monthly output table' TO CLIENT
	...
	-----------------------------------------------
	MESSAGE CAST(NOW() AS TIMESTAMP)||' | Begining M14.2 - Append H2I outputs to monthly tables' TO CLIENT
...
--*/
		...
		BEGIN
			...
			IF NOT EXISTS   (
				...
				BEGIN
						...
						MESSAGE CAST(NOW() AS TIMESTAMP)||' | Begining M14.2 - Creating new monthly table : ' || @I_TAB TO CLIENT
			...
			MESSAGE CAST(NOW() AS TIMESTAMP)||' | Begining M14.2 - Inserting results into monthly table : ' || @I_TAB || ' ...' TO CLIENT
			...
			MESSAGE CAST(NOW() AS TIMESTAMP)||' | Begining M14.2 - Inserting results into monthly table : ' || @I_TAB || ' ...DONE. ' || @@ROWCOUNT || ' rows affected.' TO CLIENT
	...
	-----------------------------------------------------------------
	-- M14.3 Create view so that points to the last 2 month's of data
	-----------------------------------------------------------------
	MESSAGE CAST(NOW() AS TIMESTAMP)||' | Begining M14.3 - Create view to point to the two latest monthly tables...' TO CLIENT
	...
	MESSAGE CAST(NOW() AS TIMESTAMP)||' | Begining M14.3 - Create view to point to the two latest monthly tables...DONE' TO CLIENT
