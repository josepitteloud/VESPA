...
create or replace procedure ${SQLFILE_ARG001}.v289_m09_Session_size_definition
AS BEGIN
    ...
    create lf index tmp1_idx_2 on #tmp1(overlap_batch)
        ...
        IF EXISTS(SELECT tname FROM syscatalog WHERE creator = '${SQLFILE_ARG001}'  and tabletype = 'TABLE' and upper(tname) = 'TEMP_EVENT')   drop table ${SQLFILE_ARG001}.temp_event
        SELECT
                        event_ID
                        , dt.account_number
                        , dt.subscriber_id
        ...
        IF EXISTS(SELECT tname FROM syscatalog WHERE creator = '${SQLFILE_ARG001}'  and tabletype = 'TABLE' and upper(tname) = 'EVENTS_1_BOX')   drop table ${SQLFILE_ARG001}.events_1_box
    ...
    WHERE adj_hh < 1
        ...
        BEGIN
                ...
                IF EXISTS(SELECT tname FROM syscatalog WHERE creator = '${SQLFILE_ARG001}'  and tabletype = 'TABLE' and upper(tname) = 'EVENTS_1_BOX')   
        ...
        MESSAGE cast(now() as timestamp)||' | @ M09.4: Multi Box events updated: '||@@rowcount TO CLIENT
                ...
                ------------------------------------------------------------------------------------------------------------------------
                ------------------------------------------------------------------------------------------------------------------------
        IF EXISTS(SELECT tname FROM syscatalog WHERE creator = '${SQLFILE_ARG001}'  and tabletype = 'TABLE' and upper(tname) = 'OVERLAPPING_EV')   drop table ${SQLFILE_ARG001}.overlapping_ev
        ...
        commit
        ------------------------------------------------------------------------------------------------------------------------
       IF EXISTS(SELECT tname FROM syscatalog WHERE creator = '${SQLFILE_ARG001}'  and tabletype = 'TABLE' and upper(tname) = 'NEW_MATRIX')   drop table  ${SQLFILE_ARG001}.new_matrix
        SELECT
              mx.thedate
            , mx.segment_id
            , mx.viewing_size
