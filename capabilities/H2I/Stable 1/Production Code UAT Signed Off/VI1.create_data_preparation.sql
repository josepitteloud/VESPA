    ...
    MESSAGE cast(now() as timestamp)||' | Begining  M07.0 - Initialising Environment' TO CLIENT
        ...
        */
        ...
        begin
                ...
                truncate table ${SQLFILE_ARG001}.V289_M07_dp_data
                commit
...
-----------------------------------------
                ...
                if exists(  select tname from syscatalog
                        ...
                        and     tabletype = 'TABLE')
                        drop table ${SQLFILE_ARG001}.v289_m07_dp_data_tempshelf
                ...
                MESSAGE cast(now() as timestamp)||' | @ M07.1: Extracting duplicates events from M06 and M17' TO CLIENT

                SELECT  a.account_number
                        , a.subscriber_id
                        , min(a.pk_viewing_prog_instance_fact)  AS event_id
                        , a.event_start_date_time_utc               AS event_start_utc
                        , case  when min(COALESCE (a.capping_end_Date_time_utc, b.capping_end_Date_time_utc)) is not null then MIN(COALESCE (a.capping_end_Date_time_utc, b.capping_end_Date_time_utc))
                                else COALESCE(a.event_end_date_time_utc, b.event_end_date_time_utc)     END AS event_end_utc
                        ,min(a.dk_event_start_datehour_dim)         AS event_start_dim
                        ,min(a.dk_event_end_datehour_dim)         AS event_end_dim
                        ,datediff(ss,event_start_utc,event_end_utc) AS event_duration_seg
                        ,min(CASE WHEN a.service_key < b.service_key THEN a.service_key ELSE b.service_key  END)    AS service_key
                        ,min(case when COALESCE(a.barb_min_start_date_time_utc, b.barb_min_start_date_time_utc)  is not null then COALESCE(a.barb_min_start_date_time_utc, b.barb_min_start_date_time_utc)
                                    else '2999-12-31 00:00:00' end)     AS barb_min_start_date_time_utc
                        ,max(case when COALESCE(a.barb_min_end_date_time_utc, b.barb_min_end_date_time_utc) is not null then  COALESCE(a.barb_min_end_date_time_utc, b.barb_min_end_date_time_utc)
                                    else '1970-01-01 00:00:00' end)     AS barb_min_end_date_time_utc
                        , CAST(NULL     AS VARCHAR)         AS provider_id
                        , CAST(-1       AS INT)             AS provider_id_number
                        , CAST(1        AS tinyint)         AS viewing_type_flag
                        ,COALESCE(a.genre_description, b.genre_description) AS programme_genre
                INTO #temp_dupes
                FROM v289_M06_dp_raw_data AS a
                JOIN v289_M17_vod_raw_data As b On a.pk_viewing_prog_instance_fact = b.pk_viewing_prog_instance_fact AND a.event_start_date_time_utc = b.event_start_date_time_utc
                GROUP BY a.account_number
                        , a.subscriber_id
                        , event_start_utc
                        , COALESCE(a.event_end_date_time_utc, b.event_end_date_time_utc)
                        , programme_genre


                COMMIT
                CREATE hg   INDEX id1 ON #temp_dupes(account_number)
                CREATE DTTM INDEX id2 ON #temp_dupes(event_start_utc)
                CREATE hg   INDEX id3 ON #temp_dupes(event_id)
                COMMIT
                MESSAGE cast(now() as timestamp)||' | @ M07.1: Extracting duplicates. Done: '||@@rowcount TO CLIENT

                SELECT a.*
                INTO #temp_dupes2
                FROM v289_M06_dp_raw_data AS a
                JOIN #temp_dupes    AS b ON a.pk_viewing_prog_instance_fact= event_id
                                        AND a.event_start_date_time_utc = b.event_start_utc


                UPDATE  #temp_dupes2
                SET a.service_key = b.service_key
                    , a.barb_min_start_date_time_utc = b.barb_min_start_date_time_utc
                    , a.barb_min_end_date_time_utc = b.barb_min_end_date_time_utc
                    , a.genre_description = b.programme_genre
                FROM #temp_dupes2 as a
                JOIN #temp_dupes as b ON a.pk_viewing_prog_instance_fact = b.event_id

                COMMIT
                CREATE hg   INDEX id1 ON #temp_dupes2(account_number)
                CREATE hg   INDEX id21 ON #temp_dupes2(subscriber_id)
                CREATE hg   INDEX id3 ON #temp_dupes2(pk_viewing_prog_instance_fact)
                CREATE DTTM INDEX id2 ON #temp_dupes2(event_start_date_time_utc)
                COMMIT


                DELETE FROM v289_M06_dp_raw_data
                FROM v289_M06_dp_raw_data as a
                JOIN #temp_dupes2 AS b ON a.account_number = b.account_number
                                    AND  a.pk_viewing_prog_instance_fact = b.pk_viewing_prog_instance_fact
                                    AND  a.event_start_date_time_utc = b.event_start_date_time_utc


                INSERT INTO v289_M06_dp_raw_data
                SELECT * from #temp_dupes2
                MESSAGE cast(now() as timestamp)||' | @ M07.1: Deduped events inserted: '||@@rowcount TO CLIENT

                COMMIT

                DELETE FROM v289_M17_vod_raw_data
                FROM v289_M17_vod_raw_data as a
                JOIN #temp_dupes AS b ON a.account_number = b.account_number
                                    AND  a.pk_viewing_prog_instance_fact = b.event_id
                                    AND  a.event_start_date_time_utc = b.event_start_utc


                DROP TABLE #temp_dupes
                DROP TABLE #temp_dupes2
                COMMIT
...
-------------------------------
        ...
        update  v289_m07_dp_data_tempshelf    dpdata
                ...
                WHERE programme_genre is null OR programme_genre  ='DUMMY'
...
-----------------------------------------
                ...
                if exists(  select tname from syscatalog
                        ...
                        and     tabletype = 'TABLE')
                drop table ${SQLFILE_ARG001}.v289_m07_events_overlap
                ...
                */
                if  exists(  select tname from syscatalog
                        ...
                        and     tabletype = 'TABLE')
                        drop table ${SQLFILE_ARG001}.v289_m07_overlaps_chunks
                ...
                */
                if  exists(  select tname from syscatalog
                        ...
                        and     tabletype = 'TABLE')
                        drop table ${SQLFILE_ARG001}.v289_m07_overlap_batches
