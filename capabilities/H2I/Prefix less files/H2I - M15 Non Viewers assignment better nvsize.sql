...
-----------------------------------------         temp_house Table Creation
        ...
        FROM V289_M08_SKY_HH_composition        AS      dt
                JOIN (SELECT DISTINCT account_number FROM V289_M07_dp_data) as b ON b.account_number = dt.account_number
                GROUP BY        dt.account_number, dt.household_size
        ...
        ------------------------------
                ...
                UPDATE  temp_house
                SET a.viewing_size = b.viewing_size,
                        dif_viewer = household_size - b.viewing_size
                FROM temp_house as a
                JOIN v289_vsizealloc_matrix as b ON a.household_size = b.hh_size
...
---- step 3: calculate an overall nv% for each hhsize implied by our viewing size calculations that we are holding on temp_inds (i.e. the numbers of nvs we want to agree to)
        ...
        select household_size,sum(cast(nonviewer_size as float))/sum(cast(household_size as float))  as hh_nv_reqd
        into #a1
        from temp_inds
        ...
        select 8 as household_size,sum(cast(nonviewer_size as float))/sum(cast(household_size as float))  as hh_nv_reqd
        into #a2
        from temp_inds
...
---- step 4: update the allocations generated in step 1 by applying a ratio of (the overall nv % that we need to get at hh size level / the nv % implied by th epiv at hh size level)
        ...
        inner join #counts as c1
        on c1.sex=vm.gender and c1.age=vm.ageband and c1.household_size=vm.hhsize
        inner join #nv_perc_sexage nps
        on c1.household_size=nps.household_size
        inner join #nv_perc_sexage_reqd npsr
        on c1.household_size=npsr.household_size
        and vm.thedate=@event_date
        and vm.hhsize>1
...
MESSAGE cast(now() as timestamp)||' | @ M15.1: '|| @iteration  TO CLIENT
MESSAGE cast(now() as timestamp)||' | @ M15.1:age '|| @age  TO CLIENT
MESSAGE cast(now() as timestamp)||' | @ M15.1: sex '|| @sex  TO CLIENT
MESSAGE cast(now() as timestamp)||' | @ M15.1: hhsize '|| @hhsize  TO CLIENT
...
--------------------------
      ...
      and running_allocs<=@total_alloc and allocatable=1 and non_viewer=0
