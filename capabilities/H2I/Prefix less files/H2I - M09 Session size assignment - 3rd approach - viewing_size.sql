...
AS BEGIN
	...
	SELECT a.segment_id, a.viewer_hhsize, a.session_size, case when a.overlap_batch is null then 0 else 1 end overr
		...
		*/
		...
		------------------------------------------------------------------------------------------------------------------------
		------------------------------------------------------------------------------------------------------------------------
        IF OBJECT_ID('overlapping_ev') IS NOT NULL    DROP  TABLE overlapping_ev
        ...
        commit
        ------------------------------------------------------------------------------------------------------------------------
       IF OBJECT_ID('new_matrix') IS NOT NULL    DROP  TABLE new_matrix
        SELECT
              mx.thedate
            , mx.segment_id
            , mx.viewing_size
            , mx.session_size
            , SUM(uk_hhwatched) * (1 - COALESCE(prop,0) )     AS new_min
