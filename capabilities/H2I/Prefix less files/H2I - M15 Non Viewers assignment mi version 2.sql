...
-----------------------------------------         temp_house Table Creation
		...
		JOIN v289_nonviewers_matrix as b ON a.household_size = b.hh_size
		...
		UPDATE temp_inds
		SET PIV 			= PIV_default,
			normalization 	= random1	* PIV_default		
		FROM temp_inds as a
		JOIN V289_M10_PIV_default2 as b ON b.hhsize = a.household_size
										AND LEFT(b.sex,1) = LEFT(a.sex,1)
										AND LEFT(b.ageband,2) = LEFT(a.age,2)
										AND b.thedate	= 	 @event_date
