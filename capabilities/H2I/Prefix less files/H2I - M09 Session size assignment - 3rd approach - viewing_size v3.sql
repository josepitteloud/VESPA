...
AS BEGIN
    ...
    WHERE adj_hh < 1
        ...
        select
				...
				,		CASE WHEN sm_session_size IS NULL THEN 'f' ELSE 'd' END default_flag
        into    #tmp3
        from
                                        #tmp2                                           as      tmp
	...
	SELECT a.segment_id, a.viewer_hhsize, a.session_size, case when a.overlap_batch is null then 0 else 1 end overr
		...
		*/
		...
		------------------------------------------------------------------------------------------------------------------------
		------------------------------------------------------------------------------------------------------------------------
        IF OBJECT_ID('overlapping_ev2') IS NOT NULL    DROP  TABLE overlapping_ev2
        ...
        create lf index lfx on overlapping_ev2(segment_id)
        create lf index lfx2 on overlapping_ev2(viewing_size)
        commit
        ------------------------------------------------------------------------------------------------------------------------
		IF OBJECT_ID('new_matrix2') IS NOT NULL    DROP  TABLE new_matrix2
        SELECT
              mx.thedate
            , mx.segment_id
            , mx.viewing_size
            , mx.session_size
            , SUM(overl_minutes) OVER (PARTITION BY b.thedate, b.viewing_size, b.segment_id)  AS T_over
            , NPiv = CASE WHEN  T_over is NOT null then ( mx.proportion - (overl_minutes / b.tot_uk_hhwatched) ) / ((b.tot_uk_hhwatched - T_over) / b.tot_uk_hhwatched ) ELSE  proportion END 
            ,   CAST (0 AS FLOAT) AS  Lower_Limit
            ,   CAST (0 AS FLOAT) AS  Upper_limit
        INTO new_matrix2
        FROM v289_sessionsize_matrix    AS mx
        left JOIN overlapping_ev2       AS b ON mx.thedate = b.thedate
                                            AND mx.segment_id = b.segment_id
                                            AND mx.viewing_size = b.viewing_size
                                            AND mx.session_size = b.session_size

        SELECT *
         ,   coalesce((SUM (NPiv) OVER (PARTITION BY thedate,segment_ID, viewing_size ORDER BY session_size ROWS BETWEEN UNBOUNDED PRECEDING AND 1 PRECEDING)),0) AS Low
        ,   SUM (NPiv) OVER (PARTITION BY thedate,segment_ID, viewing_size ORDER BY session_size ROWS BETWEEN UNBOUNDED PRECEDING AND CURRENT ROW) AS Up
INTO #ttt
FROM new_matrix2
...
JOIN #ttt as b On a.segment_id= b.segment_id
        ...
        LEFT JOIN new_matrix2                       AS sm ON  sm.segment_ID = ev.segment_id 
