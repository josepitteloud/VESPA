...
MESSAGE cast(now() as timestamp)||' | Begining M05.1 - Generating v289_nonviewers_matrix_big matrix' TO CLIENT			
	...
	SET Non_viewer =0 
	FROM #tnpiv AS a
	JOIN #ttt as b ON a.house_id  =   b.household_number AND a.person = b.person_number AND a.thedate = b.date_of_activity_db1
	-------------------------------
	select
            thedate
            ,gender as  sex
            ,ageband
            ,hhsize as hh_size
            ,viewers_count as viewers
            , base_person_count_
            , non_viewers
            , SUM(non_viewers) OVER (PARTITION BY hhsize, thedate) AS  base_person_count
            , CASE WHEN base_person_count = 0 THEN 0 ELSE cast(non_viewers as decimal(15,4)) / cast(base_person_count as decimal(15,4)) END as PIV
            , CAST (0 AS DECIMAL (15,10))         AS lower_limit
            , CAST (0 AS DECIMAL (15,10))         AS upper_limit
        INTO V289_non_viewers_PIV
        from       (select  base.thedate
                    ,case
                            when age <= 19 then 'U'
                    ...
                    ,case
                     ...
                     ,sum(processing_weight) as base_person_count_
                     , non_viewers = base_person_count_ - viewers_count
                    from   (select  hhd_views.thedate, a.house_id, a.person, a.age, a.sex
                            from    skybarb a
                            inner join   (select cast(start_time_of_session as date) as thedate, household_number from skybarb_fullview group by thedate, household_number) hhd_views
                                                                    on a.house_id = hhd_views.household_number
                           ) base

                    inner join      (select house_id, count(1) as hhsize from skybarb group by house_id) s on base.house_id = s.house_id
                    inner join      barb_weights w on base.house_id = w.household_number and base.person = w.person_number
                    left join       (select cast(start_time_of_session as date) as thedate, household_number, person_number from skybarb_fullview group by thedate, household_number, person_number) viewers
