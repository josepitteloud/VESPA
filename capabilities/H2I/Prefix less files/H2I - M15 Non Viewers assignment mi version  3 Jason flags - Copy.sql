...
-----------------------------------------         temp_house Table Creation
	...
	SELECT  account_number 
		,  COUNT(DISTINCT HH_person_number) hh_size
		,  MAX(case     
					when person_ageband = '0-19'   	AND HH_person_number = 1  AND person_gender LIKE 'U'    then 1
					when person_ageband = '20-24'  	AND HH_person_number = 1  AND person_gender LIKE 'M'    then 2
					when person_ageband = '25-34'	AND HH_person_number = 1  AND person_gender LIKE 'M'    then 3
					when person_ageband = '35-44'  	AND HH_person_number = 1  AND person_gender LIKE 'M'    then 4
					when person_ageband = '45-64'  	AND HH_person_number = 1  AND person_gender LIKE 'M'    then 5
					when person_ageband = '65+'     AND HH_person_number = 1  AND person_gender LIKE 'M'    then 6

					when person_ageband = '0-19'  	AND HH_person_number = 1  AND person_gender LIKE 'U'  then 1
					when person_ageband = '20-24' 	AND HH_person_number = 1  AND person_gender LIKE 'F'  then 2
					when person_ageband = '25-34' 	AND HH_person_number = 1  AND person_gender LIKE 'F'  then 3
					when person_ageband = '35-44' 	AND HH_person_number = 1  AND person_gender LIKE 'F'  then 4
					when person_ageband = '45-64' 	AND HH_person_number = 1  AND person_gender LIKE 'F'  then 5
					when person_ageband = '65+'     AND HH_person_number = 1  AND person_gender LIKE 'F'  then 6
					end)     as p1
		,  MAX(case     
					when person_ageband = '0-19'   	AND HH_person_number = 2  AND person_gender LIKE 'U'    then 1
					when person_ageband = '20-24'  	AND HH_person_number = 2  AND person_gender LIKE 'M'    then 2
					when person_ageband = '25-34'	AND HH_person_number = 2  AND person_gender LIKE 'M'    then 3
					when person_ageband = '35-44'  	AND HH_person_number = 2  AND person_gender LIKE 'M'    then 4
					when person_ageband = '45-64'  	AND HH_person_number = 2  AND person_gender LIKE 'M'    then 5
					when person_ageband = '65+'     AND HH_person_number = 2  AND person_gender LIKE 'M'    then 6

					when person_ageband = '0-19'   	AND HH_person_number = 2  AND person_gender LIKE 'U'  then 1
					when person_ageband = '20-24'  	AND HH_person_number = 2  AND person_gender LIKE 'F'  then 2
					when person_ageband = '25-34'	AND HH_person_number = 2  AND person_gender LIKE 'F'  then 3
					when person_ageband = '35-44'  	AND HH_person_number = 2  AND person_gender LIKE 'F'  then 4
					when person_ageband = '45-64'  	AND HH_person_number = 2  AND person_gender LIKE 'F'  then 5
					when person_ageband = '65+'             AND HH_person_number = 2  AND person_gender LIKE 'F'  then 6
					end)     as p2
		, 	CAST (NULL AS VARCHAR (3)) AS hh_type
	INTO #t2person2
	FROM V289_M08_SKY_HH_composition     as a 
	JOIN (SELECT DISTINCT account_number as acct FROM V289_M07_dp_data) as b ON a.account_number = b.acct
	GROUP BY account_number
	HAVING hh_size = 2

	MESSAGE cast(now() as timestamp)||' | @ M15.1: t2person2 Table created: '||@@rowcount TO CLIENT
	
	COMMIT
	CREATE HG INDEX scs ON #t2person2(account_number)
	
	UPDATE #t2person2
	SET hh_type = CASE 	WHEN p1 = p2 	THEN p1||'-'||p2
        ...
        SELECT   
                        ...
                        , dt.household_size			AS viewing_size
		...
		LEFT JOIN #t2person2 As t2 ON t2.account_number = dt.account_number 
		WHERE dt.household_size >1 
		GROUP BY 	dt.account_number, dt.household_size, ntile_hp, hh_type
        ...
        JOIN v289_nonviewers_matrix_small as b ON a.household_size = b.hh_size
										...
										AND a.kid       	= b.kid
        WHERE household_size BETWEEN 3 AND 4
        COMMIT
