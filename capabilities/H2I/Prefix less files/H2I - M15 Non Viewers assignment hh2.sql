...
-----------------------------------------         temp_house Table Creation
	...
	SELECT  a.account_number
		, max(household_size) hh_size
		,  MAX(case     
					when person_ageband = '0-19'  	AND hh_person_number = 1   then 1
					when person_ageband = '20-24'	AND hh_person_number = 1   then 2
					when person_ageband = '25-34'  	AND hh_person_number = 1 	then 3
					when person_ageband = '35-44'  	AND hh_person_number = 1   then 4
					when person_ageband = '45-64'  	AND hh_person_number = 1  	then 5
					when person_ageband	= '65+'		AND hh_person_number = 1  	then 6

					end)     as p1
		,  MAX(case     
					when person_ageband = '0-19'  	AND hh_person_number = 2   then 1
					when person_ageband = '20-24'	AND hh_person_number = 2   then 2
					when person_ageband = '25-34'  	AND hh_person_number = 2 	then 3
					when person_ageband = '35-44'  	AND hh_person_number = 2   then 4
					when person_ageband = '45-64'  	AND hh_person_number = 2  	then 5
					when person_ageband	= '65+'		AND hh_person_number = 2  	then 6
	...
	JOIN (SELECT DISTINCT account_number FROM temp_inds) as b ON a.account_number = b. account_number 
	GROUP BY a.account_number
	HAVING   hh_size = 2
	
	COMMIT
	CREATE HG INDEX dsc ON #t2person2 (account_number)					
	COMMIT 
	...
	COMMIT
	CREATE HG INDEX dsdc ON #tt (account_number)	
	COMMIT 
	
        SELECT   
						MIN (dt.row_id) row_id_
						, dt.account_number
						...
						, max(x.hh_type) AS hh_type
                        , CAST (null AS float) 		AS random1       
                        , household_size			AS viewing_size
						, CAST (0 AS tinyint) 		AS dif_viewer
        INTO temp_house
        FROM V289_M08_SKY_HH_composition        AS      dt
		LEFT JOIN #t2person2 as x ON x.account_number = dt.account_number 
		JOIN #tt as b ON b.account_number = dt.account_number
		...
		UPDATE temp_house
		SET  random1  =   RAND(row_id_ + DATEPART(us, GETDATE()))
