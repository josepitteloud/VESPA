CREATE OR REPLACE  PROCEDURE ${SQLFILE_ARG001}.V289_M11_03_SC3I_v1_1__add_individual_data_sv (
...
BEGIN
	...
	JOIN ${SQLFILE_ARG001}.SC3I_Segments_Lookup_vkuba AS l_sc3i ON l_sc3.isba_tv_region = l_sc3i.isba_tv_region
	...
	GROUP BY account_number
		,hh_person_number
	message convert(TIMESTAMP, now()) || ' | M11.3 Checkpoint 1' || @@rowcount TO client
	COMMIT WORK
	...
	message convert(TIMESTAMP, now()) || ' | M11.3 Checkpoint 2' || @@rowcount TO client
	COMMIT WORK
	CREATE hg INDEX ind1 ON #t3 (account_number)
	CREATE lf INDEX ind2 ON #t3 (hh_person_number)
	
	TRUNCATE TABLE ${SQLFILE_ARG001}.SC3I_Todays_panel_members_sv

	COMMIT WORK
	message convert(TIMESTAMP, now()) || ' | M11.3 Checkpoint 3' || @@rowcount TO client
	...
	COMMIT WORK
	message convert(TIMESTAMP, now()) || ' | M11.3 Checkpoint 4' || @@rowcount TO client
	INSERT INTO SC3I_Todays_panel_members_sv
	SELECT t.account_number
		,t.HH_person_number
	...
	JOIN ${SQLFILE_ARG001}.SC3I_Segments_Lookup_vkuba AS l ON p.population_scaling_segment_id = l.scaling_segment_id
