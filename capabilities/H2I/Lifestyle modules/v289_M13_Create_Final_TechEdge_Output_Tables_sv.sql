...
BEGIN
	...
	IF @fresh_start = 1  AND EXISTS(SELECT tname FROM syscatalog WHERE creator = user_name() and tabletype = 'TABLE' and upper(tname) = UPPER ('V289_M13_individual_viewing_working_table_SV'))  
		DROP TABLE V289_M13_individual_viewing_working_table_SV
		
	IF NOT EXISTS(SELECT tname FROM syscatalog WHERE creator = user_name() and tabletype = 'TABLE' and upper(tname) = UPPER ('V289_M13_individual_viewing_working_table_SV'))  
		BEGIN 
	...
	WHERE dp.barb_min_start_date_time_utc IS NOT NULL
		...
		AND convert(DATE, dp_raw.event_Start_date_time_utc) BETWEEN cmap.effective_from
			AND cmap.effective_to
