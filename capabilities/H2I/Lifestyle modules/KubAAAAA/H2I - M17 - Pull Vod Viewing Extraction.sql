...
create or replace procedure v289_m17_PullVOD_data_extraction
...
--              set @dp_tname = 'sk_prod.VESPA_STREAM_VOD_VIEWING_PROG_FACT_'
...
--------------------------
                        ...
                        execute (@query)
                        commit
						 if  NOT exists(  select tname from syscatalog where creator = user_name() and upper(tname) = upper('v289_m17_pseudo') and     tabletype = 'TABLE') GOTO fatality
                        ...
                        set     event_start_date_time_utc = dateadd(ss, cast(substring(utc_time, 5,2) as integer),
                                                                                                dateadd(mi, cast(substring(utc_time, 3,2) as integer),
                                                                                                dateadd(hh, cast(substring(utc_time, 1,2) as integer), cast(utc_day_date as timestamp))))
                        from    VIQ_DATE d, VIQ_TIME t
                        where   p.dk_event_start_datehour_dim   = d.pk_datehour_dim
                        and     p.dk_event_start_time           = cast(t.pk_time_dim as integer)
                        commit


                        update  v289_m17_pseudo p
                        set     event_end_date_time_utc = dateadd(ss, cast(substring(utc_time, 5,2) as integer),
                                                                                                dateadd(mi, cast(substring(utc_time, 3,2) as integer),
                                                                                                dateadd(hh, cast(substring(utc_time, 1,2) as integer), cast(utc_day_date as timestamp))))
                        from    VIQ_DATE d, VIQ_TIME t
                        where   p.dk_event_end_datehour_dim   = d.pk_datehour_dim
                        and     p.dk_event_end_time           = cast(t.pk_time_dim as integer)
                        commit



                        update  v289_m17_pseudo p
                        set     capping_end_Date_time_utc = dateadd(ss, cast(substring(utc_time, 5,2) as integer),
                                                                                                dateadd(mi, cast(substring(utc_time, 3,2) as integer),
                                                                                                dateadd(hh, cast(substring(utc_time, 1,2) as integer), cast(utc_day_date as timestamp))))
                        from    VIQ_DATE d, VIQ_TIME t
                        where   p.dk_capped_event_end_time_datehour_dim   = d.pk_datehour_dim
                        and     p.dk_capped_event_end_time_dim           = cast(t.pk_time_dim as integer)
                        commit
                        ...
                        update  v289_m17_pseudo p
                        set     instance_start_date_time_utc =  dateadd(ss, cast(substring(utc_time, 5,2) as integer),
                                                                                                        dateadd(mi, cast(substring(utc_time, 3,2) as integer),
                                                                                                        dateadd(hh, cast(substring(utc_time, 1,2) as integer), cast(utc_day_date as timestamp))))
                        from    VIQ_DATE d, VIQ_TIME t
                        where   p.dk_instance_start_datehour_dim   = d.pk_datehour_dim
                        and     p.dk_instance_start_time       = cast(t.pk_time_dim as integer)
                        commit


                        update  v289_m17_pseudo p
                        set     instance_end_date_time_utc =  dateadd(ss, cast(substring(utc_time, 5,2) as integer),
                                                                                                        dateadd(mi, cast(substring(utc_time, 3,2) as integer),
                                                                                                        dateadd(hh, cast(substring(utc_time, 1,2) as integer), cast(utc_day_date as timestamp))))
                        from    VIQ_DATE d, VIQ_TIME t
                        where   p.dk_instance_end_datehour_dim   = d.pk_datehour_dim
                        and     p.dk_instance_end_time       = cast(t.pk_time_dim as integer)
                        commit
...
/* -- Currently minute attribution not applied to Pull Vod so these won't work. Will have to apply logic here
                        update  v289_m17_pseudo p
                        set     barb_min_start_date_time_utc =  dateadd(ss, cast(substring(utc_time, 5,2) as integer),
                                                                                                        dateadd(mi, cast(substring(utc_time, 3,2) as integer),
                                                                                                        dateadd(hh, cast(substring(utc_time, 1,2) as integer), cast(utc_day_date as timestamp))))
                        from    VIQ_DATE d, VIQ_TIME t
                        where   p.dk_barb_min_start_datehour   = d.pk_datehour_dim
                        and     p.dk_barb_min_start_time       = cast(t.pk_time_dim as integer)
                        commit


                        update  v289_m17_pseudo p
                        set     barb_min_end_date_time_utc =  dateadd(ss, cast(substring(utc_time, 5,2) as integer),
                                                                                                        dateadd(mi, cast(substring(utc_time, 3,2) as integer),
                                                                                                        dateadd(hh, cast(substring(utc_time, 1,2) as integer), cast(utc_day_date as timestamp))))
                        from    VIQ_DATE d, VIQ_TIME t
                        where   p.dk_barb_min_end_datehour   = d.pk_datehour_dim
                        and     p.dk_barb_min_end_time       = cast(t.pk_time_dim as integer)
                        commit
*/
                        ...
                        */
                        ...
                        commit
							if  exists(  select tname from syscatalog where creator = user_name() and upper(tname) = upper('v289_m17_pseudo') and     tabletype = 'TABLE')
							drop table v289_m17_pseudo
                        commit
