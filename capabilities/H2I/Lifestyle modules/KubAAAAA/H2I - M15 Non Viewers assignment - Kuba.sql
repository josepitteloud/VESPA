...
AS BEGIN
    ...
    SELECT
            ...
            , dt.account_number
            , MAX(CASE WHEN person_ageband in ('0-11', '12-19', '0-19') THEN 1 ELSE 0 END) kid
...
/*    UPDATE temp_house
    SET    viewing_size    = household_size - nvsize,
           dif_viewer      = nvsize
    FROM   temp_house th
    JOIN (select account_number, min(nonviewer_size) as nvsize from temp_inds group by account_number) i
                                        ON th.account_number  = i.account_number
    WHERE  th.dif_viewer is null
*/
