...
begin
    ...
    -- Clean up data FROM the current target profiling Thursday
        ...
        from
                ...
                inner join      jsk01.SC3I_Segments_Lookup_vkuba         as      l_sc3i  -- on l_sc3.sky_base_universe = l_sc3i.sky_base_universe -- NOT USED IN THIS VERSION				----- KUBA replace
                                                                                                                                                        ...
                                                                                                                                                        and     case
                                                                                                                                                                        when    d.household_size > 4    then    '5+'
        ...
        */
        ...
        SELECT
                ...
                ,       case
                                when    m08.household_size > 4  then    '5+'			------- KUBA hhsize
        ...
        */
        ...
        from
                ...
                inner join      jsk01.SC3I_Segments_Lookup_vkuba 		        as      l_sc3i  on      l_sc3.isba_tv_region                                            =       l_sc3i.isba_tv_region           -- finally, match the attributes to give us the new SC3I segment IDs		----- KUBA replace
                                                                                                                                                        and l_sc3.package                                                               =       l_sc3i.package
        ...
        from
                ...
                inner join  jsk01.SC3I_Segments_Lookup_vkuba  l on p.population_scaling_segment_id = l.scaling_segment_id			----- KUBA replace
        ...
        */
        ...
        SELECT
                ...
                ,       case
                                ...
                                WHEN age >= 45                          then '45+' -- Kuba ageband
                        end                                             as      ageband
        ...
        from
                                        (
                                                ...
                                                FROM    jsk01.SC3I_Segments_Lookup_vkuba  		)       as      a	----- KUBA replace         
                cross join      (
                                                SELECT  distinct age_band
                                                FROM    jsk01.SC3I_Segments_Lookup_vkuba )       as      b		----- KUBA replace
                cross join      (
                                                SELECT  distinct viewed_tv
                                                FROM    jsk01.SC3I_Segments_Lookup_vkuba )       as      c		----- KUBA replace
                cross join      (
                                                SELECT  distinct head_of_hhd
                                                FROM    jsk01.SC3I_Segments_Lookup_vkuba )       as      d		----- KUBA replace
				COMMIT -- (^_^)
                ...
                */
                inner join      (       -- Cap household sizes
                                                SELECT
                                                                household_number
                                                        ,       case
                                                                        when hhsize < 5         then    cast(hhsize as varchar(2)) 
                                                                        else                                            '5+'		-- kuba hhsize
                                                                end             as hh_gr 
                                                FROM    (
...
*/
        ...
        inner join      jsk01.SC3I_Segments_Lookup_vkuba  l							 ----- KUBA replace
        ...
        FROM            jsk01.SC3I_Segments_Lookup_vkuba         as      l			----- KUBA replace
     ...
     from
                 ...
                 inner join jsk01.SC3I_Segments_Lookup_vkuba     as      sl              on      SC3I_weighting_working_table.scaling_segment_id =       sl.scaling_segment_id		----- KUBA replace
     COMMIT -- (^_^)
     ...
     -- week to analytics team for review. ## What exactly are we checking? can we automate any of it?
        ...
        BEGIN
                        ...
                        BEGIN
                                ...
                                EXECUTE('
                                        ...
                                        FROM
                                                ...
                                                inner join      jsk01.SC3I_Segments_Lookup_vkuba        AS      ssl             ON      srs.scaling_segment_id  =       ssl.scaling_segment_id			----- KUBA replace
                                        GROUP BY
                                                        srs.sky_base_universe
                                                ,       @scaling_var
                                                ,       ssl.'||@scaling_var||'
                ...
                EXECUTE('
                        ...
                        FROM
                                                        SC3I_weighting_working_table
                                inner join      jsk01.SC3I_Segments_Lookup_vkuba        AS      ssl             ON      SC3I_weighting_working_table.scaling_segment_id =       ssl.scaling_segment_id		----- KUBA replace
