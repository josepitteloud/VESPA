...
if @Metric_to_Measure = 'vespa_dp_instances_returned_live'
begin
...
if @Metric_to_Measure = 'vespa_dp_instances_returned_recorded'
begin
...
if @Metric_to_Measure = 'vespa_dp_instances_returned_not_live_rec'
begin
...
select a.*,
metric_benchmark_check(a.metric_result, a.metric_benchmark, a.metric_tolerance_amber, a.metric_tolerance_red) metric_rag
...
if @Metric_to_Measure = 'vespa_dp_events_duration_negative'
begin
...
----------------------------------------AC01 - EVENT START TIME > THAN CAPPED EVENT END TIME--------------------------------------------------------


if @Metric_to_Measure = 'mtc_event_start_greater_capped_end'
begin

declare @viewing_date date

set @viewing_date = (select max(VIEWING_DATE) from DATA_QUALITY_DP_DATA_aUDIT)
----------------------------------------AC02 - Collect Metric Data from Metrics table--------------------------------------------------------
...
----------------------------------------AC03 - Logic to generate Metric Benchmark result------------------------------------------

-- Check capped end time is greater thant event start time

-- Check capped end time is less than or equal event end time

select sum(case when event_start_date_time_utc > capping_end_date_time_utc then 1 else 0 end) as capped_issue
into #tmp_event_start_greater_capped_end
from data_quality_dp_data_audit
where capping_end_date_time_utc is not null

select @Metric_to_Measure metric_short_name,@viewing_date viewing_date,
capped_issue metric_result
into #tmp_event_start_greater_capped_end_final
from #tmp_event_start_greater_capped_end

----------------------------------------AC04 - Generate RAG status------------------------------------------

select a.*,
metric_benchmark_check(a.metric_result, b.metric_benchmark, b.metric_tolerance_amber, b.metric_tolerance_red) metric_rag
into #tmp_event_start_greater_capped_end_final_detail
from #tmp_event_start_greater_capped_end_final a,#data_metrics_detail b
where a.metric_short_name = b.metric_short_name

---with our results in hand, lets enter them into the repository and do summit!!

----------------------------------------AC05 - insert into Vespa Repository Table------------------------------------------
...
,getdate()
from
#tmp_event_start_greater_capped_end_final_detail

commit
-----------------------------------------------------------------AC06 - Drop Temp Tables-----------------------------------------------


drop table #data_metrics_detail
drop table #tmp_event_start_greater_capped_end
drop table #tmp_event_start_greater_capped_end_final
drop table #tmp_event_start_greater_capped_end_final_detail
