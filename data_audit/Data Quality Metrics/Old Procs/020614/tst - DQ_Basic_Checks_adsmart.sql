...
dq_chk_det.sql_where_clause, dq_chk_det.fk_dq_col_id
into #tmp_dq_values
from
...
set @fk_dq_col_id =  (select fk_dq_col_id from #tmp_dq_values)


insert into tst_int values ( @dq_det_id )
insert into tst_sql values (  @unknown_value)
insert into tst_sql values ( @column_type )
insert into tst_sql values  (@expected_value)
insert into tst_int values ( @metric_benchmark )
insert into tst_int values (  @metric_tolerance_amber )
insert into tst_int values ( @metric_tolerance_red )
insert into tst_int values (  @exception_value )
insert into tst_sql values (  @notnull_col_checks )
insert into tst_sql values (  @table_sql_where_clause )
insert into tst_int values ( @fk_dq_col_id )
...
EXECUTE logger_add_event @CP2_build_ID , 3,'UNKNOWN CHECK for '||isnumeric(@unknown_value)||'.'||'testing'||'.'||@unknown_value||''
...
grant execute on DQ_Basic_Checks_adsmart to vespa_group_low_security, sk_prodreg
