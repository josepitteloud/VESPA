...
/******************************************************************************
**
** Project Vespa: Data Quality Data Processing
**
...
**
** Code sections:
**      Part A: A01 - RunType (incremental)
**
**      Part B:       Data capture
...
if object_id('data_quality_data_processing') is not null drop procedure data_quality_data_processing
commit

go

create procedure data_quality_data_processing
@run_type varchar(20)
,@CP2_build_ID     bigint = NULL
,@count_event_days int
as
begin
...
EXECUTE logger_add_event @RunID , 3,'Data Quality Process Start', @CP2_build_ID


----------------------------------A01 - Runtype--------------------------------------------------


if upper(@run_type) = 'INCREMENTAL'
begin

set @data_days_analyze = @count_event_days
...
set @sql_stmt = 'select top '||@data_days_analyze||' event_date into #analysis_event_dates_final from (select event_date from #analysis_event_dates) t order by 1 desc'
...
EXECUTE logger_add_event @RunID , 3,'Data Quality running for count of days',@data_count
...
EXECUTE logger_add_event @RunID , 3,'Data Quality Process Start for Date '||cast (@analysis_date_current as varchar(20))
...
----here in case we need to do multiple days at the same time.  Will comment it out for now----
...
log_start_date_time_utc, duration, subscriber_id, log_received_start_date_time_utc,capped_full_flag,capped_partial_flag 
...
if @data_count = 0
begin
EXECUTE logger_add_event @RunID ,2,'Refresh of Data Audit table not worked',@data_count

end

if @data_count > 0
begin
EXECUTE logger_add_event @RunID , 3,'Count of records in data_quality_dp_data_audit table',@data_count
end
...
where viewed_start_date_key/100 = cast(replace(@analysis_date_current,'-','') as int)


commit


--look to refactor above for dimension key for performance
...
if @data_count = 0
begin
EXECUTE logger_add_event @RunID ,2,'Refresh of Data Slot table not worked',@data_count

end

if @data_count > 0
begin
EXECUTE logger_add_event @RunID , 3,'Count of records in DATA_QUALITY_SLOT_DATA_AUDIT table',@data_count
end
...
grant execute on data_quality_data_processing to vespa_group_low_security, sk_prodreg
