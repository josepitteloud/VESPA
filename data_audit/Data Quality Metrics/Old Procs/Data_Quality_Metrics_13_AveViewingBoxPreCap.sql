...
SELECT CAST(event_start_date_time_utc as date) as 'Event_Start_Date'
    , cb_change_date
    , subscriber_id
    , SUM(duration) Over (Partition By subscriber_id,event_end_date_time_utc) as 'Total_Duration' --this is kept in to compare against the DATEDIFF function
    , SUM(DATEDIFF(ss,event_start_date_time_utc,event_end_date_time_utc)) Over (Partition By subscriber_id,event_end_date_time_utc) as 'Total_Viewing_Time'
    , duration
    , DATEDIFF(ss,event_start_date_time_utc,event_end_date_time_utc) as 'Viewing_Time_DateDiff'
    , COUNT(distinct(subscriber_id)) as 'Number_of_Boxes'
    --, COUNT(subscriber_id) as 'Number of Boxes3'
    , event_start_date_time_utc
    , event_end_date_time_utc
    , AVG(DATEDIFF(ss,event_start_date_time_utc,event_end_date_time_utc)) as 'Avg_Viewing_Time'
...
AND CAST(event_start_date_time_utc as date) = '2013-04-16'
