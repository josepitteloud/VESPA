...
alter table data_quality_slot_data_audit
add
(advertiser_code varchar (10),
advertiser_name varchar  (255),
buyer_code varchar (10),
buyer_name varchar (255),
buyer_source varchar (255),
buyer_type varchar (10),
barb_sales_house_id varchar(100),
media_sales_house_name varchar (255),
media_sales_house_short_name varchar(255),
break_position varchar(10),
spot_position_in_break varchar(30),
spot_type varchar(30))



grant select on DATA_QUALITY_SLOT_DATA_AUDIT to vespa_group_low_security, sk_prodreg;
...
sql_where_clause varchar(2000),
fk_dq_col_id integer);

grant select on data_quality_check_details to vespa_group_low_security, sk_prodreg;
...
grant select on data_quality_columns to vespa_group_low_security, sk_prodreg;
...
grant select on data_quality_dp_data_audit to vespa_group_low_security, sk_prodreg;
...
grant select on data_quality_results to vespa_group_low_security, sk_prodreg;
...
grant select on data_quality_run_group to vespa_group_low_security, sk_prodreg;
...
grant select on data_quality_sky_base_upscale to vespa_group_low_security, sk_prodreg;
...
grant select on data_quality_vespa_metrics to vespa_group_low_security, sk_prodreg;
...
grant select on data_quality_vespa_repository to vespa_group_low_security, sk_prodreg;
...
grant select on data_quality_vespa_repository_reporting to vespa_group_low_security, sk_prodreg;
...
grant select,insert,delete on SC2_scaling_weekly_sample_viq_dq to vespa_group_low_security, sk_prodreg;
...
IF object_id('scaling_cbi_panel') IS NOT NULL DROP table scaling_cbi_panel
...
grant select,insert,delete on scaling_cbi_panel to vespa_group_low_security, sk_prodreg;
...
IF object_id('segmentation_index_viq_dq') IS NOT NULL DROP table segmentation_index_viq_dq
...
grant select,insert,delete on segmentation_index_viq_dq to vespa_group_low_security, sk_prodreg;
...
IF object_id('scaling_variables_viq_dq') IS NOT NULL DROP table scaling_variables_viq_dq
...
grant select,insert,delete on scaling_variables_viq_dq  to vespa_group_low_security, sk_prodreg;
...
IF object_id('data_quality_scaling_accounts') IS NOT NULL DROP table data_quality_scaling_accounts

create table data_quality_scaling_accounts
(account_number varchar(20))

grant select,insert,delete on data_quality_scaling_accounts  to vespa_group_low_security, sk_prodreg;

commit;
go

IF object_id('data_quality_channel_check') IS NOT NULL DROP table data_quality_channel_check

create table data_quality_channel_check
(pk_dq_chan_chk integer identity,
service_key integer,
channel_name varchar(100),
viewing_data_date timestamp,
live_recorded varchar(8),
num_of_instances integer,
dq_run_id integer,
load_timestamp timestamp default timestamp)

grant select,insert,delete on data_quality_channel_check to vespa_group_low_security, sk_prodreg;

commit;
go


IF object_id('data_quality_channel_service_key_list') IS NOT NULL DROP table data_quality_channel_service_key_list

create table data_quality_channel_service_key_list
(dq_cskl_id	bigint	identity, 
service_key	bigint Not Null,
load_timestamp	timestamp,
modified_date	timestamp default timestamp,
current_flag bit default 1);

grant select,insert,delete on data_quality_channel_service_key_list to vespa_group_low_security, sk_prodreg;
