...
/******************************************************************************
**
** Project Vespa: Data_Quality_Household_Basic_Checks
**
...
if object_id('Data_Quality_Household_Basic_Checks') is not null drop procedure Data_Quality_Household_Basic_Checks
commit

go

create procedure Data_Quality_Household_Basic_Checks
    @run_type varchar(40) 
...
where UPPER(metric_short_name) like 'DEMO_HH%'
and UPPER(METRIC_SHORT_NAME) not like '%PERCENT_CHECK'
...
--execute dq_basic_checks @table_name,@column_name,@creator,@dq_check_type,@dq_run_id,@target_date,@CP2_build_ID,@dq_check_detail_id

end

EXECUTE logger_add_event @RunID , 3,'Data Quality Adsmart Basic Checks End for Date '||cast (@target_date as varchar(20))
...
grant execute on Data_Quality_Household_Basic_Checks to vespa_group_low_security, sk_prodreg, buxceys, kinnairt