...
**
**
**	Part X: number of live events received on a Daily Basis
...
**
**
**	Part Y: number of recorded events received on a Daily Basis
...
**
**	Part Z: number of non live or recorded events received on a Daily Basis
**		Z01 - number of non live or recorded events received on a Daily Basis
...
**
**
**	Part AA: records where duration is negative
...
**
**
**	Part AB: CAPPED END TIME > THAN EVENT START
...
**
**
**	Part AD: CAPPED EVENT DATE NOT TIME CHK
...
**
**
**	Part AE: CAPPED FLAG CONSISTENCY CHK
...
**
**	Part AF: CAPPED FLAG END TIME CONSISTENCY CHK
...
**	
**
**	Part AG: BARB START END CONSISTENCY CHK
...
**
**	Part AH: BARB MINUTE OVERLAP CONSISTENCY CHK
...
**	 
**	Part AJ: BARB START DATE/TIME END DATE/TIME CONSISTENCY CHK
...
**	 
**	Part AL: Scaled weights per viewing day
**		AL01 - Scaled weights per viewing day
**		AL02 - Collect Metric Data from Metrics table
**		AL03 - Logic to generate Metric Benchmark result
**		AL04 - Generate RAG status
**		AL05 - insert into Vespa Repository Table
**		AL06 - Drop Temp Tables
**
**  
**	Part AM: NUMBER OF SCALED ACCOUNTS PER VIEWING DAY
**		AM01 - NUMBER OF SCALED ACCOUNTS PER VIEWING DAY
**		AM02 - Collect Metric Data from Metrics table
**		AM03 - Logic to generate Metric Benchmark result
**		AM04 - Generate RAG status
**		AM05 - insert into Vespa Repository Table
**		AM06 - Drop Temp Tables
**
** 
**	Part AN: No programme id in the programme schedule table
**		AN01 - No programme id in the programme schedule table
**		AN02 - Collect Metric Data from Metrics table
**		AN03 - Logic to generate Metric Benchmark result
**		AN04 - Generate RAG status
**		AN05 - insert into Vespa Repository Table
**		AN06 - Drop Temp Tables
**
**
**	Part AP: NO CHANNEL ID IN PROGRAMME SCHEDULE TABLE
**		AP01 - NO CHANNEL ID IN PROGRAMME SCHEDULE TABLE
**		AP02 - Collect Metric Data from Metrics table
**		AP03 - Logic to generate Metric Benchmark result
**		AP04 - Generate RAG status
**		AP05 - insert into Vespa Repository Table
**		AP06 - Drop Temp Tables
**
**	Part AQ: difference in scehdule between channel and viewing table (DISCONTINUED AFTER LOSS OF VIQ_VIEWING_DATA TABLE)
**		AQ01 - difference in scehdule between channel and viewing table
**		AQ02 - Collect Metric Data from Metrics table
**		AQ03 - Logic to generate Metric Benchmark result
**		AQ04 - Generate RAG status
**		AQ05 - insert into Vespa Repository Table
**		AQ06 - Drop Temp Tables
**
**
**	Part AR: Adsmart Slot zero duration per viewing day
**		AR01 - Adsmart Slot zero duration per viewing day
**		AR02 - Collect Metric Data from Metrics table
**		AR03 - Logic to generate Metric Benchmark result
**		AR04 - Generate RAG status
**		AR05 - insert into Vespa Repository Table
**		AR06 - Drop Temp Tables
**
**	Part AS: Adsmart - number of events with weight assigned
**		AS01 - Adsmart - number of events with weight assigned
**		AS02 - Collect Metric Data from Metrics table
**		AS03 - Logic to generate Metric Benchmark result
**		AS04 - Generate RAG status
**		AS05 - insert into Vespa Repository Table
**		AS06 - Drop Temp Tables
**
**	Part AT: Number of Linear Slots with zero duration
**		
**		AT01 - Number of Linear Slots with zero duration
**		AT02 - Collect Metric Data from Metrics table
**		AT03 - Logic to generate Metric Benchmark result
**		AT04 - Generate RAG status
**		AT05 - insert into Vespa Repository Table
**		AT06 - Drop Temp Tables
**
**	Part AU: Number of Linear slots events with weight assigned per viewing day
**		
**		AU01 - Number of Linear slots events with weight assigned per viewing day
**		AU02 - Collect Metric Data from Metrics table
**		AU03 - Logic to generate Metric Benchmark result
**		AU04 - Generate RAG status
**		AU05 - insert into Vespa Repository Table
**		AU06 - Drop Temp Tables
**
**
**
**
...
select distinct viewing_date,a.account_number,event_start_date_time_utc,event_end_date_time_utc, panel_id,
live_recorded,@Metric_to_Measure metric_short_name
into #account_event_uncapped
from
(select * from 
(select a.*, event_end_date_time_utc - event_start_date_time_utc diff
from data_quality_dp_data_audit a) a
where diff between 0 and 3) a,
data_quality_scaling_accounts b
where 
a.account_number = b.account_number
and a.panel_id in (11,12)
...
select distinct viewing_date,a.account_number,event_start_date_time_utc,
...
if @Metric_to_Measure = 'mtc_scaled_accounts_per_viewing_day'
begin
...
select @Metric_to_Measure metric_short_name,@viewing_date viewing_date, count(distinct account_number) metric_result
into #mtc_scaled_accounts_per_viewing_day_final
from data_quality_scaling_accounts

----------------------------------------AL04 - Generate RAG status------------------------------------------

select a.*,b.dq_vm_id, b.metric_tolerance_amber, b.metric_tolerance_red,
metric_benchmark_check(a.metric_result, b.metric_benchmark, b.metric_tolerance_amber, b.metric_tolerance_red) metric_rag
into #mtc_scaled_accounts_per_viewing_day_final_detail
from #mtc_scaled_accounts_per_viewing_day_final a,#data_metrics_detail b
where a.metric_short_name = b.metric_short_name
...
,getdate()
from
#mtc_scaled_accounts_per_viewing_day_final_detail

commit
-----------------------------------------------------------------AL06 - Drop Temp Tables-----------------------------------------------

drop table #data_metrics_detail
drop table #mtc_scaled_accounts_per_viewing_day_final
drop table #mtc_scaled_accounts_per_viewing_day_final_detail
...
if @Metric_to_Measure = 'mtc_viq_programme_sched_no_programme'
begin
...
select @Metric_to_Measure metric_short_name,@viewing_date viewing_date,
(1.0 * sum(case when dk_programme_id = -1 then 1 else 0 end)/ count(1)) * 100 metric_result
--(1.0 * sum(case when dk_channel_id = -1 then 1 else 0 end)/ count(1)) * 100  no_channel_percent
into #mtc_viq_programme_sched_no_programme_final
from sk_prod.viq_programme_schedule viq, sk_prod.viq_date datehour
where viq.dk_start_datehour = datehour.pk_datehour_dim
and datehour.utc_day_date = @viewing_date
group by @Metric_to_Measure,@viewing_date

----------------------------------------AN04 - Generate RAG status------------------------------------------

select a.*,b.dq_vm_id, b.metric_tolerance_amber, b.metric_tolerance_red,
metric_benchmark_check(a.metric_result, b.metric_benchmark, b.metric_tolerance_amber, b.metric_tolerance_red) metric_rag
into #mtc_viq_programme_sched_no_programme_final_detail
from #mtc_viq_programme_sched_no_programme_final a,#data_metrics_detail b
where a.metric_short_name = b.metric_short_name
...
,getdate()
from
#mtc_viq_programme_sched_no_programme_final_detail

commit
-----------------------------------------------------------------AN06 - Drop Temp Tables-----------------------------------------------

drop table #data_metrics_detail
drop table #mtc_viq_programme_sched_no_programme_final
drop table #mtc_viq_programme_sched_no_programme_final_detail
...
if @Metric_to_Measure = 'mtc_viq_programme_sched_no_channel'
begin
...
select @Metric_to_Measure metric_short_name,@viewing_date viewing_date,
--(1.0 * sum(case when dk_programme_id = -1 then 1 else 0 end)/ count(1)) * 100 metric_result
(1.0 * sum(case when dk_channel_id = -1 then 1 else 0 end)/ count(1)) * 100  metric_result
into #mtc_viq_programme_sched_no_channel_final
from sk_prod.viq_programme_schedule viq, sk_prod.viq_date datehour
where viq.dk_start_datehour = datehour.pk_datehour_dim
and datehour.utc_day_date = @viewing_date
group by @Metric_to_Measure,@viewing_date
...
select a.*,b.dq_vm_id, b.metric_tolerance_amber, b.metric_tolerance_red,
metric_benchmark_check(a.metric_result, b.metric_benchmark, b.metric_tolerance_amber, b.metric_tolerance_red) metric_rag
into #mtc_viq_programme_sched_no_channel_final_detail
from #mtc_viq_programme_sched_no_channel_final a,#data_metrics_detail b
where a.metric_short_name = b.metric_short_name
...
,getdate()
from
#mtc_viq_programme_sched_no_channel_final_detail

commit
-----------------------------------------------------------------AP06 - Drop Temp Tables-----------------------------------------------

drop table #data_metrics_detail
drop table #mtc_viq_programme_sched_no_channel_final
drop table #mtc_viq_programme_sched_no_channel_final_detail
...
--no longer active after the loss of the viq_viewing_data table from Olive.


if @Metric_to_Measure = 'mtc_viq_sched_dim_fact_channel_chk'
begin
...
into #mtc_viq_sched_dim_fact_channel_chk_final
from #viewing_prog_sched_link_chk

----------------------------------------AQ04 - Generate RAG status------------------------------------------

select a.*,b.dq_vm_id, b.metric_tolerance_amber, b.metric_tolerance_red,
metric_benchmark_check(a.metric_result, b.metric_benchmark, b.metric_tolerance_amber, b.metric_tolerance_red) metric_rag
into #mtc_viq_sched_dim_fact_channel_chk_final_detail
from #mtc_viq_sched_dim_fact_channel_chk_final a,#data_metrics_detail b
where a.metric_short_name = b.metric_short_name
...
,getdate()
from
#mtc_viq_sched_dim_fact_channel_chk_final_detail

commit
-----------------------------------------------------------------AQ06 - Drop Temp Tables-----------------------------------------------

drop table #data_metrics_detail
drop table #mtc_viq_sched_dim_fact_channel_chk_final
drop table #mtc_viq_sched_dim_fact_channel_chk_final_detail
drop table #tmp_channel_active
drop table #tmp_channel_viewing_key
drop table #tmp_channel_programme_viewing
drop table #chan_prog_view_link_chk
drop table #tmp_channel_prog_sched_active
drop table #tmp_channel_schedule
drop table #chan_prog_sched_link_chk
drop table #viewing_prog_sched_link_chk
...
--number of events with zero duration


if @Metric_to_Measure = 'mtc_adsmart_slot_zero_duration_per_viewing_day'
begin

declare @viewing_date date

set @viewing_date = (select max(broadcast_start_Day_date) from data_quality_adsmart_slot_data_audit)
----------------------------------------AR02 - Collect Metric Data from Metrics table--------------------------------------------------------
...
select @Metric_to_Measure metric_short_name,@viewing_date viewing_date, 
sum(case when (VIEWED_DURATION is null or VIEWED_DURATION < 0) then 1 else 0 end) metric_result
into #mtc_adsmart_slot_zero_duration_per_viewing_day_final
from data_quality_adsmart_slot_data_audit
group by @Metric_to_Measure, @viewing_date
...
select a.*,b.dq_vm_id, b.metric_tolerance_amber, b.metric_tolerance_red,
metric_benchmark_check(a.metric_result, b.metric_benchmark, b.metric_tolerance_amber, b.metric_tolerance_red) metric_rag
into #mtc_adsmart_slot_zero_duration_per_viewing_day_final_detail
from #mtc_adsmart_slot_zero_duration_per_viewing_day_final a,#data_metrics_detail b
where a.metric_short_name = b.metric_short_name
...
,getdate()
from
#mtc_adsmart_slot_zero_duration_per_viewing_day_final_detail

commit
-----------------------------------------------------------------AR06 - Drop Temp Tables-----------------------------------------------

drop table #data_metrics_detail
drop table #mtc_adsmart_slot_zero_duration_per_viewing_day_final
drop table #mtc_adsmart_slot_zero_duration_per_viewing_day_final_detail
...
--number of events with weight assigned


if @Metric_to_Measure = 'mtc_adsmart_slot_events_weight_assigned_per_viewing_day'
begin
...
select @Metric_to_Measure metric_short_name,@viewing_date viewing_date, 
cast(sum(case when ACTUAL_WEIGHT > 0 then 1 else 0 end)as float)/cast(count(1)as float) metric_result
into #mtc_adsmart_slot_events_weight_assigned_per_viewing_day_final
from data_quality_adsmart_slot_data_audit
group by @Metric_to_Measure, @viewing_date
...
select a.*,b.dq_vm_id, b.metric_tolerance_amber, b.metric_tolerance_red,
metric_benchmark_check(a.metric_result, b.metric_benchmark, b.metric_tolerance_amber, b.metric_tolerance_red) metric_rag
into #mtc_adsmart_slot_events_weight_assigned_per_viewing_day_final_detail
from #mtc_adsmart_slot_events_weight_assigned_per_viewing_day_final a,#data_metrics_detail b
where a.metric_short_name = b.metric_short_name
...
,getdate()
from
#mtc_adsmart_slot_events_weight_assigned_per_viewing_day_final_detail

commit
-----------------------------------------------------------------AS06 - Drop Temp Tables-----------------------------------------------

drop table #data_metrics_detail
drop table #mtc_adsmart_slot_events_weight_assigned_per_viewing_day_final
drop table #mtc_adsmart_slot_events_weight_assigned_per_viewing_day_final_detail
...
if @Metric_to_Measure = 'mtc_linear_slot_zero_duration_per_viewing_day'
begin

declare @viewing_date date

set @viewing_date = (select min(slot_start_date) from data_quality_slot_data_audit)
----------------------------------------AT02 - Collect Metric Data from Metrics table--------------------------------------------------------
...
select @Metric_to_Measure metric_short_name,@viewing_date viewing_date, 
sum(case when (VIEWED_DURATION is null or VIEWED_DURATION < 0) then 1 else 0 end) metric_result
into #mtc_linear_slot_zero_duration_per_viewing_day_final
from data_quality_slot_data_audit
group by @Metric_to_Measure, @viewing_date
...
select a.*,b.dq_vm_id, b.metric_tolerance_amber, b.metric_tolerance_red,
metric_benchmark_check(a.metric_result, b.metric_benchmark, b.metric_tolerance_amber, b.metric_tolerance_red) metric_rag
into #mtc_linear_slot_zero_duration_per_viewing_day_final_detail
from #mtc_linear_slot_zero_duration_per_viewing_day_final a,#data_metrics_detail b
where a.metric_short_name = b.metric_short_name
...
,getdate()
from
#mtc_linear_slot_zero_duration_per_viewing_day_final_detail

commit
-----------------------------------------------------------------AT06 - Drop Temp Tables-----------------------------------------------

drop table #data_metrics_detail
drop table #mtc_linear_slot_zero_duration_per_viewing_day_final
drop table #mtc_linear_slot_zero_duration_per_viewing_day_final_detail
...
if @Metric_to_Measure = 'mtc_linear_slot_events_weight_assigned_per_viewing_day'
begin
...
select @Metric_to_Measure metric_short_name,@viewing_date viewing_date, 
cast(sum(case when scaling_factor > 0 then 1 else 0 end)as float)/cast(count(1)as float) metric_result
into #mtc_linear_slot_events_weight_assigned_per_viewing_day_final
from data_quality_slot_data_audit
group by @Metric_to_Measure, @viewing_date
...
select a.*,b.dq_vm_id, b.metric_tolerance_amber, b.metric_tolerance_red,
metric_benchmark_check(a.metric_result, b.metric_benchmark, b.metric_tolerance_amber, b.metric_tolerance_red) metric_rag
into #mtc_linear_slot_events_weight_assigned_per_viewing_day_final_detail
from #mtc_linear_slot_events_weight_assigned_per_viewing_day_final a,#data_metrics_detail b
where a.metric_short_name = b.metric_short_name
...
,getdate()
from
#mtc_linear_slot_events_weight_assigned_per_viewing_day_final_detail

commit
-----------------------------------------------------------------AU06 - Drop Temp Tables-----------------------------------------------

drop table #data_metrics_detail
drop table #mtc_linear_slot_events_weight_assigned_per_viewing_day_final
drop table #mtc_linear_slot_events_weight_assigned_per_viewing_day_final_detail
...
grant execute on Data_Quality_Metrics_Collection to vespa_group_low_security, sk_prodreg, buxceys,kinnairt