...
/******************************************************************************
**
** Project Vespa: Data_Quality_Adsmart_Basic_Checks
**
...
if object_id('Data_Quality_Adsmart_Basic_Checks') is not null drop procedure Data_Quality_Adsmart_Basic_Checks
commit

go

create procedure Data_Quality_Adsmart_Basic_Checks
    @run_type varchar(40) = 'ADSMART_BASIC_CHECKS'
...
declare @metric_short_name varchar(8000)
...
dq_run_grp.dq_run_id , dq_chk_det.metric_short_name
...
and UPPER(dq_col.table_name) NOT IN ('FACT_HOUSEHOLD_SEGMENT','FACT_ADSMART_SLOT_INSTANCE')
...
set @metric_short_name = (select metric_short_name from  #data_quality_run_process
where dq_check_detail_id = @dq_check_detail_id)
...
execute dq_basic_checks @table_name,@column_name,@creator,@dq_check_type,@dq_run_id,@target_date,@CP2_build_ID, @metric_short_name
