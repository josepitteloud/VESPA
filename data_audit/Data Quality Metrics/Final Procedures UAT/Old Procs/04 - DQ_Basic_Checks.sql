...
create procedure DQ_Basic_Checks
    ...
    ,@metric_short_name varchar(8000)
...
and upper(dq_chk_det.metric_short_name) = upper(@metric_short_name)
...
begin
set @var_sql = 'select count (1) INTO @unknown_chk from '||@creator||'.'||@table_name||' WHERE convert(int,'||@column_name||') = convert(int, ('||@unknown_value||'))'
...
(select coltype, length from sys.syscolumns where upper(creator) = UPPER('''||@creator||''')
AND UPPER(TNAME) = UPPER('''||@table_name||''') AND UPPER(CNAME) = UPPER('''||@column_name||''')
 UNION  SELECT COLUMN_TYPE, COLUMN_LENGTH FROM DATA_QUALITY_COLUMNS WHERE UPPER(CREATOR) = UPPER('''||@creator||''')
 AND UPPER(TABLE_NAME) = UPPER('''||@table_name||''') AND UPPER(COLUMN_NAME) = UPPER('''||@column_name||''')) T'
...
set @var_sql = 'select min('||@column_name||') INTO @min_value from '||@creator||'.'||@table_name||''
...
set @var_sql = 'select max('||@column_name||') INTO @max_value from '||@creator||'.'||@table_name||''
...
------------------------------------K02 - Metric benchmark for distinct count------------------------------------------
