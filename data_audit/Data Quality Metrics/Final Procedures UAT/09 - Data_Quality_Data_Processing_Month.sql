...
EXECUTE logger_add_event @RunID , 3,'Data Quality Process Start Month', @CP2_build_ID


----------------------------------B02 - Collect all days where events were loaded on that load date--------------------------------------------------

--set @sql_stmt = 'select * into #analysis_event_dates from (select distinct date(event_start_date_time_utc) event_date from sk_prod.vespa_dp_prog_viewed_'||@year_month||') t'

--execute (@sql_stmt)

----------------------------------B03 - collect the event days you want to process--------------------------------------------------

--set @sql_stmt = 'select event_date into #analysis_event_dates_final from (select event_date from #analysis_event_dates) t order by 1 desc'
...
grant execute on data_quality_data_processing_month to vespa_group_low_security, sk_prodreg
