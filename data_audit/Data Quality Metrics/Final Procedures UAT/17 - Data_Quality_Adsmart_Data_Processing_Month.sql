...
if object_id('data_quality_adsmart_day_processing') is not null drop procedure data_quality_adsmart_day_processing;
commit;

go


create procedure data_quality_adsmart_day_processing
@run_type varchar(20)
...
from sk_uat.FACT_ADSMART_SLOT_INSTANCE_HISTORY
...
if @slot_cnt > 0 
begin
set @fact_tbl_name = 'sk_uat.FACT_ADSMART_SLOT_INSTANCE'
end
...
EXECUTE logger_add_event @RunID , 3,'Data Quality Adsmart Process Start for Date '||cast (@analysis_date_current as varchar(20))
...
actual_weight, sample_impressions,viewed_duration 
into #adsmart_slots
...
elapsed_hours_total from sk_uat.viq_time_shift) time_shift
...
buyer_name from sk_uat.DIM_AGENCY) agency
...
from sk_uat.DIM_BROADCAST_CHANNEL) broadcast_channel
...
from sk_uat.DIM_BROADCAST_PROGRAMME_SCHEDULE) preceding_broadcast_programme
on
...
from sk_uat.DIM_BROADCAST_PROGRAMME_SCHEDULE) succ_broadcast_programme
on
...
segment_status, segment_description from sk_uat.DIM_SEGMENT) segment
on
adsmart_slots.segment_key = segment.segment_key
left outer join
(select slot_copy_key,slot_copy_duration_seconds, slot_type, 
product_code, product_name  from sk_uat.DIM_SLOT_COPY) slot_copy
on
...
adsmart_status, adsmart_total_priority from sk_uat.DIM_SLOT_REFERENCE) slot_reference
on
...
from sk_uat.viq_time) broadcast_start_time
on
...
from sk_uat.viq_time) viewing_start_time
on
...
utc_day_in_month utc_start_day_in_month, utc_day_in_week utc_start_day_in_week,
 utc_day_long utc_start_day_long from sk_uat.viq_date) broadcast_start_date
on
...
utc_day_in_month utc_viewing_start_day_in_month, utc_day_in_week utc_viewing_start_day_in_week,
 utc_day_long utc_viewing_start_day_long from sk_uat.viq_date) viewing_start_date
on
...
FROM sk_uat.FACT_ADSMART_CAMPAIGN
...
from sk_uat.dim_adsmart_campaign) dim_campaign
...
from sk_uat.dim_segment) dim_segment
on
...
from sk_uat.FACT_HOUSEHOLD_SEGMENT
...
left outer join
(select household_key, hh_has_adsmart_stb, no_of_adsmart_stb from sk_uat.viq_household) hh
on
...
grant execute on data_quality_adsmart_day_processing to vespa_group_low_security, sk_prodreg