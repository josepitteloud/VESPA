...
UPDATE ${CBAF_DB_DATA_SCHEMA}.ADSMART
SET ROI_CABLE_AVAILABLE = COALESCE(CASE WHEN b.account_number IS NOT NULL THEN 'Yes' ELSE 'No' END , 'Unknown')
FROM ${CBAF_DB_DATA_SCHEMA}.ADSMART  AS a
...
UPDATE  ${CBAF_DB_DATA_SCHEMA}.ADSMART  a
SET A.ROI_CABLE_AVAILABLE = 'Unknown'
WHERE cb_key_household NOT IN (SELECT cb_key_household FROM ${CBAF_DB_LIVE_SCHEMA}.SKY_ROI_ADDRESS_MODEL where cb_address_status = '1' )
GO
...
UPDATE ${CBAF_DB_DATA_SCHEMA}.ADSMART  a
SET a.ROI_BROADBAND_STATUS = 'Never had BB'
WHERE a.ROI_BROADBAND_STATUS IS NULL
  AND account_number IN (SELECT account_number FROM ${CBAF_DB_LIVE_SCHEMA}.CUST_SINGLE_ACCOUNT_VIEW
                                                WHERE UPPER(PTY_COUNTRY_CODE) LIKE 'IRL'
                                                        AND (cb_key_household IS NULL OR cb_key_household = 0))
GO

UPDATE ${CBAF_DB_DATA_SCHEMA}.ADSMART  a
SET a.ROI_COUNTY = 'Unknown'
WHERE a.ROI_COUNTY IS NULL
AND account_number IN (SELECT account_number FROM ${CBAF_DB_LIVE_SCHEMA}.CUST_SINGLE_ACCOUNT_VIEW
                                                WHERE UPPER(PTY_COUNTRY_CODE) LIKE 'IRL'
                                                        AND (cb_key_household IS NULL OR cb_key_household = 0))
GO
