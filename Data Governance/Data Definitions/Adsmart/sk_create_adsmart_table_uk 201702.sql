...
-- ROI Attributes - End
   ...
   SKY_CINEMA_VIEWING  VARCHAR(14) NULL DEFAULT 'Unknown',
   SKY_PL_FOOTBALL_VIEWING  VARCHAR(14) NULL DEFAULT 'Unknown',
   SKY_SPORTS_VIEWING  VARCHAR(14) NULL DEFAULT 'Unknown',
   SKY_COMEDY_VIEWING  VARCHAR(14) NULL DEFAULT 'Unknown',
   SKY_DRAMA_VIEWING  VARCHAR(14) NULL DEFAULT 'Unknown',
   NON_SKY_DRAMA_VIEWING  VARCHAR(14) NULL DEFAULT 'Unknown',
   SKY_BROADBAND_STATUS  VARCHAR(50) NULL DEFAULT 'Unknown',
-- Adsmart Attributes/ QR2 2016 START
   SKY_ENDOFFER_TV VARCHAR(13) NULL DEFAULT 'No',
   SKY_ENDOFFER_BB VARCHAR(7) NULL DEFAULT 'No',
   SKY_ENDOFFER_TALK_LR VARCHAR(7) NULL DEFAULT 'No',
   SKY_CYCLICAL  VARCHAR(7) NULL DEFAULT 'No',
   MOBILE_USAGE_SEGMENT VARCHAR(19) NULL DEFAULT 'Unknown',
   IMPRESSION_DEMAND VARCHAR(13) NULL DEFAULT 'Percent 0-9',
   REVENUE_DEMAND VARCHAR(13) NULL DEFAULT 'Percent 0-9',
   NON_SKY_SPORT_VIEWING VARCHAR(14) NULL DEFAULT 'No Viewing',
   SKYQ VARCHAR(37) NULL DEFAULT 'Unknown',
   INCOME_BANDS VARCHAR(22) NULL DEFAULT 'Unknown'
-- Adsmart Attributes/ QR2 2016 END
 )
go
...
-- Adsmart L3 Drop 1 START
 ...
 , affluence_bands
 , bundle_type
 , sky_broadband_status
-- Adsmart L3 Drop 1 END
 , sky_cinema_viewing
 , sky_pl_football_viewing
 , sky_sports_viewing
 , sky_comedy_viewing
 , sky_drama_viewing
 , non_sky_drama_viewing
-- Adsmart Attributes/ QR2 2016 START 
 , sky_endoffer_tv
 , sky_endoffer_bb
 , sky_endoffer_talk_lr
 , sky_cyclical
 , mobile_usage_segment
 , impression_demand
 , revenue_demand
 , non_sky_sport_viewing
 , skyq
 , income_bands
-- Adsmart Attributes/ QR2 2016 END
)    
...
-- Adsmart L3 Drop 1 direct SAV mapping fields START
 ...
 , sav.affluence_bands
 , CASE 
		WHEN prod_latest_entitlement_genre IN ('Original', 'Original (Legacy 2015)',  'Original (Legacy 2016)',  'Original (Legacy 2017)') 		 	THEN 'Original'
		WHEN prod_latest_entitlement_genre =  'Variety'  																	THEN 'Variety'
		WHEN prod_latest_entitlement_genre LIKE  'Box Sets%' OR prod_latest_entitlement_genre =  'Family'		 			THEN 'Family'					---- Family or Boxsets new definition according to the requirement
		WHEN prod_latest_entitlement_genre LIKE  'Sky Q Bundle%' OR prod_latest_entitlement_genre LIKE  'SkyQ Legacy%' 		THEN 'SkyQ'							---- New Sky Q bundles added and renamed
		ELSE 'Others'
   END bundle_type 
   -- querterly attribure 1 16/17
  , CASE 
      WHEN sav.prod_active_broadband_package_desc IS NULL AND PROD_EARLIEST_BROADBAND_ACTIVATION_DT  IS NULL THEN 'Never had BB'
      ----------- Lapsed customers 
 ...
 END sky_broadband_status
, 'No Viewing' as SKY_CINEMA_VIEWING
, 'No Viewing' as SKY_PL_FOOTBALL_VIEWING
, 'No Viewing' as SKY_SPORTS_VIEWING
, 'No Viewing' as SKY_COMEDY_VIEWING
, 'No Viewing' as SKY_DRAMA_VIEWING
, 'No Viewing' as NON_SKY_DRAMA_VIEWING
, 'No' as SKY_ENDOFFER_TV
, 'No' as SKY_ENDOFFER_BB
, 'No' as SKY_ENDOFFER_TALK_LR
, 'No' as SKY_CYCLICAL
, 'Unknown' as MOBILE_USAGE_SEGMENT
, 'Percent 0-9' as IMPRESSION_DEMAND
, 'Percent 0-9' as REVENUE_DEMAND
, 'No Viewing' as NON_SKY_SPORT_VIEWING
, 'Unknown' as SKYQ
, CASE
    WHEN sav.income_bands = 'Unallocated'
    THEN 'Unknown'
    WHEN sav.income_bands IS NULL
    THEN 'Unknown'
    WHEN sav.income_bands = ''
    THEN 'Unknown'
    ELSE sav.income_bands
  END as INCOME_BANDS
...
SET  A.MOSAIC_2014_TYPES        = CASE  WHEN H_MOSAIC_UK_6_TYPE LIKE '01' THEN  'World-Class Wealth'
                                        ...
                                        WHEN H_MOSAIC_UK_6_TYPE LIKE '26' THEN  'Caféand Catchments'
...
SELECT cala.account_number
   ...
   WHERE UPPER(genre_desc) LIKE UPPER('%MOVIE%')
     AND provider_brand IN ('Sky Disney','Sky Disney HD','Sky Movies','Sky Movies HD','Sky Cinema','Sky Cinema HD','Disney Movies','Disney HD' )	-- Added: Cinema/HD 
	 AND last_modified_dt >= DATEADD(YEAR, -2, NOW())		--- Limiting the search to 2 years back due to the massive size of the source table 
...
-- Update to include IMPRESSIONS_BANDS and REVENUE_BANDS (Adsmart Attributes/ QR2 2016-2017)
--------------------------------------------
...
SELECT 
	account_number
	, max (hh_band) AS hh_bands
	, max (impressions_bands) AS impressions_bands
	, max (revenue_bands) AS revenue_bands
...
SET household_campaign_demand = COALESCE(b.hh_bands, 'Percent 0-9')
    , impression_demand = COALESCE(b.impressions_bands, 'Percent 0-9')
    , revenue_demand = COALESCE(b.revenue_bands, 'Percent 0-9')
FROM ${CBAF_DB_DATA_SCHEMA}.adsmart as a 
...
FROM ${CBAF_DB_DATA_SCHEMA}.ADSMART AS a 
JOIN ${CBAF_DB_LIVE_SCHEMA}.PROMOSMART_TRAITS  AS b ON a.account_number = b.account_number 
...
SELECT DISTINCT
	...
	, rank() OVER(PARTITION BY o.account_number 			ORDER BY offer_end_date DESC, active_offer DESC, cb_row_id)      AS rankk_1
	
INTO #temp_offers	
FROM ${CBAF_DB_LIVE_SCHEMA}.cust_product_offers AS o
...
JOIN ${CBAF_DB_LIVE_SCHEMA}.cust_subscriptions AS b ON a.subscription_id = b.subscription_id
WHERE b.price > 0 
...
COMMIT 
--- Keeping the most recent offer
DELETE FROM #temp_offers_comms	 WHERE rankk_1 > 1 
--- Keeping only offers that are ending in the next 60 days 
DELETE FROM #temp_offers_comms	 WHERE offer_end_date >=  DATEADD(day, 60, GETDATE()) 
...
FROM ${CBAF_DB_LIVE_SCHEMA}.CUST_CHANGE_ATTEMPT AS a 
...
SET MOBILE_USAGE_SEGMENT = CASE 	WHEN MOBILE_SEGMENT_PROD_HH_201608 = '1)SUPER USERS' 			THEN 'Super Users'        
									...
									ELSE 'Unknown' END 
FROM ${CBAF_DB_DATA_SCHEMA}.ADSMART AS a 
JOIN ${CBAF_DB_LIVE_SCHEMA}.MOBILE_SEGMENT_PROD AS b ON a.cb_key_household = b.cb_key_household 
go
...
SELECT DISTINCT 
	a.account_number
	, prod_latest_q_ms_activation_dt AS act
    , prod_latest_q_ms_cancellation_dt AS cancel
	...
	, CASE 	WHEN act IS NULL THEN 'Never had Sky Q'
			...
			WHEN cancel_dt <= 30				AND PROD_LATEST_Q_MS_STATUS_CODE <> 'AC' THEN 'Had Q downgraded in last 0-1 month' 
			...
			AS Q_status 
INTO #temp_MS
FROM ${CBAF_DB_LIVE_SCHEMA}.CUST_SINGLE_ACCOUNT_VIEW AS a
...
SET SKYQ = CASE 	WHEN active = 1 AND new_q = 0 AND DATEDIFF (day, effective_from_dt, GETDATE()) >= 365  				THEN 'Upgrade Q customer 12+ month'
					...
					WHEN active = 0 AND DATEDIFF (day, effective_from_dt, GETDATE()) <= 30 					THEN 'Had Q downgraded in last 0-1 month' 
					ELSE SKYQ
					END 
FROM ${CBAF_DB_DATA_SCHEMA}.ADSMART AS a 
...
UPDATE ${CBAF_DB_DATA_SCHEMA}.ADSMART
SET HOMEOWNER = 'Unknown'
WHERE HOMEOWNER IS NULL
GO

UPDATE ${CBAF_DB_DATA_SCHEMA}.ADSMART
SET MIRROR_HAS_CHILDREN = 'Unknown'
WHERE MIRROR_HAS_CHILDREN IS NULL
GO

UPDATE ${CBAF_DB_DATA_SCHEMA}.ADSMART
SET CB_ADDRESS_POSTCODE_AREA = 'Unknown'
WHERE CB_ADDRESS_POSTCODE_AREA IS NULL
GO
