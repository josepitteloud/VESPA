...
SELECT DISTINCT 
	...
	, CASE 	WHEN act IS NULL THEN 'Never had Sky Q'
			...
			WHEN cancel_dt <= 30				AND PROD_LATEST_Q_MS_STATUS_CODE <> 'AC' THEN 'Had Q downgraded in last 1 month' 
			...
			WHEN cancel IS NULL AND new_customer = 1 AND act_dt <= 30 				THEN 'New Q customer less 1 month'
			...
			WHEN cancel IS NULL AND new_customer = 0 AND act_dt <= 30 				THEN 'Upgrade Q customer less 1 month'
			ELSE 'Unknown'
			END 
			AS Q_status 
INTO #temp_MS
...
SET SKYQ = CASE 	WHEN active = 1 AND new_q = 0 AND DATEDIFF (day, effective_from_dt, GETDATE()) >= 365  				THEN 'Upgrade Q customer 12+ month'
					...
					WHEN active = 1 AND new_q = 0 AND DATEDIFF (day, effective_from_dt, GETDATE()) <= 30 				THEN 'Upgrade Q customer less 1 month' 
					...
					WHEN active = 1 AND new_q = 1 AND DATEDIFF (day, effective_from_dt, GETDATE()) <= 30 				THEN 'New Q customer less 1 month' 
					...
					WHEN active = 0 AND DATEDIFF (day, effective_from_dt, GETDATE()) <= 30 					THEN 'Had Q downgraded in less 1 month' 
					ELSE SKYQ
					END 
FROM ADSMART AS a 
