...
# Description:  Sample code for deriving Loyalty metric for VIQ (derived from 
...
SELECT
 ...
 WHERE ev.programme_trans_sk = @varProgrammeId
  ...
  -- Get the metric
SELECT (1.0 * @varTotalMinutes) / (1.0 * @varAudience * @varProgrammeDuration);
...
SELECT
 ...
 WHERE ev.pk_programme_instance_dim = @varProgrammeId
      ...
      count(distinct
                CASE
                  ...
                  WHEN (ev.INSTANCE_START_DATE_TIME_UTC <= @varProgrammeStart) AND (ev.INSTANCE_END_DATE_TIME_UTC < @varProgrammeEnd) AND (datediff(MINUTE, @varProgrammeStart, ev.INSTANCE_END_DATE_TIME_UTC) + 1 >= 3) THEN account_number
                    ELSE NULL
                END) INTO @varAudience
  FROM SK_PROD.VESPA_PROGRAMME_EVENTS ev
