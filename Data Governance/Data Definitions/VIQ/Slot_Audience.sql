...
# Description:  Sample code for deriving the Slot Audience metric for VIQ (derived from 
...
CREATE VARIABLE @varSlotMinute datetime;
...
SELECT @varSlotMinute       = '2011-02-06 12:40:00';
SELECT @varObsWindow        = 716;            -- 716 hours / 30 days

SELECT
      count(*) AS Slot_Audience
  FROM {eventsTable} ev
 WHERE ev.programme_trans_sk = @varProgrammeId
   AND ev.Event_Start <= @varSlotMinute
   AND ev.Event_End >= @varSlotMinute
   AND (
...
SELECT
 ...
 WHERE ev.dk_programme_instance_dim = @varProgrammeId
   AND ev.INSTANCE_START_DATE_TIME_UTC <= @varSlotMinute
   AND ev.INSTANCE_END_DATE_TIME_UTC >= @varSlotMinute
