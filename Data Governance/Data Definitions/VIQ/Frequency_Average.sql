...
# Description:  Sample code for deriving Frequency_Average metric for VIQ (derived from 
...
SELECT
      count(distinct
                CASE
                  WHEN (ev.INSTANCE_START_DATE_TIME_UTC  <= @varProgrammeStart) AND (ev.INSTANCE_END_DATE_TIME_UTC >= @varProgrammeEnd) AND (datediff(MINUTE, @varProgrammeStart, @varProgrammeEnd) + 1 >= 3) THEN Household
                  WHEN (ev.INSTANCE_START_DATE_TIME_UTC  > @varProgrammeStart) AND (ev.INSTANCE_END_DATE_TIME_UTC < @varProgrammeEnd) AND (datediff(MINUTE, ev.INSTANCE_START_DATE_TIME_UTC, ev.INSTANCE_END_DATE_TIME_UTC) + 1 >= 3) THEN Household
                  WHEN (ev.INSTANCE_START_DATE_TIME_UTC  > @varProgrammeStart) AND (ev.INSTANCE_END_DATE_TIME_UTC >= @varProgrammeEnd) AND (datediff(MINUTE, ev.INSTANCE_START_DATE_TIME_UTC, @varProgrammeEnd) + 1 >= 3) THEN Household
                  WHEN (ev.INSTANCE_START_DATE_TIME_UTC  <= @varProgrammeStart) AND (ev.INSTANCE_END_DATE_TIME_UTC < @varProgrammeEnd) AND (datediff(MINUTE, @varProgrammeStart, ev.INSTANCE_END_DATE_TIME_UTC) + 1 >= 3) THEN Household
                    ELSE NULL
                END) INTO @varReach
