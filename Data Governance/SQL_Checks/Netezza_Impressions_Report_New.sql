...
(case when adsmart_campaigns is null then 0 else adsmart_campaigns end) adsmart_campaigns,
(case when adsmart_segments is null then 0 else adsmart_Segments end) adsmart_segments,
(case when records_count is null then 0 else records_count end) records_count
from
(SELECT 
...
count(distinct slot.DK_adsmart_media_CAMPAIGN_DIM) adsmart_campaigns,
count(distinct slot.dk_adsmart_segment_dim) adsmart_segments
from
( select actual_impression actual_impressions,  dk_broadcast_start_datehour_dim,
dk_slot_instance_dim,DK_adsmart_media_CAMPAIGN_DIM,dk_adsmart_segment_dim
from SMI_ACCESS..V_VIEWING_SLOT_INSTANCE_FACT
--where actual_impression > 0) slot,
where DK_adsmart_media_CAMPAIGN_DIM > 0) slot,
...
and date1.broadcast_day_date between '2014-01-19' and date(now())
...
(SELECT DH.BROADCAST_DAY_DATE, SUM(VSIF_I.ACTUAL_IMPRESSION) AS TOTAL_IMPRESSION	
	...
	JOIN SMI_DW..DATEHOUR_DIM DH	
		...
		AND DH.BROADCAST_DAY_DATE between '2014-01-19' and date(now())
...
and d.ADSMART_FLAG = 'Y'
and slot.DK_adsmart_media_CAMPAIGN_DIM > 0
and date1.utc_day_date between '2014-01-19' and date(now())
group by date1.utc_day_date) C
LEFT OUTER JOIN
(select date1.utc_day_date DELETE_BROADCAST_DATE
,count(1) DELETE_records_count
,sum(case when slot.actual_impression > 0 then 1 else 0 end) DELETE_records_with_impression
,sum(case when slot.actual_impression <= 0 then 1 else 0 end) DELETE_records_with_no_impression
,sum(slot.Actual_impression) DELETE_actual_impressions_sum
...
ON C.FACT_BROADCAST_DATE = A.DELETE_BROADCAST_DATE
LEFT OUTER JOIN
(select date1.utc_day_date INSERT_BROADCAST_DATE
,count(1) INSERT_records_count
,sum(case when slot.actual_impression > 0 then 1 else 0 end) INSERT_records_with_impression
,sum(case when slot.actual_impression <= 0 then 1 else 0 end) INSERT_records_with_no_impression
,sum(slot.Actual_impression) INSERT_actual_impressions_sum
...
and slot.actual_impression > 0
and date1.utc_day_date between '2014-01-19' and date(now())
group by date1.utc_day_date) B
ON C.FACT_BROADCAST_dATE = B.INSERT_BROADCAST_DATE
left outer join
(SELECT DH.utc_day_date, SUM(VSIF_I.ACTUAL_IMPRESSION) AS TOTAL_IMPRESSION	
	FROM SMI_EXPORT..VIEWING_SLOT_INSTANCE_FACT_DELETES VSIF_D	
	JOIN SMI_EXPORT..VIEWING_SLOT_INSTANCE_FACT VSIF_I	
		ON VSIF_D.PK_VIEWING_SLOT_INSTANCE_FACT = VSIF_I.PK_VIEWING_SLOT_INSTANCE_FACT
	JOIN SMI_DW..DATEHOUR_DIM DH	
		ON VSIF_D.DK_BROADCAST_START_DATEHOUR_DIM = DH.PK_DATEHOUR_DIM
		AND VSIF_I.ACTUAL_IMPRESSION > 0
		AND DH.utc_day_date between '2014-01-19' and date(now())
	GROUP BY DH.utc_day_date) d
on c.fact_broadcast_date = d.utc_day_date) a
order by 1 
...
SELECT b.utc_day_date, count(distinct pk_household_segment_fact) number_of_household_records
  FROM SMI_DW..HOUSEHOLD_SEGMENT_FACT a,
  smi_dw..DATEHOUR_DIM b
  where a.dk_segment_date_dim = (b.pk_datehour_Dim/100)
  and dk_segment_date_dim between 20131218 and to_char(current_timestamp,'YYYYMMDD')
group by b.utc_day_date
