--   create table panel_data(   -- used to be called alt_panel_data which changed due to daily panel being included. cortb 23-09-2013
--                subscriber_id int
--               ,panel         tinyint
--               ,dt            date
...
begin
    ...
    /* --commented out as this deletes data from panel_data table to be replaced by data_dump, but the data for days changed in each subsequent file submission. cortb 13/11/2013
     if @panel = 6 begin
          select @dt = max('20' || dateformat(date1,'dd-mm-yy')) from data_dump
          delete from vespa_analysts.panel_data where dt < @dt
     end
    */ 

     set @counter=7  -- set counter according to how many days you want to update i.e. set @counter=5 for past 3 days and uncomment loop.
     ...
     --while @counter <= 7 begin
         ...
         execute (@sql)
             /*set @counter = @counter + 1  -- add back in if using while loop

     end */ 

     set @panel = @panel + 1
     if @panel = 8 begin set @panel = 11 end
...
select * from vespa_analysts.panel_data
where panel<>12
and dt < '2013-10-07'
and dt >= '2013-08-01'
*/
