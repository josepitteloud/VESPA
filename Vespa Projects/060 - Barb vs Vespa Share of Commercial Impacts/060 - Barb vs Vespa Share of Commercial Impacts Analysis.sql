...
from
        vespa_analysts.project060_all_viewing base left outer join vespa_max_caps_apr29_2012 caps
    on (
...
--Create Local Tiem version o fwhen event starts to use for VOSDAL Calculation
...
commit;
set @min_tx_start_time_local = cast ('2012-04-29 06:00:00' as datetime);
set @max_tx_end_time_local = cast ('2012-04-30 06:00:00' as datetime);
...
select * from #weighting_details;

output to 'C:\Users\barnetd\Documents\Project 060 - Share of Consolidated Impacts\subscriber_weights.csv' format ascii;

commit;

--select count(*) from vespa_analysts.scaling_weightings;

exec gen_create_table 'vespa_analysts.scaling_segments_lookup';



select subscriber_id
,minute
,sum(case when seconds_viewed_in_minute >=31 then 1 else 0 end) as total_households_viewing_multiple_channels_over_31_seconds
,sum(case when seconds_viewed_in_minute >=1 then 1 else 0 end) as total_households_viewing_multiple_channels_any_seconds
into #summary_by_sub_and_minute
from vespa_analysts.All_viewing_minute_by_minute_20120429
group by subscriber_id
,minute
;

--select top 100 * from #summary_by_sub_and_minute;
commit;


--select top 100 * from gillh.sov_daily_records_capped;
