...
---PART B  - Create Capping levels using version 1b   - 1% Capping for Playback 10% capping for Live
---PART C  - Apply Capping values to viewing data
---PART D  - Apply Scaling levels using version 1b
---PART E - Create the BARB Style Viewing Attribution per Broadcast Minute
---PART F - Import BARB Data (Spot Data, Lookups for sold spots Service Key etc.,)
---PART G - Create Outputs - Summary of Spots and Summary Of Impacts
...
--select count(distinct subscriber_id) as subscribers , count(distinct account_number) as accounts from vespa_analysts.project060_all_viewing
...
--Two Seperate Tables produced 1 for 1% capping (Playback) and One for 10% capping (Live)
--The Relevant Value form the two tables then put together in a single table to provide capping limit
--------------------------------------------------------------------------------
...
--------------------------------------------------------------------------------
--------------------------------------------------------------------------------
-- B02 - Identify extreme viewing and populate max and min daily caps
--------------------------------------------------------------------------------
...
--There are differing kinds of viewing activity
...
---Part 1 Start of minute where no whole real time minute is covered in the viewing event (or where event starts at minute start and only 1 whole minute covered)
--
/*
...
drop table vespa_analysts.project060_partial_minutes_for_allocation;
drop table vespa_analysts.project060_partial_minutes_grouped_by_broadcast_minute;
drop table vespa_analysts.project060_rank_viewing_in_minute_partial_minutes;
drop table vespa_analysts.project060_total_seconds_viewed_in_minute;
...
--select top 500 * from vespa_analysts.project060_allocated_minutes_total;

------------------------------------------------
---PART F - Import BARB Data for 29th April-----
------------------------------------------------
commit;
...
--Create Summary of Spot Volumes-----
...
;
*/
--Check for double counting in minute allocation--
--drop table vespa_analysts.project060_spot_summary_viewing_figures;
---Get Views by Spot
...
commit;
--select  * from vespa_analysts.project060_allocated_minutes_total where subscriber_id =34903 order by real_time_minute_start;
--select subscriber_id ,real_time_minute_start,count(*) as records into #dupe from vespa_analysts.project060_allocated_minutes_total group by subscriber_id ,real_time_minute_start having records >1
--select count(*) from #dupe
--select  * from #dupe
...
from vespa_analysts.project060_spot_summary_viewing_figures;
...
---select distinct epg_channel from sk_prod.vespa_epg_dim where epg_channel like '%MTV%'
select * from sk_prod.vespa_epg_dim where tx_date = '20120429' and epg_channel like '%MTV Live HD%' --epg_channel in ('MTVN HD','MTVN HD (MTV Live HD from 23/4/12)')
select * from vespa_analysts.project060_spot_file_20120429_expanded where log_station_code_for_spot = '05013'
select * from vespa_analysts.project060_spot_file_20120429_expanded where channel_name like '%MTV%'

select channel_name , service_key, log_station_code_for_spot,split_transmission_indicator , count(*) 
from vespa_analysts.project060_spot_file_20120429_expanded where channel_name like '%MTV%'
group by channel_name , service_key, log_station_code_for_spot,split_transmission_indicator

select channel_name , service_key, log_station_code_for_spot,split_transmission_indicator , count(*) 
from vespa_analysts.project060_spot_file_20120429_expanded where channel_name is null
group by channel_name , service_key, log_station_code_for_spot,split_transmission_indicator
...
select  log_station_code_for_spot,split_transmission_indicator , count(*) as records
from vespa_analysts.project060_spot_file_20120429
group by  log_station_code_for_spot,split_transmission_indicator
order by records desc

select * from vespa_analysts.project060_spot_file_20120429
where upper(preceding_programme_name)
=
'MTV LIVE: SNOW PATROL'

select distinct log_station_code_for_spot from vespa_analysts.project060_spot_file_20120429 where upper(preceding_programme_name) like '%MTV%'

commit;

select channel_name_inc_hd , sum(minutes_diff+1) as minutes_allocated from vespa_analysts.project060_full_allocated_minutes group by channel_name_inc_hd order by minutes_allocated desc

minutes_allocated
94,561,851

select top 10 * from vespa_analysts.project060_all_viewing
select top 10 *  from vespa_analysts.project060_full_allocated_minutes

select service_key ,channel_name, sum(capped_x_programme_viewed_duration/60) as minutes_duration from vespa_analysts.project060_all_viewing group by service_key ,channel_name order by minutes_duration desc


*/


/*    Comparing Seconds viewed and minutes allocated---
--Viewing Table
select service_key , sum(capped_x_programme_viewed_duration/60) as minutes_duration 
from vespa_analysts.project060_all_viewing 
group by service_key  order by minutes_duration desc

---Minute Allocation table 

select service_key , sum(datediff(minute,real_time_minute_start,real_time_minute_end)+1) as minutes_allocated 
from vespa_analysts.project060_allocated_minutes_total
group by service_key 
...
---Repeat but by Subscriber_id


select subscriber_id , sum(capped_x_programme_viewed_duration/60) as minutes_duration 
into #full_viewing_minutes_allocated_50_min
from vespa_analysts.project060_all_viewing 
group by subscriber_id 
having minutes_duration=50
 order by minutes_duration desc;

drop table #total_allocated_minutes;
select subscriber_id , sum(datediff(minute,real_time_minute_start,real_time_minute_end)+1) as minutes_allocated 
into #total_allocated_minutes
from vespa_analysts.project060_allocated_minutes_total
group by subscriber_id 
order by minutes_allocated desc


select a.*
,b.minutes_allocated
from #full_viewing_minutes_allocated_50_min as a
left outer join #total_allocated_minutes as b
on a.subscriber_id = b.subscriber_id




select * from vespa_analysts.project060_allocated_minutes_total where subscriber_id = 15849745 order by real_time_minute_start,real_time_minute_end


select service_key ,capped_x_programme_viewed_duration, channel_name_inc_hd ,  capped_x_viewing_start_time_local,capped_x_viewing_end_time_local
 from vespa_analysts.project060_all_viewing where  subscriber_id = 15849745 
...
select service_key , channel_name_inc_hd ,  capped_x_viewing_start_time_local,capped_x_viewing_end_time_local 
from vespa_analysts.project060_all_viewing where  subscriber_id = 161640 
...
select count(*)
,sum(case when total_seconds_viewed_of_broadcast_minute>=15 and total_seconds_viewed_in_minute_by_subscriber>=30 then 1 else 0 end) as allowed_min
from vespa_analysts.project060_rank_viewing_in_minute_partial_minutes as a
left outer join vespa_analysts.project060_total_seconds_viewed_in_minute as b
on a.subscriber_id=b.subscriber_id and a.real_time_minute = b.real_time_minute


select real_time_minute_start,real_time_minute_end,datediff (minute,real_time_minute_end,real_time_minute_start) from vespa_analysts.project060_allocated_minutes_total;


select datediff (minute,real_time_minute_start,real_time_minute_end)+1 as minutes_viewed , count(*) from vespa_analysts.project060_allocated_minutes_total group by minutes_viewed order by minutes_viewed;




select * from vespa_analysts.project060_allocated_minutes_total where real_time_minute_start>real_time_minute_end
...
select * from vespa_analysts.project060_spot_file_20120429_expanded_4002

select service_key , count(*)  from vespa_analysts.project060_spot_file_20120429_expanded group by service_key;



select * into  vespa_analysts.project060_spot_file_20120429_expanded_4002 from vespa_analysts.project060_spot_file_20120429_expanded where service_key=4002;

select * into  vespa_analysts.project060_allocated_minutes_total_4002 from vespa_analysts.project060_allocated_minutes_total where service_key=4002

select * into  vespa_analysts.project060_all_viewing_4002 from vespa_analysts.project060_all_viewing  where service_key=4002

select top 500 * from vespa_analysts.project060_allocated_minutes_total_4002 order by subscriber_id;


--select * from vespa_analysts.project060_allocated_minutes_total_4002 where subscriber_id = 15606;

select a.spot_start_time
,corrected_spot_transmission_start_minute
,sum(case when a.corrected_spot_transmission_start_minute between b.first_broadcast_minute and b.last_broadcast_minute then 1 else 0 end) as unweighted_views
,sum(case when a.corrected_spot_transmission_start_minute between b.first_broadcast_minute and b.last_broadcast_minute then weighting_value else 0 end) as weighted_views

from vespa_analysts.project060_spot_file_20120429_expanded_4002 as a
left outer join vespa_analysts.project060_allocated_minutes_total_4002 as b
on a.service_key=b.service_key
where a.service_key is not null and b.subscriber_id = 25026
group by a.spot_start_time
,corrected_spot_transmission_start_minute
order by spot_start_time
,corrected_spot_transmission_start_minute
;

select * from vespa_analysts.project060_spot_summary_viewing_figures_4002;



subscriber_id,service_key,real_time_minute_start,real_time_minute_end,first_broadcast_minute,last_broadcast_minute,live_timeshifted_type,weighting_value
15606,4002,'2012-04-29 19:49:00.000','2012-04-29 19:49:00.000','2012-04-29 19:49:00.000','2012-04-29 19:49:00.000','01: Live',35.76822706373966
15606,4002,'2012-04-29 19:59:00.000','2012-04-29 19:59:00.000','2012-04-29 19:59:00.000','2012-04-29 19:59:00.000','01: Live',35.76822706373966
15606,4002,'2012-04-29 19:50:00.000','2012-04-29 19:53:00.000','2012-04-29 19:50:00.000','2012-04-29 19:53:00.000','01: Live',35.76822706373966
15606,4002,'2012-04-29 20:00:00.000','2012-04-29 20:29:00.000','2012-04-29 20:00:00.000','2012-04-29 20:29:00.000','01: Live',35.76822706373966




commit;
select * from vespa_analysts.project060_full_allocated_minutes where subscriber_id = 161640 order by real_time_minute_start,real_time_minute_end

select service_key , channel_name_inc_hd ,  capped_x_viewing_start_time_local,capped_x_viewing_end_time_local
 from vespa_analysts.project060_all_viewing where  subscriber_id = 34903 
