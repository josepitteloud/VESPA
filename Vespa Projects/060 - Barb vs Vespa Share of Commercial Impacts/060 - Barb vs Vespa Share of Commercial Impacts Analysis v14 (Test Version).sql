...
,60 - datepart(second, capped_x_viewing_start_time)  as seconds_viewed_in_minute
...
--into vespa_analysts.project060_all_viewing_in_a_minute
from vespa_analysts.project060_all_viewing 
...
,datediff(minute,real_time_minute_start,real_time_minute_end),start_broadcast_minute_viewed
...
set @minute_end_time_local = cast ('2012-04-30 05:59:00' as datetime);
--set @minute_start_time_local = cast ('2012-04-29 18:05:00' as datetime);
set @minute= @minute_start_time_local;
--select @min_tx_start_time;
--select @minute_end_time_local;

---Loop by Channel---
if object_id('vespa_analysts.project060_allocated_minutes') is not null drop table vespa_analysts.project060_allocated_minutes;
commit;

create table vespa_analysts.project060_allocated_minutes
(subscriber_id  decimal(8,0)
,channel_name_inc_hd varchar(128)
,real_time_minute datetime
,broadcast_minute datetime
,live_timeshifted_type varchar(64)
,weighting_value double
);
commit;

--alter table vespa_analysts.project060_minute_allocation add test_col integer;
--select @minute;

---Start of Loop

--delete  from vespa_analysts.project060_all_viewing_in_a_minute where subscriber_id is not null; commit;


---Loop by Minute---
    WHILE @minute < @minute_end_time_local begin
   
insert into vespa_analysts.project060_all_viewing_in_a_minute
select subscriber_id
,channel_name_inc_hd
,live_timeshifted_type
,weighting
,@minute as real_time_minute
,capped_x_viewing_start_time_local 
, capped_x_viewing_end_time_local 
, viewing_record_start_time_local
 , viewing_record_end_time_local
...
,case when capped_x_viewing_start_time_local>@minute then 0 else datediff(second,capped_x_viewing_start_time_local,@minute) end as seconds_from_minute_start
, dateadd(second,seconds_from_minute_start,viewing_record_start_time_local) as first_second_viewed_in_real_time_minute
, dateadd(second,seconds_viewed_in_minute-1,first_second_viewed_in_real_time_minute) as last_second_viewed_in_real_time_minute
...
where capped_x_viewing_start_time_local is not null and live_timeshifted_type<>'04: Not Within 164h Window'
and (play_back_speed is null or play_back_speed = 2) and (
        (capped_x_viewing_start_time_local<=@minute and capped_x_viewing_end_time_local>@minute)
    or
        (capped_x_viewing_start_time_local between @minute and dateadd(second,59,@minute)))
...
--select count(*) from vespa_analysts.project060_all_viewing_in_a_minute;




create hg index idx1 on vespa_analysts.project060_all_viewing_in_a_minute(subscriber_id);
create hg index idx2 on vespa_analysts.project060_all_viewing_in_a_minute(real_time_minute);

--Create a second table which has both the first (and the second where applicable) broadcast minute viewed within the clock minute
...
,seconds_viewed_of_first_broadcast_minute as seconds_viewed_of_broadcast_minute
,capped_x_viewing_start_time_local
into vespa_analysts.project060_all_viewing_split_by_broadcast_minute_viewed
from vespa_analysts.project060_all_viewing_in_a_minute;



insert into vespa_analysts.project060_all_viewing_split_by_broadcast_minute_viewed
select subscriber_id
,channel_name_inc_hd
,live_timeshifted_type
,weighting
...
,seconds_viewed_of_second_broadcast_minute as seconds_viewed_of_broadcast_minute
,capped_x_viewing_start_time_local
from vespa_analysts.project060_all_viewing_in_a_minute
...
commit;
create hg index idx1 on vespa_analysts.project060_all_viewing_split_by_broadcast_minute_viewed(subscriber_id);
create hg index idx2 on vespa_analysts.project060_all_viewing_split_by_broadcast_minute_viewed(real_time_minute);
...
,sum(seconds_viewed_of_broadcast_minute) as total_seconds_viewed_of_broadcast_minute
into vespa_analysts.project060_all_viewing_grouped_by_broadcast_minute
from vespa_analysts.project060_all_viewing_split_by_broadcast_minute_viewed
...
commit;
create hg index idx1 on vespa_analysts.project060_all_viewing_grouped_by_broadcast_minute(subscriber_id);
create hg index idx2 on vespa_analysts.project060_all_viewing_grouped_by_broadcast_minute(real_time_minute);

--Take the maximum number of seconds viewed by viewing type/service key (where a tioe take the one that occurs first--
...
,broadcast_minute
,weighting_value
,time_broadcast_minute_event_started
...
into vespa_analysts.project060_rank_viewing_in_minute
from vespa_analysts.project060_all_viewing_grouped_by_broadcast_minute
;

commit;
create hg index idx1 on vespa_analysts.project060_rank_viewing_in_minute(most_watched_record);
create hg index idx2 on vespa_analysts.project060_rank_viewing_in_minute(subscriber_id);
create hg index idx3 on vespa_analysts.project060_rank_viewing_in_minute(real_time_minute);

delete from vespa_analysts.project060_rank_viewing_in_minute where most_watched_record <>1;
--select * from vespa_analysts.project060_rank_viewing_in_minute where subscriber_id = 95533
...
--Append Qualifying Results to summary table--


insert into vespa_analysts.project060_allocated_minutes
...
,live_timeshifted_type
,weighting_value
from vespa_analysts.project060_rank_viewing_in_minute as a
...
commit;
-----Drop all Tables that are to be recreated on Next Iteration-----
drop table vespa_analysts.project060_all_viewing_in_a_minute;
drop table vespa_analysts.project060_all_viewing_split_by_broadcast_minute_viewed;
drop table  vespa_analysts.project060_all_viewing_grouped_by_broadcast_minute;
drop table vespa_analysts.project060_rank_viewing_in_minute;
...
--select subscriber_id , count(*) as records from vespa_analysts.project060_allocated_minutes group by subscriber_id order by records desc
--select * from vespa_analysts.project060_allocated_minutes;
...
select * from vespa_analysts.project060_rank_viewing_in_minute order by subscriber_id , most_watched_record;


commit;

select * from vespa_analysts.project060_all_viewing_split_by_broadcast_minute_viewed where subscriber_id = 66016
...
dateadd(datediff(seconds,(dateadd (minute,1,@minute) , capped_x_viewing_start_time_local)),viewing_record_start_time_local)

where  (@minute between capped_x_viewing_start_time_local as minute and capped_x_viewing_end_time_local)


,dateadd(seconds,
            datediff(seconds,
                             dateadd (minute,1,@minute)
                                     , capped_x_viewing_start_time_local)
            ,viewing_record_start_time_local
        )
...
,max(case   when live_timeshifted_type = '01: Live' then  @minute
            when viewing_record_end_time_local>@minute then dateadd(second,60,@minute)
            when viewing
                as end_broadcast_minute_viewed



 and viewing_record_end_time_local>dateadd(second,59,@minute) then 60 when 
    ...
    end) as seconds_viewed_in_minute                        as broadcast_viewed_minute_min
                    end as broadcast_viewed_max

, case when live_timeshifted_type = '01: Live' then  @minute         as broadcast_viewed_minute
,live_timeshifted_type 
,weighting
...
dateadd(seconds,(datediff(second,(dateadd (minute,1,'2012-04-29 16:05:00') , capped_x_viewing_start_time)),viewing_record_start_time_local))
from  vespa_analysts.project060_all_viewing

select    capped_x_viewing_start_time
,viewing_record_start_time_local
,datediff (second,cast('2012-04-29 16:05:00' as datetime), capped_x_viewing_start_time)
,dateadd(second,(datediff (second,cast('2012-04-29 16:05:00' as datetime), capped_x_viewing_start_time)),viewing_record_start_time_local)
from  vespa_analysts.project060_all_viewing
...
select * from  sk_prod.VESPA_STB_PROG_EVENTS_20120429
where subscriber_id = 30926
