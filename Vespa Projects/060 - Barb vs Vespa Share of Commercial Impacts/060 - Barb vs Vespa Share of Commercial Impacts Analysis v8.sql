...
set @minute_end_time_local = cast ('2012-04-30 05:59:59' as datetime);
...
---Loop by Channel---
if object_id('vespa_analysts.project060_minute_allocation') is not null drop table vespa_analysts.project060_minute_allocation;
commit;
create table vespa_analysts.project060_minute_allocation
(
subscriber_id  bigint           null
,service_key  varchar(40)
,minute                 datetime            not null
,broadcast_minute_viewed          datetime            not null
,live_timeshifted_type            varchar(30)
,weighting double
,seconds_viewed_in_minute integer
);
commit;
...
---Start of Loop
--drop table vespa_analysts.project060_minute_allocation;
...
---Loop by Minute---
    ...
    insert into vespa_analysts.project060_minute_allocation
...
--,case when  live_timeshifted_type = '01: Live' then  dateadd(second,seconds_viewed_in_minute-1,@minute)      
--   else  dateadd(second,seconds_viewed_in_minute-1,viewing_record_start_time_local) end as last_broadcast_second_viewed_in_clock_minute
...
, dateadd(second,seconds_from_minute_start,viewing_record_start_time_local) as first_second_viewed_in_clock_minute
, dateadd(second,seconds_viewed_in_minute-1,first_second_viewed_in_clock_minute) as last_second_viewed_in_clock_minute

,dateadd(second, - datepart(second, first_second_viewed_in_clock_minute), first_second_viewed_in_clock_minute) as first_minute_of_broadcast_viewed
,dateadd(second, - datepart(second, last_second_viewed_in_clock_minute), last_second_viewed_in_clock_minute) as last_minute_of_broadcast_viewed

,case when first_minute_of_broadcast_viewed=last_minute_of_broadcast_viewed then seconds_viewed_in_minute
      else 60 - cast(datepart(second, first_second_viewed_in_clock_minute) as integer) end as seconds_viewed_of_first_broadcast_minute
,seconds_viewed_in_minute - seconds_viewed_of_first_broadcast_minute   as seconds_viewed_of_second_broadcast_minute
--,cast(   end as start_broadcast_minute_viewed
...
into vespa_analysts.project060_all_viewing_in_a_minute
 from vespa_analysts.project060_all_viewing 
where capped_x_viewing_start_time_local is not null
...
--and subscriber_id = 30926
order by subscriber_id , capped_x_viewing_start_time_local
;

--select * from #test_table where subscriber_id = 30926  
...
--where first_minute_of_broadcast_viewed=last_minute_of_broadcast_viewed 
--and subscriber_id = 30926
;
...
,rank() over (partition by subscriber_id,clock_minute order by total_seconds_viewed_of_broadcast_minute desc ,time_broadcast_minute_event_started) as most_watched_record
