...
select account_number
,case when b.minute is not null then b.minute else c.minute end as viewed_minute
,max(case when b.channel_name_inc_hd = 'Sky Sports 2' and b.seconds_viewed_in_minute_live>=31 then primary_box_weight else 0 end) as primary_box_sky_sports_2
,max(case when c.channel_name_inc_hd = 'Sky Sports 2' and c.seconds_viewed_in_minute_live>=31 then primary_box_weight else 0 end) as non_primary_box_sky_sports_2
into #account_summary_by_minute_viewing
...
into #account_summary_by_minute_viewing_non_primary
from #multi_box_account_summary as a
left outer join vespa_analysts.All_viewing_minute_by_minute_weighted_20111129_by_channel as b
on a.primary_subscriber_id = b.subscriber_id
--left outer join vespa_analysts.All_viewing_minute_by_minute_weighted_20111129_by_channel as c
...
from #account_summary_by_minute_viewing_non_primary
group by account_number
,viewed_minute
;
...
,sum(case when non_primary_box_sky_sports_2>0 then non_primary_box_sky_sports_2 else 0 end) as non_primary_box_only_ss2
...
,sum(case when primary_box_sky_sports_2>0 and non_primary_box_sky_sports_2>0 then non_primary_box_sky_sports_2 else 0 end) as both_boxes_ss2
from #account_summary_by_minute_viewing
group by viewed_minute
order by viewed_minute
;
