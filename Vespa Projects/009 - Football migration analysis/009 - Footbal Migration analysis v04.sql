...
select account_number
        ...
        ,SUBSCRIPTION_SUB_TYPE
        , rank() over (partition by account_number ,SUBSCRIPTION_SUB_TYPE order by effective_from_dt, cb_row_id) as rank
into #sky_HD_sub_accounts -- drop table #sky_accounts
from sk_prod.cust_subs_hist as csh
where SUBSCRIPTION_SUB_TYPE in ('DTV HD') --the DTV + Multiroom sub Type
   and status_code in ('AC','PC')               --Active Status Codes (ACtive Block Removed)
   and effective_from_dt <= @target_date             --Start on or before 1st Jan
   and effective_to_dt > @target_date                --ends after 1st Jan
   and effective_from_dt<>effective_to_dt            --ignore all but the last thing each customer did in a day
...
create hg index idx1 on #sky_HD_sub_accounts(service_instance_id);

alter table vespa_analysts.sky_base_2011_11_29 add active_hd_subscription integer default 0;

select days_returning_data_2011_11_29 ,,count(*) as boxes , sum(case when acc.service_instance_id is not null then 1 else 0 end) as hd_act
from vespa_analysts.sky_base_2011_11_29 base
left outer join #sky_HD_sub_accounts acc
on base.service_instance_id = acc.service_instance_id
group by days_returning_data_2011_11_29;
commit;
*/
--select top 500 * from #sky_HD_sub_accounts ;
...
;
commit;
--select box_type , pvr , x_pvr_type ,x_description, count(*) from vespa_analysts.sky_base_2011_11_29 group by  box_type , pvr , x_pvr_type,x_description
...
--select hd_sub , sum(boxes) as all_boxes_on_base, sum(box_data_20111129) as boxes_returning_data from vespa_analysts.stratifiedsampling_20111129 group by hd_sub order by hd_sub;
...
--------Part D    - Minute By Minute for Broadcast (Non-Weighted)  ----
...
set @min_tx_start_time_local = cast ('2011-11-29 19:00:00' as datetime);
...
if object_id('vespa_analysts.All_viewing_minute_by_minute_unweighted_20111129') is not null drop table vespa_analysts.All_viewing_minute_by_minute_unweighted_20111129;
commit;
create table vespa_analysts.All_viewing_minute_by_minute_unweighted_20111129
(
...
---Loop by Minute---
    ...
    insert into vespa_analysts.All_viewing_minute_by_minute_unweighted_20111129
...
and programme_trans_sk in (201111300000015098,
201111300000000212,
201111300000003259,
201111300000002325,
201111300000001177,
...
select minute
,sum(case when seconds_viewed_in_minute >=31 then 1 else 0 end) as total_households
,sum(case   when seconds_viewed_in_minute_playback_within_163_hours >=31 then 0 when seconds_viewed_in_minute_playback_within_163_hours>seconds_viewed_in_minute_live then 0 
            when seconds_viewed_in_minute >=31 then 1 else 0 end) as total_households_live
...
,sum(case   when seconds_viewed_in_minute_live >=31 then 0 when seconds_viewed_in_minute_live>=seconds_viewed_in_minute_playback_within_163_hours then 0 
            when seconds_viewed_in_minute >=31 then 1 else 0 end) as total_households_playback_163h

from vespa_analysts.All_viewing_minute_by_minute_unweighted_20111129
group by minute
order by minute
;
 commit;
...
--------Part E    - Minute By Minute for Broadcast (Non-Weighted) by channel ----
...
--select channel_name_inc_hd ,channel_name, count(*) from  vespa_analysts.VESPA_all_viewing_records_20110811_20110818 group by channel_name_inc_hd ,channel_name order by channel_name_inc_hd ,channel_name;
--select top 500 * from vespa_analysts.VESPA_all_viewing_records_20111129_20111206;
--select count(*) from vespa_analysts.All_viewing_minute_by_minute_20110811;
if object_id('vespa_analysts.All_viewing_minute_by_minute_unweighted_20111129_by_channel') is not null drop table vespa_analysts.All_viewing_minute_by_minute_unweighted_20111129_by_channel;
commit;
create table vespa_analysts.All_viewing_minute_by_minute_unweighted_20111129_by_channel
(
subscriber_id  bigint           null
...
---Loop by Minute---
    WHILE @minute < @max_tx_end_time_local LOOP
    insert into vespa_analysts.All_viewing_minute_by_minute_unweighted_20111129_by_channel
    select a.subscriber_id
    ,channel_name_inc_hd
    ,@minute as minute
...
from vespa_analysts.All_viewing_minute_by_minute_unweighted_20111129_by_channel
group by minute
    ,channel_name_inc_hd
order by minute
    ,channel_name_inc_hd
...
,min(case when seconds_viewed_in_minute_playback_within_163_hours >=31 then null when seconds_viewed_in_minute_playback_within_163_hours>seconds_viewed_in_minute_live then null 
            when seconds_viewed_in_minute >=31 then minute else null end) as first_minute_watched_live

,min(case   when seconds_viewed_in_minute_live >=31 then null when seconds_viewed_in_minute_live>=seconds_viewed_in_minute_playback_within_163_hours then null 
            when seconds_viewed_in_minute >=31 then minute else null end) as first_minute_watched_playback

,max(case when seconds_viewed_in_minute_playback_within_163_hours >=31 then null when seconds_viewed_in_minute_playback_within_163_hours>seconds_viewed_in_minute_live then null 
            when seconds_viewed_in_minute >=31 then minute else null end) as last_minute_watched_live

,max(case   when seconds_viewed_in_minute_live >=31 then null when seconds_viewed_in_minute_live>=seconds_viewed_in_minute_playback_within_163_hours then null 
            when seconds_viewed_in_minute >=31 then minute else null end) as last_minute_watched_playback
...
from vespa_analysts.All_viewing_minute_by_minute_weighted_20111129_by_channel
where seconds_viewed_in_minute >=31 and 
    (   (channel_name_inc_hd ='Sky Sports 2' and minute between '2011-11-29 19:30:00' and '2011-11-29 22:59:00')
     OR (channel_name_inc_hd ='Sky 3D' and minute between '2011-11-29 19:30:00' and '2011-11-29 21:59:00')
...
----Run Weighted by channel (splitting Out HD/Non HD)---
...
---Quick Analysis for Just Sky Sports 2 and Sky 3D---
select * into #ssp2_data from vespa_analysts.VESPA_all_viewing_records_20111129_20111206

where channel_name in (
'Sky Sports 2',
'Sky Sports HD2','Sky 3D');
commit;

create hg index idx1 on #ssp2_data (subscriber_id);

if object_id('vespa_analysts.All_viewing_minute_by_minute_weighted_20111129_by_programme_sk') is not null drop table vespa_analysts.All_viewing_minute_by_minute_weighted_20111129_by_programme_sk;
commit;
create table vespa_analysts.All_viewing_minute_by_minute_weighted_20111129_by_programme_sk
(
subscriber_id  bigint           null
...
---Loop by Minute---
    WHILE @minute < @max_tx_end_time_local LOOP
    insert into vespa_analysts.All_viewing_minute_by_minute_weighted_20111129_by_programme_sk
    select a.subscriber_id
    ,channel_name
    ,@minute as minute
...
--select distinct channel_name from vespa_analysts.VESPA_all_viewing_records_20111129_20111206 order by channel_name
--select * from  vespa_analysts.VESPA_all_viewing_records_20111129_20111206  where channel_name = 'Sky Sports HD2';
/*
select * from vespa_analysts.VESPA_all_viewing_records_20111129_20111206 where subscriber_id =27759;

select * into #ssp2_data_progs from vespa_analysts.VESPA_all_viewing_records_20111129_20111206
...
select top 500 * from #ssp2_data_progs 
select count(distinct subscriber_id) from #ssp2_data_progs 
commit;
*/
...
from  vespa_analysts.All_viewing_minute_by_minute_weighted_20111129_by_programme_sk
...
--select top 100 * from  vespa_analysts.All_viewing_minute_by_minute_weighted_20111129_by_programme_sk;


--------------Look at Analysis for 20:42 to 


select *  from  vespa_analysts.All_viewing_minute_by_minute_weighted_20111129_by_programme_sk where minute = '2011-11-29 20:42:00';

--drop table #viewed_20_42;
select * into #viewed_20_42 from  vespa_analysts.All_viewing_minute_by_minute_weighted_20111129_by_programme_sk where minute = '2011-11-29 20:42:00'
and seconds_viewed_in_minute>=31;

--select * from #viewed_20_42;

select count(*),sum(weighted_boxes) 
,sum(case   when seconds_viewed_in_minute <31 then 0 
            when seconds_viewed_in_minute_playback_within_163_hours >=31 then 0 
            when seconds_viewed_in_minute_playback_within_163_hours>=seconds_viewed_in_minute_live then 0 
            else weighted_boxes end) as total_households_live

,sum(case   when seconds_viewed_in_minute <31 then 0 
            when seconds_viewed_in_minute_playback_within_163_hours >=31 then weighted_boxes 
            when seconds_viewed_in_minute_playback_within_163_hours>=seconds_viewed_in_minute_live 
            then weighted_boxes else 0 end) as total_households_playback_163h
from  #viewed_20_42 
;

select pvr, package ,primary_box,case when acc.service_instance_id is not null then 1 else 0 end as hd_act
,sum(days_returning_data_2011_11_29),sum( weight_2011_11_29) as weighted_boxes_2011_11_29
,count(*) as boxes
,sum(case   when seconds_viewed_in_minute <31 then 0 
            when seconds_viewed_in_minute_playback_within_163_hours >=31 then 0 
            when seconds_viewed_in_minute_playback_within_163_hours>=seconds_viewed_in_minute_live then 0 
...
,sum(case   when seconds_viewed_in_minute is null then 0
            when seconds_viewed_in_minute <31 then 0 
            when seconds_viewed_in_minute_playback_within_163_hours >=31 then 0 
            when seconds_viewed_in_minute_playback_within_163_hours>=seconds_viewed_in_minute_live then 0 
            else 1 end) as total_households_boxes_live

,sum(case   when seconds_viewed_in_minute <31 then 0 
            when seconds_viewed_in_minute_playback_within_163_hours >=31 then 1 
            when seconds_viewed_in_minute_playback_within_163_hours>=seconds_viewed_in_minute_live 
            then 1 else 0 end) as total_households_boxes_playback_163h

from vespa_analysts.sky_base_2011_11_29 as a
left outer join #viewed_20_42 as b
on a.subscriber_id = b.subscriber_id

left outer join #sky_HD_sub_accounts acc
on a.service_instance_id = acc.service_instance_id
group by pvr, package ,primary_box, hd_act
;

commit;
--select top 100 * from vespa_analysts.sky_base_2011_11_29;

select box_type, pvr, package ,primary_box
,sum(days_returning_data_2011_11_29),sum( weight_2011_11_29) as weighted_boxes_2011_11_29
,count(*) as boxes
,sum(days_returning_data_2011_11_29*weight_2011_11_29) as tot_weighted_boxes
,sum(case   when seconds_viewed_in_minute <31 then 0 
            when seconds_viewed_in_minute_playback_within_163_hours >=31 then 0 
            when seconds_viewed_in_minute_playback_within_163_hours>=seconds_viewed_in_minute_live then 0 
            else weighted_boxes end) as total_households_live
...
left outer join #viewed_20_42 as b
on a.subscriber_id = b.subscriber_id
group by  box_type, pvr, package ,primary_box
;
/*
select sum(days_returning_data_2011_11_29*weight_2011_11_29) as tot_weighted_boxes_2011_11_29
,sum(days_returning_data_2011_11_30*weight_2011_11_30) as tot_weighted_boxes_2011_11_30
,sum(days_returning_data_2011_12_01*weight_2011_12_01) as tot_weighted_boxes_2011_12_01
,sum(days_returning_data_2011_12_02*weight_2011_12_02) as tot_weighted_boxes_2011_12_02
,sum(days_returning_data_2011_12_03*weight_2011_12_03) as tot_weighted_boxes_2011_12_03
,sum(days_returning_data_2011_12_04*weight_2011_12_04) as tot_weighted_boxes_2011_12_04
,sum(days_returning_data_2011_12_05*weight_2011_12_05) as tot_weighted_boxes_2011_12_05
,sum(days_returning_data_2011_12_06*weight_2011_12_06) as tot_weighted_boxes_2011_12_06
from vespa_analysts.sky_base_2011_11_29
*/
...
--select sum(days_returning_data_2011_11_29),sum(days_returning_data_2011_11_30)  from vespa_analysts.sky_base_2011_11_29;


--select * from vespa_analysts.VESPA_all_viewing_records_20111129_20111206 where subscriber_id = 24535650
...
select account_number
        ...
        ,effective_from_dt
        ,effective_to_dt
        from sk_prod.cust_subs_hist where account_number ='210128118762'
        order by SUBSCRIPTION_SUB_TYPE,effective_from_dt

select * from sk_prod.cust_set_top_box

select pty_country_code , acct_status_code from sk_prod.cust_single_Account_view where account_number ='210128118762'

select * from vespa_analysts.sky_base_2011_11_29  where account_number ='210128118762'
*/
...
---All Boxes that return data for 29th Nov--

select subscriber_id
into #subs_with_viewing_activity
from vespa_analysts.daily_summary_by_subscriber_20111129
where events_2011_11_29=1
;

--drop table #sub_viewing_summary;
select a.account_number
,a.subscriber_id
,max(case when c.subscriber_id is not null then 1 else 0 end) as sub_watching_football
,max(c.tot_dur) as max_seconds_game_viewed
into #sub_viewing_summary
from vespa_analysts.VESPA_all_viewing_records_20111129_20111206 as a
left outer join #subs_with_viewing_activity as b
on a.subscriber_id=b.subscriber_id
left outer join #football_viewers as c
...
select account_number
,max(max_seconds_game_viewed) as max_game_viewed_seconds
,count(*) as subs
,min(sub_watching_football) as min_watch_game
,max(sub_watching_football) as max_watch_game
into #account_summary
from  #sub_viewing_summary
group by account_number
;
...
,max_watch_game
,count(*) as records
,sum(case when max_game_viewed_seconds>=300 then 1 else 0 end) as watched_05_min_plus_game
,sum(case when max_game_viewed_seconds>=600 then 1 else 0 end) as watched_10_min_plus_game
,sum(case when max_game_viewed_seconds>=1200 then 1 else 0 end) as watched_20_min_plus_game
,sum(case when max_game_viewed_seconds>=1800 then 1 else 0 end) as watched_30_min_plus_game
,sum(case when max_game_viewed_seconds>=3600 then 1 else 0 end) as watched_60_min_plus_game
...
select sum(primary_box) ,count(*) from vespa_analysts.sky_base_2011_11_29

select programme_trans_sk,epg_title , channel_name
from sk_prod.vespa_epg_dim 
where tx_start_datetime_utc between
'2011-11-29 16:00:00' and  '2011-11-29 23:00:00'
and epg_title ='Arsenal v Man City-Live'
order by programme_trans_sk
;


programme_trans_sk,count(distinct VESPA_all_viewing_records_20111129_20111206.subscriber_id)
201111300000000346,13423
201111300000013327,3582
201111300000016803,24

programme_trans_sk,epg_title,channel_name
201111300000000212,'Arsenal v Man City-Live','Sky Sports 2'
201111300000000346,'Arsenal v Man City-Live','Sky Sports 2'
201111300000001177,'Arsenal v Man City-Live','Sky Sports 2'
201111300000002325,'Arsenal v Man City-Live','Sky Sports 2'
201111300000003259,'Arsenal v Man City-Live','Sky Sports HD2'
201111300000013327,'Arsenal v Man City-Live','Sky Sports HD2'
201111300000014851,'Arsenal v Man City-Live','Sky Sports HD2'
201111300000015098,'Arsenal v Man City-Live','Sky 3D'
201111300000016803,'Arsenal v Man City-Live','Sky 3D'


select top 100 * from vespa_analysts.VESPA_all_viewing_records_20111129_20111206 


select programme_trans_sk ,count(distinct subscriber_id) ,sum(x_programme_viewed_duration) from vespa_analysts.VESPA_all_viewing_records_20111129_20111206 
where programme_trans_sk in
(201111300000015098,
201111300000000212,
201111300000003259,
...
select count(distinct a.subscriber_id) as subs 
from  vespa_analysts.VESPA_all_viewing_records_20111129_20111206 as a    ---283082

select count(distinct a.subscriber_id) as subs 
from  vespa_analysts.VESPA_all_viewing_records_20111129_20111206 as a   ---276022
left outer join vespa_analysts.sky_base_2011_11_29 as b
on  a.subscriber_id=b.subscriber_id
where b.subscriber_id is not null
...
,vw.cb_row_ID,vw.Account_Number,vw.Subscriber_Id,vw.Cb_Key_Household,vw.Cb_Key_Family
        ...
        ,prog.Channel_Name,prog.Epg_Title,prog.Genre_Description,prog.Sub_Genre_Description
, sum(x_programme_viewed_duration) over (partition by subscriber_id, adjusted_event_start_time order by cb_row_id) as x_cumul_programme_viewed_duration 
     from sk_prod.VESPA_STB_PROG_EVENTS_20111129 as vw
          left outer join   sk_prod.VESPA_EPG_DIM as prog
          on vw.programme_trans_sk = prog.programme_trans_sk
        -- Filter for viewing events during extraction
    where subscriber_id = 27759 
/*and 
video_playing_flag = 1
     and adjusted_event_start_time <> x_adjusted_event_end_time
...
select x_type_of_viewing_event , count(*)  from sk_prod.VESPA_STB_PROG_EVENTS_20111010 group by x_type_of_viewing_event order by x_type_of_viewing_event;
commit;
*/
