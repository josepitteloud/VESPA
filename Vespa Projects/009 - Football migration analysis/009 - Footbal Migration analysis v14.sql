...
o	Editorial 
        ...
        Tables
        -------
        vespa_analysts.VESPA_all_viewing_records_20111129_20111206
...
----Add on current_short_description of that accounts primary box to ALL boxes---
--alter table vespa_analysts.sky_base_2011_11_29 delete primary_box_current_short_description;
alter table vespa_analysts.sky_base_2011_11_29 add primary_box_current_short_description varchar(20);


update vespa_analysts.sky_base_2011_11_29 as a
set primary_box_current_short_description = b.current_short_description
from vespa_analysts.sky_base_2011_11_29 as a
left outer join vespa_analysts.sky_base_2011_11_29 as b
on a.account_number = b.account_number
where a.subscriber_id<>b.subscriber_id and a.subscription_sub_type = 'DTV Extra Subscription'
;

update vespa_analysts.sky_base_2011_11_29 as a
set primary_box_current_short_description = case when primary_box_current_short_description is null then current_short_description else primary_box_current_short_description end
from vespa_analysts.sky_base_2011_11_29 as a
;

commit;

---Find if Household is a multi box household---
alter table vespa_analysts.sky_base_2011_11_29 add multi_box_household tinyint;


select account_number
,count(*) as number_of_boxes
into #number_of_boxes_in_hh
from vespa_analysts.sky_base_2011_11_29

group by account_number
;
commit;
create hg index idx1 on #number_of_boxes_in_hh(account_number);
update vespa_analysts.sky_base_2011_11_29 as a
set multi_box_household = case when b.number_of_boxes>1 then 1 else 0 end
from vespa_analysts.sky_base_2011_11_29 as a
left outer join #number_of_boxes_in_hh as b
on a.account_number = b.account_number
;
...
--select primary_box_current_short_description ,count(*) as records from vespa_analysts.sky_base_2011_11_29 where current_short_description = 'ADDSUBS' group by primary_box_current_short_description order by records desc

--select top 500 * from vespa_analysts.sky_base_2011_11_29 where primary_box_current_short_description = 'ADDSUBS' 
--select  * from vespa_analysts.sky_base_2011_11_29 where account_number in ('200000002663','200000235644')
...
---Add Tenure----
alter table vespa_analysts.sky_base_2011_11_29 add tenure varchar (32);
update vespa_analysts.sky_base_2011_11_29
   set tenure             = case
                                    when datediff(day, ph_subs_first_activation_dt,cast ('2011-11-29' as date)) <    180 then 'a) < 6 Months'
                                    when datediff(day, ph_subs_first_activation_dt,cast ('2011-11-29' as date)) <    365 then 'b) 6 Month - < 1 Year'
                                    when datediff(day, ph_subs_first_activation_dt,cast ('2011-11-29' as date)) <    730 then 'c) 1 Year - <2 Years'
                                    when datediff(day, ph_subs_first_activation_dt,cast ('2011-11-29' as date)) <    1095 then 'd) 2 Year - <3 Years'
                                    when datediff(day, ph_subs_first_activation_dt,cast ('2011-11-29' as date)) <    1825 then 'e) 3 Year - <5 Years'
                                    when datediff(day, ph_subs_first_activation_dt,cast ('2011-11-29' as date)) <   3650 then 'f) 5 Year - <10 Years'
                                    when     datediff(day, ph_subs_first_activation_dt,cast ('2011-11-29' as date)) >=3650  then 'g) 10+ Years'
                                      else 'a) < 6 Months'
                                  end
from vespa_analysts.sky_base_2011_11_29  as a
left outer join sk_prod.cust_single_account_view as b
...
---Add Value Segment----
alter table vespa_analysts.sky_base_2011_11_29 add value_segment varchar (32);
update vespa_analysts.sky_base_2011_11_29
   set value_segment             = case when b.value_segment is null then 'Bedding In' else b.value_segment end
from vespa_analysts.sky_base_2011_11_29  as a
left outer join sk_prod.VALUE_SEGMENTS_FIVE_YRS as b
on a.account_number = b.account_number
where value_seg_date='2011-11-02'
;
commit;

--
--select tenure , count(distinct account_number) from  vespa_analysts.sky_base_2011_11_29 group by tenure order by tenure;
--select value_segment , count(distinct account_number) from  vespa_analysts.sky_base_2011_11_29 group by value_segment order by value_segment;
...
----Part D Output of Minute by Minute Results
...
----Run Total of number of boxes watching each channel during the analysis period ---
----This is to detrmine which channels to include on graphs (i.e., Material volume)and which to aggregate as otehr
...
----Repeat Minute by Minure Views but for individual viewing segments
...
---Split By minute for Heavily Interested Segment---
...
on a.subscriber_id=b.subscriber_id
where b.subscriber_id is not null 
and watched_1st_half_live_minutes >=16 and watched_2nd_half_live_minutes between 16 and 30
--and minute between '2011-11-29 19:30:00' and '2011-11-29 22:59:00'
group by minute
order by minute
;
...
---Split By minute for Mainly First Half Segment---
...
on a.subscriber_id=b.subscriber_id
where b.subscriber_id is not null 
and watched_1st_half_live_minutes >=31 and watched_2nd_half_live_minutes < 16 
--and minute between '2011-11-29 19:30:00' and '2011-11-29 22:59:00'
group by minute
order by minute
;
...
---Split By minute for None of Match Segment---
...
on a.subscriber_id=b.subscriber_id
where b.subscriber_id is not null 
and watched_1st_half_live_minutes =0 and watched_2nd_half_live_minutes =0
--and minute between '2011-11-29 19:30:00' and '2011-11-29 22:59:00'
group by minute
order by minute
;
...
-----Create Profiles of Differing Viewer Segments---

select case when watched_1st_half_live_minutes >= 31 
and watched_2nd_half_live_minutes >= 31 
then '01: Fully Comitted Viewer'
when watched_1st_half_live_minutes >= 16 
and watched_2nd_half_live_minutes >= 16 
then '02: Heavily Interested'
when watched_1st_half_live_minutes < 16 
and watched_2nd_half_live_minutes >= 16 
then '03: Latecomers'
when watched_1st_half_live_minutes >=31
and watched_2nd_half_live_minutes < 16 
then '04: Mainly First Half'

when watched_1st_half_live_minutes >0 then '05: Partially Interested'
when watched_2nd_half_live_minutes > 0 then  '05: Partially Interested'

when watched_1st_half_live_minutes =0
and watched_2nd_half_live_minutes =0
then '06: None of Match'
 else '07: Other'

 end as viewer_segment
,sum(case when primary_box =1 then weighted_box_value_min else 0 end) as primary_boxes
,sum(case when primary_box =0 then weighted_box_value_min else 0 end) as secondary_boxes
,count(*) as boxes
,sum(weighted_box_value_min) as weighted_boxes

from #subscriber_events_summary as a
left outer join vespa_analysts.sky_base_2011_11_29 as b 
on a.subscriber_id=b.subscriber_id
group by viewer_segment
order by viewer_segment;

----Repeat profile for Affluence
...
,sum(case when hhafflu ='Very High' then weighted_box_value_min else 0 end) as Very_high_affluence
,sum(case when hhafflu ='High' then weighted_box_value_min else 0 end) as High_affluence
,sum(case when hhafflu ='Mid High' then weighted_box_value_min else 0 end) as Mid_high_affluence
,sum(case when hhafflu ='Mid' then weighted_box_value_min else 0 end) as Mid_affluence
,sum(case when hhafflu ='Mid Low' then weighted_box_value_min else 0 end) as Mid_Low_affluence
,sum(case when hhafflu ='Low' then weighted_box_value_min else 0 end) as Low_affluence
,sum(case when hhafflu ='Very Low' then weighted_box_value_min else 0 end) as Very_low_affluence
,sum(case when hhafflu ='Unknown' then weighted_box_value_min else 0 end) as Unknown_affluence
,count(*) as boxes
,sum(weighted_box_value_min) as weighted_boxes
...
,primary_box
,sum(case when x_box_type ='Basic' then weighted_box_value_min else 0 end) as Basic_Box
,sum(case when x_box_type ='Basic HD' then weighted_box_value_min else 0 end) as Basic_HD_Box
,sum(case when x_box_type= 'Sky+' then weighted_box_value_min else 0 end) as Sky_plus_Box
,sum(case when x_box_type ='Sky+HD' then weighted_box_value_min else 0 end) as HD_Box
,sum(case when x_box_type is null then weighted_box_value_min else 0 end) as Unknown_Box
,count(*) as boxes
,sum(weighted_box_value_min) as weighted_boxes
...
left outer join vespa_analysts.sky_base_2011_11_29 as b 
on a.subscriber_id=b.subscriber_id
group by viewer_segment ,primary_box
order by viewer_segment,primary_box;



---Repeat for DTV Package
select case when watched_1st_half_live_minutes >= 31 
and watched_2nd_half_live_minutes >= 31 
then '01: Fully Comitted Viewer'
when watched_1st_half_live_minutes >= 16 
...
,primary_box
,sum(case when c.ptb_value=4 then weighted_box_value_min else 0 end) as Top_Tier
,sum(case when c.ptb_value<>4 and prem_sports=2 then weighted_box_value_min else 0 end) as Has_Both_Sports
,sum(case when c.ptb_value<>4 and prem_sports=1 and sport_2=1 then weighted_box_value_min else 0 end) as sky_sports_2_only
,sum(case when c.ptb_value=4 then 0 when prem_sports>0 then 0 when ptb_value>0 then weighted_box_value_min else 0 end) as Other_Premium
,sum(case when c.ptb_value=0 then weighted_box_value_min else 0 end) as Non_premium
,count(*) as boxes
,sum(weighted_box_value_min) as weighted_boxes
...
left outer join vespa_analysts.sky_base_2011_11_29 as b 
on a.subscriber_id=b.subscriber_id
left outer join sk_prod.cust_entitlement_lookup as c
on b.primary_box_current_short_description=c.short_description
group by viewer_segment ,primary_box
order by viewer_segment,primary_box;
...
select 
primary_box
,sum(case when c.ptb_value=4 then 1 else 0 end) as Top_Tier
,sum(case when c.ptb_value<>4 and prem_sports=2 then 1 else 0 end) as Has_Both_Sports
,sum(case when c.ptb_value<>4 and prem_sports=1 and sport_2=1 then 1 else 0 end) as sky_sports_2_only
,sum(case when c.ptb_value=4 then 0 when prem_sports>0 then 0 when ptb_value>0 then 1 else 0 end) as Other_Premium
,sum(case when c.ptb_value=0 then 1 else 0 end) as Non_premium
,count(*) as boxes
--,sum(weighted_box_value_min) as weighted_boxes

from vespa_analysts.sky_base_2011_11_29 as b 
left outer join sk_prod.cust_entitlement_lookup as c
on b.primary_box_current_short_description=c.short_description
group by primary_box
order by primary_box;
...
-----Create Profiles of Differing Viewer Segments---
/*
select distinct subscriber_id into #subscriber_viewing_list
from vespa_analysts.All_viewing_minute_by_minute_weighted_20111129_by_channel
;
*/


---Add on segs for no viewing and have SS2 and No Viewing not have SS2


select primary_box
,multi_box_household
,count(*) as boxes
,sum(case when  watched_1st_half_live_minutes >= 31 
and watched_2nd_half_live_minutes >= 31 
then weighted_box_value_min else 0 end) as Fully_Comitted
,sum(case when watched_1st_half_live_minutes>=31 and watched_2nd_half_live_minutes >= 31 then 0
when watched_1st_half_live_minutes >= 16 
and watched_2nd_half_live_minutes >= 16 
then  weighted_box_value_min else 0 end) as Heavily_Interested
,sum(case
when watched_1st_half_live_minutes < 16 
and watched_2nd_half_live_minutes >= 16 
then weighted_box_value_min else 0 end) as Latecomers
,sum(case
when watched_1st_half_live_minutes >=31
and watched_2nd_half_live_minutes < 16 
then weighted_box_value_min else 0 end) as  Mainly_First_Half
,sum(case
when watched_1st_half_live_minutes between 0 and 30 and watched_2nd_half_live_minutes between 0 and 15
    and watched_1st_half_live_minutes+watched_2nd_half_live_minutes>0 then weighted_box_value_min else 0 end) as Partially_Interested
,sum(case 
when watched_1st_half_live_minutes =0
and watched_2nd_half_live_minutes =0
then weighted_box_value_min else 0 end) as None_of_Match
,sum(case when  watched_1st_half_live_minutes is not null and c.sport_2 =0 then weighted_box_value_min else 0 end) as non_sports_2_entitlement_with_viewing
,sum(case when  c.sport_2 =1 then 1 else 0 end) as sports_2_boxes
,sum(case when  c.sport_2 =0 then 1 else 0 end) as non_sports_2_boxes
...
on a.subscriber_id=b.subscriber_id
left outer join sk_prod.cust_entitlement_lookup as c
on a.primary_box_current_short_description=c.short_description
...
---Repeat but for Multi Box HH as well

select case when primary_box =1 and multi_box_household = 1 then '01: Primary Box in Multi Box HH'
when primary_box =1 then '02: Primary Box in Single Box HH'
when primary_box =0 then '03: Non- primary Box' end as box_sub_type
,count(*) as boxes
,sum(case when  watched_1st_half_live_minutes >= 31 
and watched_2nd_half_live_minutes >= 31 
then weighted_box_value_min else 0 end) as Fully_Comitted
...
group by box_sub_type 
order by box_sub_type ;

---Repeat but for Affluence
...
----
---Repeat but for Affluence - Primary Boxes Only
...
-----Split by Tenure/Value Segment

commit;



----Stage 3 Activity in Multiroom Households (i.e., where data from more than one box at an account is available)
--drop table #accounts_returning_multiple_boxes;
select account_number
,count(*) as boxes
into #accounts_returning_multiple_boxes
from vespa_analysts.sky_base_2011_11_29 
where days_returning_data_2011_11_29=1
group by account_number
having boxes>1
;
...
create hg index idx1 on #accounts_returning_multiple_boxes(account_number);

--select count(*) from #accounts_returning_multiple_boxes;

--drop table vespa_analysts.sky_base_2011_11_29_with_multiple_subs_returning_data;
select a.*
into vespa_analysts.sky_base_2011_11_29_with_multiple_subs_returning_data
from vespa_analysts.sky_base_2011_11_29 as a
left outer join #accounts_returning_multiple_boxes as b
on a.account_number = b.account_number
where b.account_number is not null
;
commit;

--select top 2000 * from vespa_analysts.sky_base_2011_11_29_with_multiple_subs_returning_data order by account_number ;

--drop table #multi_box_account_summary;
select account_number
,max(hhafflu) as hh_affluence
,max(lifestage) as hh_lifestage
,max(isba_tv_region) as hh_tv_region
,max(postcode) as hh_postcode
,max(primary_box_current_short_description) as hh_dtv_subscription
,max(case when subscription_sub_type = 'DTV Primary Viewing' then box_type else null end) as primary_box_type
,max(case when subscription_sub_type = 'DTV Primary Viewing' then a.subscriber_id else null end) as primary_subscriber_id
,max(case when subscription_sub_type = 'DTV Primary Viewing' then x_box_type else null end) as primary_x_box_type
,max(case when subscription_sub_type = 'DTV Primary Viewing' then weight_2011_11_29 else 0 end) as primary_box_weight
,max(case when subscription_sub_type = 'DTV Extra Subscription' then box_type else null end) as non_primary_box_type
,max(case when subscription_sub_type = 'DTV Extra Subscription' then weight_2011_11_29 else 0 end) as non_primary_box_weight
,max(case when subscription_sub_type = 'DTV Extra Subscription' then x_box_type else null end) as non_primary_x_box_type

,max(case when subscription_sub_type = 'DTV Primary Viewing' then watched_pre_match_live_minutes else 0 end) as primary_box_watched_pre_match_live_minutes
,max(case when subscription_sub_type = 'DTV Primary Viewing' then watched_1st_half_live_minutes else 0 end) as primary_box_watched_1st_half_live_minutes
,max(case when subscription_sub_type = 'DTV Primary Viewing' then watched_half_time_live_minutes else 0 end) as primary_box_watched_half_time_live_minutes
,max(case when subscription_sub_type = 'DTV Primary Viewing' then watched_2nd_half_live_minutes else 0 end) as primary_box_watched_2nd_half_live_minutes
,max(case when subscription_sub_type = 'DTV Primary Viewing' then watched_post_match_live_minutes else 0 end) as primary_box_watched_post_match_live_minutes


,max(case when subscription_sub_type = 'DTV Extra Subscription' then watched_pre_match_live_minutes else 0 end) as non_primary_box_watched_pre_match_live_minutes
,max(case when subscription_sub_type = 'DTV Extra Subscription' then watched_1st_half_live_minutes else 0 end) as non_primary_box_watched_1st_half_live_minutes
,max(case when subscription_sub_type = 'DTV Extra Subscription' then watched_half_time_live_minutes else 0 end) as non_primary_box_watched_half_time_live_minutes
,max(case when subscription_sub_type = 'DTV Extra Subscription' then watched_2nd_half_live_minutes else 0 end) as non_primary_box_watched_2nd_half_live_minutes
,max(case when subscription_sub_type = 'DTV Extra Subscription' then watched_post_match_live_minutes else 0 end) as non_primary_box_watched_post_match_live_minutes
,max(case when subscription_sub_type = 'DTV Extra Subscription' then a.subscriber_id else null end) as non_primary_subscriber_id

into #multi_box_account_summary
from vespa_analysts.sky_base_2011_11_29_with_multiple_subs_returning_data as a
left outer join  #subscriber_events_summary as b 
on a.subscriber_id=b.subscriber_id
group by account_number
;
commit;
create hg index idx1 on #multi_box_account_summary(account_number);
create hg index idx2 on #multi_box_account_summary(primary_subscriber_id);
create hg index idx3 on #multi_box_account_summary(non_primary_subscriber_id);
commit;
--select top 500 * from #multi_box_account_summary;
/*
select primary_box_watched_watched_1st_half_live_minutes+primary_box_watched_2nd_half_live_minutes as primary_box_watched_minutes
,non_primary_box_watched_watched_1st_half_live_minutes+non_primary_box_watched_2nd_half_live_minutes as non_primary_box_watched_minutes
,count(*) as accounts
from  #multi_box_account_summary
group by primary_box_watched_minutes ,non_primary_box_watched_minutes
order by primary_box_watched_minutes ,non_primary_box_watched_minutes
;
*/


select case     when primary_box_watched_1st_half_live_minutes=0 then '01: Primary Box 0 First Half Minutes Watched'
                when primary_box_watched_1st_half_live_minutes between 1 and 15 then '02: Primary Box 1-15 First Half Minutes Watched'
                when primary_box_watched_1st_half_live_minutes between 16 and 30 then '03: Primary Box 16-30 First Half Minutes Watched'
                when primary_box_watched_1st_half_live_minutes >=31 then '04: Primary Box 31+ First Half Minutes Watched' else null
end as primary_box_first_half_watched_minutes

,case     when primary_box_watched_2nd_half_live_minutes=0 then '01: Primary Box 0 Second Half Minutes Watched'
                when primary_box_watched_2nd_half_live_minutes between 1 and 15 then '02: Primary Box 1-15 Second Half Minutes Watched'
                when primary_box_watched_2nd_half_live_minutes between 16 and 30 then '03: Primary Box 16-30 Second Half Minutes Watched'
                when primary_box_watched_2nd_half_live_minutes >=31 then '04: Primary Box 31+ Second Half Minutes Watched' else null
end as primary_box_Second_half_watched_minutes

,case     when non_primary_box_watched_1st_half_live_minutes=0 then '01: non_primary Box 0 First Half Minutes Watched'
                when non_primary_box_watched_1st_half_live_minutes between 1 and 15 then '02: non_primary Box 1-15 First Half Minutes Watched'
                when non_primary_box_watched_1st_half_live_minutes between 16 and 30 then '03: non_primary Box 16-30 First Half Minutes Watched'
                when non_primary_box_watched_1st_half_live_minutes >=31 then '04: non_primary Box 31+ First Half Minutes Watched' else null
end as non_primary_box_first_half_watched_minutes

,case     when non_primary_box_watched_2nd_half_live_minutes=0 then '01: non_primary Box 0 Second Half Minutes Watched'
                when non_primary_box_watched_2nd_half_live_minutes between 1 and 15 then '02: non_primary Box 1-15 Second Half Minutes Watched'
                when non_primary_box_watched_2nd_half_live_minutes between 16 and 30 then '03: non_primary Box 16-30 Second Half Minutes Watched'
                when non_primary_box_watched_2nd_half_live_minutes >=31 then '04: non_primary Box 31+ Second Half Minutes Watched' else null
end as non_primary_box_Second_half_watched_minutes


,count(*) as accounts
,sum(primary_box_weight) as households
from  #multi_box_account_summary
group by primary_box_first_half_watched_minutes,primary_box_second_half_watched_minutes,non_primary_box_first_half_watched_minutes,non_primary_box_second_half_watched_minutes
order by primary_box_first_half_watched_minutes,primary_box_second_half_watched_minutes,non_primary_box_first_half_watched_minutes,non_primary_box_second_half_watched_minutes
;


commit;

----Look at Minute by Minute by account for Channel and Box----

--drop table #account_summary_by_minute_viewing_non_primary;

--select channel_name_inc_hd , count(*) as records from vespa_analysts.All_viewing_minute_by_minute_weighted_20111129_by_channel group by channel_name_inc_hd order by records desc;


select account_number
,b.minute  as viewed_minute
,1 as primary_box_flag
,max(case when b.channel_name_inc_hd = 'Sky Sports 1' and b.seconds_viewed_in_minute_live>=31 then primary_box_weight else 0 end) as sky_sports_1
,max(case when b.channel_name_inc_hd = 'Sky Sports 2' and b.seconds_viewed_in_minute_live>=31 then primary_box_weight else 0 end) as sky_sports_2
,max(case when b.channel_name_inc_hd = 'Sky Sports News' and b.seconds_viewed_in_minute_live>=31 then primary_box_weight else 0 end) as sky_sports_news
,max(case when b.channel_name_inc_hd = 'BBC ONE' and b.seconds_viewed_in_minute_live>=31 then primary_box_weight else 0 end) as BBC1
,max(case when b.channel_name_inc_hd = 'ITV1' and b.seconds_viewed_in_minute_live>=31 then primary_box_weight else 0 end) as ITV
,max(case when b.channel_name_inc_hd not in ('Sky Sports 1','Sky Sports 2','Sky Sports News','BBC ONE','ITV1') and b.seconds_viewed_in_minute_live>=31 then primary_box_weight else 0 end) as Other_Channel

into #account_summary_by_minute_box_viewing
from #multi_box_account_summary as a
left outer join vespa_analysts.All_viewing_minute_by_minute_weighted_20111129_by_channel as b
on a.primary_subscriber_id = b.subscriber_id
group by account_number
,viewed_minute

union all
select account_number
,c.minute  as viewed_minute
,0 as primary_box_flag
,max(case when c.channel_name_inc_hd = 'Sky Sports 1' and c.seconds_viewed_in_minute_live>=31 then primary_box_weight else 0 end) as sky_sports_1
,max(case when c.channel_name_inc_hd = 'Sky Sports 2' and c.seconds_viewed_in_minute_live>=31 then primary_box_weight else 0 end) as sky_sports_2
,max(case when c.channel_name_inc_hd = 'Sky Sports News' and c.seconds_viewed_in_minute_live>=31 then primary_box_weight else 0 end) as sky_sports_news
,max(case when c.channel_name_inc_hd = 'BBC ONE' and c.seconds_viewed_in_minute_live>=31 then primary_box_weight else 0 end) as BBC1
,max(case when c.channel_name_inc_hd = 'ITV1' and c.seconds_viewed_in_minute_live>=31 then primary_box_weight else 0 end) as ITV
,max(case when c.channel_name_inc_hd not in ('Sky Sports 1','Sky Sports 2','Sky Sports News','BBC ONE','ITV1') and c.seconds_viewed_in_minute_live>=31 then primary_box_weight else 0 end) as Other_Channel

from #multi_box_account_summary as a
left outer join vespa_analysts.All_viewing_minute_by_minute_weighted_20111129_by_channel as c
on a.non_primary_subscriber_id = c.subscriber_id
group by account_number
,viewed_minute
;
...
select account_number
,viewed_minute
,max(case when sky_sports_2>0 and primary_box_flag=1 then sky_sports_2 else 0 end) as primary_box_viewing_ss2
,max(case when sky_sports_2>0 and primary_box_flag=0 then sky_sports_2 else 0 end) as non_primary_box_viewing_ss2
into #minute_and_account_summary
from #account_summary_by_minute_box_viewing
group by account_number
,viewed_minute
;

--select top 500 * from #minute_and_account_summary;


select viewed_minute
,sum(case when primary_box_viewing_ss2>0 and non_primary_box_viewing_ss2=0 then primary_box_viewing_ss2 else 0 end) as primary_box_only_ss2
,sum(case when non_primary_box_viewing_ss2>0 and primary_box_viewing_ss2=0 then non_primary_box_viewing_ss2 else 0 end) as non_primary_box_only_ss2
,sum(case when primary_box_viewing_ss2>0 and non_primary_box_viewing_ss2>0 then non_primary_box_viewing_ss2 else 0 end) as both_boxes_ss2
from #minute_and_account_summary
group by viewed_minute
order by viewed_minute
;

--select top 500 * from #account_summary_by_minute_viewing where viewed_minute ='2011-11-29 18:00:00';
--select * from #multi_box_account_summary where account_number = '620038100181'
--select * from vespa_analysts.All_viewing_minute_by_minute_weighted_20111129_by_channel where subscriber_id = 27472482 order by minute

select viewed_minute
,sum(case when primary_box_sky_sports_2>0 and non_primary_box_sky_sports_2=0 then primary_box_sky_sports_2 else 0 end) as primary_box_only_ss2
,sum(case when primary_box_sky_sports_2=0 and non_primary_box_sky_sports_2>0 then non_primary_box_sky_sports_2 else 0 end) as non_primary_box_only_ss2
,sum(case when primary_box_sky_sports_2>0 and non_primary_box_sky_sports_2>0 then non_primary_box_sky_sports_2 else 0 end) as both_boxes_ss2
from #account_summary_by_minute_box_viewing
group by viewed_minute
order by viewed_minute
;

----Minute by Minute - Primary vs. Non-Primary------


select case when primary_box = 1 then 'Yes' else 'No' end as primary_box_yes_no
,minute 
--,sum(case   when seconds_viewed_in_minute_live>=31 then weighted_boxes else 0 end) as total_households_live
,sum(case when seconds_viewed_in_minute_live>=31 and channel_name_inc_hd = 'Sky Sports 2' then weighted_boxes else 0 end) as sky_sports_2_boxes_live
,sum(case when seconds_viewed_in_minute_live>=31 and channel_name_inc_hd = 'Sky Sports 1' then weighted_boxes else 0 end) as sky_sports_1_boxes_live
...
left outer join #subscriber_events_summary as b
on a.subscriber_id=b.subscriber_id
left outer join vespa_analysts.sky_base_2011_11_29  as c
on a.subscriber_id=c.subscriber_id
where b.subscriber_id is not null 
--and minute between '2011-11-29 19:30:00' and '2011-11-29 22:59:00'
group by primary_box_yes_no,minute
order by primary_box_yes_no,minute
;

--select top 100 * from  #subscriber_events_summary;
--select top 100 * from  vespa_analysts.sky_base_2011_11_29;


----Data Returned Analysis----


select account_number
,multi_box_household
,max(case when primary_box =1 then days_returning_data_2011_11_29 else 0 end) as primary_boxes_returning_data
,max(case when primary_box =0 then days_returning_data_2011_11_29 else 0 end) as non_primary_boxes_returning_data
into #account_box_return_summary
 from vespa_analysts.sky_base_2011_11_29
group by account_number
,multi_box_household
...
select multi_box_household
,sum(multi_box_household+1 ) as boxes
,count(distinct account_number) as households
,sum(primary_boxes_returning_data+non_primary_boxes_returning_data) as boxes_returning_data
,sum(primary_boxes_returning_data) as primary_boxes
,sum(non_primary_boxes_returning_data) as non_primary_boxes
,sum(case when primary_boxes_returning_data =1  or non_primary_boxes_returning_data=1 then 1 else 0 end) as any_box_returning_data
,sum(case when primary_boxes_returning_data =1  and non_primary_boxes_returning_data=1 then 1 else 0 end) as both_boxes_returning_data
 from #account_box_return_summary
group by multi_box_household
order by multi_box_household
;

commit;


------Create Household level segmentation
select hh_affluence
--,count(*) as boxes
,sum(primary_box_weight) as weighted_boxes
,sum(case when  primary_box_watched_1st_half_live_minutes+non_primary_box_watched_1st_half_live_minutes >= 31 
and primary_box_watched_2nd_half_live_minutes+non_primary_box_watched_2nd_half_live_minutes >= 31 
then primary_box_weight else 0 end) as Fully_Comitted
,sum(case when primary_box_watched_1st_half_live_minutes+non_primary_box_watched_1st_half_live_minutes>=31 and primary_box_watched_2nd_half_live_minutes+non_primary_box_watched_2nd_half_live_minutes >= 31 then 0
when primary_box_watched_1st_half_live_minutes+non_primary_box_watched_1st_half_live_minutes >= 16 
and primary_box_watched_2nd_half_live_minutes+non_primary_box_watched_2nd_half_live_minutes >= 16 
then  primary_box_weight else 0 end) as Heavily_Interested
,sum(case
when primary_box_watched_1st_half_live_minutes+non_primary_box_watched_1st_half_live_minutes < 16 
and primary_box_watched_2nd_half_live_minutes+non_primary_box_watched_2nd_half_live_minutes >= 16 
then primary_box_weight else 0 end) as Latecomers
,sum(case
when primary_box_watched_1st_half_live_minutes+non_primary_box_watched_1st_half_live_minutes >=31
and primary_box_watched_2nd_half_live_minutes+non_primary_box_watched_2nd_half_live_minutes < 16 
then primary_box_weight else 0 end) as  Mainly_First_Half
,sum(case
when primary_box_watched_1st_half_live_minutes+non_primary_box_watched_1st_half_live_minutes between 0 and 30 and primary_box_watched_2nd_half_live_minutes+non_primary_box_watched_2nd_half_live_minutes between 0 and 15
    and primary_box_watched_1st_half_live_minutes+non_primary_box_watched_1st_half_live_minutes+non_primary_box_watched_2nd_half_live_minutes+non_primary_box_watched_2nd_half_live_minutes>0 then primary_box_weight else 0 end) as Partially_Interested
,sum(case 
when primary_box_watched_1st_half_live_minutes+non_primary_box_watched_1st_half_live_minutes =0
and primary_box_watched_2nd_half_live_minutes+non_primary_box_watched_2nd_half_live_minutes =0
and primary_box_watched_pre_match_live_minutes
+primary_box_watched_1st_half_live_minutes
+primary_box_watched_half_time_live_minutes
+primary_box_watched_2nd_half_live_minutes
+primary_box_watched_post_match_live_minutes
+non_primary_box_watched_pre_match_live_minutes
+non_primary_box_watched_1st_half_live_minutes
+non_primary_box_watched_half_time_live_minutes
+non_primary_box_watched_2nd_half_live_minutes
+non_primary_box_watched_post_match_live_minutes>0


then primary_box_weight else 0 end) as None_of_Match
,sum(case when  primary_box_watched_pre_match_live_minutes
+primary_box_watched_1st_half_live_minutes
+primary_box_watched_half_time_live_minutes
+primary_box_watched_2nd_half_live_minutes
+primary_box_watched_post_match_live_minutes
...
+non_primary_box_watched_half_time_live_minutes
+non_primary_box_watched_2nd_half_live_minutes
+non_primary_box_watched_post_match_live_minutes>0 and c.sport_2 =0 then primary_box_weight else 0 end) as non_sports_2_entitlement_with_viewing
,sum(case when  c.sport_2 =1 then primary_box_weight else 0 end) as sports_2_boxes
,sum(case when  c.sport_2 =0 then primary_box_weight else 0 end) as non_sports_2_boxes


--,sum(case when primary_box =1 then primary_box_weight else 0 end) as primary_boxes
--,sum(case when primary_box =0 then primary_box_weight else 0 end) as secondary_boxes
--,count(*) as boxes
--,sum(primary_box_weight) as weighted_boxes

from #multi_box_account_summary as a
left outer join sk_prod.cust_entitlement_lookup as c
on a.hh_dtv_subscription=c.short_description
group by hh_affluence 
order by hh_affluence ;


------Household level profiling------
...
---get Distinct Households for charts

select hhafflu
,c.sport_2 , count(distinct account_number) as households
from vespa_analysts.sky_base_2011_11_29 as a
...
----repeat HH view for Single/Multi Box HH-----
...
--select gov_region from vespa_analysts.sky_base_2011_11_29;
...
---
----repeat by Lifestage/Tenure/Value Segment
...
--select tenure, count(*) as boxes , sum(days_returning_data_2011_11_29) , sum(days_returning_data_2011_11_29*weight_2011_11_29) from vespa_analysts.sky_base_2011_11_29 group by tenure order by tenure;
--select hhafflu, count(*) as boxes , sum(days_returning_data_2011_11_29) , sum(days_returning_data_2011_11_29*weight_2011_11_29) from vespa_analysts.sky_base_2011_11_29 group by hhafflu order by hhafflu;

--weight_2011_11_29) from vespa_analysts.sky_base_2011_11_29 where days_returning_data_2011_11_29
----repeat by Value Segment
...
-------repeat by DTV PAckage---


select case when c.prem_sports=2 and c.prem_movies =2 then '01: Top Tier' 
            when c.prem_sports=2 then '02: Dual Sports' 
else '03: Other' end as package_type
...
----Follow up For Sarah
---Number of Live Events Split by Segent

select case when viewing_events>=20 then 20 else viewing_events end as events
,count(distinct account_number) as households
,sum(case when  watched_1st_half_live_minutes >= 31 
and watched_2nd_half_live_minutes >= 31 
then weighted_box_value_min else 0 end) as Fully_Comitted
...
left outer join  #subscriber_events_summary as b 
on a.subscriber_id=b.subscriber_id
where viewing_events is not null
group by events 
order by events ;
commit;
...
--select count(*) , count(distinct subscriber_id)  from vespa_analysts.sky_base_2011_11_29
/*
select primary_box
,multi_box_household
,count(*) as boxes
,sum(days_returning_data_2011_11_29)
 from vespa_analysts.sky_base_2011_11_29
group by primary_box
,multi_box_household
order by primary_box
...
--select distinct x_box_type  from vespa_analysts.sky_base_2011_11_29
--select top 100 *  from vespa_analysts.sky_base_2011_11_29


--select sum(weight_2011_11_29) from vespa_analysts.sky_base_2011_11_29 where days_returning_data_2011_11_29 = 1;
...
--select top 500 * from #subscriber_events_summary;
--select top 500 * from #subscriber_events_summary;
...
select tenure
,case when c.prem_sports=2 and c.prem_movies =2 then '01: Top Tier' 
            when c.prem_sports=2 then '02: Dual Sports' 
else '03: Other' end as package_type
,count(*) as boxes
,count(distinct account_number) as households
,sum(days_returning_data_2011_11_29*weight_2011_11_29) as weighted_boxes
,sum(days_returning_data_2011_11_29) as boxes_returning_data
,sum(case when watched_1st_half_live_minutes>=31 then 1 else 0 end) as watch_1st_half_31_plus
,sum(case when watched_1st_half_live_minutes>=31 then weight_2011_11_29 else 0 end) as watch_1st_half_31_plus_boxes
from vespa_analysts.sky_base_2011_11_29 as a
left outer join  #subscriber_events_summary as b 
on a.subscriber_id=b.subscriber_id
left outer join sk_prod.cust_entitlement_lookup as c
on a.primary_box_current_short_description=c.short_description
group by tenure ,package_type
order by tenure ,package_type ;
