...
--select count(*) , count(distinct subscriber_id) , sum(weight_2011_08_11) from vespa_analysts.trollied_analysis_box_returning_data;
...
--select count(*) from vespa_analysts.vespa_phase1b_minute_by_minute_trollied_20110811;

--select top 500 *  from vespa_analysts.vespa_phase1b_minute_by_minute_trollied_20110811 order by subscriber_id;
...
---Create Summary per Household (account_number)---
--drop table vespa_analysts.sky_base_2011_08_11_by_account;
select account_number
        ,lifestage         
        ,HHAfflu           
        ,isba_tv_region    
        ,max(case when subscription_sub_type='DTV Primary Viewing' then package else null end) as dtv_package          
        ,max(case when pvr=1 then 1 else 0 end) as has_pvr
        ,max(case when pvr=0 then 1 else 0 end) as has_non_pvr
        ,max(case when primary_box=0 then 1 else 0 end) as multi_box_hh
into vespa_analysts.sky_base_2011_08_11_by_account
from vespa_analysts.sky_base_v2_2011_08_11
...
--select count(distinct account_number) , count(*) from vespa_analysts.sky_base_2011_08_11_by_account


--select top 500 * from vespa_analysts.sky_base_v2_2011_08_11;

--select lifestage , count(*) as accounts from vespa_analysts.sky_base_2011_08_11_by_account group by lifestage;
--select count(account_number) , count(distinct account_number) as accounts from vespa_analysts.sky_base_2011_08_11_by_account ;

--drop table vespa_analysts.sky_base_2011_08_11_count_by_account_attributes;
select lifestage         
        ,HHAfflu           
        ,isba_tv_region    
        ,dtv_package as package           
        ,has_pvr
        ,has_non_pvr
        ,multi_box_hh
        ,count(*) as accounts
into vespa_analysts.sky_base_2011_08_11_count_by_account_attributes
from vespa_analysts.sky_base_2011_08_11_by_account
group by lifestage         
        ,HHAfflu           
        ,isba_tv_region    
        ,package           
...
--drop table vespa_analysts.minute_by_minute_weighted_profile;
select minute
,b.lifestage         
        ,b.HHAfflu           
        ,b.isba_tv_region    
...
into vespa_analysts.minute_by_minute_weighted_profile
...
--select * from vespa_analysts.minute_by_minute_weighted_profile;
...
from vespa_analysts.minute_by_minute_weighted_profile
...
select minute
,sum(weighted_boxes) as households
--,sum(total_households_live) as hh_live
--,sum(total_households_playback_163h) as hh_playback_barb
...
from vespa_analysts.minute_by_minute_weighted_profile as a

group by minute
order by minute
;

--select distinct HHAfflu from vespa_analysts.minute_by_minute_weighted_profile;
--select multi_box_hh ,count(*) from vespa_analysts.minute_by_minute_weighted_profile group by multi_box_hh;
...
select count(*) from vespa_analysts.minute_by_minute_weighted_profile where hhafflu is null
...
--select top 100 * from vespa_analysts.minute_by_minute_weighted_profile;
---------
...
--select * from vespa_analysts.trollied_analysis_box_returning_data;
----create weighted figs--

--drop table vespa_analysts.minute_by_minute_weighted_profile;
select second_viewed
,sum(weighted_boxes) as households
,sum(case when viewed_playback_within_163_hours=1 then 0 else weighted_boxes end) as live_households
,sum(viewed_playback_within_163_hours*weighted_boxes ) as playback_houses
from vespa_analysts.trollied_20110811_second_by_second
group by second_viewed
order by second_viewed
;

--select top 1000 * from vespa_analysts.trollied_20110811_second_by_second;
...
select * from vespa_analysts.vespa_phase1b_minute_by_minute_trollied_20110811 order by minute;

select * from vespa_analysts.sky_base_v2_2011_08_11 where subscriber_id = 21183733

select weighted_boxes ,count(*) as records from vespa_analysts.vespa_phase1b_minute_by_minute_trollied_20110811 group by weighted_boxes order by records desc
...
--select distinct isbavespa_analysts.minute_by_minute_weighted_profile
select minute
,isba_tv_region
,sum(total_households) as households
