...
IF OBJECT_ID('molson_coors_spot_data') IS NOT NULL DROP TABLE molson_coors_spot_data
...
-- GET SPOT DATA FOR FLIGHTING REPORT - THAT IS COMPARABLE TO BARB --
/*
IF OBJECT_ID('molson_coors_spot_barb_data') IS NOT NULL DROP TABLE molson_coors_spot_barb_data;
select      b.te_channel
...
into        molson_coors_spot_barb_data
from        neighbom.BARB_MASTER_SPOT_DATA  a
join        vespa_analysts.CHANNEL_MAP_DEV_LOG_STATION_PANEL b
on          a.log_station_code = b.log_station_code
and         a.sti_code = b.sti_code
where       clearcast_commercial_no IN ('VCCMCCA021030','VCCMCCL005010','VCCMCCL028030','VCCMCCL029010','VCCMCCL032010',
                                        'AAEFOST026040','AAEFOST027030','MUMSTCI009040','MUMSTCI014030')
and         barb_date_of_transmission between '2012/08/01' and '2012/09/20'
...
select * from molson_coors_spot_barb_data
...
if object_id('molson_coors_hh_aggregate') is not null drop table molson_coors_hh_aggregate;
...
into      molson_coors_hh_aggregate
from      hh_mosaic  B
left outer join
          hh_age_gender                C
...
if object_id('molson_coors_sky_hh_aggregate') is not null drop table molson_coors_sky_hh_aggregate;
select     COALESCE(a.cb_key_household,b.cb_key_household) as cb_key_household
          ...
          ,b.buckle_segments
into   molson_coors_sky_hh_aggregate
...
if object_id('molson_coors_viewing_data') is not null drop table molson_coors_viewing_data;


create table molson_coors_viewing_data (
...
SET @var_sql = '
    insert into molson_coors_viewing_data (
                 Cb_Row_Id
                ,Account_Number
                ,Subscriber_Id
                ,cb_key_household
...
inner join       molson_coors_spot_data as spot
...
if object_id('molson_coors_spot_data2') is not null drop table molson_coors_spot_data2;

select *
       ,flag = 1
       ,sum(flag) over (order by utc_spot_start_date_time
                ROWS BETWEEN UNBOUNDED PRECEDING AND CURRENT ROW) as identifier
into molson_coors_spot_data2
...
join molson_coors_spot_data2       spot
...
from   molson_coors_viewing_data A;
...
from              molson_coors_viewing_data as cub
...
if object_id('molson_coors_hh_impacts') is not null drop table molson_coors_hh_impacts;
...
grant select on molson_coors_viewing_data to gillh;
grant select on molson_coors_viewing_data to chans;

-- JOIN ON HOUSEHOLD AGGREGATE TABLE --



if object_id('molson_coors_hh_exp_impacts') is not null drop table molson_coors_hh_exp_impacts;
...
into       molson_coors_hh_exp_impacts
from       molson_coors_hh_impacts       A
left outer join
           molson_coors_hh_aggregate     B
on         a.cb_key_household=b.cb_key_household;

grant select on molson_coors_hh_exp_impacts to gillh;
grant select on molson_coors_hh_exp_impacts to chans;



--select top 10 * from molson_coors_hh_exp_impacts
...
if object_id('molson_coors_impacts') is not null drop table molson_coors_impacts
...
into       molson_coors_impacts
from       molson_coors_hh_exp_impacts
...
select * from molson_coors_impacts;
grant select on molson_coors_impacts to gillh;
grant select on molson_coors_impacts to chans;


-- APPEND HH AGGREGATE TABLE WITH EXPOSURE TO COMMERCIALS FLAGS --

drop table molson_coors_hh_update;
select    a.cb_key_household
...
update molson_coors_hh_aggregate
 ...
 from  molson_coors_hh_aggregate    A
 left outer join
       molson_coors_hh_update       B
 on    a.cb_key_household=b.cb_key_household;

-- SKY BASE HH AGGREGATE --


alter table molson_coors_sky_hh_aggregate
      add ( carling_exposed integer
           ,stella_cidre_exposed integer
           ,fosters_exposed integer
           ,coors_light_exposed integer);
...
update molson_coors_sky_hh_aggregate
 set   a.carling_exposed=b.carling_exposed
      ,a.stella_cidre_exposed=b.stella_cidre_exposed
      ,a.fosters_exposed=b.fosters_exposed
      ,a.coors_light_exposed=b.coors_light_exposed
 from  molson_coors_sky_hh_aggregate    A
 left outer join
       molson_coors_hh_update       B
 on    a.cb_key_household=b.cb_key_household;
...
update molson_coors_hh_aggregate A
...
update molson_coors_sky_hh_aggregate A
 set   a.midday_weightings = b.midday_weightings
 from  molson_coors_sky_hh_aggregate    A
 left outer join
       molson_coors_midday_scaling_hh       B
...
alter table molson_coors_hh_exp_impacts
      add ( midday_weightings  integer);


update molson_coors_hh_exp_impacts
 set   a.midday_weightings=b.midday_weightings
 from  molson_coors_hh_exp_impacts    A
 left outer join
       molson_coors_midday_scaling_hh       B
 on    a.cb_key_household=b.cb_key_household;
...
--- added by HARRY --
-----------------------------------------------------------------------------------
------- THIS IS SOME UPDATED OUTPUT FOR THE CIDRE IMPACT CHECK S.SHEET

/*
select count(*) from molson_coors_impacts
...
select barb_date_of_transmission
       ,count(case when stella_cidre_impacts > 0 then stella_cidre_impacts else null end ) as spots_seen
       ,sum(stella_cidre_impacts) as impacts
       ,impacts*1.33 as adjusted_impacts
from molson_coors_impacts
where -- client_spot_flag = 'Stella Cidre'
        clearcast_commercial_no = 'MUMSTCI009040' -- this is more accurate!
group by barb_date_of_transmission
...
from molson_coors_impacts
where -- client_spot_flag = 'Stella Cidre'
        clearcast_commercial_no = 'MUMSTCI014030' -- this is more accurate!
group by barb_date_of_transmission
order by barb_date_of_transmission


select top 10 * from starmerh.molson_coors_spot_data2


select barb_date_of_transmission --utc_start_date
         ,count(*) as spots_aired
from starmerh.molson_coors_spot_data2
where clearcast_commercial_no = 'MUMSTCI014030'
group by barb_date_of_transmission --utc_start_date
order by barb_date_of_transmission -- utc_start_date



select distinct(clearcast_commercial_no) from starmerh.molson_coors_spot_data2
