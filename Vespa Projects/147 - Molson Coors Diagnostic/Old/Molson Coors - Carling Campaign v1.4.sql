...
select
 ...
 where broadcast_start_date_time_utc >= dateadd(month, -7, @var_period_start)
...
--select top 100 * from molson_coors_viewing_data where scaling_segment_id is null;

-- QA - Why are there missing weightings for accounts that have viewed
--select * from molson_coors_viewing_data where account_number in ('621267833823','240035235052','621553677868') and weighting_date in ('2012-08-14');
--select * from vespa_analysts.SC2_intervals where account_number in ('621267833823','240035235052','621553677868') order by account_number,reporting_starts ;
...
--select channel,count(*) from te_channel_lkup group by channel;
alter table molson_coors_viewing_data
        drop channel_new ;
...
select     weighting_date
          ,account_number
          ,max(weightings) as weightings
into       acct_wght_date
from       molson_coors_viewing_data
group by   weighting_date
          ,account_number;

select top 100 * from acct_wght_date;

select    weighting_date
         ,sum(weightings)
         ,count(distinct account_number) as boxes
from      acct_wght_date
group by  weighting_date;
*/
--------------------------------------------------------
...
-- QUICK CHECK ON NUMBER OF HOUSEHOLDS WITH ZERO SCALE WEIGHTING VALUE --
/*
select  cb_key_household
       ,barb_date_of_transmission
       ,clearcast_commercial_no
       ,sum(weightings) as weighting
into    hh_zero_weighting_check
from    molson_coors_hh_impacts
group by  cb_key_household
       ,barb_date_of_transmission
       ,clearcast_commercial_no;
...
-- MIDDAY WEIGHT 25TH AUGUST --

drop table molson_coors_midday_scaling;
select    a.account_number
         ,cast('2012-08-25' as date) as scaling_date
         ,a.reporting_starts
         ,a.reporting_ends
...
-- CREATE SUMMARY TABLE FOR HH REACH (LIKE THE IMPACTS PIVOT ABOVE)  --
