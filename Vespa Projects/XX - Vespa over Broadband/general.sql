...
drop table vobb_link_data_general;

create table     vobb_link_data_general(Subscriber_ID  int
                                           ,Account_Number varchar(30)
                                           ,Model          varchar(30)
                                           ,Location       varchar(30)
...
from '/ETL013/prod/sky/olive/data/share/clarityq/export/Decision_Sciences/Jon/vobb_link_data_general.csv'
QUOTES OFF
ESCAPES OFF
NOTIFY 1000
skip 1
;
  ...
  insert into vobb_link_data_general(Subscriber_ID
                          ,Account_Number
                          ,Model
                          ,Location
                          ,panel_expected
...
;
  ...
  insert into vobb_link_data_general(Subscriber_ID
                    ...
                    select card_subscriber_id
                          ...
                          ,'2014-02-03'
                          ,null
                      from tanghoi.VoBB_migration_groups_20140126_preM1M3M4
                     where migration_group in ('M1', 'M3', 'M4')
                       and subscriber_id not in (select subscriber_id from vobb_link_data_general)
*/
...
;
  ...
  select cal.calendar_date
        ...
        ,lnk.location
        ,lnk.date_added
        ,case when (cal.calendar_date >= date_added and con.subid is not null and net.subscriberid is not null)
                or net.subscriberid is not null then 1 else 0
          end as data_received
        ,lnk.prev_rq
    from sk_prod.sky_calendar               as cal
         cross join vobb_link_data_general  as lnk
...
create hg index idx1 on #vobb_all_data(subscriberid);
create lf index idx2 on #vobb_all_data(panelid);
create date index idx3 on #vobb_all_data(dt);
create date index idx4 on #vobb_all_data(date_added);
...
create hg index idx1 on #vobb_rq(subscriberid);
create lf index idx2 on #vobb_rq(panelid);
create date index idx3 on #vobb_rq(calendar_date);
