...
BEGIN
  ...
  -- **********
  --start process here -->
exec seg01_log 'SEG01_create_temporal_root_aggregations'
...
exec seg01_log '  creating events list'
...
exec seg01_log '  delete list for temporal_id<'||@_temporal_library_rule_id||'>'

-- check that the <pk_viewing_prog_instance_fact, temporal_library_id> is unique before insertion
--lets create a delete list, as Sybase is Sh*t
 IF object_id('SEG01_temporal_pos_delete_list_tmp') IS NOT NULL
   BEGIN
     DROP TABLE SEG01_temporal_pos_delete_list_tmp
   END

execute(' Select pk_viewing_prog_instance_fact, null '||
        '   into SEG01_temporal_pos_delete_list_tmp '||
        '   from SEG01_root_temporal_tbl e '||
        '  where e.temporal_library_id = '||@_temporal_library_rule_id)
commit


declare @delete_count integer
select @delete_count = count(1)
  from SEG01_temporal_pos_delete_list_tmp

exec seg01_log '  delete list contains<'||@delete_count||'>'


exec seg01_log '  deleting duplicates'
DELETE FROM SEG01_combined_event_tmp
FROM SEG01_combined_event_tmp n, SEG01_temporal_pos_delete_list_tmp e
where n.pk_viewing_prog_instance_fact = e.pk_viewing_prog_instance_fact
...
exec seg01_log '  inserting temporal event details'
...
--ON EXISTING SKIP
     select --top 1000
            ...
            account_number,
            @_temporal_library_rule_id temporal_library_id,
   ...
   group by pk_viewing_prog_instance_fact, account_number, temporal_library_id, max_from_date, min_to_date, total_duration, period_duration
 /*
...
select pk_viewing_prog_instance_fact, temporal_library_id, count(1) sample_count
  from SEG01_root_temporal_tbl
  group by pk_viewing_prog_instance_fact, temporal_library_id
...
--or outer join thsi to SEG01_root_temporal_tbl?
