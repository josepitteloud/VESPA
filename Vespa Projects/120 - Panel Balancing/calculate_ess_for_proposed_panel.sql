...
--Part 2 run this code to create the prcoedures
...
--Part 4 calculate effective sample size for the proposed panel
...
--429,256.058447246 from calculated sc2_weightings table
...
select @max_date = max(adjusted_event_start_date_vespa) from sk_prod.viq_viewing_data_scaling;
  ...
  select sum(calculated_scaling_weight * calculated_scaling_weight) as large
    ...
    into #ess2
    from SC2_Todays_panel_members  as tpm
         inner join sk_prod.viq_viewing_data_scaling as wei on tpm.account_number = wei.account_number
   where adjusted_event_start_date_vespa = @max_date
;--257077.515470283 from viq
