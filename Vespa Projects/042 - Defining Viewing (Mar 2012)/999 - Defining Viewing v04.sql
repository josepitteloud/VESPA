...
where (play_back_speed is null or play_back_speed = 2) and capped_flag in (0,1) and viewing_record_end_time_local is not null
...
set @programme_time_start = cast('2012-01-15 18:50:00' as datetime);
set @programme_time_end =cast('2012-01-15 19:40:00' as datetime);
...
,max(case when play_back_speed is null then 0 when play_back_speed is not null and dateadd(day,21,recorded_time_utc)<adjusted_event_start_time and dateadd(day,31,recorded_time_utc)>=adjusted_event_start_time   then 1 else 0 end) as viewed_playback_within_21_31_days
