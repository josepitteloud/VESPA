...
set @programme_time_start = cast('2012-01-15 17:58:00' as datetime);
...
,max(case when play_back_speed is null then 0 when play_back_speed =2 and dateadd(day,6,recorded_time_utc)<adjusted_event_start_time and dateadd(day,7,recorded_time_utc)>=adjusted_event_start_time   then 1 else 0 end) as viewed_playback_within_6_7_days

,max(case when play_back_speed is null then 0 when play_back_speed =2 and dateadd(day,7,recorded_time_utc)<adjusted_event_start_time and dateadd(day,14,recorded_time_utc)>=adjusted_event_start_time   then 1 else 0 end) as viewed_playback_within_7_14_days
