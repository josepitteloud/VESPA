...
*/
  ...
  as begin
            ...
            select account_number
             ...
             where (   (@country = 0 and panel_no in (5, 6, 11, 12, 10, 15))
          ...
          group by account_number
            ...
            update temp_panbal_SAV as sav
               set cbck_rate = case when datediff(month,last_callback_dt,today()) < 1 then 1
                                    when datediff(month,last_callback_dt,today()) > 6 then 0
                                    else (7 - datediff(month,last_callback_dt,today())) / 6
                               end
              from cust_stb_callback_summary as stb
             where sav.account_number = stb.account_number
          ...
          group by apd.account_number
            ...
            commit
            create unique hg index uhacc on temp_dl(account_number)
            ...
            update temp_panbal_SAV as sav
             ...
             where sav.account_number = tdl.account_number
               and bb_panel = 1
                ...
                -- use stb_connection_fact data where possible
            select account_number
                  ,count() as box
              into #stb
            ...
            update temp_panbal_SAV as sav
               set sav.boxes = stb.boxes
              from #stb as stb
             where sav.account_number = stb.account_number
                
                -- use cust_set_top_box for the rest
            select account_number
                  ,count() as boxes
              into temp_box_count
          ...
          group by account_number
           ...
           declare @mxdt date
            ...
            select account_number
                  ,case when days_data_returned + days_empty_logs > @mxdt - panel_age then 1
                        else                                                               case when days_data_returned + days_empty_logs > 30 then 30
                                                                                                else                                                days_data_returned + days_empty_logs
                                                                                            end * 1.0 / case when @mxdt - panel_age > 30 then 30
                                                                                                             else                             @mxdt - panel_age + 1
                                                                                                         end
                   end as rq
              into #rq
              from stb_connection_fact
