...
*/
  ...
  create or replace procedure V352_M03_PanBal_Segments
      ...
      as begin
           ...
           execute vespa_analysts.Regulars_Get_report_end_date @profiling_thursday output  -- proc returns a Saturday
            ...
            UPDATE temp_weekly_sample
               SET uk_standard_account = CASE WHEN b.acct_type='Standard' AND b.account_number <>'?' THEN 1
                  ...
                  ,hd_mix              = case when prod_latest_hd_status_code in ('AC','PC','AB') and prod_latest_hd_package_desc = 'HD Mix' then 1 else 0 end
          ...
          GROUP BY cv.cb_key_household
            ...
            select fbp.account_number
                  ...
                  ,case when cel.prem_sports = 2 and cel.prem_movies = 2 then                  'Top Tier'
                        ...
                        when genre in ('Box Sets', 'Sky Q Bundle') or hd_mix = 1 then          'Basic - Box Sets'
                        when kids = 0 and music = 0 and news_events = 0 and knowledge = 0 then 'Basic - Original'
                        else                                                                   'Basic - Variety'
                   end
          ...
          GROUP BY account_number
            ...
            create table #skygo(sam_profileid int)
            create unique hg index uhsam on #skygo(sam_profileid)
           declare @prevmonth varchar(6)
           declare @pvpvmonth varchar(6)
           declare @pppvmonth varchar(6)
               set @prevmonth = case when month(now())  = 1 then year(now()) - 1 || '12'
                                                            else year(now())     || right('0' || month(now()), 2)
                                end
               set @pvpvmonth = case when month(now()) <= 2 then year(now()) - 1 || 13 - month(now())
                                                            else year(now())     || right('0' || month(now()), 2)
                                end
               set @pppvmonth = case when month(now()) <= 3 then year(now()) - 1 || 13 - month(now())
                                                            else year(now())     || right('0' || month(now()), 2)
                                end
                               
           execute (' insert into #skygo
                      select sam_profileid
                        from (  select sam_profileid
                                  from comscore_' || @prevmonth || '
                              group by sam_profileid
                                 union
                                select sam_profileid
                                  from comscore_' || @pvpvmonth || '
                              group by sam_profileid
                                 union
                                select sam_profileid
                                  from comscore_' || @pppvmonth || '
                              group by sam_profileid
                             ) as sub
                    group by sam_profileid
                    ')
            ...
            update temp_PanBal_weekly_sample as bas
               set skygo = 1
              from #skygo as skg
                   inner join sky_player_registrant as spr on skg.sam_profileid = spr.sam_profile_id
             where bas.account_number = spr.account_number

                -- use non-Basic box from pvr variable
            update temp_PanBal_weekly_sample as bas
               set pvr = 0
              from cust_set_top_box as stb
             where bas.account_number = stb.account_number
              and x_box_type in ('Basic', 'Basic HD')

               -- use Downloads instead of onnet
           create table temp_dl(
                  account_number      varchar(30) null
                 )

               set @1month_ago = dateadd(month, -1, today())

            insert into temp_dl(
                   account_number
                   )
            select bas.account_number
              from temp_PanBal_weekly_sample              as bas
                   inner join cust_anytime_plus_downloads as apd on last_modified_dt > @1month_ago
          ...
          group by bas.account_number
            ...
            update temp_PanBal_weekly_sample as bas
               set bas.onnet = 1
              from temp_dl as dls
             where bas.account_number = dls.account_number
          ...
          group by account_number
            ...
            select account_number
             ...
             where active_box_flag = 'Y'
               ...
               and (x_model_number like 'DRX 890%' or x_model_number like 'DRX 895%')
          group by account_number
            ...
            update temp_PanBal_weekly_sample as bas
             ...
             where bas.account_number = ads.account_number
                ...
                -- set unused variables to a default value for ROI
            update temp_PanBal_weekly_sample as bas
               set fss = 'Not Defined'
                  ,hhcomp = 'Not Defined'
                  ,mosaic = 'Not Defined'
          ...
          truncate table PanBal_segment_snapshots
            ...
            update panbal_segments_lookup_normalised as bas
             ...
             where bas.segment_id = snp.segment_id
              ...
              drop table temp_dl
     end; --V352_M03_PanBal_Segments
 commit;

 grant execute on V352_M03_PanBal_Segments to vespa_group_low_security;
 commit;
