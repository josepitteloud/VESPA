...
select case when a.isba_tv_region is null then 'Not Defined' else a.isba_tv_region end as tv_region
,case when a.household_affluence_level is null then 999 else  a.household_affluence_level end as hh_affluence
,case when a.head_hh_agef is null then 999 else a.head_hh_agef end as head_hh_age
,a.adsmartable_box
,households
,sum(households_viewing) as total_hh_viewing
,sum(case when demograph = 'ADULTS' then households_viewing 
...
--and a.adsmartable_box=b.adsmartable_box
group by tv_region
,hh_affluence
,head_hh_age
,a.adsmartable_box
...
,min(e.tv_region) as isbatv_region
...
--select * from vespa_analysts.scaling_weightings where scaling_day = '2012-01-02'
--select top 100 * from vespa_analysts.scaling_dialback_intervals;
--select count(*) , count(distinct account_number) from #profile_details_of_all_boxes;


--drop table #counts_by_households;
select case when isbatv_region is null then 'Not Defined' else isbatv_region end as isba_tv_region
,hh_affluence_level as household_affluence_level
,headhh_agef as head_hh_agef
...
order by a.household_affluence_level 
;


commit;
--select * from #counts_by_households;
--select * from #demographic_frequency_summary order by unknown_demographic desc;


--select adsmartable ,sum(households) from #counts_by_households group by adsmartable;
