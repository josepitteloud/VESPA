...
,sum(a.adult_25_44)
,sum(a.adult_ABC1)
,sum(a.hh_with_children)
,sum(a.adult_male)
,sum(a.adult_male_18_34)
,sum(a.adult_male_ABC1)
,sum(a.adult_female_18_34)
,sum(a.adult_female_18_44)
,sum(a.adult_female_18_54)
,sum(a.adult_female_ABC1)
,sum(a.adult_female)

,sum(weighting) 
,sum(a.adult_18_34*weighting)
,sum(a.adult_18_44*weighting)
,sum(a.adult_18_54*weighting)
,sum(a.adult_25_44*weighting)
,sum(a.adult_ABC1*weighting)
,sum(a.hh_with_children*weighting)
,sum(a.adult_male*weighting)
,sum(a.adult_male_18_34*weighting)
,sum(a.adult_male_ABC1*weighting)
,sum(a.adult_female_18_34*weighting)
,sum(a.adult_female_18_44*weighting)
,sum(a.adult_female_18_54*weighting)
,sum(a.adult_female_ABC1*weighting)
,sum(a.adult_female*weighting)
...
commit;
/*
select case when socio_demographic_level = 1 then '01: AB' 
            when socio_demographic_level = 2 then '02: C1/C2' 
            when socio_demographic_level = 3 then '03: D'
            when socio_demographic_level = 4 then '04: E'
            when socio_demographic_level = 5 then '05: Retired'
            when socio_demographic_level = 6 then '06: Unclassified' else '07: Other' end as socio_level
,case when socio_demographic_level = 1 and household_affluence_level>10 then '01: AB' 
when socio_demographic_level = 1 and household_affluence_level<=10 then '02: C1' 
            when socio_demographic_level = 3 then '04: D'
            when socio_demographic_level = 4 then '05: E'
            when socio_demographic_level = 2 
            and occ_middle_management_in_hh+occ_professional_senior_in_hh+occ_education_in_hh>0 then '02: C1' 
            when socio_demographic_level = 2  and occ_office_clerical_in_hh=1 and household_affluence_level>=9 then '02: C1'
            when socio_demographic_level = 2 then '03: C2'
            when socio_demographic_level = 5 and household_affluence_level>=10 then '01: AB'
            when socio_demographic_level = 5 and household_affluence_level>=8 then '02: C1'
            when socio_demographic_level = 5 and household_affluence_level>=5 then '03: C2'
            when socio_demographic_level = 5 and household_affluence_level>=3 then '04: D'
            when socio_demographic_level = 5 and household_affluence_level<3 then '05: E'

             else '07: Other' end as socio_level_redefined

,count(*) as households

from #household_summary_details
group by socio_level , socio_level_redefined
order by socio_level ,socio_level_redefined
;


select socio_demographic_level
,sum(occ_caftsman_in_hh)   
,sum(occ_education_in_hh)   
,sum(occ_housewife_in_hh)   
,sum(occ_manual_in_hh)   
,sum(occ_middle_management_in_hh)   
,sum(occ_office_clerical_in_hh)   
,sum(occ_professional_senior_in_hh)   
,sum(occ_retired_in_hh)   
,sum(occ_other_in_hh)
,count(*) as households
from #household_summary_details
group by socio_demographic_level
order by socio_demographic_level;

    % of population (NRS 2010)  
