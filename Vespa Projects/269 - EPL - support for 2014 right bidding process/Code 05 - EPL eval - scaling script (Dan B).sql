...
,case when lower(trim(b.value_segment)) = 'unstable' then 'UNSTABLE' else 'STABLE' end as value_segment_group
...
,case when c.Days_Data_Available>=0 then 1 else 0 end as vespa_viewing_account            -- All accounts are valid accounts
...
left outer join (select
                 ...
                 where Period = 1
                 group by Account_Number
                having max(valid_account_flag) = 1) c
...
begin
    ...
    if object_id('scaling_summary') is not null drop table scaling_summary
    create table scaling_summary (
        Var                                     varchar(30)       null      default null,
        Category                                varchar(30)       null      default null,
        Universe                                bigint            null      default 0,
        Scaled_Panel                            bigint            null      default 0,
        Diff                                    bigint            null      default 0,
        Diff_Exact                              real              null      default 0
    )

    set @varSql = '
                    insert into scaling_summary
                      select
                            ''##^1^##'' as Var,
                            a.##^1^## as Category,
                            count(*) as Universe,
                            sum(case
                                  when b.account_Number is null then 1.0 * 0.0
                                    else b.weight
                                end) as xScaled_Panel,
                            abs(Universe - xScaled_Panel) as Diff,
                            abs(Universe - xScaled_Panel) as Diff_Exact
                        from account_details_for_weighting a left join account_weight b on a.account_Number = b.account_Number
                       group by a.##^1^##
                       order by 2
                    commit
                  '

    execute( replace(@varSql, '##^1^##', 'product_holding_group') )
    execute( replace(@varSql, '##^1^##', 'value_segment_group') )
    execute( replace(@varSql, '##^1^##', 'affluence_group') )
    execute( replace(@varSql, '##^1^##', 'tenure_group') )
    execute( replace(@varSql, '##^1^##', 'box_type_group') )
    execute( replace(@varSql, '##^1^##', 'tv_package_group') )
  ...
  -- Final table
if object_id('EPL_05_Scaling_Weights') is not null then drop table EPL_05_Scaling_Weights end if;
create table EPL_05_Scaling_Weights (
    Pk_Identifier                           bigint            identity,

      -- Account
    Account_Number                          varchar(20)       null      default null,
    Period                                  tinyint           null      default null,
    Scaling_Weight                          real              null      default 0,

    Updated_On                              datetime          not null  default timestamp,
    Updated_By                              varchar(30)       not null  default user_name()
);
create        hg   index idx01 on EPL_05_Scaling_Weights(Account_Number);
create        lf   index idx02 on EPL_05_Scaling_Weights(Period);
create unique hg   index idx03 on EPL_05_Scaling_Weights(Account_Number, Period);

insert into EPL_05_Scaling_Weights
      (Account_Number, Period, Scaling_Weight)
  select
        Account_Number,
        1,
        Weight
    from account_weight;
commit;
grant select on EPL_05_Scaling_Weights to vespa_group_low_security;


if object_id('account_details_for_weighting') is not null then drop table account_details_for_weighting end if;
if object_id('boxtype_ac') is not null then drop table boxtype_ac end if;
if object_id('scaling_box_level_viewing') is not null then drop table scaling_box_level_viewing end if;
if object_id('v250_single_profiling_view') is not null then drop table v250_single_profiling_view end if;
if object_id('weighting_values') is not null then drop table weighting_values end if;
if object_id('account_weight') is not null then drop table account_weight end if;
if object_id('scaling_summary') is not null then drop table scaling_summary end if;
