...
SET @c6 = 1 --  Extracting viewing data from Event table
SET @c7 = 0  -- CReating Capped Viewing table
SET @c8 = 1 --  Consolidating SAV and Viewing SAMPLED
...
IF object_id('sk_uat.WEATHER_DATA_TIME_OF_DAY') IS NOT NULL	      DROP TABLE  sk_uat.WEATHER_DATA_TIME_OF_DAY
  ...
  SELECT
	...
	INTO WEATHER_DATA_TIME_OF_DAYPG
...
INSERT INTO WEATHER_View_May_2013
  ...
  SELECT   
		...
		, CASE   WHEN DATEPART(hh, event_start_date_time_utc) BETWEEN 0 AND 7 THEN '0'
				 ...
				 WHEN DATEPART(hh, event_start_date_time_utc) BETWEEN 20 AND 24 THEN '4'
				ELSE '99'
		  END                 Time_of_Day
	...
	FROM Sk_prod.VESPA_DP_PROG_VIEWED_201305 as v
  INNER JOIN pitteloudj.WEATHER_SUBS AS s ON s.account_number = v.account_number AND s.Q_flag = 1
	WHERE 
