...
CREATE OR REPLACE VIEW weather_agg_fix AS 
SELECT 
	  district
...
FROM weather_data_aggregated_history
GROUP BY 
	  district
	, period
	, period_from	
...
CREATE OR REPLACE  VIEW WEATHER_homebase_raw
AS
SELECT
          DATE(ag.weather_date)                         AS  w_date
        , MIN(ag.min_temp)                       AS Min_Temperature
        , MAX(ag.max_temp)                       AS Max_Temperature
        , SUM(CASE WHEN ag.m_snowfall     = 0 THEN ag.m_rainfall ELSE 0 END)        AS total_rainfall
        , SUM(CASE WHEN ag.m_snowfall     = 1 THEN ag.m_rainfall ELSE 0 END)        AS total_snowfall
        , ag.district
...
-------------- Simple segment weights 
SELECT cb_address_postcode_outcode AS district
        , segment
        , count(DISTINCT a.account_number) AS hh_count
        , SUM(hh_count) OVER (PARTITION BY segment) as total_group
        , CAST(hh_count AS DECIMAL(6,2))/ total_group AS weight
INTO Segment_weight_district
FROM SIMPLE_SEGMENTS 			AS a
JOIN CUST_SINGLE_ACCOUNT_VIEW 	AS s on a.account_number = s.account_number
GROUP BY district, segment
commit
CREATE HG INDEX if1 ON Segment_weight_district(district)
CREATE LF INDEX if2 ON Segment_weight_district(segment)
COMMIT


---------------	Consolidated view Simple SEGMENTs
SELECT
    w_date
    , n_week
    , segment
...
INTO WEATHER_SKY_movies_data_segment
FROM WEATHER_homebase_raw as a
JOIN Segment_weight_district as b ON a.district = b.district
GROUP BY
     w_date
    , n_week
    , segment
...
CREATE REPLACE VIEW WEATHER_homebase_weekly
AS
SELECT
