  ...
  create variable @Sample_1_EndString varchar(4);
   ...
   while @Reference <= 201410 begin
            ...
            select account_number
                  ,rand(number(*)*multiplier) as rand_num
              into #table_churn_history1
              from citeam.View_CUST_CHURN_HIST
          group by account_number

            commit
            CREATE hg INDEX hgran ON #table_churn_history1(rand_num)
            ...
            insert into #sample_Accounts(
                   account_number
                   )
            select top @records account_number
              from #table_churn_history1
          order by rand_num

            commit
            create unique hg index uhacc on #table_churn_history2(account_number)

            select cch.account_number
                  ,typeofevent as status_code
                  ,event_dt as effective_from_dt
              into #table_churn_history2
              from #sample_accounts as bas
                   inner join citeam.View_CUST_CHURN_HIST as cch on bas.account_number = cch.account_number

            select *
              into #table_churn_history_output
              from #table_churn_history2 as bas
                   cross join sourcedates as sou
             where sou.[reference] = @reference
          ...
          ---------------------------------------------------------------------------------------------------------------------------------------------------------
           execute ('select *
            ...
            select *
              into #data_mart2
              from #data_mart1 as bas
                   inner join @sample_accounts as sam on bas.account_number = sam.account_number

            select account_number
                  ,max(case when status_code='AB' then effective_from_dt else cast(null as date) end) as status_code_AB_effective_from_dt_max
                  ,max(case when status_code='PO' then effective_from_dt else cast(null as date) end) as status_code_PO_effective_from_dt_max
                  ,max(case when status_code='SC' then effective_from_dt else cast(null as date) end) as status_code_SC_effective_from_dt_max
                  ,max(case when typeofevent='TA' then effective_from_dt else cast(null as date) end) as status_code_TA_effective_from_dt_max
              into #data_mart_output
              from #table_churn_history_output
          ---------------------------------------------------------------------------------------------------------------------------------------------------------
            select account_number
                  ,event_dt
              into #TA_call_history
              from citeam.View_CUST_CALLS_HIST as bas
                   inner join @sample_accounts as sam on bas.account_number = sam.account_number

            select account_number
                  ,max(case when status_code='TA' then effective_from_dt else cast(null as date) end) as status_code_AB_effective_from_dt_max

            select *
              into #TA_call_history_output
              from #TA_call_history
                   cross join sourcedates as sou
             where sou.[reference] = @reference
               and event_dt <= Snapshot_Date
               and event_dt > [2_Years_Prior]
          ---------------------------------------------------------------------------------------------------------------------------------------------------------
            select account_number
                  ,event_dt
              into #futureTA1
            ...
            select account_number
                  ,max(case when typeofevent = 'TA' then event_dt else cast(NULL as date) end) as type_of_event_TA_event_dt_max
              into #futureTA2
              from #futureTA1
          ...
          ---------------------------------------------------------------------------------------------------------------------------------------------------------
            select account_number
                  ,case when typeofevent = 'AB' and event_dt > Snapshot_Date and event_dt <= [3_Months_Future] then 1 else 0 end as AB_In_Next_3_Months_Flag
                  ,case when typeofevent = 'SC' and event_dt > Snapshot_Date and event_dt <= [3_Months_Future] then 1 else 0 end as SC_In_Next_4_Months_Flag
                  ,case when typeofevent = 'PO' and event_dt > Snapshot_Date and event_dt <= [3_Months_Future] then 1 else 0 end as PO_In_Next_4_Months_Flag
              into #future_output
              from citeam.View_CUST_CHURN_HIST as bas
                   inner join @sample_accounts as sam on bas.account_number = sam.account_number
                   cross join sourcedates as sou
             where sou.[reference] = @reference
          ---------------------------------------------------------------------------------------------------------------------------------------------------------
          ...
          group by account_number
                ...
                -- restructure
            select *
                  ,case when offer_category = 'Software Comms' then offer_amount_Sum else 0 end as Offer_Category_Software_Comms_offer_amount_sum
                  ,case when offer_category = 'Software DTV'   then offer_amount_Sum else 0 end as Offer_Category_Software_DTV_offer_amount_sum
                  ,case when offer_category = 'Others'         then offer_amount_Sum else 0 end as Offer_Category_others_offer_amount_sum
                  ,case when offer_category = 'Software Comms' then record_count     else 0 end as Offer_Category_Software_Comms_record_count
                  ,case when offer_category = 'Software DTV'   then record_count     else 0 end as Offer_Category_Software_DTV_record_count
          ...
          ---------------------------------------------------------------------------------------------------------------------------------------------------------
            ...
            select dmt.*
                  ...
                  ,PO_In_Next_4_Months_Flag
                  ,TA_in_3_6_Months_Flag
                  ,Type_Of_Event_TA_event_dt_Max_flag
              into #etl_main1
...
create hg index hgacc on #sub_changes(account_number);
  ...
  select account_number
        ,dt
        ,max(effective_to_dt)                                   as last_dt
        ,sum(case when effective_to_dt <= dt then 1 else 0 end) as num_ever
    into #changes
    from #sub_changes
         cross join #refs
