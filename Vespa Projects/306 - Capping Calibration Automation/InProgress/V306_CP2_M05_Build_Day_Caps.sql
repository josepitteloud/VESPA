...
as begin
	...
	*/
	...
	--calculate ntiles for caps (same grain as CP2_event_listing but we no longer need to identify each unique event anymore for the ntiling process)
	select
			*
		,	max(ntile_lp) over (
									partition by
		...
		,	1.0 * ntile_lp / max_ntile_lp							ntile_lp_pc
		,	1.0 * ntile_1 / max_ntile_1						   		ntile_1_pc
...
--     into    CP2_ntiles_week
	...
	if object_id('CP2_nt_all') is not null drop table CP2_nt_all
	commit --; --^^ to be removed
	...
	--create indexes
	create hng index idx1 on CP2_nt_all(event_start_day)
	commit --; --^^ to be removed
	create hng index idx2 on CP2_nt_all(event_start_hour)
	commit --; --^^ to be removed
	create hng index idx4 on CP2_nt_all(box_subscription)
	commit --; --^^ to be removed
	create hng index idx5 on CP2_nt_all(pack_grp)
	commit --; --^^ to be removed
	create hng index idx6 on CP2_nt_all(initial_genre)
	...
	if object_id('CP2_live') is not null drop table CP2_live
	commit --; --^^ to be removed
	...
	select
		...
		,	min	(
					case
						when	NT.median_dur_mins	>=	MET.BOX_SHUT_DOWN			then	NT.ntile_1
						else																null
					end
				)					as	pri_ntile	-- this will only be NULL if no ntiles contain a median duration >= the box_shut_down parameter. Minimum to set lower ntile bound.
		,	max(NT.ntile_1)			as	sec_ntile	-- this is simply 200 the majority of the time, but can be less where there is a lack of data. Max to set upper ntile bound.
		,	case
				when	pri_ntile is not null	then	pri_ntile - MET.THRESHOLD_NTILE
				else									sec_ntile - MET.THRESHOLD_NONTILE
			end						as	cap_ntile
		,	100.0 / (200 - MET.THRESHOLD_NONTILE)	as	cap_ntile_pc
	...
	-- Update min_dur_mins with the appropriate value for the capping ntile that we've chosen per segment.
	update	CP2_live t1
	...
	--reset capping limits that are outside of the boundaries defined by the parameters table
	update	CP2_calculated_viewing_caps	t1
	set	t1.max_dur_mins	=	case
								when	(
												t1.max_dur_mins	<	MET.MINIMUM_CUT_OFF
											or	t1.max_dur_mins	is null
										)												then	MET.MINIMUM_CUT_OFF
								when	t1.max_dur_mins	>	MET.MAXIMUM_CUT_OFF			then	MET.MAXIMUM_CUT_OFF
								else															t1.max_dur_mins
							end
	from	CP2_metadata_table		AS	MET
	where
			t1.EVENT_START_HOUR			BETWEEN	DATEPART(HOUR,MET.START_TIME)
										AND		DATEPART(HOUR,MET.END_TIME)
		AND	MET.BANK_HOLIDAY_WEEKEND	=		0
		AND	t1.Live						=		1
	commit --; --^^ to be removed
