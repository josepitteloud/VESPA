...
--commented out as not required for capping analysis
...
as begin
        ...
        set     @current_metadata_row_number    =       (
                                select  max(row_id)
                                  from  CP2_metadata_table
                                 where  @target_date    between EFFECTIVE_FROM  and  EFFECTIVE_TO
        ...
        select
                        subscriber_id
                ,       round(sum(datediff(ss, X_Viewing_Start_Time, X_Viewing_End_Time)) / 60.0, 0)    as      total_box_viewing  --minutes
        ...
        from
                ...
                inner join      (
                        ...
                        ,       max(THRESHOLD_NTILE)    OVER (PARTITION BY BANK_HOLIDAY_WEEKEND, COMMON_PARAMETER_GROUP) AS     THRESHOLD_NTILE
                                                ,       EFFECTIVE_FROM
                                                ,       EFFECTIVE_TO
                        from    CP2_metadata_table)        AS      MET
...
as begin
        ...
        select
                ...
                inner join      BARB_MINUTE_BY_MINUTE_WEIGHTED_VIEWING  bar     on      ves.UTC_DATEHOURMIN     =       bar.UTC_DATEHOURMIN
        ...
        where   met.UTC_DAY_OF_INTEREST =       str.UTC_DAY_OF_INTEREST
        commit -- ;-- ^^ originally a commit
