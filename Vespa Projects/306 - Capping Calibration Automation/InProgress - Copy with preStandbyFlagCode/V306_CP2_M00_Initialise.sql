...
--                      commit
        ...
        create table vespa_analysts.Vespa_daily_augs_YYYYMMDD (
                                        ...
                                        ,timeshifting               varchar(10)                         -- 'LIVE' or 'VOSDAL' (same day as live) or 'PLAYBACK7' (playback within 7 days) or 'PLAYBACK28' (otherwise)
                                        ,capped_flag                tinyint                             -- 0-2 depending on capping treatment: 0 -> event not capped, 1 -> event capped but doesn't effect viewing,
                                                                                                        -- 2 -> event capped & shortens viewing,
                                                                                                        -- 3 -> event capped & excludes viewing (actually 3 doesn't turn up in the table, but that's what it means
                                                                                                        --                                                                                              during processing)
        ...
        */
        ...
        create table Vespa_Daily_Augs (
                ...
                ,Viewing_Stops               datetime
                ,Viewing_Duration            bigint                             -- Capped viewing in seconds
                ,Capped_Flag                 tinyint                            -- 0-2 depending on capping treatment: 0 -> event not capped,
                                                                                -- 1 -> event capped but does not effect viewing, 2 -> event capped & shortens viewing,
                                                                                -- 3 -> event capped & excludes viewing (actually 3 will not turn up in the table, but
                                                                                --                                              that is what it means during processing)
                ,Capped_Event_End_Time       datetime                           -- Only populated for capped events
                ,Scaling_Segment_Id          bigint                             -- To help with the MBM proc builds.... -- NYIP!
