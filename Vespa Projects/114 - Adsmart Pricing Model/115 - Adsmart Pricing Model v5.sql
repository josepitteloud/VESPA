...
---Add on Corrected Mirror Segment Definition---
...
create  hg index idx1 on mirror_men_and_women_seg(cb_key_household);
--Reset mirror segments

Alter table project_114_base_Accounts Add  cb_key_household bigint;
...
update project_114_base_Accounts
set   MIRROR_MEN_MIN   =null 
,   MIRROR_WOMEN_MIN   =null
from  project_114_base_Accounts
;

update project_114_base_Accounts
set   MIRROR_MEN_MIN   =case when b.cb_key_household is null then 'MVII' else b.Mirror_Men end
,   MIRROR_WOMEN_MIN   =case when b.cb_key_household is null then 'WVII' else b.Mirror_Women end
from  project_114_base_Accounts as a
left outer join mirror_men_and_women_seg as b
on a.cb_key_household=b.cb_key_household
;
commit;
--select MIRROR_MEN_MIN , count(*) as records from project_114_base_Accounts group by MIRROR_MEN_MIN order by MIRROR_MEN_MIN
--select MIRROR_WOMEN_MIN , count(*) as records from project_114_base_Accounts group by MIRROR_WOMEN_MIN order by MIRROR_WOMEN_MIN

--select top 100 * from mirror_men_and_women_seg

/*
select Mirror_men, count(*)
from mirror_men_and_women_seg
group by Mirror_men
*/
...
---Add on Adsmartable Box details to spot data----


---Create List of Adsmartable Boxes---
SELECT base.account_number
,settop.service_instance_id
--      ,x_pvr_type
--      ,x_manufacturer
...
create  hg index idx1 on SetTop(service_instance_id);
...
create  hg index idx1 on src_lookup(src_system_id);
create  hg index idx2 on src_lookup(subscriberid);
commit;

alter table Project_114_viewing_table add adsmartable_box tinyint default 0;

update Project_114_viewing_table 
set adsmartable_box=case when c.Adsmartable=1 then 1 else 0 end
from Project_114_viewing_table as a
left outer join src_lookup as b
on a.subscriber_id = b.subscriberid
left outer join SetTop as c
...
,sum(case when adsmartable_box is null then 0 else adsmartable_box*spot_duration/30 end) as adsmartable_box_unweighted_impacts
,sum(case when adsmartable_box is null then 0 else adsmartable_box*overall_project_weighting*(spot_duration/30) end) as adsmartable_box_weighted_impacts
from Project_114_viewing_table as a 
...
,case when adsmartable_box is null then 0 else adsmartable_box end as adsmartable_box
...
--drop table project_114_all_viewing_pivot_inc_channel_info;
...
--drop table project_114_all_viewing_pivot_exc_channel_info;
--------Create Table for Pivot (Excluding Spot Details)---------
select adsmartable_box
,demographic_and_lifestyle
,affluence_band
,sky_reward_L12M
,cable_area_type
...
--------Create Table for Pivot (summary Spot Details)---------
select adsmartable_box
,case when spot_position_in_break =1 then 'a) First' when spot_position_in_break=2 then 'b) Second'
  when cast(spot_position_in_break as real)/2= round((spot_position_in_break/2),1) then 'd) Other Even' else 'c) Other Odd' end as spot_position_group
,case when spot_channel_name in ('SBO 743 PPV Event'
,'Sky News'
,'Sky News HD'
,'Sky Sports 1'
,'Sky Sports 1 HD'
...
,'Sky Sports F1'
,'Sky Sports F1 HD'
,'Sky Sports Interactive Hi 4'
,'Sky Sports Interactive Lo 0'
,'Sky Sports Interactive Lo 1'
,'Sky Sports Interactive Lo 2'
,'Sky Sports Interactive Lo 3'
,'Sky Sports Interactive Lo 4'
,'Sky Sports Interactive Lo 5'
,'Sky Sports Interactive Lo 6'
,'Sky Sports News'
,'Sky Sports News HD') then 1 else 0 end as live_sports_news_channel
...
,sum(weighted_impacts) as total_weighted_impacts
,count(*) as records
into project_114_all_viewing_pivot_with_summary_channel_info
from project_114_all_viewing_demographic_data
group by adsmartable_box
,spot_position_group
,live_sports_news_channel
,demographic_and_lifestyle
,affluence_band
,sky_reward_L12M
,cable_area_type
...
,has_skyplus
;
--drop table project_114_all_viewing_demographic_data; commit;
commit;
--select spot_position_in_break, case when cast(spot_position_in_break as real)/2= round((spot_position_in_break/2),1) then 'Even' else 'Odd' end as spot_pos , count(*) from project_114_all_viewing_demographic_data group by spot_position_in_break,spot_pos;
--select distinct spot_channel_name from  project_114_all_viewing_demographic_data order by spot_channel_name;
...
grant all on project_114_all_viewing_pivot_with_summary_channel_info to greenj, dbarnett, jacksons, stafforr, sarahm, poveys, gillh
, rombaoad, louredaj, patelj, kinnairt, sbednaszynski, vespa_group_low_security, sk_prodreg,vespa_analysts,neighbom;
commit;
--select count(*) from project_114_all_viewing_pivot_with_summary_channel_info;
--select count(*) from project_114_all_viewing_pivot_exc_channel_info;

--PART III Repeat with full uncapped viewing details----

select distinct service_key
into #distinct_service_keys_uncapped
from  BARB_SPOT_DATA_PROJECT_114
;
commit;

commit;
exec sp_create_tmp_table_idx '#distinct_service_keys_uncapped', 'service_key';
...
INTO project114_uncapped_viewing_data
FROM sk_prod.VESPA_EVENTS_VIEWED_ALL as a
left outer join #distinct_service_keys as b
on a.service_key=b.service_key
where b.service_key is not null
...
--select top 500 * from project114_uncapped_viewing_data;

--
create  hg index idx1 on project114_uncapped_viewing_data(service_key);
create  hg index idx2 on project114_uncapped_viewing_data(broadcast_start_date_time_utc);
create  hg index idx3 on project114_uncapped_viewing_data(broadcast_end_date_time_utc);
...
---PartIII E 	 - Loop Creating all viewed spots in period

if object_id('Project_114_uncapped_viewing_table') is not null drop table Project_114_uncapped_viewing_table;

--
create table Project_114_uncapped_viewing_table
(spot_column integer
,account_number varchar(20)
,subscriber_id numeric (8,0)
);
...
create variable @viewing_start_time_uncapped datetime;
create variable @viewing_end_time_uncapped datetime;
commit;
set @viewing_start_time_uncapped = cast ('2012-08-14 00:00:00' as datetime);
set @viewing_end_time_uncapped = cast ('2012-08-14 23:59:59' as datetime);

---Loop by Minute---
    
WHILE @viewing_start_time_uncapped < '2012-08-29 00:00:00' LOOP
select *
into project114_single_day_viewing_uncapped
from project114_uncapped_viewing_data
where instance_start_date_time_utc between @viewing_start_time_uncapped and @viewing_end_time_uncapped
;

---Create table of spots for that day (+ few extra hours for viewing that overlaps days--
select * into project114_single_day_spots_uncapped
 from BARB_SPOT_DATA_PROJECT_114
where utc_spot_end_date_time  between @viewing_start_time_uncapped and dateadd(hour,24,@viewing_end_time_uncapped);

insert into Project_114_uncapped_viewing_table

select spot.spot_column
,vw.account_number
,vw.subscriber_id
from  project114_single_day_viewing_uncapped as vw 
inner join project114_single_day_spots_uncapped as spot
        on  vw.service_key=spot.service_key
where  
 (     instance_start_date_time_utc <= utc_spot_start_date_time and  instance_end_date_time_utc> utc_spot_end_date_time 
    )
;



drop table project114_single_day_viewing_uncapped;
drop table project114_single_day_spots_uncapped;

set @viewing_start_time_uncapped= dateadd(day,1,@viewing_start_time_uncapped);
set @viewing_end_time_uncapped= dateadd(day,1,@viewing_end_time_uncapped)
;
...
--select count(*) from Project_114_uncapped_viewing_table;
--select * from project114_single_day_spots;



commit;
--select utc_spot_start_date_time,channel_name , count(*) as records from Project_114_uncapped_viewing_table_dump group by utc_spot_start_date_time,channel_name order by records desc;
/*
commit;
create  hg index idx1 on Project_114_uncapped_viewing_table(subscriber_id);
create  hg index idx2 on Project_114_uncapped_viewing_table(account_number);
create  hg index idx3 on Project_114_uncapped_viewing_table(service_key);
commit;
*/

grant select on Project_114_uncapped_viewing_table to greenj, dbarnett, jacksons, stafforr, sarahm, poveys, gillh
, rombaoad, louredaj, patelj, kinnairt, sbednaszynski, vespa_group_low_security, sk_prodreg,vespa_analysts,neighbom;
commit;

--drop table Project_114_uncapped_viewing_table; commit;


--Add demographic details back on----

alter table Project_114_uncapped_viewing_table add service_key int;
alter table Project_114_uncapped_viewing_table add utc_spot_start_date_time datetime;
alter table Project_114_uncapped_viewing_table add utc_spot_end_date_time datetime;
alter table Project_114_uncapped_viewing_table add spot_duration int;
alter table Project_114_uncapped_viewing_table add spot_channel_name varchar(255);
alter table Project_114_uncapped_viewing_table add spot_position_in_break int;

update Project_114_uncapped_viewing_table
set a.service_key=b.service_key
,a.utc_spot_start_date_time=b.utc_spot_start_date_time
,a.utc_spot_end_date_time=b.utc_spot_end_date_time
,a.spot_duration=b.spot_duration
,a.spot_channel_name=b.spot_channel_name
,a.spot_position_in_break=b.spot_position_in_break
from Project_114_uncapped_viewing_table as a
left outer join BARB_SPOT_DATA_PROJECT_114 as b
on a.spot_column=b.spot_column
;
commit;

alter table Project_114_uncapped_viewing_table add adsmartable_box tinyint default 0;
...
--drop table project_114_uncapped_all_viewing_demographic_data;
----
select a.spot_duration
,a.spot_position_in_break
,a.service_key
...
,case when skyplus=1 then 'Yes' else 'No' end as has_skyplus
,case 
when dateformat(utc_spot_start_date_time,'YYYY-MM-DD-HH') between '2010-03-28-02' and '2010-10-31-02' then dateadd(hh,1,utc_spot_start_date_time) 
when dateformat(utc_spot_start_date_time,'YYYY-MM-DD-HH') between '2011-03-27-02' and '2011-10-30-02' then dateadd(hh,1,utc_spot_start_date_time) 
when dateformat(utc_spot_start_date_time,'YYYY-MM-DD-HH') between '2012-03-25-02' and '2012-10-28-02' then dateadd(hh,1,utc_spot_start_date_time) 
                    else utc_spot_start_date_time  end as spot_start_time_local

into project_114_uncapped_all_viewing_demographic_data
from Project_114_uncapped_viewing_table  as a
left outer join project_114_base_Accounts as b
on a.account_number = b.account_number
where b.account_number is not null
--and right(b.account_number,3) in ('367')
;
grant all on project_114_uncapped_all_viewing_demographic_data to greenj, dbarnett, jacksons, stafforr, sarahm, poveys, gillh
, rombaoad, louredaj, patelj, kinnairt, sbednaszynski, vespa_group_low_security, sk_prodreg,vespa_analysts,neighbom;
commit;

--select top 500 * from project_114_uncapped_all_viewing_demographic_data;
--select count(*) from project_114_uncapped_all_viewing_demographic_data;
--drop table project_114_uncapped_all_viewing_pivot_with_summary_channel_info;
select adsmartable_box
,case when spot_position_in_break =1 then 'a) First' when spot_position_in_break=2 then 'b) Second'
  when cast(spot_position_in_break as real)/2= round((spot_position_in_break/2),1) then 'd) Other Even' else 'c) Other Odd' end as spot_position_group
,case when spot_channel_name in ('SBO 743 PPV Event'
...
,'Sky Sports News HD') then 1 else 0 end as live_sports_news_channel
,spot_channel_name
--,dateformat(spot_start_time_local,'HH') as spot_start_hour
,demographic_and_lifestyle
,affluence_band
--,sky_reward_L12M
,cable_area_type
...
,sum(weighted_impacts) as total_weighted_impacts
,count(*) as records
into project_114_uncapped_all_viewing_pivot_with_summary_channel_info
from project_114_uncapped_all_viewing_demographic_data
group by adsmartable_box
,spot_position_group
,live_sports_news_channel
,spot_channel_name
...
grant all on project_114_uncapped_all_viewing_pivot_with_summary_channel_info to greenj, dbarnett, jacksons, stafforr, sarahm, poveys, gillh
, rombaoad, louredaj, patelj, kinnairt, sbednaszynski, vespa_group_low_security, sk_prodreg,vespa_analysts,neighbom;
commit;
--select count(*) from project_114_uncapped_all_viewing_pivot_with_summary_channel_info;
---Repeat for Capped----drop table project_114_all_viewing_pivot_with_summary_channel_info;
select adsmartable_box
,case when spot_position_in_break =1 then 'a) First' when spot_position_in_break=2 then 'b) Second'
  when cast(spot_position_in_break as real)/2= round((spot_position_in_break/2),1) then 'd) Other Even' else 'c) Other Odd' end as spot_position_group
,case when spot_channel_name in ('SBO 743 PPV Event'
...
--select count(*) from project_114_uncapped_all_viewing_pivot_with_summary_channel_info;
--select top 500 *  from project_114_uncapped_all_viewing_demographic_data
---Create New Summary tables for Pivot 20121203
--select  spot_start_hour,count(*) from project_114_uncapped_all_viewing_pivot_with_summary_channel_info group by spot_start_hour
--select top 100 * from project_114_uncapped_all_viewing_pivot_with_summary_channel_info;
--select top 100 * from project_114_uncapped_all_viewing_demographic_data;
--
select top 1000 *  into  project_114_TEST_all_viewing_pivot_with_summary_channel_info from  project_114_all_viewing_pivot_with_summary_channel_info ;
grant all on project_114_TEST_all_viewing_pivot_with_summary_channel_info to greenj, dbarnett, jacksons, stafforr, sarahm, poveys, gillh
, rombaoad, louredaj, patelj, kinnairt, sbednaszynski, vespa_group_low_security, sk_prodreg,vespa_analysts,neighbom;
commit;
