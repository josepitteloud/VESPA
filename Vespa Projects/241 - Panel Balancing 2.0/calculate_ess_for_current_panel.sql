         ...
         -------------------------------
      -- First, you may need to create the required tables
      -- (run "SC2 - 0 - table creation.sql" (in iSQL))
         SC2_Recreate_Env; --to clear data
  ...
  commit;
  create hg index hgsub on Scaling2_00_Input(subscriber_id);
...
;
  ...
  select sav.account_number
    from vespa_analysts.panbal_sav as sav
         left join panbal_amends as ame on sav.account_number = ame.account_number
   where (panel in (11, 12) and movement <> 'Account to remove from panel 12')
      or (panel in (11, 12) and movement is null)
      or movement in ('Account to add to panel 12 from panel 6 or 7', 'Account to add to panels 6/7, eventually for panel 12')
;
...
;
         ...
         -------------------------------------------------------------
         --Part 2 run code to create the prcoedures
         -------------------------------------------------------------
...
begin
           ...
           select scaling_segment_id
                 ,sum(case when rq is null then case when cbck_rate is null then 1 else cbck_rate end else rq end * case when boxes is null then 1 else boxes end) as panel_members
             into #segment_distribs
             from SC2_Todays_panel_members as bas
...
begin
              ...
              -- Um, there's basically nothing to do here except set up the logger, so then...
                if @Scale_refresh_logging_ID is null begin
                  declare @tablespacename varchar(40)
...
commit;
  ...
  create variable @build_ID     int;
  create variable @varStartDate date;
     set @build_ID = '999';
     set @varStartDate = '2014-05-15';
 ...
 execute SC2_v2_1__scale_Vespa_panel_CUSTOM      @varStartDate, now(), @build_ID;
         ...
         ---------------------------------------------------------------
         -- Part 4 calculate effective sample size for the current panel
         ---------------------------------------------------------------
...
/*
--accounts
select count(distinct account_number) from Scaling2_00_Input
--boxes
select count(1) from Scaling2_00_Input
--returning boxes
select sum(r) from (select bas.account_number,case when rq is null then case when cbck_rate is null then 1 else cbck_rate end else rq end * boxes as r
from Scaling2_00_Input as bas inner join vespa_analysts.panbal_sav as sav on bas.account_number = sav.account_number
group by bas.account_number,r) as sub
*/
