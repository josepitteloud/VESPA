...
**Sections:
		...
		A - Cube for Home Sessions LvL 3
			A00 - MASTER LvL 3 Sessions
			A01 - MASTER LvL 3 Interaction
...
truncate table z_pa_cube_hslvl3_Sessions;commit;

Insert	into z_pa_cube_hslvl3_Sessions
with	base as		(
						select	dk_action_id
								,action_name
								,asset_uuid
...
select	step1.datehour_
		...
		,step1.stb_type
		,tenure.months_old
		,step1.home_Session_lvl3_id
		,count(distinct step1.dk_serial_number)	as nboxes
		,count(distinct	(
							case 	when length(step1.home_Session_lvl3_id) <2 then null 
									else step1.dk_serial_number||'-'||step1.home_session_lvl3_id||'-'||step1.home_session_lvl3_grain
							end
						))	as tot_journeys
		,count	(distinct
					(
						case	when step1.dk_Action_id in(02400,03000,02000,04500,03001,02100) then step1.dk_serial_number||'-'||step1.home_session_lvl3_id||'-'||step1.home_session_lvl3_grain 
								else null 
						end
					)
				)	as tot_actioned_journey
--		,sum(step1.ss_elapsed_next_action)	as sum_ss_spent_in_session
--into	z_pa_cube_hslvl3_Sessions
from	base	as step1
		inner join tenure
		on	step1.dk_Serial_number	= tenure.dk_Serial_number
where	step1.home_Session_lvl3_id <> ''
and		step1.home_Session_lvl3_id is not null
...
truncate table z_pa_cube_hslvl3_Interaction;commit;

Insert	into z_pa_cube_hslvl3_Interaction
with	base as		(
						select	dk_action_id
								,action_name
								,asset_uuid
...
select	step1.datehour_
		...
		,count(distinct step1.dk_serial_number) 																		as nboxes
		,sum(step1.ss_elapsed_next_action)																				as sum_ss_spent_in_session
		,count(distinct step1.dk_serial_number||'-'||step1.home_session_lvl3_id||'-'||step1.home_session_lvl3_grain) 	as nsessions		
--into	z_pa_cube_hslvl3_Interaction
from	base	as step1
		inner join	totstb
