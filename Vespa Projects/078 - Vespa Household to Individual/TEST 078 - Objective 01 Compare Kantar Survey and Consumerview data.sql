...
---Having to use vespa_analysts schema but will then log in to Prod 4 and create copy using dbarnett schema
...
--select count(*) ,sum(case when account_number is not null then 1 else 0 end) from dbarnett.VESPA_KANTAR_SURVEY_DATA_2012_07_HH_SUMMARY
...
--select * from dbarnett.VESPA_KANTAR_SURVEY_DATA_2012_07_HH_SUMMARY;





--select top 5000 * from dbarnett.VESPA_KANTAR_SURVEY_DATA_2011_02 where hhno = '12042';

--Match To vespa data returned in June---

--Create Household Summary
--drop table #summary_by_account;
select account_number
,count(*) as people_in_household
,min(cb_key_household) as household_key
,min(social_class) as social_class
,sum(case when age_value<16 then 1 else 0 end) as age_under_16
,sum(case when age_value>=16 then 1 else 0 end) as age_16_plus
,sum(case when age_value>=16 and sex = 'Male' then 1 else 0 end) as male_16_plus
,sum(case when age_value>=16 and sex = 'Female' then 1 else 0 end) as female_16_plus

,sum(case when age_value between 25 and 34 then 1 else 0 end) as age_25_to_34_inclusive
,sum(case when age_value between 25 and 34 and sex = 'Male' then 1 else 0 end) as male_25_to_34_inclusive
,sum(case when age_value between 25 and 34 and sex = 'Female' then 1 else 0 end) as female_25_to_34_inclusive
into #summary_by_account
from #kantar_survey_key_data
group by account_number
;
commit;
create hg index idx1 on #summary_by_account(account_number);
--select top 100 * from #summary_by_account;
commit;

---return list of accounts returning data between 13th and 19th June Inclusive---

select distinct account_number
into #vespa_skyview_accounts
from sk_prod.VESPA_EVENTS_VIEWED_ALL 
where panel_id = 1 and event_start_date_time_utc between '2012-06-13 05:00:00' and '2012-06-20 04:59:59'
;

commit;
create hg index idx1 on #vespa_skyview_accounts(account_number);

--select count(*) from #vespa_skyview_accounts;

select a.*
into #recent_vespa_accounts_survey_data
from #summary_by_account as a
left outer join #vespa_skyview_accounts as b
on a.account_number = b.account_number
where b.account_number is not null;

--select count(*) from #recent_vespa_accounts_survey_data;

commit;
create hg index idx1 on #recent_vespa_accounts_survey_data(household_key);
--select top 100 * from #recent_vespa_accounts_survey_data

--drop table #consumerview_data;
select person_age
,case when p_gender = '1' then '02: Female' when p_gender = '0' then '01: Male' else '03: Unknown' end as gender
,household_composition
,family_lifestage 
,b.account_number
,a.cb_key_household
,a.cb_key_individual
into #consumerview_data
from sk_prod.experian_consumerview as a
left outer join #recent_vespa_accounts_survey_data as b
on a.cb_key_household = b.household_key
where b.household_key is not null ;


select top 100 * from #consumerview_data order by account_number;

select * from #kantar_survey_key_data where account_number ='200000883591'
person_age,gender,household_composition,family_lifestage,account_number,cb_key_household,cb_key_individual
'3','02: Female ','10','09','200000883591',5942994943383437312,5942994943383981475
'0','02: Female ','10','09','200000883591',5942994943383437312,5942994943383981490
'5','02: Female ','05','11','200000921524',3450832155983216640,3450832155983485229
'0','01: Male   ','00','09','200001082573',3263711521607254016,3263711521607688033
'3','02: Female ','00','09','200001082573',3263711521607254016,3263711521607688042
'0','02: Female ','02','09','200001087077',6671528926643224576,6671528926643494854
'0','02: Female ','02','09','200001087077',6671528926643224576,6671528926643557416
'3','02: Female ','02','09','200001087077',6671528926643224576,6671528926643494856
'0','01: Male   ','02','09','200001087077',6671528926643224576,6671528926643499562
'4','01: Male   ','02','09','200001087077',6671528926643224576,6671528926643494849
...
from 
sk_prod.ilu 
where cb_key_household = 5942994943383437312;
---Consumerview Data
select  * from #consumerview_data where account_number ='200000883591';
...
,sex
,age_value
 from #kantar_survey_key_data where account_number ='200000883591'

--select  * from SK_PROD.CUST_SINGLE_ACCOUNT_VIEW where account_number ='200000883591';
--select  * from SK_PROD.CUST_ALL_ADDRESS where account_number ='200000883591';
...
from 
sk_prod.ilu 
where cb_key_household = 3450832155983216640;


---Consumerview Data
select  * from #consumerview_data where account_number ='200000921524';
...
,sex
,age_value
 from #kantar_survey_key_data where account_number ='200000921524'
